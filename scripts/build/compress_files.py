...
BROTLI_LOCATION = path.join(_CURRENT_DIR, 'brotli.js')

BROTLI_CONST = b'\x1e\x9b'


def node_brotli_file(src, dst):
    f_in = open(src, 'rb')
    src_data = f_in.read()
    f_in.close()

    brotli = subprocess.Popen([NODE_LOCATION, BROTLI_LOCATION],
...
def compress_file(input_file_name):
    ...
    if prev_hash != curr_hash:
        node_brotli_file(input_file_name, input_file_name + ".compressed")
