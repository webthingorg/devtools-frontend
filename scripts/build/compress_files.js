...
async function brotli(src, dst) {
  const source_data = await readBinaryFile(src);
  const size_bytes = source_data.length;
  ...
  new DataView(array.buffer).setBigUint64(0, BigInt(size_bytes), true);
  ...
  return new Promise((resolve, reject) => {
    pipeline(fs.createReadStream(src), brotli, output, err => {
...
async function compressFile(filename) {
  ...
  const prevHash = await readTextFile(hashFilename);
  const currHash = sha1(await readBinaryFile(filename));
  if (prevHash !== currHash) {
    await writeTextFile(hashFilename, currHash);
    await brotli(filename, compressedFilename);
  }
}

async function main(argv) {
  const file_list_position = argv.indexOf('--file_list');
  const file_list = argv[file_list_position + 1];
  const file_list_contents = await readTextFile(file_list);
  const files = file_list_contents.split(' ');
