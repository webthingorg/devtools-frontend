...
describe('generating CSS JS files', () => {
  afterEach(() => {
    delete process.env.WATCH_BUILD;
  });
  ...
  it('does not include hot reloading code when debug mode is on and the WATCH_BUILD environment variable is not true',
     async () => {
       process.env.WATCH_BUILD = 'false';
       const css = `
    @container (width<1024px) {
      .test {
        color: #fff;
  ...
  it('includes hot reloading code when debug mode is on and the WATCH_BUILD environment variable is set to true',
     async () => {
       process.env.WATCH_BUILD = 'true';
       const css = `
    @container (width<1024px) {
      .test {
        color: #fff;
