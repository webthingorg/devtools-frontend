    ...
    return {
      ExpressionStatement(node) {
        if (node.expression.type === 'CallExpression' &&
            ...
            context.report({
              ...
              fix(fixer) {
                return [
                  ...
                  fixer.insertTextAfter(wasShownFunction.value.body.body[0], adoptedStyleSheetsText), fixer.remove(node)
                ];
              }
            });
          ...
          const wasShownText = `\n  wasShown() : void {
    super.wasShown();${adoptedStyleSheetsText}\n  }`;
          context.report({
            node,
