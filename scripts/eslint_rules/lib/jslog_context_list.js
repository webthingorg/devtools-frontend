  ...
  create: function(context) {
    ...
    return {
      CallExpression(node) {
        ...
        if (node.callee?.property?.name === 'registerActionExtension') {
          checkPropertyValue('actionId', node.arguments[0]);
        }
        if (node.callee?.property?.name === 'registerViewExtension') {
          checkPropertyValue('id', node.arguments[0]);
        }
        if (node.callee?.property?.name === 'registerSettingExtension') {
          checkPropertyValue('settingName', node.arguments[0]);
        }
        if (node.callee?.property?.name === 'createSetting') {
          checkValue(node.arguments[0]?.value, node);
        }
      },
      ...
      'Program:exit'() {
        ...
        const finalContents = 'export const knownContextValues = [\n'
