...
function isLitHtmlTemplateCall(taggedTemplateExpression) {
  ...
  return taggedTemplateExpression.object.name === 'LitHtml' && taggedTemplateExpression.property.name === 'html';
}
...
function findIndexOfDataSetterUsageForNode(taggedTemplateExpression) {
   ...
   * parts of a template. So if we have <foo .data=${something}></foo>, there
   * are two template parts:
   ...
   * repreesenting all the dynamic parts of the template.
   */
    ...
    return {
      TaggedTemplateExpression(node) {
        ...
        if (!dataUsageHasTypeCast) {
          context.report({node: node, message: 'LitHtml .data=${} calls must be typecast.'});
          return;
        }

        if (!dataSetterAsUsesInterface(node, indexOfDataSetterCall)) {
          context.report(
              {node: node, message: 'LitHtml .data=${} calls must be typecast to a type reference, not a literal.'});
          return;
        }
      },
    };
