    ...
    return {
      ImportDeclaration(node) {
        ...
        if (importMatchesExemptThirdParty) {
          /* We don't impose any rules on third_party DEPS whose structure doesn't allow us
           * to import as we'd like
           */
          return;
        }
        ...
        if (isStarAsImportSpecifier(node.specifiers)) {
          ...
          if (computeTopLevelFolder(importingFileName) !== computeTopLevelFolder(exportingFileName)) {
            ...
            if (importPath.includes('third_party')) {
              message +=
                  ' If the third_party dependency does not allow this update es_modules_import.js to make it exempt.';
            }
