...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'i18n.i18n.registerUIStrings must receive the current files path as the first argument',
  ...
  create: function(context) {
    return {
      CallExpression(callExpression) {
        if (!IsModuleScope || !IsRegisterUIStringsCall(callExpression)) {
          return;
        }
        ...
        const allowedPathArguments = [
          currentFileRelativeToFrontEnd,
          path.relative(FRONT_END_DIRECTORY, path.join(currentModuleDirectory, 'ModuleUIStrings.js')),
          path.relative(FRONT_END_DIRECTORY, path.join(currentModuleDirectory, 'ModuleUIStrings.ts')),
        ];

        if (!allowedPathArguments.includes(callExpression.arguments[0].value)) {
          context.report({
            node: callExpression,
