...
const PERFOAMCE_PANEL_INTERACTION_TESTS_PATH = 'test/interactions/panels/performance/';
module.exports = {
  meta : {
    type : 'problem',
  ...
  create : function(context) {
    ...
    function reportPathIfInvalid(node) {
      if(!fileName.includes(PERFOAMCE_PANEL_INTERACTION_TESTS_PATH)) {
        context.report({
          node,
    ...
    return {
      'CallExpression[callee.type="Identifier"][callee.name="itScreenshot"], MemberExpression'(node) {
        if ((node.property?.name === 'skip' && node.object?.name === 'itScreenshot' )|| node.type === 'CallExpression') {
