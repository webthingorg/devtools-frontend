...
class Project(ABC):
    ...
    def _run_command(cmd, dir, shell=False):
        ...
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                                   stderr=subprocess.STDOUT, cwd=dir, shell=shell)
        while True:
...
class DevtoolsProject(Project):
    ...
    def run(self):
        ...
        cmd = ['npm', 'run', 'e2etest',
               '--test-file=test/e2e/sources/dwarf-cxx-language-plugin.ts']
        Project._run_command(cmd, project_path)
...
def run_open_cv_test(config):
    ...
    language_server = threading.Thread(target=run_language_server,
                                       args=(config[LLVM_PROJECT_NAME], localhost_url, webcam_url))
    webcamera_server = threading.Thread(target=run_webcamera_server,
                                        args=(config[WEBCAMERA_PROJECT_NAME],))
...
def run_e2e_test(config):
    ...
    language_server = threading.Thread(target=run_language_server,
                                       args=(config[LLVM_PROJECT_NAME], localhost_url, devtools_url))
