...
class LLVMProject(Project):
    ...
    def _update_deps(self):
        ...
        with open(deps_file, 'w') as f:
            f.write(deps_content.replace("'build_symbol_server': False", "'build_symbol_server': True"))
    ...
    def build(self):
        ...
        build_path = os.path.join(self._install_dir, "back_end", self._project_name, "build")
        create_dir(build_path)
        cmd = ["cmake", "-DSYMBOL_SERVER_BUILD_FORMATTERS=ON",
    ...
    def run(self, search_path, timeout):
        project_path = os.path.join(self._install_dir, "back_end", self._project_name)
        starter_script = os.path.join(project_path, "tools/symbol-server.py")
        language_server = os.path.join(project_path, "build/bin/DWARFSymbolServer")
