...
const eslintignore_path = path.join(ROOT_DIRECTORY, '.eslintignore');

let directories_or_files_to_lint = process.argv.slice(2);

if (directories_or_files_to_lint.length === 0) {
  directories_or_files_to_lint = DEFAULT_DIRECTORIES_TO_LINT;
}

const cli = new CLIEngine({
  extensions: ['.js', '.ts'],
  ignorePath: eslintignore_path,
  fix: true,
});
...
// problematic paths.
const filtered_files_to_lint = directories_or_files_to_lint.filter(path => !cli.isPathIgnored(path));

const report = cli.executeOnFiles(filtered_files_to_lint);
