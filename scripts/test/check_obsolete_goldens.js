...
const INTERACTION_TEST_ROOT = path.join(__dirname, '..', '..', 'test', 'interactions');

const GOLDENS_LOCATION = path.join(INTERACTION_TEST_ROOT, 'goldens');
...
async function checkGoldensForPlatform(platform) {
  ...
  for await (const golden of goldens) {
    ...
    console.log(
        relativeGoldenPath, unixRelativeGoldenPath, `grep -r ${unixRelativeGoldenPath} ${INTERACTION_TEST_ROOT}`);
    try {
      // If this doesn't throw, that means we found a match and we're fine.
      const textSearchCommand = os.platform().includes('win') ?
          `Select-String -Path "${INTERACTION_TEST_ROOT}" -Pattern "${unixRelativeGoldenPath}" -CaseSensitive` :
          `grep -r ${unixRelativeGoldenPath} ${INTERACTION_TEST_ROOT}`;
      await exec(textSearchCommand);
    } catch (error) {
      if (error.code === 1) {
