...
def get_css_files():
    ...
    files = []
    for root, dirnames, filenames in walk(FRONT_END_DIRECTORY):
...
def main():
    ...
    exec_command.extend(get_css_files())
    exec_command.append('--fix')
    # The glob pattern does not appear to work on Windows. Setting
    # the --allow-empty-input flag makes the build not fail, but no CSS
