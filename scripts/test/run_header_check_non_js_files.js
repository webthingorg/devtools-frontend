...
].map(fileLocation => path.join(ROOT_DIRECTORY, ...fileLocation)));
...
if (filesToLint.length === 0) {
  ...
  filesToLint = glob.sync(`{${topLevelDirectories}}/**/{BUILD.gn,*.gni,*.css}`)
                    .map(fileLocation => fileLocation.replace('/', path.sep));
}

for (const fileLocation of filesToLint) {
  if (fileLocation.startsWith(path.join(FRONT_END_DIRECTORY, 'third_party'))) {
    continue;
  }
  if (EXEMPTED_FILES.has(fileLocation)) {
    continue;
  }
