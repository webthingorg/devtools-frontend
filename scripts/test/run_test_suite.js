...
function getGclientAwareTestSuitePath() {
  ...
  let testSuitePathParts = pathInput.split('/');
  if (testSuitePathParts[0] !== 'gen') {
    err('Failed: Test suite path must start with gen/');
    process.exit(1);
  }

  if (yargsObject['gclient-root'] !== yargsObject['cwd']) {
    // devtools isn't the gclient root, its a dependency of another project (probably chromium)
    const testSuiteGenFilesPrefix = yargsObject['cwd'].substring(yargsObject['gclient-root'].length + 1);
...
function getAbsoluteTestSuitePath(target) {
  ...
  const fullPath = path.join(yargsObject['gclient-root'], 'out', target, ...testSuitePathParts);
...
function main() {
  ...
  const targetPath = path.join(yargsObject['gclient-root'], 'out', target);
