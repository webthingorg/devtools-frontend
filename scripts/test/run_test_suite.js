...
async function executeTestSuite({
  ...
  if (jobs > 1) {
    ...
    for (const k in testListOutputJSON['tests']) {
      testsList.push(testListOutputJSON['tests'][k]['fullTitle']);
    }
      ...
      return new Promise(async resolve => {
        ...
        while (test) {
          const task = childProcess.spawn(
              nodePath(), argumentsForNode.concat([`--fgrep='${testsList.pop()}'`]),
    ...
    const pool = [];
    for (let i = 0; i < 2 /* jobs*/; i++) {
      pool.push(getWorker(testsList));
    }
