...
async function executeTestSuite({
  ...
  if (jobs > 1) {
    ...
    const my_list = childProcess.spawnSync(nodePath(), argumentsForNode.concat(['--dry-run']), {encoding: 'utf-8'})
                        .stdout.split('\n');
    const tests_list = [];
    for (const k in my_list) {
      if (my_list[k].includes('✔')) {
        tests_list.push(my_list[k].slice(my_list[k].indexOf('✔') + 1, my_list[k].lastIndexOf(': NaNms')).trim());
      }
    }
