#!/usr/bin/env python3
# Copyright 2021 The DevTools Authors. All rights reserved.
...
GS_ROOT = 'gs://devtools-internal-screenshots'
...
def build_parser():
    ...
    trigger_parser = sp.add_parser('trigger',
            ...
            help=trigger_help)
    trigger_parser.add_argument('--ignore-triggered',
        action='store_true',
...
def trigger_screenshots(verbose, builders=None):
    ...
    try_command = 'git cl try -B devtools-internal/try ' + ' '.join(
...
def apply_patch_to_local(patchset, wait_sec, ignore_failed, retry, verbose):
    ...
    retry, should_wait = check_all_success(
            results, wait_sec, ignore_failed, retry)
    if retry:
...
def check_all_success(results, wait_sec, ignore_failed, retry):
    ...
    if in_progress:
        warn_on_exceptions(results, in_progress, WARNING_BUILDERS_STILL_RUNNING)
...
def download_patches(results, destination_dir, verbose):
    ...
    for builder, result in results.items():
        gs_path = [
            GS_FOLDER, builder,
            str(result['cl']), str(result['patch']),
            'screenshot.patch'
        ]
...
def git_apply_patch(patches, verbose):
    ...
    if screenshot_patches:
        run_command(['git', 'apply', *screenshot_patches], verbose,
