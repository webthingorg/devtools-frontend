...
async function requestHandler(request, response) {
  ...
  const fileExists = await promisify(fs.exists)(absoluteFilePath);
  if (!fileExists) {
  ...
  const headersFileExists = await promisify(fs.exists)(absoluteFilePath + '.headers');
  if (headersFileExists) {
    try {
      const headersFile = await promisify(fs.readFile)(absoluteFilePath + '.headers', 'utf8');
  ...
  try {
    data = await promisify(fs.readFile)(absoluteFilePath, encoding);
