...
import {ProtocolInterface, Protocol as P} from './protocol_schema';

const PROTOCOL_JSON_PATH = path.resolve(__dirname, '../../third_party/blink/public/devtools_protocol/browser_protocol.json');
...
// const jsProtocol: ProtocolInterface = require(path.resolve(process.cwd(), argv.jsProtocol));
// const browserProtocol: ProtocolInterface = require(path.resolve(process.cwd(), argv.browserProtocol));
const protocolDomains: P.Domain[] = protocolJson.domains;
...
// Main
const destProtocolFilePath = `${__dirname}/protocol.d.ts`;
...
const destMappingFilePath = `${__dirname}/protocol-mapping.d.ts`;
...
const destApiFilePath = `${__dirname}/protocol-proxy-api.d.ts`;
