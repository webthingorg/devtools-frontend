...
const emitHeaderComments = () => {
  ...
  emitLine(' * Re-generate with: npm run generate-protocol-type-defs.');
  emitLine(' */');
  emitLine();
};
...
const ensureFilesAreClangFormatted = (files: string[]) => {
  const clangFilesArg = files.join(' ');
  const clangCommand = `git cl format --js ${clangFilesArg}`;
  childProcess.spawnSync(clangCommand);
  console.log('Ran clang on protocol files', clangCommand);
};
...
const main = () => {
  ...
  ensureFilesAreClangFormatted([
    destProtocolFilePath,
    destMappingFilePath,
    destApiFilePath,
  ]);
};
