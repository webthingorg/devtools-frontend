...
const walkNode = (node: ts.Node, state?: WalkerState) => {
  if (!state) {
    state = {
  ...
  if (ts.isClassDeclaration(node)) {
      ...
      classNode.members.forEach(member => {
        if (ts.isMethodDeclaration(member)) {
          ...
          if (!isPrivate) {
            ...
            // now find its interfaces that we need to make public
    ...
    if (ts.isPropertyAccessExpression(node.expression.expression)) {
      ...
      const leftSideText = (propertyAcess.expression as ts.Identifier).escapedText.toString();
      const rightSideText = (propertyAcess.name as ts.Identifier).escapedText.toString();
