...
exports.walkTree = exports.filePathToTypeScriptSourceFile = exports.nodeIsReadOnlyArrayInterfaceReference = exports.nodeIsReadOnlyInterfaceReference = void 0;
const fs = require("fs");
const path = require("path");
const ts = require("typescript");
const utils_1 = require("./utils");
const classExtendsHTMLElement = (classNode) => {
 ...
 */
const nodeIsReadOnlyInterfaceReference = (node) => {
    return ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) && node.typeName.escapedText === 'Readonly';
};
exports.nodeIsReadOnlyInterfaceReference = nodeIsReadOnlyInterfaceReference;
/*
 * Detects if a Node is of type ReadonlyArray<X>.
 */
const nodeIsReadOnlyArrayInterfaceReference = (node) => {
    return ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) &&
        node.typeName.escapedText === 'ReadonlyArray';
};
exports.nodeIsReadOnlyArrayInterfaceReference = nodeIsReadOnlyArrayInterfaceReference;
/**
 ...
 */
const findTypeReferencesWithinNode = (state, node) => {
     ...
     */
    if (exports.nodeIsReadOnlyInterfaceReference(node) || exports.nodeIsReadOnlyArrayInterfaceReference(node)) {
    ...
    if (ts.isArrayTypeNode(node) && ts.isTypeReferenceNode(node.elementType) &&
        ...
        if (node.typeName.escapedText === 'Map' || node.typeName.escapedText === 'Set') {
            ...
            const referencesWithinGenerics = node.typeArguments.flatMap(node => [...findTypeReferencesWithinNode(state, node)]);
...
const taggedTemplateExpressionIsLitHtmlCall = (node) => {
...
const checkTemplateSpanForTypeCastOfData = (matchingSpan) => {
    ...
    const typeCastIsTypeReference = ts.isAsExpression(matchingSpan.expression) && ts.isTypeReferenceNode(matchingSpan.expression.type);
...
const walkNode = (node, startState) => {
    ...
    if (ts.isClassDeclaration(node)) {
        ...
        if (extendsHtmlElement) {
            ...
            node.members.forEach(member => {
                if (ts.isMethodDeclaration(member)) {
                    ...
                    const methodName = member.name.escapedText.toString();
        ...
        if (builtInTypeScriptTypes.has(interfaceName)) {
            throw new Error(`Found interface ${interfaceName} that conflicts with TypeScript's built-in type. Please choose a different name!`);
        }
        ...
        if (builtInTypeScriptTypes.has(typeName)) {
            throw new Error(`Found type ${typeName} that conflicts with TypeScript's built-in type. Please choose a different name!`);
        }
        state.foundInterfaces.add(node);
    }
        ...
        if (!allMembersAreExplictlyInitialized) {
            throw new Error(`Found enum ${node.name.escapedText.toString()} whose members do not have manually defined values.`);
        }
        state.foundEnums.add(node);
    }
    else if (ts.isImportDeclaration(node)) {
        const filePath = node.moduleSpecifier.text;
        ...
        else if (node.importClause && node.importClause.namedBindings && ts.isNamespaceImport(node.importClause.namedBindings)) {
...
const filePathToTypeScriptSourceFile = (filePath) => {
    return ts.createSourceFile(filePath, fs.readFileSync(filePath, { encoding: 'utf8' }), ts.ScriptTarget.ESNext);
};
exports.filePathToTypeScriptSourceFile = filePathToTypeScriptSourceFile;
const findNestedInterfacesInInterface = (state, interfaceDec) => {
    const foundNestedInterfaceNames = new Set();
...
const findNestedReferencesForTypeReference = (state, interfaceOrTypeAliasDeclaration) => {
    const foundNestedReferences = new Set();
        ...
        */
            interfaceOrTypeAliasDeclaration.type.types.forEach(nestedType => {
                if (ts.isTypeLiteralNode(nestedType)) {
                    ...
                    const nestedTypeReference = utils_1.findNodeForTypeReferenceName(state, typeReferenceName);
        ...
        if (interfaceOrTypeAliasDeclaration.heritageClauses) {
            interfaceOrTypeAliasDeclaration.heritageClauses.forEach(heritageClause => {
                ...
                extendNames.forEach(interfaceName => {
                    const interfaceDec = utils_1.findNodeForTypeReferenceName(state, interfaceName);
                    if (!interfaceDec) {
...
const populateTypeReferencesToConvert = (state) => {
    state.typeReferencesToConvert.forEach(typeReferenceName => {
        const typeDeclaration = utils_1.findNodeForTypeReferenceName(state, typeReferenceName);
...
const walkTree = (startNode, resolvedFilePath) => {
    ...
    const allFoundTypeReferencesNames = new Set([
        ...Array.from(state.foundInterfaces, foundInterface => {
            return foundInterface.name.escapedText.toString();
        }),
        ...Array.from(state.foundEnums, foundEnum => {
     ...
     * else, error loudly
     */
    const importsToCheck = new Set();
    missingTypeReferences.forEach(missingInterfaceName => {
        if (builtInTypeScriptTypes.has(missingInterfaceName)) {
            return;
        }
        const importForMissingInterface = Array.from(state.imports).find(imp => 'namedImports' in imp && imp.namedImports.has(missingInterfaceName));
        if (!importForMissingInterface) {
            throw new Error(`Could not find definition for type reference ${missingInterfaceName} in the source file or any of its imports.`);
        }
    ...
    importsToCheck.forEach(fullPathToImport => {
        const sourceFile = exports.filePathToTypeScriptSourceFile(fullPathToImport);
        const stateFromSubFile = exports.walkTree(sourceFile, fullPathToImport);
