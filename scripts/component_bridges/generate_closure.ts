...
const typeNodeToJSDocClosureType = (node: ts.TypeNode, options: TypeNodeToJSDocOptions): string => {
  const isOptional = options.nodeIsOptional;
  ...
  if (ts.isTypeReferenceNode(node)) {
    const paramString = ['!', paramValue, isOptional ? '=' : ''].join('');
...
export const generateClosureClass = (state: WalkerState): string[] => {
  ...
  state.getters.forEach(getter => {
    let jsDocForFunc = ['/**'];
    const name = (getter.name as ts.Identifier).escapedText.toString();
    ...
    // Only output JSDoc if the getter has an explicit return type
    if (getter.type) {
      output.push(jsDocForFunc.join('\n'));
    }

    output.push(indent(`get ${name}() {}`, 2));
  });
  ...
  state.setters.forEach(setter => {
    ...
    output.push(indent(`set ${name}(${setterParamName}) {}`, 2));
  });
