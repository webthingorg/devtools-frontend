...
export const generateClosureClass = (state: WalkerState): string[] => {
  ...
  // the line is used as a Closure typedoc.
  ...
  state.publicMethods.forEach(method => {
    ...
    method.parameters.forEach(param => {
      ...
      /* the rules for ? or ! inside params are subtle
       * when we are given an interface that's optional in TypeScript land
       * we need to tell Closure that it's optional _but not nullable_.
       * ?Foo in Closure would let Foo be null
       * so instead we go for {!Foo=} which states it's optional but not nullable.
       * but note that if we pass a union of Foo | Null, that should become {?Foo}, which is dealt with within valueForTypeNode().
       *
       * TS                    <|> Closure
