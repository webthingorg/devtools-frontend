...
exports.valueForTypeNode = exports.nodeIsPrimitive = void 0;
const ts = require("typescript");
const walk_tree_1 = require("./walk_tree");
...
const valueForTypeNode = (node, isFunctionParam = false) => {
    let value = '';
    if (ts.isTypeReferenceNode(node)) {
        /* These check for Readonly<X> or ReadonlyArray<X> and unwrap them
         * so we parse out the inner type instead.
         */
        if (walk_tree_1.nodeIsReadOnlyInterfaceReference(node) && node.typeArguments) {
            return exports.valueForTypeNode(node.typeArguments[0], isFunctionParam);
        }
        if (walk_tree_1.nodeIsReadOnlyArrayInterfaceReference(node) && node.typeArguments) {
            ...
            return exports.valueForTypeNode(arrayNode, isFunctionParam);
        }
        if (ts.isIdentifier(node.typeName)) {
            ...
            if (value === 'Map' && node.typeArguments) {
                const keyType = exports.valueForTypeNode(node.typeArguments[0]);
                let valueType = exports.valueForTypeNode(node.typeArguments[1]);
                if (!exports.nodeIsPrimitive(node.typeArguments[1])) {
                    valueType = `!${valueType}`;
                }
                value = `Map<${keyType}, ${valueType}>`;
            }
            else if (value === 'Set' && node.typeArguments) {
                let valueType = exports.valueForTypeNode(node.typeArguments[0]);
                if (!exports.nodeIsPrimitive(node.typeArguments[0])) {
                    valueType = `!${valueType}`;
                }
                value = `Set<${valueType}>`;
            }
        ...
        const isPrimitive = exports.nodeIsPrimitive(node.elementType);
        const modifier = isPrimitive ? '' : '!';
        value = `Array.<${modifier}${exports.valueForTypeNode(node.elementType)}>`;
    }
        ...
        if (isNullUnion) {
            ...
            const value = exports.valueForTypeNode(notNullNode);
            return `?${value}`;
        }
        const parts = node.types.map(n => exports.valueForTypeNode(n, isFunctionParam));
        return parts.join('|');
    }
    else if (ts.isFunctionTypeNode(node)) {
        let returnType = exports.valueForTypeNode(node.type);
        ...
        const params = node.parameters
            ...
            const valueForParam = exports.valueForTypeNode(param.type, true);
            if (exports.nodeIsPrimitive(param.type)) {
            ...
            return [
                '!',
                exports.valueForTypeNode(param.type, true),
                param.questionToken ? '=' : '',
            ].join('');
        })
            .join(', ');
        ...
        const members = node.members
            ...
            if (ts.isPropertySignature(member) && member.type) {
                ...
                return {
                    ...
                    value: requiredOptionalFlag + exports.valueForTypeNode(member.type, isFunctionParam),
                };
            }
            return null;
        })
...
exports.valueForTypeNode = valueForTypeNode;
//# sourceMappingURL=value_for_type_node.js.map