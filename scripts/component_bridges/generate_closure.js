...
exports.generateClosureBridge = exports.generateImports = exports.generateTypeReferences = exports.generateClosureClass = exports.generateCreatorFunction = void 0;
// Copyright 2020 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
const ts = require("typescript");
const utils_js_1 = require("./utils.js");
const value_for_type_node_js_1 = require("./value_for_type_node.js");
const generateCreatorFunction = (state) => {
    ...
    const call = state.customElementsDefineCall.expression;
    const customElementTagArgument = call.arguments[0];
...
exports.generateCreatorFunction = generateCreatorFunction;
const generatedClassInterfaceName = (componentClass) => {
...
const indent = (str, amountOfSpaces) => {
    return ' '.repeat(amountOfSpaces) + str;
};
const paramIsMarkedAsOptionalOrRequired = (param) => {
    const startChars = ['!', '?'];
    return startChars.some(c => param.startsWith(c));
};
const typeNodeToJSDocClosureType = (node, options) => {
    const paramValue = value_for_type_node_js_1.valueForTypeNode(node, true);
    ...
    if (ts.isArrayTypeNode(node)) {
        ...
        if (value_for_type_node_js_1.nodeIsPrimitive(internalType)) {
            output += value_for_type_node_js_1.valueForTypeNode(internalType, true);
        }
    ...
    if (needsOptionalModifier) {
        ...
        else if (value_for_type_node_js_1.nodeIsPrimitive(node) || ts.isTypeLiteralNode(node)) {
...
const generateClosureClass = (state) => {
    ...
    state.publicMethods.forEach(method => {
        ...
        const argsForFunc = method.parameters
            .map(param => {
            return param.name.escapedText.toString();
        })
            .join(', ');
        let jsDocForFunc = ['/**'];
        method.parameters.forEach(param => {
            ...
            const paramName = param.name.escapedText.toString();
    ...
    state.getters.forEach(getter => {
        let jsDocForFunc = ['/**'];
        const getterName = getter.name.escapedText.toString();
        if (!getter.type) {
    ...
    state.setters.forEach(setter => {
        let jsDocForFunc = ['/**'];
        const setterName = setter.name.escapedText.toString();
        if (setter.parameters.length === 0) {
            throw new Error(`Found invalid setter with no parameter: ${setterName}`);
        }
        const setterParamName = setter.parameters[0].name.escapedText.toString();
...
exports.generateClosureClass = generateClosureClass;
const generateInterfaceMembers = (members, interfaceName) => {
    const output = [];
    members.forEach(member => {
        if (ts.isPropertySignature(member)) {
            ...
            const keyIdentifer = member.name;
            const memberIsOptional = !!member.questionToken;
            const keyName = keyIdentifer.escapedText;
            let nodeValue = value_for_type_node_js_1.valueForTypeNode(member.type);
            ...
            if (memberIsOptional) {
                if (value_for_type_node_js_1.nodeIsPrimitive(member.type)) {
 ...
 */
const membersForTypeReference = (state, typeReference) => {
    ...
    const interfaceOrTypeAlias = utils_js_1.findNodeForTypeReferenceName(state, interfaceName);
...
const gatherMembersForInterface = (state, currentInterface) => {
    const allMembers = [];
    ...
    if (currentInterface.heritageClauses) {
        currentInterface.heritageClauses.forEach(heritageClause => {
            ...
            extendNames.forEach(interfaceName => {
                const interfaceDec = utils_js_1.findNodeForTypeReferenceName(state, interfaceName);
                if (!interfaceDec) {
                    throw new Error(`Could not find interface or type alias: ${interfaceName}`);
                }
...
const checkUnionTypeValid = (node, typeName) => {
...
const checkInterfaceMembersValid = (state, members, interfaceName) => {
    members.forEach(member => {
        if (ts.isPropertySignature(member)) {
            if (!member.type) {
                throw new Error(`Interface member ${ts.SyntaxKind[member.kind]} did not have a type key, aborting.`);
            ...
            if (ts.isTypeReferenceNode(member.type) && ts.isQualifiedName(member.type.typeName)) {
                const leftType = member.type.typeName.left;
                const leftNode = utils_js_1.findNodeForTypeReferenceName(state, leftType.escapedText.toString());
                if (!leftNode || !state.foundEnums.has(leftNode)) {
...
const generateClosureForInterface = (state, interfaceName) => {
    const typeReferenceNode = utils_js_1.findNodeForTypeReferenceName(state, interfaceName);
    ...
    if (ts.isInterfaceDeclaration(typeReferenceNode)) {
        ...
        const unionTypeConverted = typeReferenceNode.type.types.map(v => value_for_type_node_js_1.valueForTypeNode(v)).join('|');
      ...
      */
        ...
        typeReferenceNode.type.types.forEach(typePart => {
             ...
             */
                ...
                throw new Error(`Unsupported: a type extended something that the bridges generator doesn't understand: ${ts.SyntaxKind[typePart.kind]}`);
            }
        });
        /**
...
const generateTypeReferences = (state) => {
    const finalCode = [];
...
exports.generateTypeReferences = generateTypeReferences;
const getEnumType = (node) => {
...
const generateEnumOutput = (enumDeclaration) => {
    const enumOutput = [];
    ...
    enumDeclaration.members.forEach(member => {
        ...
        const enumValue = member.initializer && member.initializer.text;
...
const generateClosureBridge = (state) => {
    const result = {
        types: exports.generateTypeReferences(state),
        closureClass: exports.generateClosureClass(state),
        creatorFunction: exports.generateCreatorFunction(state),
        moduleImports: exports.generateImports(state),
    };
    return result;
};
exports.generateClosureBridge = generateClosureBridge;
//# sourceMappingURL=generate_closure.js.map