...
/**
 * Take a series of CTC format ICU messages and converts them to DTL format by
 ...
 * @return {Record<string, DtlMessage>}
 */
function bakePlaceholders(messages) {
  /** @type {Record<string, DtlMessage>} */
  const bakedMessages = {};
...
/**
 * @param {string} path
 * @param {Record<string, DtlMessage>} localeStrings
 */
function saveDtlStrings(path, localeStrings) {
...
function collectAndBakeCtcStrings(dir, outputDir) {
  ...
  for (const filename of fs.readdirSync(dir)) {
    ...
    if (filename.endsWith('.ctc.json')) {
      ...
      saveDtlStrings(outputFile, strings);
      dtlFilenames.push(path.basename(filename));
    }
  }
  return dtlFilenames;
}
