...
const {collectAndBakeCtcStrings} = require('./bakeCtcToDtl.js');

const SRC_ROOT = path.join(__dirname, '..', '..');
const UISTRINGS_REGEX = /\sUIStrings = .*?\};/s;

/** @typedef {import('./bakeCtcToDtl.js').CtcMessage} CtcMessage */
...
/**
 * Take a series of DTL format ICU messages and converts them
...
function getToken(node) {
  if (!node.initializer) throw new Error('no Token found');
  let token = '';
...
/**
 * Collects all DTL messsages defined in UIString from Javascript files in dir,
...
function collectAllStringsInDir(dir) {
  ...
  for (const relativeToRootPath of files) {
    ...
    for (const [key, parsed] of Object.entries(parsedMessages)) {
      ...
      const pathRelativeToFrontend = relativeToRootPath.slice(10);
...
function writeStringsToCtcFiles(locale, strings) {
  const localesDirectory = path.resolve(SRC_ROOT, 'scripts', 'localizationV2', 'locales');
  if (!fs.existsSync(localesDirectory)){
      fs.mkdirSync(localesDirectory);
      console.log(`Locales directory created at ${localesDirectory}.`);
  }
  const fullPath = path.join(localesDirectory, `${locale}.ctc.json`);
  /** @type {Record<string, CtcMessage>} */
  const output = {};
...
if (require.main === module) {
  ...
  // Bake the ctc en-US and en-XL files into en-US and en-XL DTL format
  const dtl = collectAndBakeCtcStrings(path.join(SRC_ROOT, 'scripts/localizationV2/locales/'),
    path.join(SRC_ROOT, 'scripts/localizationV2/locales/'));
  dtl.forEach(function(locale) {
    console.log(`Baked ${locale} into DTL format.`);
  });
}
