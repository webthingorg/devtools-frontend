...
function registerLocale(locale) {
  registeredLocale = lookupLocale(locale);
}

function getLocalizedString(str_, id, values) {
  const icuMessage = str_(id, values);
  return getFormatted(icuMessage, registeredLocale);
}

function registerUIStrings(path, UIStrings) {
  return (id, value) => {
    try {
      const i18nInstance = createMessageInstanceIdFn(path, UIStrings);
...
function collectAllCustomElementsFromICU(icuElements, seenElementsById = new Map()) {
  for (const el of icuElements) {
    ...
    if (!el.format || el.format.type !== 'pluralFormat') {
      continue;
    }
...
function _preformatValues(icuMessage, messageFormatter, values) {
  ...
  for (const {id, format} of argumentElements) {
    ...
    if (id && (id in values) === false) {
      throw new Error(
          `ICU Message "${icuMessage}" contains a value reference ("${id}") ` +
    ...
    if (typeof value !== 'number') {
      throw new Error(
          `ICU Message "${icuMessage}" contains a numeric reference ("${id}") ` +
  ...
  for (const valueId of Object.keys(values)) {
    ...
    throw new Error(
        `Provided value "${valueId}" does not match any placeholder in ` +
...
function _formatIcuMessage(locale, icuMessageId, uiStringMessage, values = {}) {
  ...
  if (!localeMessage && uiStringMessage) {
    ...
    if (!LOCALES['en-US'][icuMessageId] || localeMessage !== LOCALES['en-US'][icuMessageId].message) {
      console.log(
          'i18n',
          `Message "${icuMessageId}" does not match its 'en-US' counterpart. ` +
              'Run \'i18n\' to update.');
    }
  }
...
/** @param {string[]} path */
function _formatPathAsString(path) {
  let pathAsString = '';
  for (const property of path) {
    if (/^[a-z]+$/i.test(property)) {
      if (pathAsString.length) {
        pathAsString += '.';
      }
      pathAsString += property;
    } else {
      if (/]|"|'|\s/.test(property)) {
        throw new Error(`Cannot handle "${property}" in i18n`);
      }
      pathAsString += `[${property}]`;
    }
  }
...
/**
 * Register a file's UIStrings, return function to generate the string ids.
 *
...
function createMessageInstanceIdFn(filename, fileStrings) {
  ...
  const getMessageInstanceIdFn = (icuMessage, values) => {
    ...
    if (!keyname) {
      throw new Error(`Could not locate: ${icuMessage}`);
    }
...
function getFormattedFromIdAndValues(locale, icuMessageId, values) {
  ...
  if (!icuMessageIdRegex.test(icuMessageId)) {
    throw new Error('This is not an ICU message ID');
  }
...
function _resolveIcuMessageInstanceId(icuMessageInstanceId, locale) {
  ...
  if (!matches) {
    throw new Error(`${icuMessageInstanceId} is not a valid message instance ID`);
  }
  ...
  const {formattedString} =
      _formatIcuMessage(locale, icuMessageId, icuMessageInstance.icuMessage, icuMessageInstance.values);
 ...
 * string references and replace them with their localized values.
 * @param {*} inputObject
 * @param {Locale} locale
 * @return {I18NMessages}
 */
function replaceIcuMessageInstanceIds(inputObject, locale) {
  /**
   * @param {*} subObject
   * @param {I18NMessages} icuMessagePaths
   * @param {string[]} path
   */
  function replaceInObject(subObject, icuMessagePaths, path = []) {
    if (typeof subObject !== 'object' || !subObject) {
      return;
    }
    ...
    for (const [property, value] of Object.entries(subObject)) {
      ...
      // Check to see if the value looks like a string reference. If it is, replace it.
      if (typeof value === 'string' && isIcuMessage(value)) {
        ...
        const messageInstances = icuMessagePaths[icuMessageInstance.icuMessageId] || [];
        const currentPathAsString = _formatPathAsString(currentPath);

        messageInstances.push(
            icuMessageInstance.values ? {values: icuMessageInstance.values, path: currentPathAsString} :
                                        currentPathAsString);

        subObject[property] = formattedString;
        icuMessagePaths[icuMessageInstance.icuMessageId] = messageInstances;
      } else {
        replaceInObject(value, icuMessagePaths, currentPath);
      }
    }
  }
...
/** @typedef {import('./locales').DtlMessages} dtlMessages */

/**
 * Populate the i18n string lookup dict with locale data
 * Used when the host environment selects the locale and serves the intended locale file
 * @see https://docs.google.com/document/d/1jnt3BqKB-4q3AE94UWFA0Gqspx8Sd_jivlB7gQMlmfk/edit
 * @param {Locale} locale
 * @param {dtlMessages} dtlMessages
 */
function registerLocaleData(locale, dtlMessages) {
  LOCALES[locale] = dtlMessages;
}
 ...
 * @return {I18NStrings}
 */
function getFormattedStrings(locale) {
  const localeMessages = LOCALES[locale];
  if (!localeMessages) {
    throw new Error(`Unsupported locale '${locale}'`);
  }

  const icuMessageIds = Object.keys(localeMessages).filter(f => f.includes('front_end'));
  const strings = /** @type {I18NStrings} */ ({});
  for (const icuMessageId of icuMessageIds) {
    // eslint-disable-next-line no-unused-vars
    const [filename, varName] = icuMessageId.split(' | ');

    const key = /** @type {keyof I18NStrings} */ (varName);
