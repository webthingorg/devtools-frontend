...
function rootPath() {
  ...
  if (path.basename(devtoolsFrontendPath) === 'renderer') {
    // TODO(1011259): remove Chromium repository handling
    // Chromium repository
    return path.dirname(path.dirname(path.dirname(devtoolsFrontendParentPath)));
  }
...
exports.thirdPartyPath = () => path.join(rootPath(), 'third_party');
...
exports.nodePath = () => {
  ...
  return path.join(exports.thirdPartyPath(), 'node', paths[os.platform]);
};

exports.devtoolsRootPath = () => {
  return path.dirname(path.dirname(ABS_PATH_TO_CURRENT_FILE));
};

exports.nodeModulesPath = () => {
  return path.join(exports.devtoolsRootPath(), 'node_modules');
};
