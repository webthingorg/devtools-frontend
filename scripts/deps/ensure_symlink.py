...
# ensure your .gclient file that manages chromium.src contains after your
# list of solutions:
#
# hooks = [
#   {
#     # Ensure devtools is symlinked in the correct location
...
def ensure_symlink(options):
    chromium_devtools_path = os.path.join(
        options.chromium_dir,
        os.path.normpath(DEVTOOLS_FRONTEND_CHROMIUM_LOCATION))
    if not os.path.exists(chromium_devtools_path):
        symlink(optisons.devtools_dir, chromium_devtools_path)
