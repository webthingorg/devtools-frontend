...
const COMPUTING_COVERAGE_PREFIX = '/compute-coverage/';
...
async function requestHandler(request, response) {
  let filePath = parseURL(request.url).pathname;
  if (filePath === '/favicon.ico') {
    send404(response, '404, no favicon');
    return;
  }

  let isComputingCoverageRequest = false;

  if (filePath.startsWith(COMPUTING_COVERAGE_PREFIX)) {
    filePath = filePath.replace(COMPUTING_COVERAGE_PREFIX, '/');
    isComputingCoverageRequest = true;
  }
      ...
      if (previouslyGeneratedInstrumentedSource) {
        ...
        if (!isComputingCoverageRequest) {
          ...
          console.error(
              ...
              'of your interactions test, before declaring any tests. Alternatively, you can open the current page ' +
              `and prepend "${COMPUTING_COVERAGE_PREFIX}" to your current URL path.
              `);
          return;
        }
