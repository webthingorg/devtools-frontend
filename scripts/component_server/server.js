...
function createComponentIndexFile(componentPath, componentExamples) {
  const componentName = componentPath.replace('/component_docs/', '').replace(/_/g, ' ');
  // clang-format off
  return `<!DOCTYPE html>
  <html>
    <head>
    ...
    <body>
      ...
      ${componentExamples.map(example => {
        const fullPath = path.join('/front_end', componentPath, example);
    ...
    <body>
      ...
      <ul>
        ${componentNames.map(name => {
          ...
          return `<li><a href='/component_docs/${name}'>${niceName}</a></li>`;
        }).join('\n')}
      </ul>
    </body>
  </html>`;
...
async function getExamplesForPath(filePath) {
  const componentDirectory = path.join(devtoolsRootFolder, 'front_end', filePath);
...
async function requestHandler(request, response) {
  ...
  } else if (filePath.startsWith('/component_docs') && path.extname(filePath) === '') {
