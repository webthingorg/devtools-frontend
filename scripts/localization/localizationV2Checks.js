...
async function checkUIStrings(shouldAutoFix) {
  ...
  for (const [filePath, uiStringsEntries] of uiStringsMap.entries()) {
    let errorList;
    if (filePath.endsWith('heap_snapshot_worker\\ModuleUIStrings.js')) {
      // heap_snapshot is a web worker and has its own memory space, it's strings are
      // not displayed but serialized and send over to the profiler to be displayed.
      // To keep the strings in sync we declared them here and reuse the structure
      // in the profiler.
      continue;
    } else if (filePath.endsWith('ModuleUIStrings.js')) {
