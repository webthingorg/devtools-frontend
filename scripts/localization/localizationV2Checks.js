...
async function autoFixUIStringsCheck(errorMap) {
  ...
  for (const [filePath, unusedUIStringsEntries] of errorMap.entries()) {
    ...
    const textToRemoveList = await getTextToRemove(filePath, content, unusedUIStringsEntries);
    ...
    if (node.kind === ts.SyntaxKind.VariableDeclaration && node.name.escapedText === 'UIStrings') {
      ...
      for (const property of node.initializer.properties) {
        for (const entry of unusedUIStringsEntries) {
          if (property.name.escapedText === entry.stringId) {
            // get the full text of the entry including descriptions and placeholders
            textToRemoveList.push(property.getFullText());
            uiStringsNodeFound = true;
          }
        }
      }
    }
    if (!uiStringsNodeFound) {
...
function checkNoV1CallsInMigratedDir() {
  ...
  if (filesContainV1Calls.size > 0) {
    ...
    fileMigratedError += `\nFor example:
      ls("An example string") ---> common.i18n.getLocalizedString(_str, UIStrings.theExampleString)
