...
describe('getLocalizationCaseAndVersion', () => {
  ...
  it('returns {locCase: "Common.i18n.getLocalizedString", locVersion: 2} for Common.i18n.getLocalizedString', () => {
    const ast = parseCode('Common.i18n.getLocalizedString(_str, UIStrings.fakeID)');
    assert.deepStrictEqual(
        getLocalizationCaseAndVersion(ast.body[0].expression),
        {locCase: 'Common.i18n.getLocalizedString', locVersion: 2});
  });
  it('returns locCase: "Common.i18n.getFormatLocalizedString", locVersion: 2} for Common.i18n.getFormatLocalizedString',
     () => {
       const ast = parseCode('Common.i18n.getFormatLocalizedString(_str, UIStrings.fakeID)');
       assert.deepStrictEqual(
           getLocalizationCaseAndVersion(ast.body[0].expression),
           {locCase: 'Common.i18n.getFormatLocalizedString', locVersion: 2});
     });
...
describe('findUIStringsNode', () => {
  it('assert the result is not null if UIStrings exists', () => {
    ...
    assert(findUIStringsNode(sourceFile) !== null);
  });
  it('assert the result is null if UIStrings does not exist', () => {
