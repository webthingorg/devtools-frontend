...
function shouldParseDirectory(directoryPath) {
  let result = true;
  for (const exclusionPath of excludeDirs) {
    if (path.normalize(directoryPath).includes(path.normalize(exclusionPath))) {
      result = false;
      break;
    }
  }
