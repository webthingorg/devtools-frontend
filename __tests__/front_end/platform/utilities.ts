...
describe('Utilities', () => {
  ...
  it('calculates the lower bound', () => {
    let testArrays = [[], [1], [-1, -1, 0, 0, 0, 0, 2, 3, 4, 4, 4, 7, 9, 9, 9]];
  ...
  it('q selects', () => {
    ...
    function testArray(array) {
      ...
      let sorted = array.slice(0).sort(compare);

      let reference = {min: sorted[0], median: sorted[Math.floor(sorted.length / 2)], max: sorted[sorted.length - 1]};
  ...
  it('sorts ranges', () => {
    let testArrays = [[], [1], [2, 1], [6, 4, 2, 7, 10, 15, 1], [10, 44, 3, 6, 56, 66, 10, 55, 32, 56, 2, 5]];
  ...
  it('sorts natural order', () => {
    ...
    for (let i = 0, n = testArray.length; i < n; ++i) {
      for (let j = 0; j < n; ++j) {
        let a = testArray[i];
        let b = testArray[j];
        let diff = String.naturalOrderComparator(a, b);
        if (diff === 0)
  ...
  it('hashes strings', () => {
    let stringA = ' '.repeat(10000);
    let stringB = stringA + ' ';
    let hashA = String.hashCode(stringA);
  ...
  it('trims URLs', () => {
    let baseURLDomain = 'www.chromium.org';
    let testArray = [
  ...
  it('converts to base64', () => {
    ...
    for (let i = 0; i < testArray.length; i += 2) {
      let string = testArray[i];
      let encodedString = testArray[i + 1];
  ...
  it('trims the middle of strings', () => {
    ...
    for (let string of testArray) {
      for (let maxLength = string.length + 1; maxLength > 0; --maxLength) {
        let trimmed = string.trimMiddle(maxLength);
