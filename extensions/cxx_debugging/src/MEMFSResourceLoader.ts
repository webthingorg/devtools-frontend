...
import type * as WorkerRPC from './WorkerRPC.js';

export class ResourceLoader implements DWARFSymbols.ResourceLoader {
  protected async fetchSymbolsData(rawModule: DWARFSymbols.RawModule, url: URL, hostInterface: WorkerRPC.HostInterface):
    ...
    if (symbolsResponse.ok) {
      ...
      try {
        ...
        const errorMessage = `Failed to fetch dwp file: ${e}`;
        await hostInterface.reportResourceLoad(dwpUrl, {success: false, errorMessage});
      ...
      if (!(symbolsDwpResponse && symbolsDwpResponse.ok)) {
        ...
        const statusText =
            symbolsDwpResponse ? symbolsDwpResponse?.statusText || `status code ${symbolsDwpResponse.status}` : '';
        await hostInterface.reportResourceLoad(
            dwpUrl, {success: false, errorMessage: `Failed to fetch dwp file: ${statusText}`});
      }
      ...
      await hostInterface.reportResourceLoad(url.href, {success: true, size: symbolsData.byteLength});
      if (symbolsDwpData) {
        await hostInterface.reportResourceLoad(`${url.href}.dwp`, {success: true, size: symbolsDwpData.byteLength});
      }
    ...
    if (rawModule.url !== url.href) {
      ...
      await hostInterface.reportResourceLoad(url.href, {success: false, errorMessage});
  ...
  async loadSymbols(
      ...
      hostInterface: WorkerRPC.HostInterface):
