...
export class Debugger {
  ...
  getWasmLocal(local: number, stopId: bigint): Promise<WasmValue> {
    return this.evaluateOnCallFrameId<WasmValue>(
        true, this.convertWasmValue, `locals[${Number(local)}]`, this.getCallFrameId(stopId));
  }
  getWasmGlobal(global: number, stopId: bigint): Promise<WasmValue> {
    return this.evaluateOnCallFrameId<WasmValue>(
        true, this.convertWasmValue, `globals[${Number(global)}]`, this.getCallFrameId(stopId));
  }
  getWasmOp(op: number, stopId: bigint): Promise<WasmValue> {
    return this.evaluateOnCallFrameId<WasmValue>(
        true, this.convertWasmValue, `operands[${Number(op)}]`, this.getCallFrameId(stopId));
  }
}
