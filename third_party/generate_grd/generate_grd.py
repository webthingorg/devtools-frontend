...
def _generate_include_row(grd_prefix, filename, pathname, \
    ...
    type = 'BINDATA'  # TODO: solve chrome_html
    ...
    if ('vulcanized' in pathname or 'crisper' in pathname):
        return GRD_INCLUDE_TEMPLATE_PP.format(file=pathname,
                                              path=resource_path,
                                              name=name,
                                              type=type)

    return GRD_INCLUDE_TEMPLATE.format(file=pathname,
                                       path=resource_path,
                                       name=name,
                                       type=type)
...
def main(argv):
    ...
    if args.grdp_files != None:
        for grdp_file in args.grdp_files:
            grdp_path = os.path.relpath(grdp_file,
                                        os.path.dirname(args.out_grd))
    ...
    if args.input_files != None:
        ...
        for filename in args.input_files:
            ...
            grd_file.write(
                _generate_include_row(args.grd_prefix, filename, filepath,
                                      resource_path_rewrites,
        ...
        for manifest_file in args.manifest_files:
            ...
            with open(manifest_path, 'r') as f:
                ...
                base_dir = os.path.normpath(
                    os.path.join(_CWD, data['base_dir']))
