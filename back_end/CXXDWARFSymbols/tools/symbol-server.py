...
PORT = 8888
SymbolServer = [options.tool, *itertools.chain.from_iterable(('-I', '{0}'.format(subst)) for subst in options.I)]

print('Executing {0}'.format(' '.join(SymbolServer)))
LC = subprocess.Popen(SymbolServer, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
StdIn = LC.stdin
StdOut = LC.stdout
...
class SymbolServerServerHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self, *args):
        ...
        StdIn.write('Content-Length: {0}\r\n\r\n{1}'.format(len(Input), Input).encode())
        StdIn.flush()
...
Tool = Observer()
Tool.schedule(ToolWatchdog(), path=os.path.dirname(options.tool), recursive=False)
Tool.start()

while True:
    try:
        with socketserver.TCPServer(('127.0.0.1', options.port), SymbolServerServerHandler) as httpd:
