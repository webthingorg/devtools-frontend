...
def stage1(source_dir, OPTIONS):
    ...
    subprocess.check_call([
        ...
        '-DSYMBOL_SERVER_BUILD_FORMATTERS=ON', *goma_args
    ],
                          cwd=binary_dir)
    if OPTIONS.check:
        subprocess.check_call(['autoninja', 'clang', 'check-symbol-server'],
                              cwd=binary_dir)
    else:
        subprocess.check_call(['autoninja', 'clang', 'all'], cwd=binary_dir)
    return binary_dir
...
def stage2(source_dir, stage1_dir, OPTIONS):
    ...
    print(' '.join([
        ...
        '-DSYMBOL_SERVER_BUILD_FORMATTERS=ON', '-DLLVM_TABLEGEN={}'.format(
            os.path.join(llvm_tools_dir,
                         'llvm-tblgen')), '-DCLANG_TABLEGEN={}'.format(
                             os.path.join(llvm_tools_dir, 'clang-tblgen')),
        '-DLLDB_TABLEGEN={}'.format(os.path.join(llvm_tools_dir,
                                                 'lldb-tblgen')),
        '-DNATIVE_LLVM={}'.format(llvm_tools_dir)
    ]))
    subprocess.check_call([
        'cmake', source_dir, '-DCMAKE_EXPORT_COMPILE_COMMANDS=ON', '-GNinja',
        '-DCMAKE_BUILD_TYPE=Release', '-DCMAKE_TOOLCHAIN_FILE={}'.format(
        ...
        '-DSYMBOL_SERVER_BUILD_FORMATTERS=ON', '-DLLVM_TABLEGEN={}'.format(
            os.path.join(llvm_tools_dir,
                         'llvm-tblgen')), '-DCLANG_TABLEGEN={}'.format(
                             os.path.join(llvm_tools_dir, 'clang-tblgen')),
        '-DLLDB_TABLEGEN={}'.format(os.path.join(
            llvm_tools_dir,
            'lldb-tblgen')), '-DNATIVE_LLVM={}'.format(llvm_tools_dir)
    ],
                          cwd=binary_dir)
    if OPTIONS.check:
        subprocess.check_call(['autoninja', 'all', 'check-symbol-server'],
                              cwd=binary_dir)
    else:
        subprocess.check_call(['autoninja', 'all'], cwd=binary_dir)
    return binary_dir
