...
template <>
size_t index_traits<Module>::size(Module& c) {  // NOLINT
  return c.GetNumCompileUnits();
}
template <>
auto index_traits<Module>::at(Module& c, size_t n) {  // NOLINT
...
llvm::SmallVector<std::string, 1> WasmModule::GetSourceScripts() const {
  ...
  for (size_t idx = 0; idx < module_->GetNumCompileUnits(); idx++) {
    ...
    for (auto f : Indexed(compile_unit->GetSupportFiles())) {
      ...
      if (!all_files.insert(std::make_pair(dir, filename)).second) { continue; }
...
lldb::VariableSP FindVariableAtOffset(lldb_private::Module* module,
  ...
  for (auto var : Indexed(var_list)) {
    if (var->GetName().GetStringRef() == name) { return var; }
  }
  var_list.Clear();
  module->FindGlobalVariables(lldb_private::RegularExpression(name), 1,
                              var_list);
  if (!var_list.Empty()) { return var_list.GetVariableAtIndex(0); }
  return {};
}
}  // namespace
...
llvm::Optional<std::string> ModuleCache::ResolveLocalModuleFile(
  ...
  if (llvm::sys::fs::exists(url)) { return url.str(); }
...
const WasmModule* ModuleCache::FindModule(llvm::StringRef script_id) const {
  ...
  if (i != modules_.end()) { return i->second.get(); }
  return nullptr;
}
...
const WasmModule* ModuleCache::GetModuleFromUrl(llvm::StringRef id,
                                                llvm::StringRef url) {
  if (auto m = FindModule(id)) return m;
...
llvm::Expected<Binary> WasmModule::GetVariableFormatScript(
    llvm::StringRef name,
    lldb::addr_t frame_offset) const {
  ...
  auto wasm_code = formatter.GenerateCode(&**code);
