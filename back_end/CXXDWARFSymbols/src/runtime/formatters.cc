...
void EmitValueInto(Value<T> v, StringSlice* output) {
  ...
  while (d != 0 && !i.Empty()) {
    ...
    if (x < 0) { x *= -1; }  // Avoid overflow: negate every digit individually.
    *i.begin() = digits[x];
    ++i;
    d /= 10;
  }
...
class Printer {
 ...
 private:
  ...
  friend Printer& operator<<(Printer& p, std::nullptr_t) {
    if (p.Valid()) { *p.output_window_ = '\0'; }
    return p;
  }

  friend Printer& operator<<(Printer& p, const char* msg) {
    for (const char* c = msg; *c != 0; ++c) {
      if (p.output_window_.Empty()) { break; }
