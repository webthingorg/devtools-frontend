...
export function buildHierarchy(
  ...
  for (const [pid, process] of processes) {
    for (const [tid, thread] of process.threads) {
      ...
      const samplesIntegrator = cpuProfile && new Helpers.CPUSampling.SamplesIntegrator(cpuProfile, pid, tid);
      if (cpuProfile) {
        profileCalls = samplesIntegrator?.callsFromProfileSamples(pid, tid) || [];
      }
      const events = [...profileCalls, ...thread.events];
      // Step 2. Massage the data.
      Helpers.Trace.sortTraceEventsInPlace(events);
      // Step 2. Build the tree.
      thread.tree = treify(events, options, samplesIntegrator);
      const constructedProfileCalls = samplesIntegrator?.getConstructedProfileCalls() || [];
      thread.tree.entries = [...thread.tree.entries, ...constructedProfileCalls];
      Helpers.Trace.sortTraceEventsInPlace(thread.tree.entries);
    }
  }
}
...
export function treify(
    events: (RendererEntry)[], options: {filter: {has: (name: Types.TraceEvents.KnownEventName) => boolean}},
    samplesIntegrator?: Helpers.CPUSampling.SamplesIntegrator): RendererEventTree {
  const stack = [];
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (stack.length === 0) {
      ...
      samplesIntegrator?.onTraceEventStart(event);
    ...
    if (startsAfterParent) {
      samplesIntegrator?.onTraceEventEnd(parentEvent);
      stack.pop();
      i--;
    ...
    if (Types.TraceEvents.isProfileCall(event)) {
      samplesIntegrator?.onProfileCall(event, parentEvent);
      continue;
    }
