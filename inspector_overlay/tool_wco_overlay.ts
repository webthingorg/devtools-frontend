...
export class WCOOverlay extends Overlay {
  ...
  override install() {
    ...
    const wcoIcons = ['chevron', 'ellipsis', 'minimize', 'maximize', 'close'];

    for (const iconName of wcoIcons) {
  ...
  drawWindowControlsOverlay(config: WCOConfig) {
    // need to user proper type for config
    if (config.showCSS) {
      // now hardcoded to true in inspector_overlay_agent.cc
  ...
  drawTitlebarAreaCSSVarsForPlatform(platfom: string) {
    ...
    switch (platfom) {
      case 'Windows':
        ...
        text = `titlebar-area-width: ${formatNumber(viewportSize.width - 238)}px \n titlebar-area-height: 33px`;
        break;
      case 'Mac':
        ...
        text = `titlebar-area-width: ${formatNumber(viewportSize.width - 185)}px \n titlebar-area-height: 40px`;
        break;
      case 'Linux':
        ...
        text = `titlebar-area-width: ${formatNumber(viewportSize.width - 196)}px \n titlebar-area-height: 34px`;
        break;
      default:
        // Default to Windows
        text = `titlebar-area-width: ${formatNumber(viewportSize.width - 238)}px \n titlebar-area-height: 33px`;
