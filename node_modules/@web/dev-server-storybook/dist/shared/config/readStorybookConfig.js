...
function validateMainJs(mainJs) {
    ...
    if (mainJs.addons != null) {
        ...
        if (mainJs.addons.some(addon => addon.startsWith('@storybook'))) {
            throw createError('Official storybook addons are not es modules, and cannot be loaded from this storybook implementation.');
        }
    }
    return mainJs;
}
export async function readStorybookConfig(pluginConfig) {
    ...
    if (!mainJsExists && !commonJsMainExists) {
        throw createError(`Could not find any storybook configuration at ${mainJsPath} or ${commonJsMainPath}. You can change the storybook config directory using the configDir option.`);
    }
