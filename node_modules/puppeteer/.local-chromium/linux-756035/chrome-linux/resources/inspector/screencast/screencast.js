import{SDKModel,DOMModel,OverlayModel,ResourceTreeModel,NetworkManager,EmulationModel,ScreenCaptureModel}from'../sdk/sdk.js';import{UIString,Settings,Color,ResourceType}from'../common/common.js';import{Widget,KeyboardShortcut,ARIAUtils,UIUtils,Toolbar,RootView,SplitWidget}from'../ui/ui.js';import{InspectorFrontendHost}from'../host/host.js';class InputModel extends SDKModel.SDKModel{constructor(target){super(target);this._inputAgent=target.inputAgent();this._activeTouchOffsetTop=null;this._activeTouchParams=null;}
...
dimensions.width*=window.devicePixelRatio;dimensions.height*=window.devicePixelRatio;this._screenCaptureModel.startScreencast('jpeg',80,Math.floor(Math.min(maxImageDimension,dimensions.width)),Math.floor(Math.min(maxImageDimension,dimensions.height)),undefined,this._screencastFrame.bind(this),this._screencastVisibilityChanged.bind(this));for(const emulationModel of SDKModel.TargetManager.instance().models(EmulationModel.EmulationModel)){emulationModel.overrideEmulateTouch(true);}
if(this._overlayModel){this._overlayModel.setHighlighter(this);}}
_stopCasting(){if(!this._isCasting){return;}
this._isCasting=false;this._screenCaptureModel.stopScreencast();for(const emulationModel of SDKModel.TargetManager.instance().models(EmulationModel.EmulationModel)){emulationModel.overrideEmulateTouch(false);}
...
_onSuspendStateChange(event){if(SDKModel.TargetManager.instance().allTargetsSuspended()){this._stopCasting();}else{this._startCasting();}
this._updateGlasspane();}
_updateGlasspane(){if(this._targetInactive){this._glassPaneElement.textContent=UIString.UIString('The tab is inactive');this._glassPaneElement.classList.remove('hidden');}else if(SDKModel.TargetManager.instance().allTargetsSuspended()){this._glassPaneElement.textContent=UIString.UIString('Profiling in progress');this._glassPaneElement.classList.remove('hidden');}else{this._glassPaneElement.classList.add('hidden');}}
...
return;}
const position=this._convertIntoScreenSpace(event);const node=await this._domModel.nodeForLocation(Math.floor(position.x/this._pageScaleFactor+this._scrollOffsetX),Math.floor(position.y/this._pageScaleFactor+this._scrollOffsetY),Settings.Settings.instance().moduleSetting('showUAShadowDOM').get());if(!node){return;}
...
const shortcutKey=KeyboardShortcut.KeyboardShortcut.makeKeyFromEvent((event));const handler=this._shortcuts[shortcutKey];if(handler&&handler(event)){event.consume();return;}
...
if(!node&&object){const domModel=object.runtimeModel().target().model(DOMModel.DOMModel);if(domModel){node=await domModel.pushObjectAsNodeToFrontend(object);}}
...
return Color.Color.fromRGBA([color.r,color.g,color.b,color.a]).asString(Color.Format.RGBA)||'';}
...
_createNavigationBar(){this._navigationBar=this.element.createChild('div','screencast-navigation');this._navigationBack=this._navigationBar.createChild('button','back');this._navigationBack.disabled=true;ARIAUtils.setAccessibleName(this._navigationBack,ls`back`);this._navigationForward=this._navigationBar.createChild('button','forward');this._navigationForward.disabled=true;ARIAUtils.setAccessibleName(this._navigationForward,ls`forward`);this._navigationReload=this._navigationBar.createChild('button','reload');ARIAUtils.setAccessibleName(this._navigationReload,ls`reload`);this._navigationUrl=UIUtils.createInput();ARIAUtils.setAccessibleName(this._navigationUrl,ls`Address bar`);this._navigationBar.appendChild(this._navigationUrl);this._navigationUrl.type='text';this._navigationProgressBar=new ProgressTracker(this._resourceTreeModel,this._networkManager,this._navigationBar.createChild('div','progress'));if(this._resourceTreeModel){this._navigationBack.addEventListener('click',this._navigateToHistoryEntry.bind(this,-1),false);this._navigationForward.addEventListener('click',this._navigateToHistoryEntry.bind(this,1),false);this._navigationReload.addEventListener('click',this._navigateReload.bind(this),false);this._navigationUrl.addEventListener('keyup',this._navigationUrlKeyUp.bind(this),true);this._requestNavigationHistory();this._resourceTreeModel.addEventListener(ResourceTreeModel.Events.MainFrameNavigated,this._requestNavigationHistoryEvent,this);this._resourceTreeModel.addEventListener(ResourceTreeModel.Events.CachedResourcesLoaded,this._requestNavigationHistoryEvent,this);}}
...
_requestNavigationHistoryEvent(event){this._requestNavigationHistory();}
...
InspectorFrontendHost.InspectorFrontendHostInstance.inspectedURLChanged(url);this._navigationUrl.value=url;}
_focusNavigationBar(){this._navigationUrl.focus();this._navigationUrl.select();return true;}}
const _bordersSize=44;const _navBarHeight=29;const _HttpRegex=/^http:\/\/(.+)/;const _SchemeRegex=/^(https?|about|chrome):/;class ProgressTracker{constructor(resourceTreeModel,networkManager,element){this._element=element;if(resourceTreeModel){resourceTreeModel.addEventListener(ResourceTreeModel.Events.MainFrameNavigated,this._onMainFrameNavigated,this);resourceTreeModel.addEventListener(ResourceTreeModel.Events.Load,this._onLoad,this);}
if(networkManager){networkManager.addEventListener(NetworkManager.Events.RequestStarted,this._onRequestStarted,this);networkManager.addEventListener(NetworkManager.Events.RequestFinished,this._onRequestFinished,this);}}
...
const request=(event.data);if(request.type===ResourceType.resourceTypes.WebSocket){return;}
...
var ScreencastView$1=Object.freeze({__proto__:null,ScreencastView:ScreencastView,_bordersSize:_bordersSize,_navBarHeight:_navBarHeight,_HttpRegex:_HttpRegex,_SchemeRegex:_SchemeRegex,ProgressTracker:ProgressTracker});let _appInstance;class ScreencastApp{constructor(){this._enabledSetting=Settings.Settings.instance().createSetting('screencastEnabled',true);this._toggleButton=new Toolbar.ToolbarToggle(UIString.UIString('Toggle screencast'),'largeicon-phone');this._toggleButton.setToggled(this._enabledSetting.get());this._toggleButton.setEnabled(false);this._toggleButton.addEventListener(Toolbar.ToolbarButton.Events.Click,this._toggleButtonClicked,this);SDKModel.TargetManager.instance().observeModels(ScreenCaptureModel.ScreenCaptureModel,this);}
static _instance(){if(!_appInstance){_appInstance=new ScreencastApp();}
return _appInstance;}
presentUI(document){const rootView=new RootView.RootView();this._rootSplitWidget=new SplitWidget.SplitWidget(false,true,'InspectorView.screencastSplitViewState',300,300);this._rootSplitWidget.setVertical(true);this._rootSplitWidget.setSecondIsSidebar(true);this._rootSplitWidget.show(rootView.element);this._rootSplitWidget.hideMain();this._rootSplitWidget.setSidebarWidget(self.UI.inspectorView);self.UI.inspectorView.setOwnerSplit(this._rootSplitWidget);rootView.attachToDocument(document);rootView.focus();}
modelAdded(screenCaptureModel){if(this._screenCaptureModel){return;}
this._screenCaptureModel=screenCaptureModel;this._toggleButton.setEnabled(true);this._screencastView=new ScreencastView(screenCaptureModel);this._rootSplitWidget.setMainWidget(this._screencastView);this._screencastView.initialize();this._onScreencastEnabledChanged();}
...
class ToolbarButtonProvider{item(){return ScreencastApp._instance()._toggleButton;}}
class ScreencastAppProvider{createApp(){return ScreencastApp._instance();}}
var ScreencastApp$1=Object.freeze({__proto__:null,ScreencastApp:ScreencastApp,ToolbarButtonProvider:ToolbarButtonProvider,ScreencastAppProvider:ScreencastAppProvider});export{InputModel$1 as InputModel,ScreencastApp$1 as ScreencastApp,ScreencastView$1 as ScreencastView};