...
class ExpandableTextPropertyValue extends ObjectPropertyValue{constructor(element,text,maxLength){super(element);const container=element.createChild('span');this._text=text;this._maxLength=maxLength;container.textContent=text.slice(0,maxLength);container.title=`${text.slice(0, maxLength)}...`;this._expandElement=container.createChild('span');this._maxDisplayableTextLength=10000000;const encoder=new TextEncoder();const buffer=encoder.encode(text);const totalBytes=Number.bytesToString(buffer.byteLength);if(this._text.length<this._maxDisplayableTextLength){this._expandElementText=ls`Show more (${totalBytes})`;this._expandElement.setAttribute('data-text',this._expandElementText);this._expandElement.classList.add('expandable-inline-button');this._expandElement.addEventListener('click',this._expandText.bind(this));this._expandElement.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){this._expandText();}});ARIAUtils.markAsButton(this._expandElement);}else{this._expandElement.setAttribute('data-text',ls`long text was truncated (${totalBytes})`);this._expandElement.classList.add('undisplayable-text');}
