...
function rule(blocklistInput, options) {
	const blocklist = [].concat(blocklistInput);
	...
	return (root, result) => {
		...
		function check(node, nodeIndex, valueNode, input, option) {
			...
			if (!unit || (unit && !blocklist.includes(unit.toLowerCase()))) {
				return;
			}
