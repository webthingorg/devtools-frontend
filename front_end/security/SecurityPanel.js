...
Security.SecurityPanel = class extends UI.PanelWithSidebar {
  ...
  _updateSecurityState(newSecurityState, explanations, summary) {
    // Note: please use _updateVisibleSecurityState instead.
  ...
  _onSecurityStateChanged(event) {
    // Note: please use _onVisibleSecurityStateChanged instead.
...
Security.SecurityMainView = class extends UI.VBox {
  ...
  updateSecurityState(newSecurityState, explanations, summary) {
    // Note: please use updateVisibleSecurityState instead.
    // Remove old state.
  ...
  _explainSafetyTipSecurity(visibleSecurityState, summary, explanations) {
    ...
    if (securityStateIssueIds.includes('bad-reputation')) {
      ...
      currentExplanations.push({
        ...
        description: ls`This site's hostname looks similar to ${
            safetyTipInfo
                .safeUrl}. Attackers sometimes mimic sites by making small, hard-to-see changes to the domain name.\n\nIf you believe this is shown in error please visit https://bugs.chromium.org/p/chromium/issues/entry?template=Safety+Tips+Appeals.`
      });
    }
