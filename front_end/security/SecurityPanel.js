...
export class SecurityOriginView extends UI.Widget.VBox {
   ...
   * @return {!Element}
   */
  _createConnectionSection(securityDetails) {
   ...
   * @return {!Element}
   */
  _createCertificateSection(securityDetails, forSignedExchange) {
    ...
    if (forSignedExchange) {
      certificateDiv.textContent = ls`Certificate of Signed Exchange`;
    } else {
   ...
   * @return {?Element}
   */
  _maybeCreateSctSection(securityDetails, forSignedExchange) {
    ...
    if (forSignedExchange) {
      sctDiv.textContent = ls`Certificate Transparency of Signed Exchange`;
    } else {
  ...
  */
  updateOriginState(originState) {
    ...
    if (originState.securityDetails) {
      this.element.appendChild(this._createConnectionSection(originState.securityDetails));
      this.element.appendChild(
          this._createCertificateSection(originState.securityDetails, false /* forSignedExchange */));
      const sctSection = this._maybeCreateSctSection(originState.securityDetails, false /* forSignedExchange */);
      if (sctSection) {
    ...
    if (originState.securityDetailsOfSignedExchange) {
      this.element.appendChild(
          this._createCertificateSection(originState.securityDetailsOfSignedExchange, true /* forSignedExchange */));
      const sctSection =
          this._maybeCreateSctSection(originState.securityDetailsOfSignedExchange, true /* forSignedExchange */);
      if (sctSection) {
        this.element.appendChild(sctSection);
      }
    }
