...
export class AnimationModel extends SDK.SDKModel.SDKModel {
  ...
  _createGroupFromPendingAnimations() {
    ...
    const pendingAnimationId = this._pendingAnimations.values().next().value;
    this._pendingAnimations.remove(pendingAnimationId);
    const groupedAnimations = [this._animationsById.get(pendingAnimationId)];
