...
export class AnimationModel extends SDK.SDKModel.SDKModel {
  ...
  animationStarted(payload) {
    ...
    if (animation.type() === 'WebAnimation') {
      const keyframesRule = animation.source().keyframesRule();
      if (keyframesRule && keyframesRule.keyframes().length === 0) {
...
export class AnimationImpl {
   ...
   */
  remoteObjectPromise() {
    return this._animationModel._agent.invoke_resolveAnimation({animationId: this.id()})
        .then(payload => payload && this._animationModel._runtimeModel.createRemoteObject(payload.remoteObject));
  }
