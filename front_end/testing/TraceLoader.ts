...
export class TraceLoader {
  ...
  /**
   * Load an array of raw events from the trace file.
   * Will default to typing those events using the types from Trace, but
  ...
  static async traceEngine(
    ...
    TraceBounds.BoundsManager.instance({forceNew: true});
    ...
    TraceLoader.initTraceBoundsManager(traceEngineData.parsedTrace);
    Timeline.ModificationsManager.ModificationsManager.reset();
