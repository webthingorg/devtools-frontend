...
export class TraceLoader {
  ...
  static async fixtureContents(context: Mocha.Context|Mocha.Suite|null, name: string):
    ...
    const urlForTest = new URL(`../panels/timeline/fixtures/traces/${name}`, window.location.origin);
    const urlForComponentExample = new URL(`../panels/timeline/fixtures/traces/${name}`, window.location.origin);
