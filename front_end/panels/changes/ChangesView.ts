...
export class ChangesView extends UI.Widget.VBox {
  ...
  wasShown(): void {
    this.refreshDiff();
    this.registerCSSFiles([changesViewStyles, CodeHighlighter.Style.default]);
  }
  ...
  private hideDiff(message: string): void {
    ...
    this.toolbar.setEnabled(false);
    this.diffContainer.textContent = '';
  ...
  private renderDiffRows(diff: Diff.Diff.DiffArray|null): void {
    if (!diff || (diff.length === 1 && diff[0][0] === Diff.Diff.Operation.Equal)) {
      ...
      const mime = (this.selectedUISourceCode as Workspace.UISourceCode.UISourceCode).mimeType();
      DiffRenderer.render(diff, mime, this.diffContainer);
    }
  }
}
