...
export class ConsoleMessageSource {
  ...
  async getPrompt(): Promise<string> {
    ...
    const leadingCode: string[] = [];
    let leadingCodeSize = 0;
    ...
    if (callframe && debuggerModel) {
      ...
      if (text) {
        ...
        while (lines[currentLineNumber] !== undefined && (leadingCodeSize + lines[currentLineNumber].length < 500)) {
          leadingCode.push(lines[currentLineNumber]);
          leadingCodeSize += lines[currentLineNumber].length;
          currentLineNumber--;
        }
        leadingCode.reverse();
        currentLineNumber = (mappedLocation?.lineNumber as number) + 1;
        while (lines[currentLineNumber] !== undefined && (leadingCodeSize + lines[currentLineNumber].length < 1000)) {
          leadingCode.push(lines[currentLineNumber]);
          leadingCodeSize += lines[currentLineNumber].length;
          currentLineNumber++;
        }
