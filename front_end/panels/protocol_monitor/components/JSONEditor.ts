...
export class JSONEditor extends LitElement {
    ...
    return html`
      ...
      return html`
            <div class="row attribute padded double" data-attribute="type">
              <div style=${parameters[key].optional && 'color: blue'}>${key}<span class="separator">:</span></div>
              <devtools-recorder-input
                .disabled=${false}
   ...
   * If you input a command inside the input bar it will be displayed inside the editor, but you should not then try to type another command inside the editor.
   */
