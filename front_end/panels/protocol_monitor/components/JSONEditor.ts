...
export class JSONEditor extends LitElement {
  ...
  #handlePopoverDescriptions(event: MouseEvent): { box: AnchorBox, show: (popover: UI.GlassPane.GlassPane) => Promise<boolean> } | null {
    ...
    return {
      ...
      show: async (popover: UI.GlassPane.GlassPane): Promise<boolean> => {
        const popupElement = new ElementsComponents.CSSHintDetailsView.CSSHintDetailsView({
          ...
          'getLearnMoreLink': (): string => `https://chromedevtools.github.io/devtools-protocol/tot/${this.command.split('.')[0]}/`,
        });
  ...
  #getDescriptionAndTypeForElement(hintElement: HTMLElement): { description: string , type: ParameterType | string } | undefined {
