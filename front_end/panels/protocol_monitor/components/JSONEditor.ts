...
const UIStrings = {
  /**
   *@description The text that is displayed inside the Suggestion Box for boolean parameter.
   */
  true: 'true',
  /**
   *@description The text that is displayed inside the Suggestion Box for boolean parameter.
  ...
  /**
   *@description The title of a button that resets to undefined the value of a primitive parameter.
   */
  resetUndefined: 'Reset to undefined',
  /**
   *@description The title of a button that reset the value to default value for object and array parameters.
   */
...
export class JSONEditor extends LitElement {
  ...
  #defaultValueByType: Map<string, string|number|boolean> = new Map();
  ...
  constructor() {
    ...
    this.#defaultValueByType.set('string', '');
    this.#defaultValueByType.set('number', 0);
    this.#defaultValueByType.set('boolean', false);
  }
  ...
  #convertObjectToParameterSchema(key: string, value: unknown, schema?: Parameter, initialSchema?: Parameter[]):
    ...
    switch (type) {
      ...
      case 'boolean':
      case 'number':
        return this.#convertPrimitiveParameter(key, value, schema, initialSchema);
      case 'object':
        return this.#convertObjectParameter(key, value, schema, initialSchema);
      case 'array':
        return this.#convertArrayParameter(key, value, schema, initialSchema);
    }
    return {
      type,
      name: key,
  ...
  #getDescriptionAndTypeForElement(hintElement: HTMLElement):
    ...
    if (hintElement.matches(COMMAND_CLASS)) {
    ...
    if (hintElement.matches(PARAMETER_CLASS)) {
  ...
  populateParametersForCommandWithDefaultValues(): void {
    ...
    this.parameters = commandParameters.map((parameter: Parameter) => {
        ...
        return {
          ...
          value: typeInfos.map(type => {
            return {
              ...
              value: type.optional ? undefined : this.#defaultValueByType.get(type.type),
            } as Parameter;
          }),
          name: parameter.name,
        };
      ...
      return {
        ...
        value: parameter.optional ? undefined : this.#defaultValueByType.get(parameter.type),
        name: parameter.name,
        description: parameter.description,
      } as Parameter;
    });
  ...
  #handleParameterInputBlur = (event: Event): void => {
    ...
    if (value === '') {
      object.value = this.#defaultValueByType.get(object.type);
    } else {
      object.value = value;
    }
  ...
  #handleAddParameter(parameterId: string): void {
    const createNestedParameter = (type: Parameter, name: string): Parameter => {
      ...
      return {
        ...
        value: this.#defaultValueByType.get(type.type),
        description: type.description,
      } as Parameter;
    };
    ...
    if (parameter.type === 'array') {
      ...
      const nestedType = this.typesByName.get(typeRef) ?? [];
      const nestedValue: Parameter[] = nestedType.map(nestedType => createNestedParameter(nestedType, nestedType.name));
      ...
      parameter.value = this.#defaultValueByType.get(parameter.type);
    }
  ...
  #handleClearParameter(parameterId: string): void {
    ...
    if (parameter.type === 'object') {
      ...
      parameter.value = parameter.optional ? undefined : this.#defaultValueByType.get(parameter.type);
    }
  ...
  #computeDropdownValues(parameter: Parameter): string[] {
    ...
    if (parameter.type === 'boolean') {
      return [i18nString(UIStrings.true), i18nString(UIStrings.false)];
    }
    return [];
  }
    ...
    return html`
      <ul>
        ${repeat(parameters, parameter => {
          ...
          const classes = {colorBlue: parameter.optional, parameter: true, colorGrey: parameter.value === undefined && parameter.optional};
          return html`
                <li class="row">
                  <div class="row">
                    <div class=${classMap(classes)} data-paramId=${parameterId}>${parameter.name}<span class="separator">:</span></div>
                        ...
                        ${(isArray && parameter.value.length !== 0) || parameter.type === ParameterType.Object ? this.#renderInlineButton({
                        ...
                        `: nothing}
                          ...
                          <!-- In case  the parameter is not optional or its value is not undefined render the input field -->
                          ${isPrimitive && (parameter.value !== undefined || !parameter.optional) && parentParameter?.type !== 'array' ? html`
                          <devtools-recorder-input
                            data-paramId=${parameterId}
                            .options=${parameter.type === 'string' || parameter.type === 'boolean' ? this.#computeDropdownValues(parameter) : []}
                            .autocomplete=${false}
                            .value=${live(parameter.value ?? '')}
                            .placeholder=${parameter.value === '' ? EMPTY_STRING : `<${this.#defaultValueByType.get(parameter.type)}>`}
                          ...
                          ${isPrimitive && parentParameter?.type !== 'array' && parameter.optional ?
                            html`${parameter.value === undefined ? html`  ${this.#renderInlineButton({
                              ...
                              title: i18nString(UIStrings.resetUndefined),
                              iconName: 'clear',
                              onClick: () => this.#handleClearParameter(parameterId),
                              classMap: { deleteButton: true },
                            })}`}` : nothing}
