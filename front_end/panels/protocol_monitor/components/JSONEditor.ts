...
export class JSONEditor extends LitElement {
  ...
  #handleAddParameter(parameterId: string): void {
    ...
    } else if (parameter.type === ParameterType.Object) {
      const typeRef = parameter.typeRef;
      if (!typeRef) {
        return;
      }
                  ...
                  <div class="row">
                    <div class=${classMap(classes)} data-paramId=${parameterId}>${parameter.name}<span class="separator">:</span></div>
                        ...
                        `: nothing}
                          ...
                          ${isPrimitive && (parameter.value !== undefined || !parameter.optional) && parentParameter?.type !== ParameterType.Array ? html`
                          <devtools-recorder-input
                            data-paramId=${parameterId}
                            .options=${parameter.type === ParameterType.String || parameter.type === ParameterType.Boolean ? this.#computeDropdownValues(parameter) : []}
                            .autocomplete=${false}
                          ...
                          ${isPrimitive && parentParameter?.type !== ParameterType.Array && parameter.optional ?
                          ...
                          ${parentParameter && parentParameter.type === ParameterType.Array ? html`
                          <!-- If the parameter is an object we don't want to display the input field we just want the delete button-->
                          ${parameter.type !== ParameterType.Object ? html`   <devtools-recorder-input
                            data-paramId=${parameterId}
                            .options=${parameter.type === ParameterType.String || parameter.type === ParameterType.Boolean ? this.#computeDropdownValues(parameter) : []}
                            .autocomplete=${false}
                            .value=${live(parameter.value ?? '')}
