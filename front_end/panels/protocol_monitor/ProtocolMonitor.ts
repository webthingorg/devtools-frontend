...
export class ProtocolMonitorDataGrid extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(
  ...
  constructor(splitWidget: UI.SplitWidget.SplitWidget) {
    ...
    const populateToolbarInput = (): void => {
      // Type casting is needed here because the property jsonEditor does not exists directly on type Widget.
      ...
      const commandJson = command || parameters ? JSON.stringify({command, parameters}) : undefined;
      if (commandJson) {
