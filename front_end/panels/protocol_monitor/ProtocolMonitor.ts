...
function widgetRef<T extends UI.Widget.Widget, Args extends unknown[]>(
  ...
  return ref((e?: Element) => {
    ...
    const widget = UI.Widget.Widget.getOrCreateWidget(e, type);
    if (!widget) {
...
export type ViewInput = {
  ...
  dataGridData: DataGrid.DataGridController.DataGridControllerData,
...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
  ...
  constructor(private view?: View) {
    super(true);
    this.impl = new this.Impl(this, view);
  }
  ...
  private Impl = class {
    ...
    started = false;
    startTime = 0;
    readonly requestTimeForId = new Map<number, number>();
    readonly dataGridRowForId = new Map<number, DataGrid.DataGridUtils.Row>();
    infoWidget!: InfoWidget;
    dataGrid!: DataGrid.DataGridController.DataGridController;
    readonly textFilterUI = this.#createTextFilterUI();
    ...
    dataGridData = this.#createDataGridInitialData();
    bottomToolbar!: ToolbarElement;
    ...
    #createClearButton(): UI.Toolbar.ToolbarButton {
      ...
      clearButton.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () => {
        ...
        //   rows: [],
        // };
        this.dataGridData.rows = [];
        this.infoWidget.render(null);
        void this.widget.update();
      });
      return clearButton;
    }
    ...
    #createTextFilterUI(): UI.Toolbar.ToolbarInput {
      ...
      textFilterUI.addEventListener(UI.Toolbar.ToolbarInput.Event.TextChanged, event => {
        ...
        this.dataGridData.filters = filterParser.parse(query);
        void this.widget.update();
      });
      return textFilterUI;
    }
    ...
    private messageReceived(message: Message, target: ProtocolClient.InspectorBackend.TargetBase|null): void {
      if ('id' in message && message.id) {
        ...
        this.dataGrid.data.rows[matchingExistingRowIndex] = newRowWithUpdate;
        void this.widget.update();
        return;
      }
      ...
      // };
      this.dataGrid.data.rows.push(newRow);
      void this.widget.update();
    }
    ...
    constructor(public widget: UI.Widget.Widget, public view: View = (input, output, target) => {
      render(
          html`<devtools-split-widget .options=${{vertical: true, defaultSidebarWidth: input.sideBarMinWidth}}
              ...
              <div slot="main" class="widget vbox protocol-monitor">
                ...
                <devtools-split-widget .options=${{
                  ...
                  <devtools-data-grid-controller
                    .data=${input.dataGridData}
                    @cellfocused=${input.onCellFocused}
                    @newuserfiltertext=${input.onNewFilterText}
                    slot="main"
                    ${typedRef(DataGridController, e => output.dataGrid = e)}>
                  </devtools-data-grid-controller>
                ...
                <devtools-toolbar class="protocol-monitor-bottom-toolbar"
                   ...
                   ${typedRef(ToolbarElement, e => output.bottomToolbar = e)}>
                </devtools-toolbar>
              </div>
              <div is="devtools-protocol-monitor-json-editor"
                   jslog=${VisualLogging.pane('command-editor').track({resize: true})}
                   ${typedRef(JSONEditor, e => output.editor = e)}
                   slot="sidebar"
                   style="overflow:hidden"
                   @submiteditor=${input.onSubmitEditor}>
              </div>
            </div>`,
          target,
          {host: input},
    ...
    doUpdate() {
      this.view(this, this, this.widget.contentElement);
    }
  };
}
...
export class InfoWidget extends UI.Widget.VBox {
  ...
  static override createOnDemand(element: HTMLElement): InfoWidget {
    return new InfoWidget(element);
  }
