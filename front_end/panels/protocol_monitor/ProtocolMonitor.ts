...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const downloadButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.download), 'largeicon-download');
    downloadButton.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () => {
      this._download();
    });
    topToolbar.appendToolbarItem(downloadButton);
  ...
  _messageReceived(message: Message, target: ProtocolClient.InspectorBackend.TargetBase|null): void {
    if (this._isRecording) {
      this._messages.push({...message, type: 'recv', domain: '-'});
    }
  ...
  _messageSent(
    ...
    if (this._isRecording) {
      this._messages.push({...message, type: 'send'});
    }
    const sdkTarget = target as SDK.SDKModel.Target | null;
    const newRow: DataGrid.DataGridUtils.Row = {
      cells: [
  ...
  async _download(): Promise<void> {
    ...
    stream.write(JSON.stringify(this._messages, null, '  '));
    stream.close();
  }
}
