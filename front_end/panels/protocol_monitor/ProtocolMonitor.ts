...
import {SubmitEditorEvent} from './components/JSONEditor.js';
...
export class EditorWidget extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    this.jsonEditor.addEventListener(SubmitEditorEvent.eventName, (event: Event) => {
      this.dispatchEventToListeners(Events.CommandSent, (event as SubmitEditorEvent).data);
    });
  }
