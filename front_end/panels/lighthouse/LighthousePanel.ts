...
export class LighthousePanel extends UI.Panel.Panel {
  ...
  private async resetEmulationAndProtocolConnection(): Promise<void> {
    ...
    if (this.stateBefore) {
      ...
      // For some reason, detaching a session after overriding device metrics will prevent other sessions from overriding device metrics in the future.
      // A workaround is to call "Emulation.clearDeviceMetricOverrides" which is the result of the next line.
      emulationModel.emulate(EmulationModel.DeviceModeModel.Type.None, null, null);
