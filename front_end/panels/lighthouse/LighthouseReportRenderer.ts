...
export function renderLighthouseReport(
    lhr: ReportJSON, artifacts?: RunnerResultArtifacts, opts?: RenderReportOpts): HTMLElement {
  ...
  async function onPrintOverride(rootEl: HTMLElement): Promise<void> {
    ...
    await linkifyNodeDetails(clonedReport as HTMLElement);
  ...
  function getStandaloneReportHTML(): void {
  ...
  void waitForMainTargetLoad().then(() => {
    void linkifyNodeDetails(reportEl);
    void linkifySourceLocationDetails(reportEl);
  });
...
async function linkifyNodeDetails(el: Element): Promise<void> {
  const mainTarget = SDK.TargetManager.TargetManager.instance().mainTarget();
  if (!mainTarget) {
    return;
  }
  ...
  for (const origElement of el.getElementsByClassName('lh-node')) {
    ...
    const element =
        await Common.Linkifier.Linkifier.linkify(node, {tooltip: detailsItem.snippet, preventKeyboardFocus: undefined});
...
async function linkifySourceLocationDetails(el: Element): Promise<void> {
