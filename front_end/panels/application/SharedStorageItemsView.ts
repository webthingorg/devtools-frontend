...
export namespace SharedStorageItemsDispatcher {
  ...
  export type EventTypes = {
    ...
    [Events.ItemDeleted]: void,
    [Events.ItemEdited]: void,
...
export class SharedStorageItemsView extends StorageItemsView {
  ...
  #sharedStorageItemsDispatcher: Common.ObjectWrapper.ObjectWrapper<SharedStorageItemsDispatcher.EventTypes>;
  ...
  async #editingCallback(
    ...
    await this.refreshItems();
    this.#sharedStorageItemsDispatcher.dispatchEventToListeners(SharedStorageItemsDispatcher.Events.ItemEdited);
  ...
  async #deleteCallback(node: DataGrid.DataGrid.DataGridNode<unknown>): Promise<void> {
    ...
    await this.#sharedStorage.deleteEntry(node.data.key);
    await this.refreshItems();
    this.#sharedStorageItemsDispatcher.dispatchEventToListeners(SharedStorageItemsDispatcher.Events.ItemDeleted);
  ...
  async #previewEntry(entry: DataGrid.DataGrid.DataGridNode<unknown>|null): Promise<void> {
    ...
    if (entry && entry.data && entry.data.key && entry.data.value) {
      console.assert(Boolean(wrappedEntry));
  ...
  getSharedStorageItemsDispatcherForTesting():
      Common.ObjectWrapper.ObjectWrapper<SharedStorageItemsDispatcher.EventTypes> {
    return this.#sharedStorageItemsDispatcher;
  }
}
