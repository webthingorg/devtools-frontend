...
export class PreloadingView extends UI.Widget.VBox {
  ...
  private bottom = new PreloadingDetailsView(null);
  ...
  private onModelUpdated(): void {
    ...
    const rows = this.model.getAll().map(convert);
  ...
  async getBottomForTest(): Promise<PreloadingComponents.PreloadingDetailsReportView.PreloadingDetailsReportView> {
    await this.bottom.forceDoUpdateForTest();
    return this.bottom.contentElement.children[0] as
        PreloadingComponents.PreloadingDetailsReportView.PreloadingDetailsReportView;
  }
}
...
class PreloadingDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async forceDoUpdateForTest(): Promise<void> {
    await this.doUpdate();
  }
}
