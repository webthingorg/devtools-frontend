...
export class PreloadingView extends UI.Widget.VBox {
  ...
  constructor(model: SDK.PreloadingModel.PreloadingModel) {
    ...
    this.preloadingStateDisabledWarningsPromise = this.getPreloadPagesState().then(this.onGetPreloadPagesState());
  }
  ...
  private onGetPreloadPagesState(status: String): void {
    if (status !== '') {
      this.showInfobar(
