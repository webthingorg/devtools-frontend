...
export class PreloadingView extends UI.Widget.VBox {
  ...
  constructor(model: SDK.PrerenderingModel.PrerenderingModel) {
    ...
    void this.getFeatureFlags().then(x => this.onGetFeatureFlags(x));
  }
  ...
  async getFeatureFlags(): Promise<FeatureFlags> {
    const preloadingHoldback = (await this.model.target().systemInfo().invoke_getFeatureState({
                                 featureState: 'PreloadingHoldback',
                               })).featureEnabled ??
        null;
    const prerender2Holdback = (await this.model.target().systemInfo().invoke_getFeatureState({
