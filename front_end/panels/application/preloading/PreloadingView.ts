...
export class PreloadingRuleSetView extends UI.Widget.VBox {
  ...
  constructor(model: SDK.PreloadingModel.PreloadingModel) {
    ...
    this.shouldAutoPrettyPrint = true;
    const prettyToggle =
        new UI.Toolbar.ToolbarToggle(i18nString(UIStrings.prettyPrint), 'brackets', undefined, 'pretty-print');
    prettyToggle.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () => {
      this.shouldAutoPrettyPrint = !this.shouldAutoPrettyPrint;
      // this.ruleSetDetails.shouldPrettyPrint = this.shouldAutoPrettyPrint;
      this.updateRuleSetDetails();
    });
    prettyprintToolbar.appendToolbarItem(prettyToggle);
