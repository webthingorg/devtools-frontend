...
export class PreloadingDisabledInfobar extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  #dataEqual(a: Protocol.Preload.PreloadEnabledStateUpdatedEvent, b: Protocol.Preload.PreloadEnabledStateUpdatedEvent):
      boolean {
    let key: keyof Protocol.Preload.PreloadEnabledStateUpdatedEvent;
    for (key in a) {
      if (a[key] !== b[key]) {
        return false;
      }
  ...
  set data(data: Protocol.Preload.PreloadEnabledStateUpdatedEvent) {
    if (this.#dataEqual(data, this.#data)) {
      return;
    }
  ...
  #maybeSections(shouldShow: boolean, header: string, description: string|Element): LitHtml.LitTemplate {
    if (!shouldShow) {
      return LitHtml.nothing;
    }
