...
const UIStrings = {
   ...
   */
  mismatchedHeadersDetail: 'Mismatched HTTP request headers detail',
};
...
export class UsedPreloadingView extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  #renderInternal(): LitHtml.LitTemplate {
    ...
    return LitHtml.html`
      <${ReportView.ReportView.Report.litTagName}>
        ...
        ${this.#maybeMismatchedHTTPHeadersSections(kind)}
  ...
  #maybeMismatchedHTTPHeadersSections(kind: UsedKind): LitHtml.LitTemplate {
    if (kind !== UsedKind.PrerenderFailed || this.#data.attempts.length === 0) {
      return LitHtml.nothing;
    }

    const attempt = this.#data.attempts.find(
        attempt =>
