...
export interface RuleSetGridRow {
  ...
  preloadsStatusSummary: RuleSetGridPreloadStatusSummary;
  validity: string;
  location: string;
}

export interface RuleSetGridPreloadStatusSummary {
  summary: string;
...
export class RuleSetGrid extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  #buildReportRows(): DataGrid.DataGridUtils.Row[] {
    function preloadsRenderer(json: DataGrid.DataGridUtils.CellValue): LitHtml.TemplateResult {
      const preloadsStatusSummary = JSON.parse(json as string) as RuleSetGridPreloadStatusSummary;

      const revealAttemptViewWithFilter = (): void => {
        void Common.Revealer.reveal(
            new PreloadingHelper.PreloadingForward.AttemptViewWithFilter(preloadsStatusSummary.ruleSetId));
      };
    ...
    return this.#rows.map(row => ({
                            cells: [
                              ...
                              {columnId: 'processLocalId', value: row.processLocalId},
                              {
                                ...
                                value: JSON.stringify(row.preloadsStatusSummary as unknown as JSON),
