...
export class RuleSetDetailsView extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  async #render(): Promise<void> {
    const sourceText = await this.#renderSource();
  ...
  generateSourceByTemplate(sourceText: string): LitHtml.LitTemplate {
  ...
  async #renderSource(): Promise<LitHtml.LitTemplate> {
    if (this.#shouldPrettyPrint && this.#data?.sourceText !== undefined) {
      ...
      return this.generateSourceByTemplate(formattedResult.formattedContent);
    }

    return this.generateSourceByTemplate(this.#data?.sourceText || '');
  }
}
