...
export class MismatchedPreloadingGrid extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  #render(): void {
    ...
    const reportsGridData: DataGrid.DataGridController.DataGridControllerData = {
      columns: [
        {
          id: 'url',
          title: i18nString(UIStrings.preloadedURLs),
          widthWeighting: 40,
          hideable: false,
          visible: true,
          sortable: true,
  ...
  #buildReportRows(): DataGrid.DataGridUtils.Row[] {
    function urlRenderer(url: string, pageURL: string): LitHtml.TemplateResult {
      function span(additionalProps: {'background-color'?: string}, s: string): LitHtml.TemplateResult {
        // float: 'left' is for eliminating spaces.
      ...
      const color =
          document.documentElement.classList.contains('-theme-with-dark-background') ? COLOR_DARK : COLOR_LIGHT;
      const diffs = Diff.Diff.DiffWrapper.charDiff(url, pageURL);
      const content = diffs.reduce((acc, diffOp) => {
        const s = diffOp[1];
        let diff;
        switch (diffOp[0]) {
          ...
          case Diff.Diff.Operation.Insert:
            diff = span({'background-color': color.green}, s);
            break;
          case Diff.Diff.Operation.Delete:
            diff = span({'background-color': color.red}, s);
            break;
    ...
    // Prevent TS2531: Object is possibly 'null'.
    const pageURL = this.#data.pageURL;

    // DESC diffScore, i.e. most similar one first.
