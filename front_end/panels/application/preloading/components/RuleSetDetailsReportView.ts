...
export class RuleSetDetailsReportView extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  ...
  set prerenderedUrl(prerenderedUrl: String) {
  ...
  async #render(): Promise<void> {
    await coordinator.write('RuleSetDetailsReportView render', () => {
      ...
      // clang-format off
      const reportData = {reportTitle: this.#prerenderedUrl};
      LitHtml.render(LitHtml.html`
      <${ReportView.ReportView.Report.litTagName} (.data={${reportData} as ReportView.ReportView.ReportData })>
