...
export interface FrameDetailsReportViewData {
  data: SDK.ResourceTreeModel.ResourceTreeModel;
}
...
export class BackForwardCacheView extends UI.ThrottledWidget.ThrottledWidget {
  constructor() {
    super(true, 1000);
    // this.target = target;
    // this.model = target.model(SDK.ResourceTreeModel.ResourceTreeModel);
  ...
  private async showStatus(): Promise<void> {
    ...
    if (resourceTreeModel) {
      const results = await resourceTreeModel.navigationHistory();
      await resourceTreeModel.navigate('https://blog.youtube/');
      // await resourceTreeModel.reloadPage(/*ignoreCache?: */ false, );
      if (results) {
        resourceTreeModel.navigateToHistoryEntry(results.entries[results.currentIndex]);
      }
    }
  }
