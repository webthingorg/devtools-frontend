...
export class BackForwardCacheView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  private showStatus(): void {
    ...
    if (resourceTreeModel) {
      resourceTreeModel.navigate('https://www.google.com');
      resourceTreeModel.navigationHistory().then(results => {
        if (!results) {
          // resourceTreeModel.navigateToHistoryEntry(results.entries[results.currentIndex - 1]);
        }
      });
    }
  }
