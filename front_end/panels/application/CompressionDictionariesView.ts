            ...
            (menu: UI.ContextMenu.ContextMenu, _columns: readonly DataGrid.DataGridUtils.Column[],
             row: Readonly<DataGrid.DataGridUtils.Row>, _rows: readonly DataGrid.DataGridUtils.Row[]): void => {
              ...
              menu.overrideSection().appendItem('Delete selected', () => {
                ...
                let match = '';
                let matchDest = '';
                row.cells.filter(cell => cell.columnId === 'dictionary-url').forEach(cell => {
                  dictionaryUrl = cell.value as string;
                });
                ...
                this.#model.deleteDictionary(
                    this.#data.topFrameSite, this.#data.frameOrigin, dictionaryUrl, match,
                    matchDest === '' ? [] : matchDest.split(' ').map(i => i.slice(1, -1)));
              });
              menu.overrideSection().appendItem('Delete all', () => {
                assertNotNullOrUndefined(this.#model);
                assertNotNullOrUndefined(this.#data);
                void this.#model.deleteAllDictionaries(this.#data.topFrameSite, this.#data.frameOrigin);
              });
            },
      },
