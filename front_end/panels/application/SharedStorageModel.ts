...
export class SharedStorageModel extends SDK.SDKModel.SDKModel<EventTypes> implements
  ...
  private storagesInternal: Map<string, SharedStorage>;
  ...
  private async securityOriginAdded(event: Common.EventTarget.EventTargetEvent<string>): Promise<void> {
    await this.maybeAddOrigin(event.data);
  }

  private async maybeAddOrigin(securityOrigin: string): Promise<void> {
