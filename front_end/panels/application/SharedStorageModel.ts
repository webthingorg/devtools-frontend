...
export class SharedStorageModel extends SDK.SDKModel.SDKModel<EventTypes> implements
  ...
  async #maybeAddOrigin(securityOrigin: string): Promise<void> {
    // debug
    console.error('maybe add origin: ', securityOrigin);
    ...
    if (this.#storages.has(securityOrigin)) {
      // debug
      console.error('already added origin');
      return;
    }
    ...
    if (typeof metadataResponse.getError() !== 'undefined') {
      // debug
      console.error('origin not using ss');
      console.error(JSON.stringify(metadataResponse));
      return;
    }
  ...
  storages(): IterableIterator<SharedStorage> {
    return this.#storages.values();
  }

  storageForOrigin(origin: string): SharedStorage|null {
    return this.#storages.get(origin) || null;
  }

  sharedStorageAccessed(event: Protocol.Storage.SharedStorageAccessedEvent): void {
    if (event.type === Protocol.Storage.SharedStorageAccessType.DocumentSet ||
