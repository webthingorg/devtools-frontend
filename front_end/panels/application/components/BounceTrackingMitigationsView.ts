...
const UIStrings = {
   ...
   * indicate that bounce tracking mitigations are disabled.
   */
...
export class BounceTrackingMitigationsView extends HTMLElement {
  ...
  set data(data: BounceTrackingMitigationsViewData) {
    this.#trackingSites = data.trackingSites;
    this.#seenButtonClick = data.seenButtonClick;
    this.#render();
  }
  ...
  #renderDeletedSitesOrNoSitesMessage(): LitHtml.TemplateResult {
    ...
    if (this.#trackingSites.length === 0) {
      ...
      return LitHtml.html`
        <${ReportView.ReportView.ReportSection.litTagName}>
          ${i18nString(this.#screenStatus === ScreenStatusType.Running ? UIStrings.checkingPotentialTrackers : UIStrings.noPotentialBounceTrackersIdentified)}
        </${ReportView.ReportView.ReportSection.litTagName}>
      `;
      // clang-format on
    }
