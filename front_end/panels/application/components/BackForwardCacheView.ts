...
export class BackForwardCacheView extends HTMLElement {
  ...
  #buildFrameTreeDataRecursive(
    ...
    const node: TreeOutline.TreeOutlineUtils.TreeNode<FrameTreeNodeData> = {
      treeNodeData: {
    ...
    if (children.length) {
      (node as unknown as TreeOutline.TreeOutlineUtils.TreeNodeWithChildren<FrameTreeNodeData>).children =
          (): Promise<TreeOutline.TreeOutlineUtils.TreeNode<FrameTreeNodeData>[]> => Promise.resolve(children);
