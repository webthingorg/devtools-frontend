...
const UIStrings = {
  ...
  framesTitle: 'Frames',
  /**
   * @description Top level summary of the number of frames with issues found.
   * @example {3} PH1
   * @example {2} PH2
   */
  issuesInFrames: '{PH1} issues found in {PH2} frames',
  /**
   * @description Shows the number of frames with a particular issue.
   * @example {5} PH1
   */
  framesPerIssue: '{PH1} frames',
};
...
export class BackForwardCacheView extends HTMLElement {
  ...
  async #render(): Promise<void> {
    await coordinator.write('BackForwardCacheView render', () => {
      ...
      LitHtml.render(LitHtml.html`
        <${ReportView.ReportView.Report.litTagName} .data=${{ reportTitle: i18nString(UIStrings.backForwardCacheTitle) } as ReportView.ReportView.ReportData
        }>
  ...
  #maybeRenderFrameTree(explanationTree: Protocol.Page.BackForwardCacheNotRestoredExplanationTree|
    ...
    const counts = this.#maybeAddFrameSubTree(urlTreeElement, explanationTree);
    const frameCount = counts[0];
    const issueCount = counts[1];
    urlTreeElement.expand();
    treeOutline.appendChild(urlTreeElement);
    urlTreeElement.title = i18nString(UIStrings.issuesInFrames, {PH1: String(issueCount), PH2: String(frameCount)});
  ...
  #maybeAddFrameSubTree(
      ...
      explanationTree: Protocol.Page.BackForwardCacheNotRestoredExplanationTree|undefined): [number, number] {
    if (!explanationTree || (explanationTree.explanations.length === 0 && explanationTree.children.length === 0)) {
      return [0, 0];
    }
    ...
    let framecount = 0;
    if (explanationTree.explanations.length > 0 || explanationTree.children.length > 0) {
      const urlTreeElement = new UI.TreeOutline.TreeElement('');
      urlTreeElement.setLeadingIcons([icon]);
      explanationTree.explanations.map(explanation => {
  ...
  #maybeRenderExplanations(
    ...
    // clang-format off
    return LitHtml.html`
      ${this.#renderExplanations(i18nString(UIStrings.pageSupportNeeded), i18nString(UIStrings.pageSupportNeededExplanation), pageSupportNeeded, explanationTree)}
      ${this.#renderExplanations(i18nString(UIStrings.supportPending), i18nString(UIStrings.supportPendingExplanation), supportPending, explanationTree)}
      ${this.#renderExplanations(i18nString(UIStrings.circumstantial), i18nString(UIStrings.circumstantialExplanation), circumstantial, explanationTree)}
    `;
    // clang-format on
  }
  ...
  #renderExplanations(
    ...
    return LitHtml.html`
      ${explanations.length > 0 ? LitHtml.html`
        ...
        ${explanations.map(explanation => this.#renderReason(explanation, explanationTree))}
      ` : LitHtml.nothing}
    `;
    // clang-format on
  }

  #findFramesWithReason(
      explanation: Protocol.Page.BackForwardCacheNotRestoredExplanation,
      explanationTree: Protocol.Page.BackForwardCacheNotRestoredExplanationTree): string[] {
    let frames: string[];
    frames = [];
    explanationTree.explanations.forEach(treeExplanation => {
      if (explanation.reason === treeExplanation.reason) {
        frames = [explanationTree.url];
      }
    });
    explanationTree.children.map(child => {
      frames = frames.concat(this.#findFramesWithReason(explanation, child));
    });
    return frames;
  }
  ...
  #renderReason(
      explanation: Protocol.Page.BackForwardCacheNotRestoredExplanation,
      explanationTree: Protocol.Page.BackForwardCacheNotRestoredExplanationTree|undefined): LitHtml.TemplateResult {
    // Find all the frame URLs that have this reason, and build a frame tree for them.
    const urlsWithReason: string[] = explanationTree ? this.#findFramesWithReason(explanation, explanationTree) : [];
    const treeOutline = new UI.TreeOutline.TreeOutlineInShadow();
    if (urlsWithReason.length > 0) {
      const topLevelTitle = i18nString(UIStrings.framesPerIssue, {PH1: String(urlsWithReason.length)});
      const topTreeElement = new UI.TreeOutline.TreeElement(topLevelTitle);
      urlsWithReason.map(url => {
        topTreeElement.appendChild(new UI.TreeOutline.TreeElement(url));
      });
      treeOutline.appendChild(topTreeElement);
    }
    // clang-format off
    return LitHtml.html`
      <${ReportView.ReportView.ReportSection.litTagName}>
      ...
      <div class='explanation-frames'>
        ${urlsWithReason.length > 0 ? treeOutline.element : LitHtml.nothing}
      </div>
    `;
    // clang-format on
  }
