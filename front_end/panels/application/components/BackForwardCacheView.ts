...
export class BackForwardCacheView extends HTMLElement {
  ...
  #renderReasonContext(explanation: Protocol.Page.BackForwardCacheNotRestoredExplanation): LitHtml.TemplateResult {
    ...
    return LitHtml.html`
      id=<x-link href="chrome://extensions/?id=${explanation.context}" class="link">
${explanation.context}
</a>`;
      // clang-format on
    } else {
  ...
  #renderReason(explanation: Protocol.Page.BackForwardCacheNotRestoredExplanation): LitHtml.TemplateResult {
    ...
    ${this.#renderReasonContext(explanation)}` :
            LitHtml.nothing}
      </${ReportView.ReportView.ReportSection.litTagName}>
      <div class='gray-text'>
        ${explanation.reason}
