...
export class BackForwardCacheView extends HTMLElement {
  ...
  async #onNavigatedAway(): Promise<void> {
    SDK.TargetManager.TargetManager.instance().removeModelListener(
        ...
        this);
    await this.#waitAndGoBackInHistory();
  }

  async #waitAndGoBackInHistory(): Promise<void> {
    const mainTarget = SDK.TargetManager.TargetManager.instance().mainTarget();
    ...
    // It is still possible to mess things up by pressing the browser's back/
    // forward buttons while the BFCache test is running.
    if (historyResults.currentIndex === this.#historyIndex) {
      window.setTimeout(this.#waitAndGoBackInHistory.bind(this), 500);
    } else {
