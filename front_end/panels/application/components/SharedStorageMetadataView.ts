...
export class SharedStorageMetadataView extends StorageMetadataView {
  ...
  constructor(sharedStorageMetadataGetter: SharedStorageMetadataGetter, owner: string) {
    ...
    this.data = {origin: owner as Platform.DevToolsPath.UrlString};
    void this.render();
  }
  ...
  override async renderReportContent(storageKey: SDK.StorageKeyManager.DisplayStorageKey):
      Promise<LitHtml.LitTemplate> {
    ...
    // clang-format off
    return LitHtml.html`
      ${await super.renderReportContent(storageKey)}
  ...
  #renderResetBudgetButton(): LitHtml.TemplateResult {
    ...
    return LitHtml.html`<${SharedStorageResetBudgetButton.litTagName}
     .data=${{resetBudgetHandler: this.#resetBudget} as SharedStorageResetBudgetButtonData}
