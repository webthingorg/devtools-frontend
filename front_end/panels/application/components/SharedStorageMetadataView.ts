...
const UIStrings = {
  ...
  */
  remainingBudget: 'Remaining Privacy Budget',
  /**
...
export class SharedStorageMetadataView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor(sharedStorage: SharedStorage) {
    ...
    this.#reportView.origin = this.#sharedStorage.securityOrigin;
    this.update();
    this.#sharedStorage.addEventListener(SharedStorage.Events.SharedStorageChanged, this.update, this);
  }

  async doUpdate(): Promise<void> {
    const metadata = await this.#sharedStorage.getMetadata();
...
export interface SharedStorageMetadataData {
...
export class SharedStorageMetadataReportView extends HTMLElement {
  ...
  set data(data: SharedStorageMetadataData) {
...
<${ReportView.ReportView.Report.litTagName} .data=${titleForReport as ReportView.ReportView.ReportData}>
  ...
  #renderEntriesSection(): LitHtml.LitTemplate {
    ...
    return LitHtml.html`
      <${ReportView.ReportView.ReportSectionHeader.litTagName}
        title=${i18nString(UIStrings.entries)}
