...
class SharedStorageResetBudgetButton extends HTMLElement {
  ...
  #render(): void {
    ...
    LitHtml.render(LitHtml.html`
      <!-- Wrap the button in a container, in order to better align it inside the column. -->
      <span class="button-container">
        <button class="reset-budget-button"
          title=${i18nString(UIStrings.resetBudget)}
          @click=${(): void => this.resetBudgetHandler()}>
          <${IconButton.Icon.Icon.litTagName} .data=${
        {iconName: 'refresh_12x12_icon', color: 'var(--color-text-secondary)', width: '9px', height: '14px'} as
