...
export class ProtocolHandlersView extends HTMLElement {
  ...
  #renderTestProtocolSection(): LitHtml.TemplateResult {
    if (this.#protocolHandlers.length === 0) {
      Host.userMetrics.actionTaken(Host.UserMetrics.Action.PwaWithoutProtocol);
      return LitHtml.html``;
    }
    Host.userMetrics.actionTaken(Host.UserMetrics.Action.PwaWithProtocol);
    ...
    return LitHtml.html`
       <div class="protocol-handlers-row">
        <select id="protocol-select" class="chrome-select" @change=${this.#handleProtocolSelect}>
           ${protocolOptions}
        </select>
        <input id="user-input" class="harmony-input" type="text" @change=${this.#handleQueryInputChange}/>
        <${Buttons.Button.Button.litTagName} .type=${'submit'} .variant=${Buttons.Button.Variant.PRIMARY} @click=${
  ...
  #render(): void {
    ...
    LitHtml.render(LitHtml.html`
      ...
      ${this.#renderTestProtocolSection()}
