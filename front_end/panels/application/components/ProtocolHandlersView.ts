...
export class ProtocolHandlersView extends HTMLElement {
  ...
  #protocolURL: Platform.DevToolsPath.UrlString = Platform.DevToolsPath.EmptyUrlString;
  ...
  #handleTestProtocol(evt: Event): void {
    evt.preventDefault();
    const formData = new FormData(evt.target as HTMLFormElement);
    const selectedProtocol = formData.get('protocolSelect');
    const userInput = formData.get('userInput');
    this.#protocolURL = (selectedProtocol + '://' + userInput) as Platform.DevToolsPath.UrlString;
    Host.InspectorFrontendHost.InspectorFrontendHostInstance.openInNewTab(this.#protocolURL);
  }
  ...
  #renderTestProtocolForm(): LitHtml.TemplateResult {
    if (this.#protocolHandlers.length) {
      return LitHtml.html`
     <form id="form" class="protocol-handlers-row" @submit=${this.#handleTestProtocol}>
