...
export class ProtocolHandlersView extends HTMLElement {
  ...
  set data(data: ProtocolHandlersData) {
    ...
    this.#update(isNewManifest);
  }

  #update(isNewManifest: boolean): void {
    const inputElement = (this.shadowRoot?.querySelector('.user-input') as HTMLInputElement);
    if (inputElement && isNewManifest) {
      inputElement.value = '';
    }
    if (!this.#selectedProtocolState) {
  ...
  #renderProtocolTest(): LitHtml.LitTemplate {
    ...
    return LitHtml.html`
       <div class="protocol-handlers-row">
        ...
        <input class="harmony-input user-input" type="text" .value=${this.#queryInputState} @change=${
