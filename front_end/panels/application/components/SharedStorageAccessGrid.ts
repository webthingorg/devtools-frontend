...
const UIStrings = {
  ...
  */
  allSharedStorageEvents: 'All shared storage events.',
  /**
...
function onlyUniqueEvents(
    value: Protocol.Storage.SharedStorageAccessedEvent, index: number,
    self: Array<Protocol.Storage.SharedStorageAccessedEvent>): boolean {
  for (const [i, val] of self.entries()) {
    if (eventEquals(val, value)) {
      return i === index;
    }
  }
  return false;
...
export class SharedStorageAccessGrid extends HTMLElement {
  ...
  #render(): void {
    ...
    LitHtml.render(LitHtml.html`
      <div>
        <span class="heading">Shared Storage</span>
  ...
  #buildRows(): DataGrid.DataGridUtils.Row[] {
    return this.#datastores.filter(onlyUniqueEvents)
        .map(event => ({
               cells: [
