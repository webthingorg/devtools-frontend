...
export class FrameDetailsReportView extends
  ...
  #maybeRenderCSPStatus(infos: Protocol.Network.ContentSecurityPolicyStatus[]|undefined): LitHtml.LitTemplate {
    if (!infos) {return LitHtml.nothing;}
    const policies = [];
    for (const info of infos) {
      policies.push(LitHtml.html`
      <${ReportView.ReportView.ReportKey.litTagName}>${info.isEnforced ? i18n.i18n.lockedString('Content-Security-Policy') : i18n.i18n.lockedString('Content-Security-Policy-Report-Only')}
      ${info.isEnforced ? '' : LitHtml.html`<x-link class="report-only-link" href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy-Report-Only">
      ...
      ${info.source === Protocol.Network.ContentSecurityPolicySource.HTTP ? i18n.i18n.lockedString('HTTP header') : i18n.i18n.lockedString('<Meta tag')}
      ${this.parseEffectiveDirectives(info.effectiveDirectives)}
      </${ReportView.ReportView.ReportValue.litTagName}>
      `);
    }
    return LitHtml.html`<${ReportView.ReportView.ReportSectionHeader.litTagName}>${i18nString(UIStrings.contentSecurityPolicy)}
                        </${ReportView.ReportView.ReportSectionHeader.litTagName}>
                        ${policies}`;
  }
