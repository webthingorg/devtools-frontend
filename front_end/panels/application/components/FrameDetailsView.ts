...
const UIStrings = {
  ...
  */
  ...
  /**
   *@description Report key text for relevant origin trials (trials with at least 1 token present).
   */
  relevantTrials: 'Relevant Trials',
};
...
export class FrameDetailsReportView extends HTMLElement {
  ...
  private renderOriginTrial(): LitHtml.TemplateResult|{} {
    ...
    this.originTrialTreeView.data = {trials: originTrials};

    return LitHtml.html`
    <${ReportView.ReportView.ReportSectionHeader.litTagName}>${i18nString(UIStrings.originTrials)}
    </${ReportView.ReportView.ReportSectionHeader.litTagName}>
    <${ReportView.ReportView.ReportKey.litTagName}>${i18nString(UIStrings.relevantTrials)}</${
