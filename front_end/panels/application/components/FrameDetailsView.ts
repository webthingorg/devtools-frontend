...
import type {PermissionsPolicySectionData, ShowPermissionsDisallowedDetailsEvent} from './PermissionsPolicySection.js';
...
export class FrameDetailsReportView extends HTMLElement {
  ...
  set data(data: FrameDetailsReportViewData) {
    ...
    if (!this.permissionsPolicies) {
      this.permissionsPolicies = (this.frame && this.frame.getPermissionsPolicyState());
    }
    this.render();
  }

  onShowPermissionsPolicyDetailsEvent(event: ShowPermissionsDisallowedDetailsEvent): void {
    this.showPermissionsPolicyDetails = event.showDetails;
  }
          ...
          ${LitHtml.Directives.until(this.permissionsPolicies?.then(policies => {
            return LitHtml.html`
              <${PermissionsPolicySection.litTagName}
                .data=${{policies, showDetails: this.showPermissionsPolicyDetails} as PermissionsPolicySectionData}
                @showdetails=${this.onShowPermissionsPolicyDetailsEvent}
              >
              </${PermissionsPolicySection.litTagName}>
            `;
          }), LitHtml.nothing)}
