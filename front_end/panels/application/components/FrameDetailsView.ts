...
export class FrameDetailsReportView extends HTMLElement {
  ...
  #renderPrerenderingSection(): LitHtml.LitTemplate {
    ...
    if (this.#frame.prerenderReasonDetails) {
      const detailIndex: string = this.#frame.prerenderFinalStatus + 'DetailSection';
      const detailSection = Prerender2ReasonDescription[detailIndex].name();
      return LitHtml.html`
      <${ReportView.ReportView.ReportSectionHeader.litTagName}>
      ...
      <${ReportView.ReportView.ReportValue.litTagName}>
      <div class="text-ellipsis" title=${this.#frame.prerenderReasonDetails}>
        ${this.#frame.prerenderReasonDetails}
      </div>
      </${ReportView.ReportView.ReportValue.litTagName}>
      <${ReportView.ReportView.ReportSectionDivider.litTagName}></${
          ReportView.ReportView.ReportSectionDivider.litTagName}>`;
