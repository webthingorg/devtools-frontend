...
export class FrameDetailsReportView extends HTMLElement {
  ...
  #maybeRenderCSPStatus(info: string|undefined, policyName:string): LitHtml.LitTemplate {
    const parsed = info ? new CspEvaluator.CspParser(info).csp : null;
    const evaluated = parsed ? new CspEvaluator.CspEvaluator(parsed).evaluate() : null;
    console.log(evaluated ? evaluated : "NOTHING");
    return LitHtml.html`
    <${ReportView.ReportView.ReportKey.litTagName}>${policyName}</${ReportView.ReportView.ReportKey.litTagName}>
    <${ReportView.ReportView.ReportValue.litTagName}>
      ${
      evaluated ? LitHtml.html`<span class="inline-name">Policy: </span>${evaluated}` :
                 LitHtml.nothing}
    </${ReportView.ReportView.ReportValue.litTagName}>
  `;
  };
