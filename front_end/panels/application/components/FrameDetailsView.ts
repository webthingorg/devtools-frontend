...
function constructOriginTrialTree(originTrial: Protocol.Page.OriginTrial): TreeNode<OriginTrialTreeNodeData> {
  const constructTokenDetailsNodes =
      (token: Protocol.Page.OriginTrialTokenWithStatus): TreeNode<OriginTrialTreeNodeData>[] => {
        ...
        const parsedTokenDetailNodes = parsedToken ?
            [
              {treeNodeData: `${UIStrings.origin}: ${parsedToken.origin}`},
              {treeNodeData: `${UIStrings.expiryTime}: ${parsedToken.expiryTime}`},
              {treeNodeData: `${UIStrings.usageRestriction}: ${parsedToken.usageRestriction}`},
              {treeNodeData: `${UIStrings.isThirdParty}: ${parsedToken.isThirdParty}`},
              {treeNodeData: `${UIStrings.matchSubDomains}: ${parsedToken.matchSubDomains}`},
            ] :
            [];
...
function originTrialRenderer(data: TreeNode<OriginTrialTreeNodeData>): LitHtml.TemplateResult {
  ...
  if (isOriginTrial(content)) {
    return LitHtml.html`
    ${content.trialName} (${content.status})
    (${content.tokensWithStatus.length > 1 ? content.tokensWithStatus.length : ''})
    `;
  }
