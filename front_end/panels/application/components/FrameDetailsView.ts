...
export class FrameDetailsReportView extends HTMLElement {
  ...
  set data(data: FrameDetailsReportViewData) {
    ...
    console.log(  // eslint-disable-line no-console
        'setting data for FrameDetailsReportView; has policies: ', Boolean(this.policies));
    if (!this.policies) {
      console.log('fetching policies');  // eslint-disable-line no-console
    ...
    await coordinator.write('FrameDetailsView render', () => {
      ...
      LitHtml.render(LitHtml.html`
        ...
        <${ReportView.ReportView.Report.litTagName} .data=${{reportTitle: this.frame.displayName()} as ReportView.ReportView.ReportData}>
          ...
          ${LitHtml.Directives.until(this.policies?.then(policies => {
            console.log('FrameDetailsView until directive then'); // eslint-disable-line no-console
            return LitHtml.html`
