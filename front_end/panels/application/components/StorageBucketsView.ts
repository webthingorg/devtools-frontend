...
const UIStrings = {
  ...
  loading: 'Loading…',
  /**
   * @description The Storage Bucket's storage key.
   */
  storageKey: 'Storage Key',
  /**
   * @description The Storage Bucket's persistent (https://wicg.github.io/storage-buckets/explainer#storage-policy-persistence)
   */
  persistent: 'Persistent',
  /**
   * @description The Storage Bucket's durability (https://wicg.github.io/storage-buckets/explainer#storage-policy-durability)
   */
  durability: 'Durability',
  /**
   * @description The Storage Bucket's quota (https://wicg.github.io/storage-buckets/explainer#storage-policy-quota)
   */
  quota: 'Quota',
  /**
   * @description The Storage Bucket's expiration (https://wicg.github.io/storage-buckets/explainer#storage-policy-expiration)
   */
...
export class StorageBucketsViewWrapper extends UI.ThrottledWidget.ThrottledWidget {
  ...
  override async doUpdate(): Promise<void> {
    this.#storageBucketsView.data = {model: this.#model, bucketInfo: this.#bucketInfo};
  }
}
...
export class StorageBucketsView extends HTMLElement {
  ...
  #bucketInfo: Protocol.Storage.StorageBucketInfo|null = null;
  ...
  #bucketName(): string {
    ...
    return `${bucket.name} - ${bucket.storageKey}`;
  }
  ...
  #getExpirationString(): string {
    ...
    const {expiration} = this.#bucketInfo;
  ...
  #renderBucketInformation(): LitHtml.TemplateResult {
    ...
    const {bucket: {storageKey}, persistent, durability} = this.#bucketInfo;
  ...
  #renderRow(key: string, value: string | number | boolean): LitHtml.TemplateResult {
    return LitHtml.html`
      <${ReportView.ReportView.ReportKey.litTagName}>${key
      }</${ReportView.ReportView.ReportKey.litTagName}>
      <${ReportView.ReportView.ReportValue.litTagName}>${value
  ...
  async #deleteBucket(): Promise<void> {
    ...
    if (ok) {
      this.#model.deleteBucket(this.#bucketInfo.bucket);
    }
  }
}
