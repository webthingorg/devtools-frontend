...
export class PermissionsPolicySection extends HTMLElement {
  ...
  private policies: Protocol.Page.PermissionsPolicyFeatureState[] = [];
  private showPermissionsDisallowedDetails = false;

  connectedCallback(): void {
    console.log(  // eslint-disable-line no-console
        'PermissionsPolicySection connectedCallback', this.showPermissionsDisallowedDetails);
  }

  disconnectedCallback(): void {
    console.log(  // eslint-disable-line no-console
        'PermissionsPolicySection disconnectedCallback', this.showPermissionsDisallowedDetails);
  }
  ...
  private renderAllowed(): LitHtml.TemplateResult|{} {
    const allowed = this.policies.filter(p => p.allowed).map(p => p.feature).sort();
    if (!allowed.length) {
  ...
  private async renderDisallowed(): Promise<LitHtml.TemplateResult|{}> {
    const disallowed = this.policies.filter(p => !p.allowed).sort((a, b) => a.feature.localeCompare(b.feature));
