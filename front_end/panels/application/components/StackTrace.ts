...
export class StackTrace extends HTMLElement {
  ...
  createRowTemplates(): LitHtml.TemplateResult[] {
    ...
    if (hiddenCallFramesCount) {
      ...
      // clang-format off
      expandableRows.push(LitHtml.html`
      <${StackTraceLinkButton.litTagName} .data=${{onShowAllClick: this.onShowAllClick, hiddenCallFramesCount: hiddenCallFramesCount} as StackTraceLinkButtonData}></${StackTraceLinkButton.litTagName}>
      `);
      // clang-format on
    }
