...
export class ComressionDictionariesTreeElement extends ApplicationPanelTreeElement {
  ...
  constructor(resourcesPanel: ResourcesPanel, info: Protocol.Network.CompressionDictionaryStorageInfo) {
    super(resourcesPanel, info.frameOrigin, false);
    this.#info = info;
  }

  static createElement(resourcesPanel: ResourcesPanel, info: Protocol.Network.CompressionDictionaryStorageInfo):
      ComressionDictionariesTreeElement {
    const treeElement = new ComressionDictionariesTreeElement(resourcesPanel, info);
  ...
  refreshTree(): void {
    ...
    for (const [key, info] of storages) {
      if (!this.#treeElements.has(key)) {
        const element = ComressionDictionariesTreeElement.createElement(this.resourcesPanel, info);
