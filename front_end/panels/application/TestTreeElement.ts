...
export class TestTreeElement extends ExpandableApplicationPanelTreeElement {
  private originPrivateFileSystemApi?: ProtocolProxyApi.OriginPrivateFileSystemApi;
  ...
  async initialize(target: SDK.Target.Target): Promise<void> {
    this.originPrivateFileSystemApi = target.originPrivateFileSystemAgent();
    await this.originPrivateFileSystemApi.invoke_populateListWithMockData({
      storageKey: '',
    });
  }
  ...
  private populateListWithMockData(): void {
    // TODO: Add log to test functionality here
  }
}
