...
function eventEquals(
  ...
  return (
      a.accessTime === b.accessTime && a.type === b.type && a.ownerOrigin === b.ownerOrigin && a.params === b.params);
}
...
export class SharedStorageEventsView extends UI.SplitWidget.SplitWidget {
  ...
  private async onFocus(event: Event): Promise<void> {
    ...
    const wrappedEvent:
        WrappedEvent = {accessTime: eventTime, accessType: eventType, ownerOrigin: origin, eventParams: params};

    const wrappedEventJSON: JSON = <JSON>wrappedEvent;

    const jsonView = await SourceFrame.JSONView.JSONView.createView(JSON.stringify(wrappedEventJSON));
