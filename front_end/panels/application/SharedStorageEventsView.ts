...
export class SharedStorageEventsView extends UI.SplitWidget.SplitWidget {
  ...
  constructor() {
    ...
    this.contentElement.setAttribute('jslog', `${VisualLogging.pane().context('shared-storage')}`);
    ...
    this.#sharedStorageEventGrid.setAttribute('jslog', `${VisualLogging.section().context('shared-storage.events')}`);
