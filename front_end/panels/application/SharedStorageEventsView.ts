...
export class SharedStorageEventsView extends UI.SplitWidget.SplitWidget {
  ...
  private async onFocus(event: Event): Promise<void> {
    ...
    const eventArray = new Array(
        '{accessTime: ', eventTime, ', accessType: ', eventType, ', ownerOrigin: ', origin, ', eventParams : ', params,
        '}');

    const wrappedEvent: string = eventArray.join('');

    const jsonView = await SourceFrame.JSONView.JSONView.createView(wrappedEvent);
