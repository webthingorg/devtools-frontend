...
export class ServiceWorkersView extends UI.Widget.VBox implements
  ...
  constructor() {
    ...
    seeOthers.setAttribute('jslog', `${VisualLogging.action().track({click: true}).context('see-all-registrations')}`);
    ...
    // should be record hover for these? (tooltip with an explainer is shown)
    const forceUpdate =
...
export class Section {
  ...
  constructor(
    ...
    // should this be a link? it opens the network panel and highlights the requests, it looks like a link and not an not an icon though
    this.networkRequests.element.setAttribute(
        'jslog', `${VisualLogging.action().track({click: true}).context('network-requests')}`);
  ...
  private createSyncNotificationField(
    ...
    editor.setAttribute('jslog', `${VisualLogging.textField().track({keydown: true}).context(context)}`);
    form.appendChild(editor);
    const button = UI.UIUtils.createTextButton(label);
    button.type = 'submit';
    button.setAttribute('jslog', `${VisualLogging.action().track({click: true}).context('submit')}`);
