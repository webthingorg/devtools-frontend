...
export class ServiceWorkerCacheView extends UI.View.SimpleView {
  ...
  private cacheContentUpdated(event: Common.EventTarget.EventTargetEvent): void {
    const nameAndOrigin = event.data;
    if (this.cache.securityOrigin !== nameAndOrigin.origin || this.cache.cacheName !== nameAndOrigin.cacheName) {
      return;
    }
