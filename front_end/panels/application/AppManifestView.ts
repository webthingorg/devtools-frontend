...
export class AppManifestView extends UI.Widget.VBox implements SDK.TargetManager.Observer {
  ...
  async _renderManifest(
    ...
    let squareSizedIconAavailable = false;
    for (const icon of icons) {
      ...
      squareSizedIconAavailable = result.squareSizedIconAavailable || squareSizedIconAavailable;
    }
    if (!squareSizedIconAavailable) {
  ...
  async _appendImageResourceToSection(
      ...
      Promise<{imageResourceErrors: Platform.UIString.LocalizedString[], squareSizedIconAavailable?: boolean}> {
    ...
    if (!imageResource.sizes) {
      ...
      for (const size of sizes) {
        ...
        squareSizedIconAavailable = squareSizedIconAavailable || hasSquareSize;
        if (error) {
    ...
    field.appendChild(wrapper);
    return {imageResourceErrors, squareSizedIconAavailable};
  }
}
