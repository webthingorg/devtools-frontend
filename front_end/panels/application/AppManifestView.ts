...
export class AppManifestView extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox)
  ...
  private async renderManifest(
    ...
    if (hasWco) {
      ...
      const wcoStyleSheetText = await this.overlayModel.hasStyleSheetTextForWindowControlsOverlay(url);
      if (!wcoStyleSheetText) {
        return;
      }

      this.overlayModel.setwindowControlsOverlayModeActive(false);
      this.overlayModel.toggleWindowControlsOverlayToolbar(false);
      ...
      wcoOsCheckbox.checkboxElement.addEventListener('click', async () => {
        this.overlayModel?.toggleWindowControlsOverlayToolbar(wcoOsCheckbox.checkboxElement.checked);
        if (!this.overlayModel?.windowControlsOverlayModeActive()) {
          // If Window Controls Overlay mode is not active, uncheck the Window Controls Overlay CSS checkbox
      ...
      osSelectElement.addEventListener('change', async () => {
        ...
        if (this.overlayModel) {
          this.overlayModel.updateWindowControlsOverlayPlatform(selectedOS);
          this.overlayModel.toggleWindowControlsOverlayToolbar(wcoOsCheckbox.checkboxElement.checked);
        }
      });
      ...
      wcoCssCheckbox.checkboxElement.addEventListener('click', async () => {
        this.overlayModel?.toggleWindowControlsOverlayCss(wcoCssCheckbox.checkboxElement.checked);
      ...
      this.overlayModel.updateWindowControlsOverlayThemeColor(stringProperty('theme_color'));
    } else {
