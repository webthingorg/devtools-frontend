...
const UIStrings = {
  ...
  screenshotsMustHaveSameAspectRatio:
      'All screenshots with the same "form_factor" must have the same aspect ratio. Some screenshots will be ignored.',
  /**
...
export class AppManifestView extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox)
  ...
  private async renderManifest(
    ...
    const screenshotAspectRatio = new Map<string, number>();
    ...
    for (const screenshot of screenshots) {
      ...
      const {imageResourceErrors: screenshotErrors, naturalAspectRatio} =
      ...
      if (screenshot.form_factor && naturalAspectRatio) {
        screenshotAspectRatio.has(screenshot.form_factor) ||
            screenshotAspectRatio.set(screenshot.form_factor, naturalAspectRatio);
        haveScreenshotsDifferentAspectRatio = haveScreenshotsDifferentAspectRatio ||
            (screenshotAspectRatio.get(screenshot.form_factor) !== naturalAspectRatio);
      }
  ...
  private async appendImageResourceToSection(
    ...
    return {imageResourceErrors, squareSizedIconAvailable, naturalAspectRatio};
  }
