...
export class AppManifestView extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox)
  ...
  constructor(
    ...
    this.errorsSection.element.setAttribute('jslog', `${VisualLogging.section().context(UIStrings.errorsAndWarnings)}`);
    this.installabilitySection = this.reportView.appendSection(i18nString(UIStrings.installability));
    this.installabilitySection.element.setAttribute(
        'jslog', `${VisualLogging.section().context(UIStrings.installability)}`);
    this.identitySection = this.reportView.appendSection(i18nString(UIStrings.identity));
    this.identitySection.element.setAttribute('jslog', `${VisualLogging.section().context(UIStrings.identity)}`);
    this.presentationSection = this.reportView.appendSection(i18nString(UIStrings.presentation));
    this.presentationSection.element.setAttribute(
        'jslog', `${VisualLogging.section().context(UIStrings.presentation)}`);
    ...
    this.protocolHandlersSection.element.setAttribute(
        'jslog', `${VisualLogging.section().context(UIStrings.protocolHandlers)}`);
    ...
    this.iconsSection.element.setAttribute('jslog', `${VisualLogging.section().context(UIStrings.icons)}`);
    ...
    this.windowControlsSection.element.setAttribute(
        'jslog', `${VisualLogging.section().context(UIStrings.windowControlsOverlay)}`);
  ...
  private async renderManifest(
    ...
    if (appId && recommendedId) {
      ...
      if (!stringProperty('id')) {
        ...
        copyButton.setAttribute('jslog', `${VisualLogging.action().track({click: true, hover: true}).context('copy')}`);
    ...
    if (startURL) {
      ...
      if (completeURL) {
        ...
        link.tabIndex = 0;
        link.setAttribute('jslog', `${VisualLogging.link().track({click: true}).context(UIStrings.startUrl)}`);
    ...
    setIconMaskedCheckbox.setAttribute(
        'jslog', `${VisualLogging.toggle().track({change: true}).context(UIStrings.showOnlyTheMinimumSafeAreaFor)}`);
    ...
    for (const shortcut of shortcuts) {
      ...
      // or should this be item?
      shortcutSection.element.setAttribute('jslog', `${VisualLogging.section().context(UIStrings.shortcutS)}`);
      ...
      link.setAttribute('jslog', `${VisualLogging.link().track({click: true}).context('shortcut-item')}`);
      link.tabIndex = 0;
    ...
    wcoDocumentationLink.setAttribute(
        'jslog', `${VisualLogging.link().track({click: true}).context(UIStrings.customizePwaTitleBar)}`);
