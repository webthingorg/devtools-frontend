...
const UIStrings = {
  ...
  heur: 'heur',
  /**
   * @description Label for checkbox in the Autofill pane. If checked, this pane will open
   * automatically whenever a form is being autofilled.
   */
  autoShow: 'Automatically open pane on autofill event',
};
...
export class AutofillView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  connectedCallback(): void {
    ...
    const formFilledEvent = autofillManager.getAddressFormFilled();
    if (formFilledEvent) {
  ...
  #onAddressFormFilled(
      {data}: Common.EventTarget.EventTargetEvent<
          AutofillManager.AutofillManager.EventTypes[AutofillManager.AutofillManager.Events.AddressFormFilled]>): void {
