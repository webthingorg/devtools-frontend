  ...
  static createUserAgentSelectAndInput(title: string): {
    ...
    function userAgentSelected(): void {
      ...
      if (value !== customOverride.value) {
        ...
        SDK.NetworkManager.MultitargetNetworkManager.instance().setUserAgentOverride(value, userAgentMetadata);
      } else {
    ...
    for (const userAgentVersion of userAgentDescriptor.values) {
      if (userAgent ==
        ...
        if (!userAgentVersion.metadata)
          return null
          SDK.NetworkManager.MultitargetNetworkManager.patchUserAgentMetadataWithChromeVersion(
              userAgentVersion.metadata);
