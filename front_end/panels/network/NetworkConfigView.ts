...
export class NetworkConfigView extends UI.Widget.VBox {
  ...
  static createUserAgentSelectAndInput(title: string): {
    ...
    uaClientHintsBrandsContainer2.classList.add('network-config-ua-client-hints-container');
    ...
    uaClientHintsDeviceContainer.appendChild(addButton);
    ...
    function addCustomizedUAClientHints(): void {
      ...
      const brands = [];
      const brandContainers = uaClientHintsBrandsContainer.childNodes;
      // skip the first child which is a 'User agent' label
      for (let i = 1; i < brandContainers.length; i++) {
        const brand = getTextValueFromSuggestBox(brandContainers[i].childNodes[0]);
        const version = getTextValueFromSuggestBox(brandContainers[i].childNodes[1]);
        if (brand) {
