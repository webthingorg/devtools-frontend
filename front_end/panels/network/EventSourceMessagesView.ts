...
const UIStrings = {
   ...
   */
  enterRegex: 'Enter regex, for example: (http)?s',
};
...
export class EventSourceMessagesView extends UI.Widget.VBox {
  ...
  private clearMessages(): void {
    _clearMessageOffsets.set(this.request, this.request.eventSourceMessages().length);
    this.refresh();
  }
  ...
  refresh(): void {
    ...
    const offset = _clearMessageOffsets.get(this.request) || 0;
...
// TODO(crbug.com/1172300) Ignored during the jsdoc to ts migration)
// eslint-disable-next-line @typescript-eslint/naming-convention
const _clearMessageOffsets = new WeakMap<SDK.NetworkRequest.NetworkRequest, number>();
