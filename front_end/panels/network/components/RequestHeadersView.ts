...
export class RequestHeadersComponent extends HTMLElement {
  ...
  #renderGeneralSection(): LitHtml.TemplateResult {
    ...
    let statusCodeImageName = 'red-ball-icon';
    if (this.#request.statusCode < 300 || this.#request.statusCode === 304) {
      statusCodeImageName = 'green-ball-icon';
    } else if (this.#request.statusCode < 400) {
      statusCodeImageName = 'yellow-ball-icon';
    }
      ...
      <${Category.litTagName} .data=${{name: 'general', title: i18nString(UIStrings.general)} as CategoryData}>
        ...
        ${this.#request.statusCode? html`
          ...
          <div class="row">
            ...
            <div class="header-value ${statusFromCache ? 'status-from-cache' : ''}"><${IconButton.Icon.Icon.litTagName} class="inline-icon" .data=${{
              iconName: statusCodeImageName,
              color: '',
              width: '11px',
              height: '11px',
            } as IconButton.Icon.IconData}>
            </${IconButton.Icon.Icon.litTagName}>${statusText}</div>
          </div>
        ` : ''}
