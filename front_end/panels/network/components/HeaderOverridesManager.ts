...
export class HeaderOverridesManager {
  ...
  static generateHeaderDescriptors(request: Readonly<SDK.NetworkRequest.NetworkRequest>): HeaderDescriptor[] {
    ...
    if (request.wasBlocked()) {
      const headerWithIssues = BlockedReasonDetails.get((request.blockedReason() as Protocol.Network.BlockedReason));
      if (headerWithIssues) {
    ...
    headers = mergeHeadersWithIssues(headers, headersWithIssues);
