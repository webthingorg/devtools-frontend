...
export class ResponseHeaderSection extends HTMLElement {
  ...
  set data(data: ResponseHeaderSectionData) {
    ...
    // the Chromium backend. DevTools therefore uses a previously stored copy.
    const setCookieHeaders =
        (this.#request.getAssociatedData(SDK.NetworkManager.SET_COOKIE_HEADER_OVERRIDES_DATA_KEY) || []) as
        Protocol.Fetch.HeaderEntry[];
    const headers = this.#request.sortedResponseHeaders.concat(setCookieHeaders);
