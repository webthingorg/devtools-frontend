...
export class ResponseHeaderSection extends HTMLElement {
  ...
  #onHeaderValueChanged(event: HeaderValueChangedEvent): void {
    ...
    // For the case in which there are multiple headers with the same name, we
    // either replace all of them with the (single) overridden value, or they
    // are already overrides in which case we need to keep them all.
    ...
    // If the last applyTo-block matches 'rawFileName', use this block.
    // Otherwise just append a new block at the end. We are not using earlier
    // blocks, because they could be overruled by blocks with wildcards coming
    // later.
