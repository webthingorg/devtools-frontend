    ...
    render(html`
      ${headerDescriptors.map((header, index) => html`
        <${HeaderSectionRow.litTagName} .data=${{
          header: header,
        } satisfies HeaderSectionRowData} @headeredited=${this.#onHeaderEdited} @headerremoved=${this.#onHeaderRemoved} @enableheaderediting=${this.#onEnableHeaderEditingClick} data-index=${index}></${HeaderSectionRow.litTagName}>
      `)}
