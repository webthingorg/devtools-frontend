...
const UIStrings = {
  /**
  *@description Label for a button which allows adding an HTTP header. The '+' character symbolizes that we're adding something (There isn't anything to the left of it).
  */
...
export class ResponseHeaderSection extends HTMLElement {
  ...
  #updateOverrides(headerName: Platform.StringUtilities.LowerCaseString, headerValue: string, index: number): void {
    ...
    if (this.#headers[index].name !== previousName) {
      const previousOverrideEntry =
          block.headers.find(header => header.name === previousName && header.value === previousValue);
      if (previousOverrideEntry) {
        const idx = block.headers.indexOf(previousOverrideEntry);
        if (idx >= 0) {
