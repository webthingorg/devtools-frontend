...
export class NetworkWaterfallColumn extends UI.Widget.VBox {
  ...
  private static buildResourceTypeStyle(): Map<Common.ResourceType.ResourceType, _LayerStyle>[] {
    ...
    function toBorderColor(color: string): string|null {
      ...
      s /= 2;
      l -= Math.min(l, 0.2);
      return new Common.Color.HSL(parsedColor.h, s, l, parsedColor.alpha, undefined)
    ...
    function toWaitingColor(color: string): string|null {
      ...
      let {l} = parsedColor;
      l *= 1.1;
      return new Common.Color.HSL(parsedColor.h, parsedColor.s, l, parsedColor.alpha, undefined)
          .canonicalize({clipToGamut: true, zeroPowerlessComponents: true})
          .asString();
    }
  }
