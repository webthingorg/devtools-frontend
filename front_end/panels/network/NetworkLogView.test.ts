...
describeWithMockConnection('NetworkLogView', () => {
  ...
  it('"Copy all" commands respects filters', async () => {
    ...
    const copyAllURLsId = copyAllURLs.buildDescriptor().id;
    assert.isDefined(copyAllURLsId);
    contextMenu.invokeHandler(copyAllURLsId);
...
url-header-und-content-overridden`]);
    ...
    const copyAllCurlComnmandsId = copyAllCurlComnmands.buildDescriptor().id;
    assert.isDefined(copyAllCurlComnmandsId);
    contextMenu.invokeHandler(copyAllCurlComnmandsId);
    await expectCalled(copyText);
    assert.strictEqual(copyText.callCount, 1);
...
curl 'url-header-und-content-overridden'`]);
    ...
    const copyAllFetchCallId = copyAllFetchCall.buildDescriptor().id;
    assert.isDefined(copyAllFetchCallId);
    contextMenu.invokeHandler(copyAllFetchCallId);
    await expectCalled(copyText);
    assert.strictEqual(copyText.callCount, 1);
...
fetch("url-header-und-content-overridden", {
  ...
  "credentials": "omit"
    ...
    const copyAllPowerShellId = copyAllPowerShell.buildDescriptor().id;
    assert.isDefined(copyAllPowerShellId);
    contextMenu.invokeHandler(copyAllPowerShellId);
    await expectCalled(copyText);
    assert.strictEqual(copyText.callCount, ++callCount);
