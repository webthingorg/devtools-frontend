...
export class IssueAggregator extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  private aggregateIssue(issue: IssuesManager.Issue.Issue): AggregatedIssue {
    const aggregatedIssue = issue.isHidden() ? this.aggregateIssueByStatus(this.hiddenAggregatedIssuesByCode, issue) :
                                               this.aggregateIssueByStatus(this.aggregatedIssuesByCode, issue);
