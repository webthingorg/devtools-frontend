...
export class HideIssuesMenu extends HTMLElement {
  ...
  // private callback: ((eventType: string, data?: unknown) => void)|null = null;
  private callback: ((eventType: string) => void)|null = null;
  ...
  clickHandler(event: Event): void {
    ...
    contextMenu.headerSection().appendItem(
        i18nString(UIStrings.hideIssueByKind, {PH1: kind}), () => this.hideIssueClickHandler(MenuEntryAction.Kind));
    contextMenu.headerSection().appendItem(
  ...
  hideIssueClickHandler(action: MenuEntryAction): void {
    ...
    if (this.callback) {
      setting.set(values);
      this.callback(IssuesManager.Issue.IssueAction.HideIssue);
    }
  }
