...
export class CoverageModel extends SDK.SDKModel.SDKModel<EventTypes> {
  ...
  private async resolveSourceMapsAndUpdate(): Promise<void> {
    ...
    this.processSourceMapBacklog = [];
    // this.sourceInfo = await this.getSourceInfo();
    for (const {script, sourceMap} of currentBacklog) {
      await this.resolveSourceMap(script, sourceMap);
    }
  ...
  private calculateSizeForSources(sourceMap: SDK.SourceMap.SourceMap, text: TextUtils.Text.Text, contentLength: number):
    ...
    // Map shows that which source file contribute to the size in the generated file. For example:
    // Map(2) {url1 => 593, url2 => 232, url3 => 52}
    ...
    // [{end: 593, sourceUrl: url1}, {end: 781, sourceUrl: url2}, {end: 833, sourceUrl: url3}, {end: 881, sourceUrl: url1}]
  ...
  private async addCoverage(
    ...
    if (!urlCoverage) {
      ...
      if (sourceMap) {
        ...
        urlCoverage.setSourceSegments(sourceSegments);
        for (const sourceURL of sourceMap.sourceURLs()) {
          const subentry = this.addCoverageForSource(
              sourceURL, sourceSizeMap.get(sourceURL) || 0, type as CoverageType, urlCoverage);
          if (subentry) {
            coverageInfos.push(subentry);
          }
        }
      }
    }
