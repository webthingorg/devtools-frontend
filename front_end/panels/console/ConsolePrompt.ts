...
export class ConsolePrompt extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.Widget>(
  ...
  private editorKeymap(): readonly CodeMirror.KeyBinding[] {
    return [
      {key: 'UpArrow', run: (): boolean => this.moveHistory(-1), shift: (): boolean => this.moveHistory(-1, true)},
      {key: 'DownArrow', run: (): boolean => this.moveHistory(1), shift: (): boolean => this.moveHistory(1, true)},
