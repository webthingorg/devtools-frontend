...
export class ConsolePrompt extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.Widget>(
  ...
  #waitingForSelfXssWarningResponse = false;
  ...
  showSelfXssWarning(): void {
    ...
    this.#waitingForSelfXssWarningResponse = true;
  }

  private async handleEnter(forceEvaluate?: boolean): Promise<void> {
    if (this.#waitingForSelfXssWarningResponse && this.text() === i18nString(UIStrings.allowPasting)) {
      ...
      Common.Settings.Settings.instance()
          ...
          .set(true);
      this.#waitingForSelfXssWarningResponse = false;
      return;
    }
    this.#waitingForSelfXssWarningResponse = false;
