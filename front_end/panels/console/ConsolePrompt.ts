...
export class ConsolePrompt extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.Widget>(
    UI.Widget.Widget) {
  #addCompletionsFromHistory: boolean;
  #historyInternal: ConsoleHistoryManager;
  #initialText: string;
  #editor: UI.TextEditor.TextEditor|null;
  readonly #eagerPreviewElement: HTMLDivElement;
  #textChangeThrottler: Common.Throttler.Throttler;
  readonly #formatter: ObjectUI.RemoteObjectPreviewFormatter.RemoteObjectPreviewFormatter;
  #requestPreviewBound: () => Promise<void>;
  readonly #innerPreviewElement: HTMLElement;
  readonly #promptIcon: UI.Icon.Icon;
  readonly #iconThrottler: Common.Throttler.Throttler;
  readonly #eagerEvalSetting: Common.Settings.Setting<boolean>;
  #previewRequestForTest: Promise<void>|null;
  #defaultAutocompleteConfig: UI.TextEditor.AutocompleteConfig|null;
  #highlightingNode: boolean;
...
export class ConsoleHistoryManager {
  ...
  pushHistoryItem(text: string): void {
    if (this.#uncommittedIsTop) {
      this.#data.pop();
      this.#uncommittedIsTop = undefined;
    }
