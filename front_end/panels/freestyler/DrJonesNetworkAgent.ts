...
QUERY: Explain this network request
...
ANSWER: This request aims to retrieve a list of products matching the search query "laptop" within the "electronics" category. The successful 200 OK status confirms that the server fulfilled the request and returned the relevant data.
`;
/* clang-format on */
...
export class DrJonesNetworkAgent {
  ...
  static parseResponse(response: string): {answer?: string} {
    ...
    while (i < lines.length) {
      ...
      if (trimmed.startsWith('ANSWER:') && !answer) {
        const answerLines = [
  ...
  async * run(options: {
    ...
    const query = `${
        options.selectedNetworkRequest ?
            ...
            ''}QUERY: ${EXPLAIN_THIS_NETWORK_REQUEST}`;
    ...
    const {answer} = DrJonesNetworkAgent.parseResponse(response);
    if (answer) {
