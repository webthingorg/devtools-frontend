...
const ENABLE_ELEMENT_DATA = false;
const ENABLE_CONSIDERATIONS = true;

/* clang-format off */
const preamble = `You are the most advanced CSS debugging assistant integrated into Chrome DevTools.
You always suggest considering the best web development practices and the newest platform features such as view transitions.
The user selected a DOM element in the browser's DevTools and sends a CSS-related query about the page or the selected DOM element.
${ENABLE_CONSIDERATIONS ? `\n# Considerations
...
export class FreestylerAgent {
  ...
  static async getElementData(element: SDK.DOMModel.DOMNode): Promise<string> {
    if (!element) {
      return '';
    }
    ...
    if (parentNode) {
      ...
      if (parentChildrenNodes) {
        ...
        switch (childElementNodes.length - 1) {
          case 0:
            output += '\n* It doesn\'t have any sibling element nodes';
            break;
          case 1:
            output += '\n* It has only 1 sibling element node';
            break;
          default:
            output += `\n* It has ${childElementNodes.length - 1} sibling element nodes`;
            break;
        }
        ...
        switch (siblingTextNodes.length) {
          case 0:
            output += '\n* It doesn\'t have any sibling text nodes';
            break;
          case 1:
            output += '\n* It has only 1 sibling text node';
            break;
          default:
            output += `\n* It has ${siblingTextNodes.length - 1} sibling text nodes`;
            break;
        }
      }
    }
  ...
  async * run(query: string, options: {
    ...
    const structuredLog = [];
    query = `${
        options.selectedElement && ENABLE_ELEMENT_DATA ?
            `# Inspected element\n${
                await FreestylerAgent.getElementData(options.selectedElement)}\n\n# User request\n\n` :
            ''}QUERY: ${query}`;
