...
async function executeJsCode(code: string, {throwOnSideEffect}: {throwOnSideEffect: boolean}): Promise<string> {
  ...
  let frameId = selectedNode?.frameId();
  let target: SDK.Target.Target|null|undefined = selectedNode?.domModel().target();
  if (!target) {
    target = UI.Context.Context.instance().flavor(SDK.Target.Target);
