...
export class FreestylerAgent {
  ...
  async #generateObservation(action: string, {throwOnSideEffect, confirm, runDeniedMessage: denyErrorMessage}: {
    throwOnSideEffect: boolean,
    confirm?: (this: FreestylerAgent, action: string) => Promise<boolean>,
    runDeniedMessage?: string,
  }): Promise<string> {
    ...
    try {
      ...
      if (error instanceof SideEffectError) {
        return await this.#generateObservation(action, {
          ...
          confirm: this.#confirmSideEffect,
          runDeniedMessage: error.message,
        });
      }
