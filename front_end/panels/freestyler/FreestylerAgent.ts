...
function parseResponse(_response: string): {thought?: string, action?: string, answer?: string} {
  // TODO: imlement response parsing.
  return {};
}
  ...
  async run(query: string, onStep: (step: Step, stepOutput: string) => void): Promise<void> {
    ...
    for (let i = 0; i < MAX_STEPS; i++) {
      ...
      if (!thought && !action && !answer) {
        onStep(Step.ANSWER, 'Sorry, I could not help you with this query.');
        break;
      }

      if (answer) {
        onStep(Step.ANSWER, answer);
        chatHistory.push({
          text: `ANSWER: ${answer}`,
