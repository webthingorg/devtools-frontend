...
const preamble = (selectedElementData: string): string =>
...
* **CRITICAL** Use \`window.getComputedStyles\` ALWAYS with property access, like \`window.getComputedStyle($0.parentElement)['color']\`
...
const data = {
  ...
  elementStyles: {
    ...
    visibility: window.getComputedStyle($0)['visibility'],
    position: window.getComputedStyle($0)['position'],
    clipPath: window.getComputedStyle($0)['clip-path'],
    zIndex: window.getComputedStyle($0)['z-index']
  },
  parentStyles: {
    display: window.getComputedStyle($0)['display'],
    visibility: window.getComputedStyle($0)['visibility'],
...
${selectedElementData ? `# Selected element data\n${selectedElementData}` : ''}
`;
  ...
  async * run(query: string, options: {
    ...
    for (let i = 0; i < MAX_STEPS; i++) {
      ...
      const getData = async(): Promise<string> => {
        ...
        let output = `\n* Its selector is \`${options.selectedElement.simpleSelector()}\``;
        const childNodes = await options.selectedElement.getChildNodesPromise();
        if (childNodes) {
      ...
      const request = FreestylerAgent.buildRequest({
        input: query,
        preamble: preamble(await getData()),
