...
* ANSWER
Use INFORMATION_RETRIEVAL to evaluate JS code (without markdown) on the page to gather all the data needed to answer the query and put it inside the data variable - then return STOP.
OBSERVATION will be the result of running the JS code on the page.
You can then answer the question with ANSWER or run another INFORMATION_RETRIEVAL query.
Please run INFORMATION_RETRIEVAL again if the information you got is not enough to answer the query.

* YOU CANNOT USE 3rd PARTY LIBRARIES WHILE EXECUTING CODE!
  ...
  static parseResponse(response: string): {thought?: string, action?: string, answer?: string} {
    ...
    while (i < lines.length) {
      ...
      if (trimmed.startsWith('THOUGHT:') && !thought) {
        ...
        i++;
      } else if (trimmed.startsWith('INFORMATION_RETRIEVAL') && !action) {
        const actionLines = [];
        let j = i + 1;
        ...
        while (j < lines.length) {
          ...
          if (line.startsWith('INFORMATION_RETRIEVAL') || line.startsWith('OBSERVATION:') ||
