...
export class FreestylerChatUi extends HTMLElement {
  ...
  #renderSelectAnElement = (): LitHtml.TemplateResult => {
    ...
    return LitHtml.html`
      <${Buttons.Button.Button.litTagName} .data=${{
        ...
        title: i18n.i18n.lockedString(TempUIStrings.sendButtonTitle),
      } as Buttons.Button.ButtonData} @click=${this.#props.onInspectElementClick}></${Buttons.Button.Button.litTagName}>
      <span class="select-an-element-text">${i18n.i18n.lockedString(TempUIStrings.selectAnElement)}</span>
    `;
    // clang-format on
  };
  ...
  #renderEmptyState = (): LitHtml.TemplateResult => {
    ...
    return LitHtml.html`<div class="empty-state-container">
      ...
      ${i18n.i18n.lockedString(TempUIStrings.emptyStateText)}
    </div>`;
    // clang-format on
  };
        ...
        <form class="input-form" @submit=${this.#handleSubmit}>
          ...
          <div class="chat-input-container">
            <input type="text" class="chat-input" .disabled=${!this.#props.selectedNode}
              placeholder=${i18n.i18n.lockedString(TempUIStrings.inputPlaceholder)}>
            <${Buttons.Button.Button.litTagName}
              class="step-actions"
              type="submit"
              title=${i18n.i18n.lockedString(TempUIStrings.sendButtonTitle)}
              aria-label=${i18n.i18n.lockedString(TempUIStrings.sendButtonTitle)}
          ...
          <span class="chat-input-disclaimer">${i18n.i18n.lockedString(TempUIStrings.inputDisclaimer)}</span>
        </form>
      </div>
    `;
    // clang-format on
