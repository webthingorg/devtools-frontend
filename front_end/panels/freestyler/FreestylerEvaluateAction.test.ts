...
describe('FreestylerEvaluateAction', () => {
  describe('error handling', () => {
    ...
    it('should throw an Freestyler.ExecutionError with the description of the exception if response included exception details',
       async () => {
         try {
  ...
  describeWithRealConnection('serialization', () => {
    ...
    it('should stop serializing cycles after some max depth (5) is reached', async () => {
      assert.strictEqual(
          await executeForTest(`
        const obj = { a: 1 };
        obj.itself = obj;
        obj
      `),
          '{a: 1, itself: {a: 1, itself: {a: 1, itself: {a: 1, itself: {a: (max depth), itself: (max depth)}}}}}');
    });
  });
});
