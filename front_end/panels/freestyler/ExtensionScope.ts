 ...
 *
 * TODO: The scope should use the execution context of the selected node.
 */
export class ExtensionScope {
  ...
  async #simpleEval(context: SDK.RuntimeModel.ExecutionContext, expression: string): Promise<void> {
  ...
  async #bindingCalled(executionContext: SDK.RuntimeModel.ExecutionContext, event: {
    ...
    const response = await executionContext.evaluate(
        {
          expression: `freestyler.getArgs(${id})`,
          replMode: true,
          includeCommandLineAPI: false,
          returnByValue: true,
          silent: false,
    ...
    const arg = JSON.parse(response.object.value);
...
globalThis.freestyler = (args) => {
  ...
  __freestyler(String(freestyler.id));
  freestyler.id++;
}
freestyler.id = 1;
