...
export class PlayerListView extends UI.Widget.VBox implements TriggerDispatcher {
  ...
  private readonly playerEntriesWithHostnameFrameTitle: Set<string>private readonly mainContainer: MainView;
  ...
  private CreatePlayerListEntry(playerID: string) {
  ...
  private selectPlayer(playerID: string, element: Element) {
  ...
  private hidePlayersExcept(playerID: string|null) {
  }
  ...
  private setMediaElementFrameTitle(playerID: string, frameTitle: string, isHostname: boolean) {
  ...
  private setMediaElementPlayerTitle(playerID: string, playerTitle: string) {
    if (!this.playerEntryFragments.has(playerID))
      return;
    const fragment = this.playerEntryFragments.get(playerID);
  ...
  private setMediaElementPlayerIcon(playerID: string, iconName: string) {
    if (!this.playerEntryFragments.has(playerID))
      return;
    const fragment = this.playerEntryFragments.get(playerID);
    if (fragment === undefined)
  ...
  private formatAndEvaluate(playerID: string, func: Function, candidate: string, min: number, max: number) {
  ...
  onEvent(playerID: string, event: PlayerEvent): void {
    ...
    if (eventType === 'kLoad') {
      ...
      const video_name = url.substring(url.lastIndexOf('/') + 1);
      this.formatAndEvaluate(playerID, this.setMediaElementPlayerTitle, video_name, 1, 20);
      return;
    }
