...
export class PlayerMessagesView extends UI.Widget.VBox {
  ...
  private errorToDiv(error: Protocol.Media.PlayerError): Element {
    ...
    if (Object.keys(error.data).length !== 0) {
      ...
      const data_content = entry.$('status-error-data').createChild('div');
      for (const [key, value] of Object.entries(error.data)) {
        const datum_content = data_content.createChild('div');
        UI.UIUtils.createTextChild(datum_content, `${key}: ${value}`);
      }
    }
    ...
    if (error.stack.length !== 0) {
      ...
      const stack_content = entry.$('status-error-stack').createChild('div');
      for (const stack_entry of error.stack) {
        const frame_box = stack_content.createChild('div');
        UI.UIUtils.createTextChild(frame_box, `${stack_entry.file}:${stack_entry.line}`);
      }
    }
