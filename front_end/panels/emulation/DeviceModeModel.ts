...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper implements SDK.SDKModel.SDKModelObserver {
  ...
  _scaleSetting: Common.Settings.Setting<any>;
  _scale: number;
  _widthSetting: Common.Settings.Setting<any>;
  _heightSetting: Common.Settings.Setting<any>;
  _uaSetting: Common.Settings.Setting<any>;
  _deviceScaleFactorSetting: Common.Settings.Setting<any>;
  _deviceOutlineSetting: Common.Settings.Setting<any>;
  _toolbarControlsEnabledSetting: Common.Settings.Setting<any>;
  _type: Type;
  ...
  _calculateAndEmulate(resetPageScaleFactor: boolean): void {
    ...
    if (this._type === Type.Device && this._device && this._mode) {
      ...
      this._applyDeviceMetrics(new UI.Geometry.Size(screenWidth, screenHeight), new UI.Geometry.Insets(0, 0, 0, 0), new UI.Geometry.Insets(0, 0, 0, 0), this._scaleSetting.get(), this._deviceScaleFactorSetting.get() || defaultDeviceScaleFactor, mobile, screenHeight >= screenWidth ? Protocol.Emulation.ScreenOrientationType.PortraitPrimary :
        Protocol.Emulation.ScreenOrientationType.LandscapePrimary, resetPageScaleFactor);
  ...
  _applyDeviceMetrics(screenSize: UI.Geometry.Size, insets: UI.Geometry.Insets, outline: UI.Geometry.Insets, scale: number, deviceScaleFactor: number, mobile: boolean, screenOrientation: Protocol.Emulation.ScreenOrientationType | null, resetPageScaleFactor: boolean, forceMetricsOverride: boolean | undefined = false): void {
    ...
    const screenOrientationAngle = screenOrientation === Protocol.Emulation.ScreenOrientationType.LandscapePrimary ? 90 : 0;
  ...
  async captureScreenshot(fullSize: boolean, clip?: Protocol.Page.Viewport): Promise<string | null> {
    ...
    const screenshot = await screenCaptureModel.captureScreenshot(Protocol.Page.CaptureScreenshotRequestFormat.Png, 100, clip);
