...
export class DeviceModeView extends UI.Widget.VBox {
  ...
  private createUI(): void {
    ...
    this.responsivePresetsContainer.setAttribute(
        'jslog', `${VisualLogging.item().context('responsivePresetsContainer')}`);
...
export class Ruler extends UI.Widget.VBox {
  ...
  constructor(horizontal: boolean, applyCallback: (arg0: number) => void) {
    ...
    this.element.setAttribute('jslog', `${VisualLogging.item().context('deviceModeRuler').track({click: true})}`);
