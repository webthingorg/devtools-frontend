...
export class AdvancedApp implements Common.App.App {
  ...
  constructor() {
    UI.DockController.DockController.instance().addEventListener(UI.DockController.Events.BeforeDockSideChanged, this._openToolboxWindow, this);
  }
  ...
  presentUI(document: Document): void {
    ...
    UI.DockController.DockController.instance().addEventListener(UI.DockController.Events.BeforeDockSideChanged, this._onBeforeDockSideChange, this);
    UI.DockController.DockController.instance().addEventListener(UI.DockController.Events.DockSideChanged, this._onDockSideChange, this);
    UI.DockController.DockController.instance().addEventListener(UI.DockController.Events.AfterDockSideChanged, this._onAfterDockSideChange, this);
  ...
  _onBeforeDockSideChange(event: {
    ...
    if ( /** @type {string} */(event.data.to as string) === UI.DockController.State.Undocked && this._toolboxRootView) {
  ...
  _onDockSideChange(event?: {
    ...
    const toDockSide = event ? event.data.to as string : UI.DockController.DockController.instance().dockSide();
    if (toDockSide === UI.DockController.State.Undocked) {
      this._updateForUndocked();
    }
    else if (this._toolboxRootView && event &&
      (event.data.from as string) === UI.DockController.State.Undocked) {
