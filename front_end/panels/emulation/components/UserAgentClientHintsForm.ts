...
export class UserAgentClientHintsForm extends HTMLElement {
  ...
  private renderPlatformSection(): LitHtml.TemplateResult {
    ...
    return LitHtml.html`
      ...
      <input class="input-field half-row" type="text" @input="${handlePlatformVersionChange}" .value="${
        platformVersion}" placeholder="${i18nString(UIStrings.platformVersion)}" aria-label="${
        i18nString(UIStrings.platformVersion)}" />
    `;
  }
  ...
  private renderDeviceModelSection(): LitHtml.TemplateResult {
    ...
    return LitHtml.html`
      ...
      <input id="device-model" class="input-field ${
        this.showMobileCheckbox ? 'device-model-input' : 'full-row'}" type="text" @input="${
    ...
    return LitHtml.html`
      ...
      <div class="add-container full-row" role="button" tabindex="0" @click="${this.handleAddBrandClick}" @keypress="${
        ...
        <${IconButton.Icon.Icon.litTagName} aria-hidden="true"
          .data=${
        {color: 'var(--color-text-primary)', iconName: 'add-icon', width: '10px'} as IconButton.Icon.IconData}>
        </${IconButton.Icon.Icon.litTagName}>
  ...
  private render(): void {
    ...
    const submitButton = this.showSubmitButton ? LitHtml.html`
      <button type="submit" class="submit-button full-row">${i18nString(UIStrings.update)}</button>
    ` :
                                                 LitHtml.html``;
