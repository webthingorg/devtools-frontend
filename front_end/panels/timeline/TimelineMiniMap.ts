...
export class TimelineMiniMap extends
  ...
  constructor() {
    ...
    this.#overviewComponent.addEventListener(PerfUI.TimelineOverviewPane.Events.WindowChanged, event => {
      ...
      this.dispatchEventToListeners(PerfUI.TimelineOverviewPane.Events.WindowChanged, {...event.data, breadcrumb: {
        min: this.#breadcrumbs?.lastBreadcrumb.window.min + this.#minTime,
        max: this.#breadcrumbs?.lastBreadcrumb.window.max + this.#minTime,
        range: this.#breadcrumbs?.lastBreadcrumb.window.max - this.#breadcrumbs?.lastBreadcrumb.window.min
      }});
    });
