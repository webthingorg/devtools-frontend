  ...
  setData(data: OverviewData): void {
    ...
    if (data.traceParsedData) {
      ...
      if (this.#threadTracksSource === ThreadTracksSource.NEW_ENGINE) {
        this.#controls.push(
            new TimelineEventOverviewCPUActivity(null, data.traceParsedData, Boolean(data.isCpuProfile)));
      }
    }
    ...
    if (data.performanceModel && this.#threadTracksSource !== ThreadTracksSource.NEW_ENGINE) {
      this.#controls.push(
