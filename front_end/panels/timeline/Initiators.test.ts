...
describe('Initiators', () => {
  it('returns the initiator data', async function() {
    const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'set-timeout-long-task.json.gz');
  ...
  it('returns the initiator data for network requests', async function() {
    const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'network-requests-initiators.json.gz');
  ...
  it('will walk back through the initiators to find the entire chain', async function() {
    const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'nested-initiators.json.gz');
