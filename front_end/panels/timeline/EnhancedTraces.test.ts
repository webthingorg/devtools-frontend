...
describe('EnhancedTracesEngine', () => {
  ...
  function createTestTargetRundownTrace(
      target: EnhancedTraces.Target, executionContext: EnhancedTraces.ExecutionContext,
      script: EnhancedTraces.Script): TraceEngine.Types.TraceEvents.TraceEventTargetRundown {
    return {
      ...defaultTraceEvent,
      args: {
        data: {
  ...
  function createTestV8SourceRundownTrace(script: EnhancedTraces.Script):
      TraceEngine.Types.TraceEvents.TraceEventScriptRundown {
    return {
      ...defaultTraceEvent,
      args: {
        data: {
  ...
  function createTestV8SourceTextRundownTrace(script: EnhancedTraces.Script):
      TraceEngine.Types.TraceEvents.TraceEventScriptRundownSource {
    return {
      ...defaultTraceEvent,
      args: {
        data: {
  ...
  function createMockTrace(): TraceEngine.Types.TraceEvents.TraceEventData[] {
    ...
    result.push(createTestTargetRundownTrace(mockTarget1, mockExecutionContext1, mockScript1));
    result.push(createTestV8SourceRundownTrace(mockScript1));
    return result;
  }
  ...
  it('captures correct script info and source text', async function() {
    const mockTrace = createMockTrace();
    mockTrace.push(createTestV8SourceTextRundownTrace(mockScript1));
    const enhancedTraces = new EnhancedTraces.EnhancedTracesEngine(mockTrace);
  ...
  it('generates correct info with multiple targets, execution contexts and scripts', async function() {
    const mockTrace = createMockTrace();
    mockTrace.push(createTestV8SourceTextRundownTrace(mockScript1));
    mockTrace.push(createTestTargetRundownTrace(mockTarget1, mockExecutionContext2, mockScript2));
    mockTrace.push(createTestTargetRundownTrace(mockTarget2, mockExecutionContext3, mockScript3));
    mockTrace.push(createTestV8SourceRundownTrace(mockScript2));
    mockTrace.push(createTestV8SourceRundownTrace(mockScript3));
    mockTrace.push(createTestV8SourceTextRundownTrace(mockScript2));
    mockTrace.push(createTestV8SourceTextRundownTrace(mockScript3));
    const enhancedTraces = new EnhancedTraces.EnhancedTracesEngine(mockTrace);
    const targets = enhancedTraces.captureTargetsFromTraces();
    const executionContexts = enhancedTraces.captureExecutionContextFromTraces();
    const scripts = enhancedTraces.captureScriptFromTraces();
