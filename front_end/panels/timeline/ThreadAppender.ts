...
export class ThreadAppender implements TrackAppender {
  ...
  constructor(
    ...
    this.#groupedChildIndex = rasterCount;
   ...
   */
    ...
    if (this.threadType === ThreadType.RASTERIZER || this.threadType === ThreadType.THREAD_POOL) {
      this.#appendGroupedTrackHeaderAndTitle(trackStartLevel);
    } else {
   ...
   */
  #appendGroupedTrackHeaderAndTitle(trackStartLevel: number): void {
    if (this.#groupedChildIndex === 1) {
      const trackIsCollapsible = this.#entries.length > 0;
    ...
    const rasterizerTitle = this.threadType === ThreadType.RASTERIZER
      ? i18nString(UIStrings.rasterizerThreadS, {PH1: this.#groupedChildIndex})
      : i18nString(UIStrings.threadPoolThreadS, {PH1: this.#groupedChildIndex});
    const titleGroup =
