...
export class ThreadAppender implements TrackAppender {
  ...
  #appendNodesAtLevel(
    ...
    for (const node of nodes) {
      ...
      // To represent ignore listed stack frames, we add a flamechart
      // entry for the bottom frame with a "ignore listed" mark, the
      // rest of the frames in the ignored listed stack are ignored.
      const canAppendIgnoredListedEvent = !entryIsIgnoreListed || !parentIsIgnoredListed;

      if (entryIsVisible && canAppendIgnoredListedEvent) {
  ...
  isIgnoreListedEntry(entry: TraceEngine.Types.TraceEvents.TraceEventData): boolean {
    if (!Root.Runtime.experiments.isEnabled('ignoreListJSFramesOnTimeline')) {
      return false;
    }
