...
describeWithEnvironment('SidebarAnnotationsTab', () => {
  ...
  it('renders annotations list in the sidebar', async function() {
    ...
    // Create Entry Label overlays
    const entryLabelOverlay: TraceEngine.Types.File.OverlayAnnotation = {
      type: 'ENTRY_LABEL',
      entry: defaultTraceEvents[0],
      label: 'Entry Label 1',
    };

    const entryLabelOverlay2: TraceEngine.Types.File.OverlayAnnotation = {
      type: 'ENTRY_LABEL',
      entry: defaultTraceEvents[1],
      label: 'Entry Label 2',
    };

    component.updateAnnotationOverlays([entryLabelOverlay, entryLabelOverlay2]);
    assert.isNotNull(component.shadowRoot);
  ...
  it('updates annotations list in the sidebar when a new list is passed in', async function() {
    ...
    entryLabelOverlay.label = 'New Entry Label 1';
    entryLabelOverlay2.label = 'New Entry Label 2';
