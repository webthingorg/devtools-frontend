...
export class NetworkRequestTooltip extends HTMLElement {
  ...
  #renderPriority(): LitHtml.TemplateResult|null {
    ...
    if (this.#networkRequest.args.data.priority === this.#networkRequest.args.data.initialPriority) {
      return LitHtml.html`
        <div class="priority">${UIStrings.priority}: ${this.#networkRequest.args.data.priority}</div>
      `;
    }
    return LitHtml.html`
      <div class="priority">${UIStrings.priority}: ${this.#networkRequest.args.data.initialPriority} -> ${
        this.#networkRequest.args.data.priority}</div>
    `;
  }
  ...
  #renderTimings(): LitHtml.TemplateResult|null {
    ...
    const end = this.#networkRequest.ts + this.#networkRequest.dur - syntheticData.finishTime;
    const endTime = (end > 0 ? end : 0) as TraceEngine.Types.Timing.MicroSeconds;

    const category = TimelineUIUtils.syntheticNetworkRequestCategory(this.#networkRequest);
    ...
    return LitHtml.html`
      <ul>
        <li>
          <span class="whisker-left"> <span class="horizontal"></span> </span>
        ...
        <li>
          <span class="whisker-right"> <span class="horizontal"></span> </span>
          ${UIStrings.waitingOnMainThread}
          <span class="time">${i18n.TimeUtilities.formatMicroSecondsTime(endTime)}</span>
        </li>
      </ul>
    `;
  }
