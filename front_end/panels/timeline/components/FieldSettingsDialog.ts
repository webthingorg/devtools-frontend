...
export class FieldSettingsDialog extends HTMLElement {
  ...
  // Cannot use Lit template automatic binding because this event function is technically added to a different component
  // Arrow key events are used by the data grid to move around, but this makes it hard to edit the input values so we
  // should prevent key events from reaching the grid if they are coming from an input.
  #blockPropagation = (event: Event): void => {
    event.stopPropagation();
  };
        ...
        cells: [
          {
            ...
            renderer: value => html`
              <input
                ...
                aria-label=${i18nString(UIStrings.developmentOrigin)}
                style=${inputStyle}
                .value=${value}
                title=${value}
                @keyup=${this.#blockPropagation}
          ...
          },
          {
            ...
            renderer: value => html`
              <input
                ...
                aria-label=${i18nString(UIStrings.productionOrigin)}
                style=${inputStyle}
                .value=${value}
                title=${value}
                @keyup=${this.#blockPropagation}
