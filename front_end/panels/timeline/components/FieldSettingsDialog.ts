...
const UIStrings = {
  ...
  whenPerformanceIsShown:
      'When DevTools Performance panel is shown, the URLs you visit will be sent to Google to query field data. These requests are not tied to your Google account.',
  /**
...
export class FieldSettingsDialog extends HTMLElement {
  ...
  async #submit(enabled: boolean): Promise<void> {
    if (enabled && this.#urlOverrideEnabled && this.#urlOverride) {
      const cruxManager = CrUXManager.CrUXManager.instance();
