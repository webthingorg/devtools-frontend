...
export class TimespanBreakdownOverlay extends HTMLElement {
  ...
  // stores the sections of durations
  #sections: Array<Overlays.EntryBreakdown>|null = null;
  ...
  set sections(sections: Array<Overlays.EntryBreakdown>|null) {
  ...
  afterOverlayUpdate(): void {
    const label = this.#shadow.querySelector<HTMLElement>('.timespan-breakdown-overlay-section-label');
    if (!label) {
      return;
    }
  ...
  #render(): void {
    LitHtml.render(
        LitHtml.html`
            ${this.#sections?.map(section => LitHtml.html`
                <div class="timespan-breakdown-overlay-section">
                  <div class="timespan-breakdown-overlay-label">${section.label}
                    <div>${TraceEngine.Helpers.Timing.microSecondsToMilliseconds(section.bounds.range)} ms</div>
                  </div>
                </div>
            `)}
        `,
