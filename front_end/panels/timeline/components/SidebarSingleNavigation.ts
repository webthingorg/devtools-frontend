...
import {type ActiveInsight, Events as SidebarEvents, type EventTypes as SidebarEventsType} from './Sidebar.js';
...
export class SidebarSingleNavigation extends HTMLElement {
  ...
  #referenceEvent(event: TraceEngine.Types.TraceEvents.TraceEventData) {
    return () => {
      const evt = new CustomEvent<SidebarEventsType[SidebarEvents.SidebarMetricClick]>(
          SidebarEvents.SidebarMetricClick, {detail: {metricEvent: event}, bubbles: true, composed: true});
