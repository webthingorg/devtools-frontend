...
export class SidebarSingleNavigation extends HTMLElement {
  ...
  #referenceEvent(event: TraceEngine.Types.TraceEvents.TraceEventData|null): boolean {
    return Boolean(event && this.dispatchEvent(new EventReferenceClick(event)));
  }
  ...
  #renderMetricValue(
    ...
    return this.#metricIsVisible(label) ? LitHtml.html`
      <div class="metric" @click=${() => this.#referenceEvent(event)}>
  ...
  #calculateCLSScore(
    ...
    let worstShift: TraceEngine.Types.TraceEvents.TraceEventData|null = null;
    let largestScore: number = 0;

    worstCluster?.events.forEach(shift => {
    ...
    return {maxScore, worstShfitEvent: worstShift};
  }
