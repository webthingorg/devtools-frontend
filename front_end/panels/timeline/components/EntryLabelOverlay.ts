...
export class EntryLabelOverlay extends HTMLElement {
  ...
  #entryLabelParams: {height: number, width: number, cutOffHeight: number, chart: string}|null = null;
...
*/
  ...
  constructor(label: string) {
    super();
    this.#render();
    this.#labelPartsWrapper = this.#shadow.querySelector('.label-parts-wrapper') as HTMLElement;
    this.#labelBox = this.#labelPartsWrapper?.querySelector('.label-box') as HTMLElement;
    this.#connectorLineContainer = this.#labelPartsWrapper?.querySelector('.connectorContainer') as SVGAElement;
    this.#entryHighlightWrapper = this.#labelPartsWrapper?.querySelector('.entry-highlight-wrapper') as HTMLElement;
    this.#label = label;
  ...
  set entryLabelParams(entryLabelParams: {height: number, width: number, cutOffHeight: number, chart: string}) {
    if (entryLabelParams.height === this.#entryLabelParams?.height &&
        entryLabelParams.width === this.#entryLabelParams?.width &&
        entryLabelParams.cutOffHeight === this.#entryLabelParams?.cutOffHeight) {
      return;
    }
  ...
  #drawEntryHighlightWrapper(): void {
    ...
    const {height, width, cutOffHeight, chart} = this.#entryLabelParams;

    if(chart === 'main') {
      this.#labelBox.style.visibility = cutOffHeight > 0 ? 'hidden' : 'visible';
      this.#connectorLineContainer.style.visibility = cutOffHeight > 0 ? 'hidden' : 'visible';
      this.#entryHighlightWrapper.style.borderTopWidth = cutOffHeight > 0 ? '0' : '2px';
    } else if(chart === 'network') {
      this.#entryHighlightWrapper.style.borderBottomWidth = cutOffHeight > 0 ? '0' : '2px';
    }

    // PART 3: draw the box that highlights the entry with a label
    this.#entryHighlightWrapper.style.height = `${height - cutOffHeight}px`;
