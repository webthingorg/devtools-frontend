...
export class TimeRangeOverlay extends HTMLElement {
  ...
  #visibleOverlayWidth(overlayRect: DOMRect): number {
    ...
    if (startX >= canvasStartX && endX <= canvasEndX) {
      return width;
    }

    // Window is scrolled off the left hand edge.
    if (startX < canvasStartX && endX < canvasEndX) {
      // Visible portion is the overlay end - the start of the canvas
      return endX - canvasStartX;
    }

    if (startX > canvasStartX && endX > canvasEndX) {
      // Visible portion is the end of the canvas - the start of the overlay.
      return canvasEndX - startX;
    }

    // The time range spans the entire canvas and more so just return the canvas width;
    return canvasEndX - canvasStartX;
  }
  ...
  afterOverlayUpdate(): void {
    ...
    const visibleOverlayWidth = this.#visibleOverlayWidth(overlayRect);
