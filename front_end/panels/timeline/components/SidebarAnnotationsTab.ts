...
function findTextColorForContrast(bgColorText: string): string {
  ...
  const darkColorToken = 'var(--app-color-performance-sidebar-label-text-dark)';
  const darkColorText =
  ...
  return contrastRatio >= 4.5 ? darkColorToken : 'var(--app-color-performance-sidebar-label-text-light)';
}
