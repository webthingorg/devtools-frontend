...
export class SidebarUI extends HTMLElement {
  ...
  #inpLongestEvent: TraceEngine.Types.TraceEvents.SyntheticInteractionPair|null = null;
  ...
  set traceParsedData(traceParsedData: TraceEngine.Handlers.Types.TraceParseData|null) {
    ...
    this.#clsMetric = {
      clsScore,
      clsScoreClassification: TraceEngine.Handlers.ModelHandlers.LayoutShifts.stateForLayoutShiftScore(clsScore),
    };

    this.#inpLongestEvent = traceParsedData.UserInteractions.longestInteractionEvent;
  ...
  #renderINPMetric(): LitHtml.TemplateResult|null {
    if (!this.#inpLongestEvent) {
      return null;
    }
    const timeString = i18n.TimeUtilities.formatMicroSecondsTime(this.#inpLongestEvent.dur);
