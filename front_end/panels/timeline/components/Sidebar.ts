...
export class SidebarUI extends HTMLElement {
  ...
  #renderIndicator(): void {
    const insightsTabHeaderElement =
        this.#shadow.querySelector('.tabs-header input:nth-child(1)');  // Get the active tab
    const annotationTabHeaderElement =
        this.#shadow.querySelector('.tabs-header input:nth-child(2)');   // Get the active tab
    const tabSliderElement = this.#shadow.querySelector('.tab-slider');  // Get the active tab
  ...
  render(expanded: boolean): void {
    ...
    const output = LitHtml.html`<div class=${LitHtml.Directives.classMap({
      ...
      <div class="tab-headers-bottom-line"></div> </div>
