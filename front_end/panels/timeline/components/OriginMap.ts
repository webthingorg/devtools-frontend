...
interface Item extends CrUXManager.OriginMapping {
  isTitleRow?: boolean;
}

export class OriginMap extends UI.Widget.Widget implements UI.ListWidget.Delegate<Item> {
  readonly #list: UI.ListWidget.ListWidget<Item>;
  #editor?: UI.ListWidget.Editor<Item>;

  constructor() {
    super();

    this.#list = new UI.ListWidget.ListWidget<Item>(this, false /* delegatesFocus */, true /* isTable */);
  ...
  renderItem(originMapping: Item): Element {
  ...
  removeItemRequested(_item: Item, index: number): void {
    const mappings = this.#pullMappingsFromSetting();
  ...
  commitEdit(originMapping: Item, editor: UI.ListWidget.Editor<Item>, isNew: boolean): void {
  ...
  beginEdit(originMapping: Item): UI.ListWidget.Editor<Item> {
  ...
  #developmentValidator(_item: Item, index: number, input: UI.ListWidget.EditorControl): UI.ListWidget.ValidatorResult {
  ...
  #productionValidator(_item: Item, _index: number, input: UI.ListWidget.EditorControl): UI.ListWidget.ValidatorResult {
    const origin = this.#getOrigin(input.value);
  ...
  #createEditor(): UI.ListWidget.Editor<Item> {
    if (this.#editor) {
      return this.#editor;
    }
