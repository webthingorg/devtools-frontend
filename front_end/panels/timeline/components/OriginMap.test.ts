...
function getOriginMappings(view: Element): Array<[string, string, string?]> {
  const rows =
      view.shadowRoot!.querySelector('.vbox')!.shadowRoot!.querySelectorAll('.origin-mapping-row:not(.header)');
...
function getDevInput(view: Element): HTMLInputElement|null {
  return view.shadowRoot!.querySelector('.vbox')!.shadowRoot!.querySelector('.development-origin-input input');
}

function getProdInput(view: Element): HTMLInputElement|null {
  return view.shadowRoot!.querySelector('.vbox')!.shadowRoot!.querySelector('.production-origin-input input');
}

function getConfirmButton(view: Element): HTMLElementTagNameMap['devtools-button']|null {
  const buttons = view.shadowRoot!.querySelector('.vbox')!.shadowRoot!.querySelectorAll('devtools-button');
...
function getValidationErrors(view: Element): string {
  const errors = view.shadowRoot!.querySelector('.vbox')!.shadowRoot!.querySelector<HTMLElement>(
...
describeWithMockConnection('OriginMap', () => {
  ...
  it('should show mappings from setting', async () => {
    ...
    const view = renderElementIntoDOM(new Components.OriginMap.OriginMap());
  ...
  it('should pre-fill new mapping fields', async () => {
    const originMap = new Components.OriginMap.OriginMap();
    const view = renderElementIntoDOM(originMap);
    originMap.startCreation();
    await coordinator.done();

    const devInput = getDevInput(view)!;
    assert.strictEqual(devInput.value, 'http://localhost:8080');

    const prodInput = getProdInput(view)!;
  ...
  it('should ignore cancelled entries', async () => {
    ...
    getCancelButton(view)!.click();

    await coordinator.done();

    const mappings = getOriginMappings(view);
    assert.deepStrictEqual(mappings, []);
    assert.isNull(getDevInput(view));
    assert.isNull(getProdInput(view));
  });
