...
const UIStrings = {
   ...
   * @example {401ms} PH1
   */
  estimatedSavings: 'Est savings: {PH1}ms',
};
    ...
    const output = LitHtml.html`
      <div class=${containerClasses}>
        <header @click=${this.#dispatchInsightToggle} jslog=${VisualLogging.action('timeline.toggle-insight').track({click: true})}>
          ...
          ${this.#estimatedSavings && this.#estimatedSavings > 0 ? LitHtml.html`
            <slot name="insight-savings" class="insight-savings">${i18nString(UIStrings.estimatedSavings, {PH1: this.#estimatedSavings})}</slot>
          </div>`
          : LitHtml.nothing}
        </header>
