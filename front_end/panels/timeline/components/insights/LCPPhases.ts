...
export class LCPPhases extends HTMLElement {
  ...
  set insights(insights: TraceEngine.Insights.Types.TraceInsightData<typeof Handlers.ModelHandlers>|null) {
    this.#insights = insights;
    this.#phaseData = this.getPhaseData(this.#insights);
  ...
  getPhaseData(insights: TraceEngine.Insights.Types.TraceInsightData<typeof Handlers.ModelHandlers>|
               null): Array<{phase: string, timing: number|TraceEngine.Types.Timing.MilliSeconds, percent: string}> {
    if (!insights) {
      return [];
    }
