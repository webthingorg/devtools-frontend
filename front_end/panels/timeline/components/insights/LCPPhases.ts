...
export class LCPPhases extends HTMLElement {
  ...
  readonly insightName = 'lcp-by-phase';
  ...
  set activeCategory(category: InsightsCategories) {
  ...
  #isActive(): boolean {
    const isActive = this.#activeInsight && this.#activeInsight.name === this.insightName &&
  ...
  #hasDataToRender(): boolean {
    return this.#phaseData ? this.#phaseData.length > 0 : false;
  }

  #render(): void {
    const shouldRender = this.#shouldRenderForCateogory() && this.#hasDataToRender();
    const output = shouldRender ? this.#renderLCPPhases() : LitHtml.nothing;
