...
export class ThirdParties extends BaseInsight {
  ...
  override createOverlays(): Overlays.Overlays.TimelineOverlay[] {
    ...
    const insight = insightsByNavigation.ThirdPartyWeb;
  ...
  #render(data: TraceEngine.Insights.Types.InsightResults['ThirdPartyWeb']): LitHtml.TemplateResult {
    ...
    const rows1 =
        entries.sort((a, b) => b[1].transferSize - a[1].transferSize)
            .slice(0, 6)
            .map(([entity, summary]) => [entity.name, Platform.NumberUtilities.bytesToString(summary.transferSize)]);
    const rows2 =
        entries.sort((a, b) => b[1].mainThreadTime - a[1].mainThreadTime)
            .slice(0, 6)
            .map(([
                   entity,
                   summary,
                 ]) => [entity.name, i18n.TimeUtilities.millisToString(Math.floor(summary.mainThreadTime / 1000))]);
                  ...
                  <div class="table-container">
                    <dl>
                      ...
                      <dd class="dl-title">${i18nString(UIStrings.columnTransferSize)}</dd>
                        ${rows1.map(([v1, v2]) => LitHtml.html`
                          <dt>${v1}</dt>
                          <dd class="dl-value">${v2}</dd>
                        `)}
                    </dl>
                  </div>
                  <div class="table-container">
