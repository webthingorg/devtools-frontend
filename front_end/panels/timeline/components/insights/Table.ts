...
export class Table extends HTMLElement {
  ...
  async #render(): Promise<void> {
    ...
    const hoverable = this.#rows.some(row => row.overlays);
    ...
    LitHtml.render(
        LitHtml.html`<table
          ...
          @mouseleave=${hoverable ? this.#onMouseLeave : null}>
        <thead>
          <tr>
        ...
        <tbody
          @mouseover=${hoverable ? this.#onHoverRow : null}
          @click=${hoverable ? this.#onClickRow : null}
        >
          ${this.#rows.map(row => {
