...
export class DocumentLatency extends BaseInsight {
  ...
  #renderInsight(insight: TraceEngine.Insights.Types.InsightResults['DocumentLatency']): LitHtml.LitTemplate {
    const didRedirectPass = insight.redirectDuration === 0;
    const didServerResponseTimePass = insight.serverResponseTime === 0;
    const didTextCompressionPass = insight.uncompressedResponseBytes === 0;
    // clang-format off
    return LitHtml.html`
    <div class="insights">
        ...
        <div slot="insight-description" class="insight-description">
          <ul class="insight-results insight-icon-results">
              <li class="insight-entry">
                ${this.#adviceIcon(didRedirectPass)}
                <span>${i18nString(UIStrings.redirects)}</span>
              </li>
              <li class="insight-entry">
                ${this.#adviceIcon(didServerResponseTimePass)}
                <span>${i18nString(UIStrings.serverResponseTime)}</span>
              </li>
              <li class="insight-entry">
                ${this.#adviceIcon(didTextCompressionPass)}
