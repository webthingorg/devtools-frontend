  ...
  private onWindowChanged(event: Common.EventTarget.EventTargetEvent<WindowChangedEvent>): void {
    ...
    if (event.data.breadcrumbWindow) {
      ...
      const rangeMilliseconds =
          TraceEngine.Helpers.Timing.microSecondsToMilliseconds(event.data.breadcrumbWindow.range);
      const minMilliseconds = TraceEngine.Helpers.Timing.microSecondsToMilliseconds(event.data.breadcrumbWindow.min);
      this.mainFlameChart.setTotalAndMinimumBreadcrumbValues(rangeMilliseconds, minMilliseconds);
      this.networkFlameChart.setTotalAndMinimumBreadcrumbValues(rangeMilliseconds, minMilliseconds);
      this.mainFlameChart.update();
    }
