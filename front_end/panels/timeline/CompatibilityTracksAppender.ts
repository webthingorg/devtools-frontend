...
export class CompatibilityTracksAppender {
  ...
  #eventsForTrack = new Map<TrackAppenderName, readonly TraceEngine.Types.TraceEvents.TraceEventData[]>();
  #trackEventsForTreeview = new Map<TrackAppenderName, readonly TraceEngine.Types.TraceEvents.TraceEventData[]>();
  ...
  eventsInTrack(trackAppenderName: TrackAppenderName): readonly TraceEngine.Types.TraceEvents.TraceEventData[] {
   ...
   *   |------- e2 --------|
   */
  canBuildTreeFromEvents(events: readonly TraceEngine.Types.TraceEvents.TraceEventData[]): boolean {
   ...
   */
  eventsForTreeView(trackAppenderName: TrackAppenderName): readonly TraceEngine.Types.TraceEvents.TraceEventData[] {
    ...
    if (!this.canBuildTreeFromEvents(trackEvents)) {
      // Some tracks can include both async and sync events. When this
      // happens, we use all events for the tree views if a tree can be
   ...
   */
  groupEventsForTreeView(group: PerfUI.FlameChart.Group): readonly TraceEngine.Types.TraceEvents.TraceEventData[]|null {
