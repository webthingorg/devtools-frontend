...
export class CompatibilityTracksAppender {
  ...
  checkActionsAvailability(group: PerfUI.FlameChart.Group, node: TraceEngine.Types.TraceEvents.TraceEntry, flameChartView: PerfUI.FlameChart.FlameChart): void{
    const threadTrackAppender = this.#trackForGroup.get(group);
    if (threadTrackAppender instanceof ThreadAppender) {
      threadTrackAppender.checkActionsAvailability(node, flameChartView);
    } else {
      console.warn('Could not modify tree in not thread track');
    }
  }
