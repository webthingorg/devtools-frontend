...
export class CompatibilityTracksAppender {
  ...
  /**
   * @param events ordered array of events.
   * @returns a boolean that determines if the given events can be organized
   * into a tree structure.
   */
  canBuildTreeFromEvents(events: TraceEngine.Types.TraceEvents.TraceEventData[]): boolean {
    const stack: TraceEngine.Types.TraceEvents.TraceEventData[] = [];
    for (const event of events) {
      const startTime = event.ts;
      let endTime: number|(number | undefined) = event.ts + (event.dur || 0);
   ...
   * that can be arranged in a tree shape (nestable).
   *
   * At the moment this returns an array of legacy
   * SDK.TracingModel.Event, this is due to the lack of compatibility of
   * the Tree View artifacts with the new engine types. In the future,
   * support for the new engine should be added incrementally and the
   * return type updated to the new engine's types.
   */
