...
export class CompatibilityTracksAppender {
  ...
  /**
   * Returns number of tracks of given type already registered.
   * Used to name the "Raster Thread 6" tracks, etc
   */
  getTrackCountForThreadType(threadType: ThreadType.RASTERIZER|ThreadType.THREAD_POOL): number {
    const threadAppenderTracks = Array.from(this.#trackForGroup.values()) as
        ThreadAppender[];  // The argument type ensures these are ThreadAppenders
    return threadAppenderTracks.filter(appender => appender.threadType === threadType).length;
  }
