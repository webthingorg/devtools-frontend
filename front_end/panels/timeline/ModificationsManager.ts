...
export class ModificationsManager extends EventTarget {
  ...
  createAnnotation(newAnnotation: TraceEngine.Types.File.Annotation, loadeFromFile: boolean = false): void {
    // If the new annotation created was not loaded form the file, set the annotations visibility setting to true.
    if (!loadeFromFile) {
      ...
      // visibility to true if annotations label is added. This is done in updateAnnotation().
      if (!(newAnnotation.type === 'TIME_RANGE')) {
        this.#annotationsVisibilitySetting.set(true);
      }
    }
