...
export class ModificationsManager extends EventTarget {
  ...
  private constructor({traceParsedData, traceBounds, modifications}: ModificationsManagerData) {
    ...
    this.#annotationsHiddenSetting = Common.Settings.Settings.instance().createSetting('annotations-hidden', true);
  ...
  createAnnotation(newAnnotation: TraceEngine.Types.File.Annotation, loadeFromFile: boolean = false): void {
    ...
    if (!loadeFromFile) {
      ...
      // hidden to true if annotations label is added. This is done in updateAnnotation().
