...
export class ModificationsManager extends EventTarget {
  ...
  #annotationForOverlay: Map<TimelineOverlay, TraceEngine.Types.File.Annotation>;
  ...
  private constructor({traceParsedData, traceBounds, modifications}: ModificationsManagerData) {
    ...
    this.#annotationForOverlay = new Map();
  }
  ...
  createOverlayForAnnotation(newOverlayAnnotation: TraceEngine.Types.File.Annotation): void {
    const newOverlay = {
      type: 'ENTRY_LABEL',
      entry: newOverlayAnnotation.entry,
      label: '',
    } as EntryLabel;
    this.#annotationForOverlay.set(newOverlay, newOverlayAnnotation);
  ...
  removeAnnotationOverlay(removedOverlay: TimelineOverlay): void {
    this.#annotationForOverlay.delete(removedOverlay);
