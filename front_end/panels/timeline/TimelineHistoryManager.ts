...
export class TimelineHistoryManager {
  ...
  #getActiveIndexForListControl(): number {
    ...
    return this.recordings.findIndex(r => {
      // We already know the last active trace must be a trace index, but we
      // need to convince TypeScript again.
      return this.lastActiveTrace?.type === 'TRACE_INDEX' &&
          r.traceParseDataIndex === this.lastActiveTrace.traceParseDataIndex;
    });
  }
  ...
  async showHistoryDropDown(): Promise<RecordingData|null> {
    ...
    const activeTraceIndex = await DropDown.show(
        this.recordings.map(recording => recording.traceParseDataIndex), this.#getActiveIndexForListControl(),
