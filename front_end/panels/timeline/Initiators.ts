  ...
  while (currentEvent) {
    ...
    if (currentInitiator) {
      ...
      // if(hiddenEvents.includes(currentEvent)) {
        let pair = getPairVisibleParents(currentEventVisible, currentInitiatorVisible, traceEngineData, modifiedEntries, hiddenEvents);
      // }
...
function getPairVisibleParents(event: TraceEngine.Types.TraceEvents.TraceEventData, initiator: TraceEngine.Types.TraceEvents.TraceEventData, traceEngineData: TraceEngine.Handlers.Types.TraceParseData, modifiedEntries: TraceEngine.Types.TraceEvents.TraceEventData[], hiddenEvents: TraceEngine.Types.TraceEvents.TraceEventData[]): InitiatorPair {

   if(hiddenEvents.includes(event)) {
    let nextParent = traceEngineData.Renderer.entryToNode.get(event)?.parent;
    while(nextParent?.entry && !modifiedEntries.includes(nextParent?.entry)) {
      nextParent = nextParent.parent ?? undefined;
    }
    event = nextParent?.entry ?? event;
  }
   
  if(hiddenEvents.includes(initiator)) {
    let nextParent = traceEngineData.Renderer.entryToNode.get(initiator)?.parent;
    while(nextParent?.entry && !modifiedEntries.includes(nextParent?.entry)) {
      nextParent = nextParent.parent ?? undefined;
    }
    initiator = nextParent?.entry ?? initiator;
  }

  return {event: event, initiator: initiator};
