...
describeWithEnvironment('EntriesFilter', function() {
  ...
  it('supports the user merging an entry into its parent', async function() {
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.MERGE_FUNCTION, entry: entryTwo});
  ...
  it('adds the collapsed entry into the expandableEntries array', async function() {
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.COLLAPSE_FUNCTION, entry: entryTwo});
  ...
  it('adds the next visible parent of the merged entry into the expandableEntries array if the direct parent is hidden',
     async function() {
       ...
       stack.applyFilterAction({
         type: TraceEngine.Types.TraceEvents.FilterAction.COLLAPSE_REPEATING_DESCENDANTS,
       ...
       stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.MERGE_FUNCTION, entry: foo2Calls[1]});
  ...
  it('supports collapsing an entry', async function() {
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.COLLAPSE_FUNCTION, entry: basicTwoCallEntry});
  ...
  it('supports collapsing all repeating entries among descendants', async function() {
    ...
    stack.applyFilterAction(
        {type: TraceEngine.Types.TraceEvents.FilterAction.COLLAPSE_REPEATING_DESCENDANTS, entry: firstFooCallEntry});
  ...
  it('supports undo all filter actions by applying context menu undo action', async function() {
    ...
    stack.applyFilterAction(
        {type: TraceEngine.Types.TraceEvents.FilterAction.COLLAPSE_REPEATING_DESCENDANTS, entry: fibonacciCalls[0]});
    // We collapsed identical descendants after the first `foo` entry - so it should not be included in the invisible list itself,
    // but all foo() calls below it in the stack should now be invisible.
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.MERGE_FUNCTION, entry: basicStackOne});
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.UNDO_ALL_ACTIONS, entry: basicTwoCallEntry});
  ...
  it('supports resetting all hidden children of a selected entry', async function() {
    ...
    stack.applyFilterAction({type: TraceEngine.Types.TraceEvents.FilterAction.RESET_CHILDREN, entry: foo2Calls[1]});
  ...
  it('correctly assigns a visible parent to expandable entries if the direct parent is not visible', async function() {
    ...
    stack.applyFilterAction(
        {type: TraceEngine.Types.TraceEvents.FilterAction.MERGE_FUNCTION, entry: anonymousEntryWithInvisibleParent});
