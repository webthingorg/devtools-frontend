...
export function makeAriaAnnouncementForAnnotationModified(event: AnnotationModifiedEvent): void {
  const {overlay, action} = event;
  switch (action) {
    case 'Remove': {
      const text = ariaDescriptionForOverlay(overlay);
      if (text) {
        UI.ARIAUtils.alert(i18nString(UIStrings.srAnnotationRemoved, {PH1: text}));
      }
      break;
    }
    case 'Add': {
      const text = ariaDescriptionForOverlay(overlay);
      if (text) {
        UI.ARIAUtils.alert(i18nString(UIStrings.srAnnotationAdded, {PH1: text}));
      }
      break;
    }
    case 'UpdateLabel': {
      ...
      if (label) {
        UI.ARIAUtils.alert(i18nString(UIStrings.srLabelTextUpdated, {PH1: label}));
      }
      break;
    }
    case 'UpdateTimeRange': {
      UI.ARIAUtils.alert(i18nString(UIStrings.srTimeRangeBoundsUpdated));
      break;
    }
    case 'UpdateLinkToEntry': {
      if (isEntriesLink(overlay) && overlay.entryFrom && overlay.entryTo) {
        ...
        UI.ARIAUtils.alert(i18nString(UIStrings.srEntriesLinked, {PH1: from, PH2: to}));
      }
      break;
    }
    case 'EnterLabelEditState': {
      UI.ARIAUtils.alert(i18nString(UIStrings.srEnterLabelEditMode));
      break;
    }
    default:
