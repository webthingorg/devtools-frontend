...
export class InteractionsTrackAppender implements TrackAppender {
   ...
   */
  #appendEventAtLevel(
      syntheticEvent: TraceEngine.Handlers.ModelHandlers.UserInteractions.SyntheticInteractionEvent,
      level: number): number {
    ...
    this.#entryData.push(syntheticEvent.startEvent);
  ...
  */
  ...
  #findSyntheticEventForStartEvent(startEvent: TraceEngine.Types.TraceEvents.TraceEventEventTimingStart):
      TraceEngine.Handlers.ModelHandlers.UserInteractions.SyntheticInteractionEvent|null {
    return this.#traceParsedData.UserInteractions.interactionEvents.find(event => {
      return event.startEvent === startEvent;
    }) ||
        null;
  }
}
