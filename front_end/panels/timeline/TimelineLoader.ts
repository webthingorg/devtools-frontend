...
export class TimelineLoader implements Common.StringOutputStream.OutputStream {
  ...
  static async loadFromURL(url: Platform.DevToolsPath.UrlString, client: Client): Promise<TimelineLoader> {
    ...
    async function finishedCallback(
      ...
      loader.#loadTraceFileOrCPUProfileFromJSON(trace);
      await loader.close();
    }
  ...
  #loadTraceFileOrCPUProfileFromJSON(trace: ParsedJSONTrace): void {
  ...
  async write(chunk: string, endOfFile: boolean): Promise<void> {
    ...
    if (endOfFile) {
      let trace;
      try {
        trace = JSON.parse(this.buffer) as ParsedJSONTrace;
        this.#loadTraceFileOrCPUProfileFromJSON(trace);
        return Promise.resolve();
      } catch (e) {
...
type ParsedJSONTrace = TraceEngine.Types.File.Contents|Protocol.Profiler.Profile;
