...
export class TimelineLoader implements Common.StringOutputStream.OutputStream {
  ...
  constructor(client: Client, saveToFile: boolean) {
    this.client = client;

    this.backingStorage = new Bindings.TempFile.TempFileBackingStorage();
    this.tracingModel = new SDK.TracingModel.TracingModel(this.backingStorage, saveToFile);
  ...
  static async loadFromFile(file: File, client: Client): Promise<TimelineLoader> {
    const loader = new TimelineLoader(client, /* saveToFile= */ true);
  ...
  static loadFromCpuProfile(profile: Protocol.Profiler.Profile|null, client: Client): TimelineLoader {
    const loader = new TimelineLoader(client, /* saveToFile= */ false);
