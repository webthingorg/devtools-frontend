...
const UIStrings = {
  ...
  elapsed: 'Elapsed (µs)',
  /**
   *@description Column name for count of elements that were fast rejected
   */
  fastRejectCount: 'Fast Reject Count',
  /**
...
export class TimelineSelectorStatsView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.#datagrid.data = {
      ...
      columns: [
        ...
        {
          id: SelectorTimingsKey.FastRejectCount as Lowercase<string>,
          title: i18nString(UIStrings.fastRejectCount),
          sortable: true,
          widthWeighting: 1,
          visible: true,
          hideable: true,
  ...
  private async createRowsForTable(timings: SelectorTimings[]): Promise<DataGrid.DataGridUtils.Row[]> {
    ...
    const rows = await Promise.all(timings.map(async x => {
      ...
      return {
        cells: [
          {columnId: SelectorTimingsKey.Elapsed, value: x[SelectorTimingsKey.Elapsed]},
          {columnId: SelectorTimingsKey.FastRejectCount, value: x[SelectorTimingsKey.FastRejectCount]},
