...
export class TimelineFlameChartNetworkDataProvider implements PerfUI.FlameChart.FlameChartDataProvider {
  ...
  entryIndexForSelection(selection: TimelineSelection|null): number {
    ...
    const index =
        this.#events.indexOf(selection.object as TraceEngine.Types.TraceEvents.TraceEventSyntheticNetworkRequest);
    if (index !== -1) {
  ...
  #updateTimelineData(startTime: number, endTime: number): void {
    ...
    this.#maxLevel = this.#networkTrackAppender.updateTimelineData(startTime, endTime);
