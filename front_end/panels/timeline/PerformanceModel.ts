...
export class PerformanceModel extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  async addSourceMapListeners(): Promise<void> {
    ...
    for (const node of this.#cpuProfileNodes()) {
      ...
      const debuggerModelToListen = this.#maybeGetDbgModelForNode(node);
  ...
  #maybeGetDbgModelForNode(node: SDK.CPUProfileDataModel.CPUProfileNode): SDK.DebuggerModel.DebuggerModel|null {
