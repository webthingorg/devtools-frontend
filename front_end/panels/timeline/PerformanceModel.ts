...
export class PerformanceModel extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  private cpuProfileNodes(): SDK.CPUProfileDataModel.CPUProfileNode[] {
  ...
  async addSourceMapListeners(): Promise<void> {
    ...
    for (const node of this.cpuProfileNodes()) {
      ...
      const debuggerModelToListen = this.getDebuggerModelToListen(node);
  ...
  private getDebuggerModelToListen(node: SDK.CPUProfileDataModel.CPUProfileNode): SDK.DebuggerModel.DebuggerModel|null {
