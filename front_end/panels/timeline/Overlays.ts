...
export class Overlays {
  ...
  #positionEntryLabelOverlay(overlay: EntryLabel, element: HTMLElement): void {
    ...
    const labelAndCnnectorAngleOffset = 40;
    const entryHeight = height + labelPadding * 2;

    element.style.top = `${y - entryHeight-labelConnectorHeight}px`;
    element.style.height = `${height+ entryHeight + labelConnectorHeight}px`;
    ...
    labelElement.style.marginRight = `${labelAndCnnectorAngleOffset}px`;
    ...
    const startX = line.width/2 - labelAndCnnectorAngleOffset;
    const startY = 0;
    ...
    const entryWrap = element.querySelector(".entry-wrap") as HTMLElement;
    entryWrap.style.height = `${height}px`;
    entryWrap.style.width = `${finalWidth}px`;
  }
  ...
  #createElementForNewOverlay(overlay: TimelineOverlay): HTMLElement {
    switch (overlay.type) {
      case 'ENTRY_LABEL': {
        ...
        div.createChild("div", "entry-wrap")
