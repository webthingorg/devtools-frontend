...
export class Overlays {
  ...
  #positionEntryLabelOverlay(overlay: EntryLabel, element: HTMLElement): {entryHeight: number, entryWidth: number}|
    ...
    element.style.top =
        `${y - labelHeightWithPadding - Components.EntryLabelOverlay.EntryLabelOverlay.LABLE_CONNECTOR_HEIGHT}px`;
    element.style.height = `${
        entryHeight + labelHeightWithPadding +
        Components.EntryLabelOverlay.EntryLabelOverlay.LABLE_CONNECTOR_HEIGHT}px`;
    ...
    // The reason we cannot directly place it in the middle is that the width of the whole entry label overlayt
    // depends of which element is wider - the lable or the entry it belongs to. Therefore we cannot make sure the
