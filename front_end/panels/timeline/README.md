...
Serializing/Deserializing events allows for the creation of annotations associated with events and enabling to save them to/load them from the trace file. Several classes handle the serialization and application of these serialized annotations:
...
3. `ModificationsManager` - Takes the serialized modifications in a trace file (under `metadata.modifications`) and applies it to the current timeline after EventsSerializer mapped the keys to events (by initializing entriesFilter with the loaded modifications), as well as creates the object that gets saved under `metadata.modifications` when a trace file with modifications is saved.