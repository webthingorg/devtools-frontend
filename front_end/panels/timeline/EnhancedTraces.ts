  ...
  captureScriptFromTraces(): Script[] {
    ...
    this.traceEvents.forEach(traceEvent => {
      if (traceEvent.cat === 'disabled-by-default-devtools.v8-source-rundown') {
        ...
        // problematic! script id could be the same, do isolate plus script id equation
      ...
      } else if (traceEvent.cat === 'disabled-by-default-devtools.v8-source-rundown-sources') {
        const data = traceEvent.args?.data;
...
export interface EnhancedTraces {
  metaData: EnhancedTracesMetadata;
  targets: Target[];
