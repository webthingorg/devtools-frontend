...
export class TimelineDetailsView extends UI.Widget.VBox {
  ...
  private showSelectorStats(event: TraceEngine.Legacy.CompatibleTraceEvent): void {
    ...
    const hasSelectorStats = selectorStatsView.setEvent(event);
    if (!hasSelectorStats) {
      return;
    }
  ...
  private showAggregatedSelectorStats(events: TraceEngine.Legacy.Event[]): void {
    ...
    const hasSelectorStats = selectorStatsView.setAggregatedEvent(events);
    if (!hasSelectorStats) {
      return;
    }
  ...
  private updateSelectedRangeStats(startTime: number, endTime: number): void {
    ...
    const isEnabledAdvancedPaint =
        Common.Settings.Settings.instance().createSetting('timelineCaptureLayersAndPictures', false).get();
    if (this.#selectedEvents && isEnabledAdvancedPaint) {
      const eventsInRange = TimelineModel.TimelineModel.TimelineModelImpl.findRecalculateStyleEvents(
          this.#selectedEvents as unknown as TraceEngine.Legacy.Event[], startTime, endTime);
