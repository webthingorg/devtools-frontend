...
export class TimelineDetailsView extends UI.Widget.VBox {
   ...
   * debounced. The forceSingleUpdate param configures the debouncing
   * behaviour.
   */
  private scheduleUpdateContentsFromWindow(forceSingleUpdate: boolean = false): void {
    if (!this.model) {
  ...
  async setSelection(selection: TimelineSelection|null): Promise<void> {
    ...
    if (!this.selection) {
      // Update instantly using forceSingleUpdate, since we are only
      // making a single call and don't need to debounce.
      this.scheduleUpdateContentsFromWindow(/* forceSingleUpdate */ true);
      return;
    }
