...
*/
  ...
  #drawLabel(initialLabel?: string): void {
    ...
    if (!this.#shouldDrawBelowEntry) {
      // If the label is drawn above, the connector goes up and to the left, so we pull the label back slightly to align it nicely.
    ...
    if (this.#shouldDrawBelowEntry && this.#entryLabelParams) {
      ...
      // 2. the height of the connector, so we have room to draw it
      ...
      const verticalTransform = this.#entryLabelParams.height - this.#entryLabelParams.cutOffEntryHeight +
          ...
          EntryLabelOverlay.LABEL_CONNECTOR_HEIGHT + 4;
