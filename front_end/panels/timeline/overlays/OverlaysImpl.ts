...
export class Overlays extends EventTarget {
  ...
  readonly #annotationsVisibilitySetting: Common.Settings.Setting<boolean>;
  ...
  constructor(init: {
    ...
    // This method is also called in multiple SidebarAnnotationsTab, but calling this multiple times doesn't recreate the setting.
    // Instead, after the second call, the cached setting is returned.
    this.#annotationsVisibilitySetting = Common.Settings.Settings.instance().createSetting('annotations-visible', true);
    this.#annotationsVisibilitySetting.addChangeListener(this.update.bind(this));
  ...
  #positionOverlay(overlay: TimelineOverlay, element: HTMLElement): void {
    const annotationsAreVisible = this.#annotationsVisibilitySetting.get();
    switch (overlay.type) {
      case 'ENTRY_SELECTED': {
      ...
      case 'TIME_RANGE': {
        ...
        // Therefore, if the range does not have a label, show the range regardless of whether the annotations visibility setting is on or off.
        if (overlay.label.length) {
          this.#setOverlayElementVisibility(element, annotationsAreVisible);
        }
      ...
      case 'ENTRY_LABEL': {
        ...
        this.#setOverlayElementVisibility(element, entryVisible && annotationsAreVisible);
        if (entryVisible) {
