...
export class Overlays extends EventTarget {
  ...
  #entriesLinkInProgress: Components.EntriesLinkOverlay.EntriesLinkOverlay|null;
  ...
  #updateMouseCoordinatesProgressEntriesLink(event: Event): void {
    ...
    if (!this.#entriesLinkInProgress || this.#entriesLinkInProgress.entryToExists) {
      return;
    }
    ...
    this.#entriesLinkInProgress.coordinateTo = {x: this.#lastMouseOffsetX, y: this.#lastMouseOffsetY + networkHeight};
  }
    ...
    switch (overlay.type) {
      ...
      case 'ENTRIES_LINK': {
        ...
        if (component) {
          ...
          if (overlay.entryTo) {
            // If the link has the entry it is pointing to, remove the mousemove event
            // that is used to track the mouse when it the link is created
            ...
            this.#entriesLinkInProgress = component;
