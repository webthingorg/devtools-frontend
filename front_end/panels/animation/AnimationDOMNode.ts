...
export class AnimationDOMNode {
  ...
  async #addReportScrollPositionBinding(): Promise<void> {
    ...
    this.#scrollBindingListener = ev => {
      ...
      const {scrollTop, scrollLeft, id} = JSON.parse(payload) as {scrollTop: number, scrollLeft: number, id: string};
  ...
  async addScrollEventListener(onScroll: ({scrollLeft, scrollTop}: {scrollLeft: number, scrollTop: number}) => void):
    ...
    await object.callFunction(scrollListenerInPage, [
      ...
      SCROLL_LISTENER_NAME_IN_PAGE(id),
  ...
  async removeScrollEventListener(id: string): Promise<void> {
    ...
    await object.callFunction(
        ...
        [SCROLL_LISTENER_NAME_IN_PAGE(id)].map(arg => SDK.RemoteObject.RemoteObject.toCallArgument(arg)));
    object.release();
