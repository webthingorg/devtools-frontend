...
export class AnimationUI {
  ...
  private drawDelayLine(parentElement: HTMLElement): void {
    ...
    console.log('this.delay()', this.delayOrStartTime());
    this.#delayLine.setAttribute('x2', (this.delayOrStartTime() * this.#timeline.pixelMsRatio() + margin).toFixed(2));
  ...
  redraw(): void {
    ...
    (this.#activeIntervalGroup as HTMLElement).style.transform =
        'translateX(' + (this.delayOrStartTime() * this.#timeline.pixelMsRatio()).toFixed(2) + 'px)';

    this.#nameElement.style.transform = 'translateX(' +
        (this.delayOrStartTime() * this.#timeline.pixelMsRatio() + Options.AnimationMargin).toFixed(2) + 'px)';
