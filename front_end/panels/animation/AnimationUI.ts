...
export class AnimationUI {
  ...
  constructor(animation: AnimationImpl, timeline: AnimationTimeline, parentElement: Element) {
    ...
    this.#activeIntervalGroup.setAttribute(
        'jslog', `${VisualLogging.item().context('animations.active-interval-group').track({drag: true})}`);
  ...
  private drawPoint(iteration: number, parentElement: Element, x: number, keyframeIndex: number, attachEvents: boolean):
    ...
    circle.setAttribute('jslog', `${VisualLogging.item().context('animations.keyframe-circle').track({drag: true})}`);
    circle.tabIndex = 0;
