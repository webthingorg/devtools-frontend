...
export class AnimationTimeline extends UI.Widget.VBox implements SDK.TargetManager.SDKModelObserver<AnimationModel> {
  ...
  #scrollListenerId?: string|null;
  #collectedGroups: AnimationGroup[];
  #createPreviewForCollectedGroupsTimeout: number|undefined;
  ...
  private clearTimeline(): void {
    if (this.#selectedGroup && this.#scrollListenerId) {
      void this.#selectedGroup.scrollNode().then((node: AnimationDOMNode|null) => {
        void node?.removeScrollEventListener(this.#scrollListenerId as string);
  ...
  private animationGroupStarted({data}: Common.EventTarget.EventTargetEvent<AnimationGroup>): void {
    ...
    if (this.#createPreviewForCollectedGroupsTimeout) {
      window.clearTimeout(this.#createPreviewForCollectedGroupsTimeout);
    }

    this.#createPreviewForCollectedGroupsTimeout = window.setTimeout(() => {
      this.createPreviewForCollectedGroups();
    }, 10);
  }
  ...
  private createPreviewForCollectedGroups(): void {
    // TODO(ergunsh): What should be the order here?
  ...
  private addAnimationGroup(group: AnimationGroup): void {
    ...
    // Collect all the groups that arrive in a close timeframe
    // before creating the preview so that we can sort them stabilly in the UI.
  ...
  private timeToText(time: number): string {
    return this.#selectedGroup?.isScrollDriven() ? `${time.toFixed(0)}px` : i18n.TimeUtilities.millisToString(time);
  }
  ...
  private setCurrentTimeText(time: number): void {
    ...
    this.#currentTime.textContent = this.timeToText(time);
  }
  ...
  private renderGrid(): void {
    ...
    for (let time = 0; time < this.duration(); time += gridSize) {
      ...
      if (lastDraw === undefined || gridWidth - lastDraw > 50) {
        ...
        label.textContent = this.timeToText(time);
  ...
  private animateTime(currentTime: number): void {
    ...
    // thus we don't animate the scrubber based on time for that case.
