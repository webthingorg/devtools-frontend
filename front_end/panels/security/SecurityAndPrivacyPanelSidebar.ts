...
export class SecurityAndPrivacyPanelSidebar extends UI.Widget.VBox {
  ...
  constructor(
      ...
      private readonly setVisibleViewinPanel: (arg0: UI.Widget.VBox) => void, mainView: SecurityMainView) {
    super();
    ...
    this.securityOverviewElement = new OriginTreeElement(
        {
          onSelect: this.setVisibleViewinPanel.bind(this, mainView),
  ...
  #renderTreeElement(element: SecurityPanelSidebarTreeElement): void {
    if (element instanceof OriginTreeElement) {
      ...
      element.setLeadingIcons([element.getIconForSecurityState(securityState)]);
      if (element.listItemElement.lastChild) {
        element.listItemElement.removeChild(element.listItemElement.lastChild);
      }
      element.listItemElement.appendChild(element.getTitleForSecurityState(securityState));
    }
  }
}
