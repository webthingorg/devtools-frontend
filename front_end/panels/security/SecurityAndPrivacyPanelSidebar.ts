...
export class SecurityAndPrivacyPanelSidebar extends UI.Widget.VBox {
  ...
  constructor(private readonly revealOrigin: (arg0: Origin) => void, private readonly revealMainView: () => void) {
    super();
    ...
    this.securityOverviewElement = new OriginTreeElement(
        this.revealMainView.bind(this), 'security-main-view-sidebar-tree-item', this.#renderTreeElement);
  ...
  addOrigin(origin: Platform.DevToolsPath.UrlString, securityState: Protocol.Security.SecurityState): void {
    ...
    const originElement = new OriginTreeElement(
        this.revealOrigin.bind(this, origin), 'security-sidebar-tree-item', this.#renderTreeElement, origin);
  ...
  #renderTreeElement(element: SecurityPanelSidebarTreeElement): void {
    if (element instanceof OriginTreeElement) {
      ...
      const {icon, elementTitle} = isOverviewElement ?
          {
            icon: getSecurityStateIconForOverview(securityState, `lock-icon lock-icon-${securityState}`),
            elementTitle:
                (():
                     Element => {
          ...
          } :
          {
            icon: getSecurityStateIconForDetailedView(
                securityState, `security-property security-property-${securityState}`),
            elementTitle: createHighlightedUrl(element.origin() ?? Platform.DevToolsPath.EmptyUrlString, securityState),
          };
