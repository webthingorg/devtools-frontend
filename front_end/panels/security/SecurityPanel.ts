...
export class SecurityPanelSidebarTree extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  addOrigin(origin: Platform.DevToolsPath.UrlString, securityState: Protocol.Security.SecurityState): void {
    ...
    const originElement = new SecurityPanelSidebarDetailTreeElement(
...
class SecurityPanelSidebarOverviewTreeElement extends SecurityPanelSidebarTreeElement {
  ...
  override getIconForSecurityState(securityState: Protocol.Security.SecurityState): IconButton.Icon.Icon {
    return getSecurityStateIconForOverview(securityState, `lock-icon lock-icon-${securityState}`);
  }
}

class SecurityPanelSidebarDetailTreeElement extends SecurityPanelSidebarTreeElement {
  constructor(textElement: Element, selectCallback: () => void) {
    super(textElement, selectCallback);
  ...
  override getIconForSecurityState(securityState: Protocol.Security.SecurityState): IconButton.Icon.Icon {
    return getSecurityStateIconForDetailedView(securityState, `security-property security-property-${securityState}`);
  }
}
