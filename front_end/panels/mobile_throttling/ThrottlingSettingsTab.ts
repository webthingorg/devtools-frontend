...
export class ThrottlingSettingsTab extends UI.Widget.VBox implements
  ...
  private createEditor(): UI.ListWidget.Editor<SDK.NetworkManager.Conditions> {
    ...
    const packetQueueLengthInput =
        editor.createInput('packetQueueLength', 'text', i18n.i18n.lockedString('packet'), packetQueueLengthValidator);
    ...
    if (!valid) {
      const errorMessage =
          i18nString(UIStrings.packetQueueLengthMustBeAnIntegerGreaterOrEqualToZero);
      return {valid, errorMessage};
    }
    return {valid, errorMessage: undefined};
  }
