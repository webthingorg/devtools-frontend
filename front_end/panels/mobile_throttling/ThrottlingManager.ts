...
export class ThrottlingManager {
  ...
  private hardwareConcurrencyOverrideEnabled = false;
  ...
  private updatePanelIcon(): void {
    ...
    UI.Tooltip.Tooltip.install(icon, tooltips.join('\n'));
  ...
  createHardwareConcurrencySelector(): {
    ...
    const warning = new UI.Toolbar.ToolbarItem(
        UI.UIUtils.createIconLabel(i18nString(UIStrings.excessConcurrency), 'smallicon-warning'));
    reset.element.style.visibility = 'hidden';
    warning.element.style.visibility = 'hidden';
    ...
    void this.cpuThrottlingManager.getHardwareConcurrency().then(defaultValue => {
      ...
      const setHardwareConcurrency = (value: number): void => {
        ...
        reset.element.style.visibility = value === defaultValue ? 'hidden' : 'visible';
        warning.element.style.visibility = value <= defaultValue ? 'hidden' : 'visible';
      };
