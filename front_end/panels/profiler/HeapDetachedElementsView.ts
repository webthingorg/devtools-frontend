...
const UIStrings = {
   ...
   */
  toReplace: 'THIS IS FILLER TEXT: REPLACE!!',
  /**
   *@description Text in Heap Snapshot View of a profiler tool
   */
...
export class DetachedElementsProfileView extends UI.View.SimpleView implements DataDisplayDelegate {
  //   override profileHeader: DetachedElementsProfileHeader;
  ...
  constructor(dataDisplayDelegate: DataDisplayDelegate, profile: DetachedElementsProfileHeader) {
    ...
    this.updateElementsGrid(profile.detachedElements);
  }
  ...
  updateElementsGrid(detachedElements: Protocol.DOM.DetachedElementInfo[]|null): void {
    ...
    detachedElements?.forEach(detachedElementInfo => {
      // Create a row object for each node
    ...
    // this.profileType.removeEventListener(DetachedElementsProfileType.Events.DetachedElementsObtained, this.updateElementsGrid, this)
  }
  ...
  private createRowForNode(node: SDK.DOMModel.DOMNode, size: number): DataGrid.DataGridUtils.Row {
    return {
      cells: [
        ...
        {
          columnId: 'node_memory_id',
          value: node.backendNodeId(),
          renderer: DataGrid.DataGridRenderers.primitiveRenderer,
        },
      ],
      hidden: false,
  ...
  private initGridContainer(): GridContainer {
    ...
    const dataGridInitialData: DataGrid.DataGridController.DataGridControllerData = {
      columns: [
        ...
        {
          id: 'node_memory_id',
          title: i18nString(UIStrings.nodeID),
          sortable: false,
          widthWeighting: 1,
          visible: true,
          hideable: false,
...
export class DetachedElementsProfileType extends
  ...
  async getDetachedElements(): Promise<void> {
    ...
    const domModels: SDK.DOMModel.DOMModel[] = SDK.TargetManager.TargetManager.instance().models(SDK.DOMModel.DOMModel);
    const data = await domModels[0].getDetachedDOMNodes();

    let profile: DetachedElementsProfileHeader = new DetachedElementsProfileHeader(
        heapProfilerModel, this, data);  // MAKE SURE ALL NODES ARE BEING READ BEFORE SHOWING
    ...
    // profile.title = i18nString(UIStrings.detachedD, {PH1: profile.uid});
    // profile.setTitle(i18nString(UIStrings.detachedD, {PH1: profile.uid}));\
    // profile.finishLoad();

    // this.dispatchEventToListeners(DetachedElementsProfileType.Events.DetachedElementsObtained, data);
  ...
  //     return new DetachedElementsProfileHeader(null, this, title);
  //   }
}
...
//   constructor() {
//     super(DetachedElementsProfileType.TypeId, i18nString(UIStrings.allocationSampling));
