...
export class DetachedElementsProfileView extends UI.View.SimpleView implements DataDisplayDelegate {
  ...
  constructor(dataDisplayDelegate: DataDisplayDelegate, profile: DetachedElementsProfileHeader) {
    ...
    this.gridContainer = this.initGridContainer(content);
  ...
  populateElementsGrid(detachedElements: Protocol.DOM.DetachedElementInfo[]|null): void {
    ...
    const rows = detachedElements?.map(detachedElementInfo => {
      ...
      return this.createRowForNode(DOMNode, this.getNodeSize(detachedElementInfo));
    }) ??
        [];
    const grid = this.gridContainer.grid;
    grid.update({...grid.data(), rows: rows});
  }
  ...
  private nodeRenderer = (node: DataGrid.DataGridUtils.CellValue): LitHtml.TemplateResult => {
    if (!(node instanceof SDK.DOMModel.DOMNode)) {
      throw new Error(`Unexpected cell value ${node}`);
    }
    ...
    while (nodes.length > 0) {
      ...
      if (this.nodeIDsSet.has(descendantNode.backendNodeId() as number)) {
  ...
  private initGridContainer(content: HTMLElement): GridContainer {
    const gridsContainer = content.createChild('div', 'detached-elements-nodes-container') as HTMLDivElement;
    const targetContainer = gridsContainer.createChild('div') as HTMLDivElement;
    targetContainer.classList.add('target-container');
    const header = targetContainer.createChild('div') as HTMLDivElement;
    header.classList.add('header');
    const gridContainer = targetContainer.createChild('div') as HTMLDivElement;
    ...
    return {container: targetContainer, header: header, grid: dataGridIntegrator};
  }
