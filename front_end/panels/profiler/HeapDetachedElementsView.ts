  ...
  private nodeRenderer = (node: DataGrid.DataGridUtils.CellValue): LitHtml.TemplateResult => {
    ...
    while (nodes.length > 0) {
      ...
      if (this.nodeIDsSet.has(nodeptr.backendNodeId() as number)) {
        ...
        treeOutline.findTreeElement(nodeptr)?.setLeadingIcons([icon]);
        treeOutline.findTreeElement(nodeptr)?.listItemNode.classList.add('detached-elements-detached-node');
        treeOutline.findTreeElement(nodeptr)?.listItemNode.style.setProperty('display', '-webkit-box');
      }
    }
...
export class DetachedElementsProfileType extends
  ...
  async getDetachedElements(): Promise<void> {
    ...
    await domModels[0].target().model(SDK.OverlayModel.OverlayModel)?.overlayAgent.invoke_disable();
    const data = await domModels[0].getDetachedDOMNodes();
    await domModels[0].target().model(SDK.OverlayModel.OverlayModel)?.overlayAgent.invoke_enable();
