...
export class SyncSection extends HTMLElement {
  static readonly litTagName = LitHtml.literal`sync-section`;
  ...
  if (!syncInfo.arePreferencesSynced) {
    return LitHtml.html`
      <span class="warning">
        ${
        i18nString(
            UIStrings.preferencesSyncDisabled)} <x-link href="chrome://settings/syncSetup/advanced" class="link">${
        i18nString(UIStrings.settings)}</x-link>
      </span>`;
  }
  return LitHtml.html`
...
ComponentHelpers.CustomElements.defineComponent('sync-section', SyncSection);
