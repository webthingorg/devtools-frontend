...
export class FrameworkIgnoreListSettingsTab extends UI.Widget.VBox implements
  ...
  constructor() {
    ...
    const ignoreListContentScripts = ignoreListOptions.createChild('div', 'ignore-list-content-scripts');
    ...
    const automaticallyIgnoreList = ignoreListOptions.createChild('div', 'automatically-ignore-list');
  ...
  renderItem(item: Common.Settings.RegExpSettingItem, _editable: boolean): Element {
    ...
    const checkbox = element.createChild('input') as HTMLInputElement;
    checkbox.type = 'checkbox';
    checkbox.checked = !item.disabled;
    checkbox.ariaLabel = i18nString(UIStrings.ignoreScriptsWhoseNamesMatchS, {PH1: item.pattern});
    checkbox.addEventListener('change', inputChanged, false);

    element.classList.add('ignore-list-item');
    const pattern = element.createChild('label', 'ignore-list-pattern');
    pattern.textContent = item.pattern;
    UI.Tooltip.Tooltip.install(element, i18nString(UIStrings.ignoreScriptsWhoseNamesMatchS, {PH1: item.pattern}));
