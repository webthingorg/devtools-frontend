...
export class DevicesSettingsTab extends UI.Widget.VBox implements
  ...
  commitEdit(
    ...
    if (userAgentControlValue) {
      device.userAgentMetadata = {
        ...
        mobile:
            (uaType === Emulation.DeviceModeModel.UA.Mobile || uaType === Emulation.DeviceModeModel.UA.MobileNoTouch),
      };
    }
    if (isNew) {
  ...
  _createEditor(): UI.ListWidget.Editor<Emulation.EmulatedDevices.EmulatedDevice> {
    ...
    const uaTypeOptions = [
      Emulation.DeviceModeModel.UA.Mobile,
    ...
    function titleValidator(
      ...
      if (value.length >= Emulation.DeviceModeModel.MaxDeviceNameLength) {
        errorMessage =
            i18nString(UIStrings.deviceNameMustBeLessThanS, {PH1: Emulation.DeviceModeModel.MaxDeviceNameLength});
    ...
    function widthValidator(
        item: Emulation.EmulatedDevices.EmulatedDevice, index: number,
        input: UI.ListWidget.EditorControl): UI.ListWidget.ValidatorResult {
      return Emulation.DeviceModeModel.DeviceModeModel.widthValidator(input.value);
    }

    function heightValidator(
        item: Emulation.EmulatedDevices.EmulatedDevice, index: number,
        input: UI.ListWidget.EditorControl): UI.ListWidget.ValidatorResult {
      return Emulation.DeviceModeModel.DeviceModeModel.heightValidator(input.value);
    }

    function scaleValidator(
        item: Emulation.EmulatedDevices.EmulatedDevice, index: number,
        input: UI.ListWidget.EditorControl): UI.ListWidget.ValidatorResult {
      return Emulation.DeviceModeModel.DeviceModeModel.scaleValidator(input.value);
    }
  }
}
