...
describe('DebuggerPlugin', () => {
  describe('computeExecutionDecorations', () => {
    ...
    it('correctly returns no decorations when no execution location is passed', () => {
      const doc = 'console.log("Hello World!");';
      const state = CodeMirror.EditorState.create({doc, extensions});
      const decorations = computeExecutionDecorations(state, null);
    ...
    it('correctly returns line and token decorations', () => {
      ...
      const state = CodeMirror.EditorState.create({doc, extensions});
      const decorations = computeExecutionDecorations(state, doc.indexOf('debugger'));
...
<html xmlns="http://www.w3.org/1999/xhtml"></html>`;
      ...
      const decorations = computeExecutionDecorations(state, doc.indexOf('<html'));
