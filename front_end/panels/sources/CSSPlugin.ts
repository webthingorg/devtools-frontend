...
function findColorsAndCurves(
    ...
    onWideGamutColor: (pos: number, colorText: string) => void,
  ...
  tree.iterate({
    ...
    enter: node => {
      ...
      if (content) {
        ...
        } else if (Common.Color.Color.canBeWideGamut(content)) {
          onWideGamutColor(node.from, content);
        } else {
...
function computeSwatchDeco(state: CodeMirror.EditorState, from: number, to: number): CodeMirror.DecorationSet {
  ...
  findColorsAndCurves(
      ...
      (pos, wideGamutColorText) => {
        builder.add(
            pos, pos, CodeMirror.Decoration.widget({widget: new CircularColorSwatchWidget(wideGamutColorText)}));
      },
      (pos, curve, text) => {
