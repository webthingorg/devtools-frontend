...
export class BreakpointView extends HTMLElement {
  ...
  #render() {
    if (!ComponentHelpers.ScheduledRender.isScheduledRender(this)) {
      throw new Error('BreakpointView render was not scheduled');
    }
  ...
  #renderBreakpointGroup(group: BreakpointGroup): LitHtml.TemplateResult {
    // clang-format off
    return LitHtml.html`
    <details ?open=${group.expanded} @toggle=${(e: Event) => this.#onDetailsToggled(e, group.url)}>
      <summary>
        <div class='icon-and-group-info'>
          ${this.#renderFileIcon()}
          <span data-group-name=true>${Common.ParsedURL.ParsedURL.extractName(group.url)}</span>
        </div>
      </summary>

      ${group.breakpointEntries.map(entry => this.#renderBreakpointEntry(entry))}
    </details>
     `
    // clang-format on
  }
  ...
  #renderFileIcon(): LitHtml.TemplateResult {
    return LitHtml.html`
      <${IconButton.Icon.Icon.litTagName} .data=${{iconName: 'ic_file_script', color: '', width: '16px', height: '16px'} as IconButton.Icon.IconWithName}></${IconButton.Icon.Icon.litTagName}>
    `
  }
  ...
  #renderBreakpointEntry(item: BreakpointItem): LitHtml.TemplateResult {
    ...
    <div class=${LitHtml.Directives.classMap(classMap)}>
      ...
      <span class='code-snippet' @click=${() => this.dispatchEvent(new BreakpointSelectedEvent(item))}>${this.#getCodeSnippet(item)}</span>
    </div>
    `
    // clang-format on
  }
  ...
  #onDetailsToggled(e: Event, url: Platform.DevToolsPath.UrlString) {
    const details = e.currentTarget as HTMLDetailsElement;
    this.dispatchEvent(new ExpandedStateChangedEvent(url, details.open));
  }
