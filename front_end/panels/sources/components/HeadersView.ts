...
export class HeadersViewComponent extends HTMLElement {
  ...
  #parsingError = false;
  #focusElement: {type: BlockOrHeader, blockIndex: number, headerIndex?: number}|null = null;
  ...
  #onClick(e: Event): void {
    ...
    if (target.matches('.add-header')) {
      ...
      this.#focusElement = {type: BlockOrHeader.Header, blockIndex, headerIndex};
      ...
      this.#focusElement = {type: BlockOrHeader.Block, blockIndex};
      this.#onHeadersChanged();
  ...
  #render(): void {
    ...
    if (this.#focusElement) {
      ...
      if (this.#focusElement.type === BlockOrHeader.Header) {
        focusElement =
            this.#shadow.querySelector(`[data-block-index="${this.#focusElement.blockIndex}"][data-header-index="${
                (this.#focusElement.headerIndex || 0) + 1}"] .header-name`);
      } else {
        focusElement =
            this.#shadow.querySelector(`[data-block-index="${this.#focusElement.blockIndex + 1}"] .apply-to`);
      }
      if (focusElement) {
  ...
  #renderApplyToRow(pattern: string, blockIndex: number): LitHtml.TemplateResult {
    ...
    return LitHtml.html`
      <div class="row" data-block-index=${blockIndex}>
        ...
        ${this.#renderAddRemoveButtons(BlockOrHeader.Block)}
      </div>
    `;
    // clang-format on
  }
  ...
  #renderHeaderRow(header: Header, blockIndex: number, headerIndex: number): LitHtml.TemplateResult {
    ...
    return LitHtml.html`
      <div class="row padded" data-block-index=${blockIndex} data-header-index=${headerIndex}>
        ...
        ${this.#renderAddRemoveButtons(BlockOrHeader.Header)}
      </div>
    `;
    // clang-format on
  }
