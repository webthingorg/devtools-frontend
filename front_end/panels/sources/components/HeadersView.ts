...
export interface ContentEditableData {
  value: string;
  seed: number;
}

export class ContentEditable extends HTMLElement {
  static readonly litTagName = LitHtml.literal`devtools-editor-editable`;

  set data(data: ContentEditableData) {
...
export class HeadersViewComponent extends HTMLElement {
  ...
  #onHeadersChanged(): void {
    ...
    // Prevent `onWorkingCopyChanged()` to cause an unnecessary re-render
  ...
  #renderEditable(value: string, className?: string): LitHtml.TemplateResult {
    // If `value` has not changed compared to the previous render, the
    // `ContentEditable`'s `innerHTML` would not be updated without the seed
    // which is guaranteed to always be different.
    // This happens when the user makes an edit, closes the file without saving
    // and then re-opens the file.
    // clang-format off
    return LitHtml.html`
      <${ContentEditable.litTagName} contenteditable="true" class="editable ${className}"
        tabindex="0" .data=${{value, seed: Date.now()} as ContentEditableData}></${
      ContentEditable.litTagName}>`;
    // clang-format on
  }
}

ComponentHelpers.CustomElements.defineComponent('devtools-editor-editable', ContentEditable);
...
declare global {
  ...
  interface HTMLElementTagNameMap {
    ...
    'devtools-editor-editable': ContentEditable;
  }
}
