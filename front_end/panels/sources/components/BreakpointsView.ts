...
export class BreakpointsView extends HTMLElement {
  ...
  #handleHomeOrEndKey(key: 'Home'|'End'): Promise<void> {
    if (key === 'Home') {
    ...
    if (key === 'End') {
      ...
      if (lastGroup.expanded) {
        const lastBreakpointItem =
            this.#shadow.querySelector<HTMLElement>(`details:nth-of-type(${numGroups}) > .breakpoint-item:last-child`);
        return this.#setSelected(lastBreakpointItem);
      }
      const lastGroupSummaryElement =
          this.#shadow.querySelector<HTMLElement>(`details:nth-of-type(${numGroups}) > summary`);
    ...
    return LitHtml.html`
      <details class=${LitHtml.Directives.classMap(classMap)}
          ...
          <summary @contextmenu=${contextmenuHandler}
                   tabindex=${tabIndex}
                   @click=${clickHandler}
                   track-dom-node-to-element-id=${trackDOMNodeToElementId(this.#domNodeToTreeNodeMap, group.url)}>
  ...
  #renderBreakpointEntry(breakpointItem: BreakpointItem, editable: boolean): LitHtml.TemplateResult {
    ...
    <div class=${LitHtml.Directives.classMap(classMap)}
         ...
         track-dom-node-to-element-id=${trackDOMNodeToElementId(this.#domNodeToTreeNodeMap, breakpointItem.id)}>
      <label class='checkbox-label'>
