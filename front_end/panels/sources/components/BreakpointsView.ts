...
import {findNextNodeForKeyboarNavigation, trackDOMNodeToElementId, type ElementId} from './BreakpointsViewUtils.js';
...
export class BreakpointsView extends HTMLElement {
  ...
  #domNodeToTreeNodeMap: WeakMap<HTMLElement, ElementId> = new WeakMap();
  ...
  #keyDownHandler(event: KeyboardEvent): Promise<void> {
    ...
    if (event.key === 'Home') {
      const firstGroupElement = this.#shadow.querySelector<HTMLElement>('details > summary');
      assertNotNullOrUndefined(firstGroupElement);
      return this.#setSelected(firstGroupElement);
    }
    if (event.key === 'End') {
      const numGroups = this.#breakpointGroups.length;
      const lastGroup = this.#breakpointGroups[numGroups - 1];
      if (lastGroup.expanded) {
        const lastBreakpointItem =
            this.#shadow.querySelector<HTMLElement>(`details:nth-of-type(${numGroups}) > .breakpoint-item:nth-of-type(${
                lastGroup.breakpointItems.length})`) ||
            null;
        return this.#setSelected(lastBreakpointItem);
      }
      const lastGroupSummaryElement =
          this.#shadow.querySelector<HTMLElement>(`details:nth-of-type(${numGroups}) > summary`) || null;
    ...
    // No need to handle other key events.
    event.consume();
    return Promise.resolve();
  }
  ...
  async #setSelected(element: HTMLElement|null): Promise<void> {
    ...
    const id = this.#domNodeToTreeNodeMap.get(element);
  ...
  async #handleArrowKey(key: Platform.KeyboardUtilities.ArrowKey, target: HTMLElement): Promise<void> {
    ...
    const setGroupExpandedState = (detailsElement: HTMLDetailsElement, expanded: boolean): Promise<unknown> => {
      if (expanded) {
    ...
    const nextNode = await findNextNodeForKeyboarNavigation(target, key, setGroupExpandedState);
    ...
    return LitHtml.html`
      <details class=${LitHtml.Directives.classMap(classMap)}
          ...
          <summary @contextmenu=${contextmenuHandler}
                   tabindex=${tabIndex}
                   @click=${clickHandler}
                   track-dom-node-to-tree-node=${trackDOMNodeToElementId(this.#domNodeToTreeNodeMap, group.url)}>
  ...
  #renderBreakpointEntry(breakpointItem: BreakpointItem, editable: boolean): LitHtml.TemplateResult {
    ...
    <div class=${LitHtml.Directives.classMap(classMap)}
         ...
         track-dom-node-to-tree-node=${trackDOMNodeToElementId(this.#domNodeToTreeNodeMap, breakpointItem.id)}>
      <label class='checkbox-label'>
