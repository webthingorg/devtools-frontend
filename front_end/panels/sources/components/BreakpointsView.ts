...
export class BreakpointsView extends HTMLElement {
    ...
    return LitHtml.html`
      <details class=${LitHtml.Directives.classMap(classMap)}
          ...
          <summary @contextmenu=${contextmenuHandler}
                   ...
                   track-dom-node-to-element-id=${ComponentHelpers.Directives.nodeRenderedCallback((domNode: Element) => {
                      if (!(domNode instanceof HTMLElement)) {
                        throw new Error('domNode is expected to an HTMLElement.');
                      }
                      this.#domNodeToIdMap.set(domNode, group.url);
                    })}>
  ...
  #renderBreakpointEntry(
    ...
    <div class=${LitHtml.Directives.classMap(classMap)}
         ...
         @keydown=${this.#keyDownHandler}
         track-dom-node-to-element-id=${nodeRenderedCallback((domNode: Element) => {
          if (!(domNode instanceof HTMLElement)) {
            throw new Error('domNode is expected to an HTMLElement.');
          }
          this.#domNodeToIdMap.set(domNode, breakpointItem.id);
         })}>
      <label class='checkbox-label'>
