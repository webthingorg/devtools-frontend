...
export class RecorderPanel extends UI.Panel.Panel {
  private recording: Recorder.Recording.Recording|null;
  ...
  constructor() {
    ...
    this.recording = {
      ...
      sections: [],
    } as Recorder.Recording.Recording;
    ...
    recordingView.addEventListener('recordingtoggled', async (e: Event) => {
      ...
      recordingView.data = {
        recording: (currentSession ? currentSession.getRecording() : this.recording) as Recorder.Recording.Recording,
        isRecording: event.data,
      };

      if (currentSession) {
        currentSession.addEventListener('recording-updated', async ({data}: {data: Recorder.Recording.Recording}) => {
          this.recording = data;
          recordingView.data = {
            recording: data,
            isRecording: event.data,
