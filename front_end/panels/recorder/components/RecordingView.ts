...
export class RecordingView extends HTMLElement {
  ...
  set data(data: RecordingViewData) {
    ...
    if (this.userFlow !== data.recording) {
      this.userFlow = data.recording;
      this.steps = this.userFlow.sections.reduce((prev, curr) => [...prev, ...curr.steps], [] as Recorder.Steps.Step[]);
    }
  ...
  private getStepState(step: Recorder.Steps.Step): State {
    ...
    if (step === this.currentStep) {
      return this.currentError ? State.Error : State.Current;
    }
      ...
      <div class="wrapper ${this.isRecording ? 'is-recording' : null} ${this.isPlaying ? 'is-playing' : null}">
        ...
        <div class="footer">
          <div class="controls">
            ...
            <button class="control" @click=${this.handleTogglePlaying.bind(this)}>
              <div class="icon"></div>
