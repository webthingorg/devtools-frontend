...
describe('CSSExpressionHelper', () => {
  describe('resolveCSSExpressionWithVar', () => {
      ...
      const STYLE_VARIABLE_VALUE_MAP: Map<SDK.CSSStyleDeclaration.CSSStyleDeclaration, NameValueMap> = new Map([
        [
          mockCssStyleDeclaration,
          {
            ...
            '--cycle-a': {value: 'var(--cycle-b, 20px)', declaration: createMockDeclaration(mockCssStyleDeclaration)},
            '--cycle-b': {value: 'var(--cycle-a)', declaration: createMockDeclaration(mockCssStyleDeclaration)},
            '--out-of-cycle':
    ...
    describe('cyclic references', () => {
      ...
      it('should return `null` if the var reference is inside the cycle', () => {
        assert.isNull(resolveWithMocks('var(--cycle-a)'));
      });

      // TODO(ergunsh): These are valid `var()` reference cases that we currently
      // don't handle correctly.
      it.skip('[crbug.com/329626445]: should return fallback value if the expression is not inside the cycle', () => {
    ...
    describe('var references inside fallback', () => {
      it('should resolve a `var()` reference inside fallback value too', () => {
        assert.strictEqual(
            resolveCSSExpressionWithVar(
                mockMatchedStyles, mockCssStyleDeclaration, 'var(--non-existent, var(--a) other)'),
            'a other');
      });

      it('should return null when the fallback value contains a cyclic reference', () => {
        assert.isNull(resolveCSSExpressionWithVar(
            mockMatchedStyles, mockCssStyleDeclaration, 'var(--non-existent, var(--cycle-1) other)'));
      });

      it('should return null when the fallback value is non existent too', () => {
        assert.isNull(resolveCSSExpressionWithVar(
            mockMatchedStyles, mockCssStyleDeclaration, 'var(--non-existent, var(--other-non-existent))'));
      });
    });
