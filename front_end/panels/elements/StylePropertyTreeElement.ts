...
export class ShadowRenderer extends ShadowMatch {
  ...
  shadowModel(shadow: CodeMirror.SyntaxNode[], context: RenderingContext): null|ShadowModel {
    ...
    for (let item = queue.shift(); item; item = queue.shift()) {
      ...
      if (value.name === 'NumberLiteral') {
      ...
      } else if (value.name === 'ValueName' && text === 'inset') {
        ...
        const computedValueAst = tokenizePropertyValue(computedValue);
        if (!computedValueAst) {
          return null;
        }
        const matches =
  ...
  override render(node: CodeMirror.SyntaxNode, context: RenderingContext): Node[] {
    ...
    for (const shadow of shadows) {
      ...
      if (!model) {
        const {nodes} = Renderer.render(shadow.concat(isImportant ? [isImportant] : []), context);
        result.push(...nodes);
        continue;
      }
      ...
      popoverHelper.addEventListener(ShadowSwatchPopoverHelper.Events.ShadowChanged, () => {
        model.renderContents(swatch);
        void this.#treeElement.applyStyleText(this.#treeElement.renderedPropertyText(), false);
      });
      result.push(swatch);
