...
export class VariableRenderer extends VariableMatch {
  ...
  render(context: RenderingContext): Node[] {
    ...
    const {declaration, value} = this.#treeElement.matchedStyles().computeCSSVariable(this.#style, this.name) ?? {};

    const computedValue = this.computedText();
    const fromFallback = !value;
    ...
    if (varSwatch.link) {
      this.#pane.addPopover(
          varSwatch.link, () => this.#treeElement.getVariablePopoverContents(this.name, value ?? null));
    }
...
export class ColorMixRenderer extends ColorMixMatch {
  override render(context: RenderingContext): Node[] {
    const hookUpColorArg = (node: Node, onChange: (newColorText: string) => void): string|null => {
    ...
    const color1 = Renderer.renderInto(this.color1.color, context, contentChild).cssControls.get('color') ?? [];
    this.color1.percentage && Renderer.renderInto(this.color1.percentage, context, contentChild);
    contentChild.appendChild(document.createTextNode(', '));
    const color2 = Renderer.renderInto(this.color2.color, context, contentChild).cssControls.get('color') ?? [];
    this.color2.percentage && Renderer.renderInto(this.color2.percentage, context, contentChild);
    contentChild.appendChild(document.createTextNode(')'));

    if ([...this.space, this.color1.color, this.color1.percentage, this.color2.color, this.color2.percentage].some(
            n => n && context.matchedResult.hasUnresolvedVars(n))) {
      return [contentChild];
    }

    const swatch = new InlineEditor.ColorMixSwatch.ColorMixSwatch();
    const firstColorText = color1[0] ? hookUpColorArg(color1[0], text => swatch.setFirstColor(text)) : null;
    const secondColorText = color2[0] ? hookUpColorArg(color2[0], text => swatch.setSecondColor(text)) : null;

    if (!firstColorText || !secondColorText) {
      return [contentChild];
    }

    const space = this.space.map(space => context.matchedResult.getComputedText(space)).join(' ');
    const p1Text = this.color1.percentage ? ' ' + context.matchedResult.getComputedText(this.color1.percentage) : '';
    const p2Text = this.color2.percentage ? ' ' + context.matchedResult.getComputedText(this.color2.percentage) : '';
    swatch.appendChild(contentChild);
    swatch.setColorMixText(`color-mix(${space}, ${firstColorText}${p1Text}, ${secondColorText}${p2Text})`);
    context.addControl('color', swatch);
    return [swatch];
  }
