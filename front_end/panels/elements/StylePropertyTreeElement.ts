...
export class ShadowRenderer implements MatchRenderer<ShadowMatch> {
  ...
  shadowModel(shadow: CodeMirror.SyntaxNode[], context: RenderingContext, shadowMatch: ShadowMatch): null|ShadowModel {
    ...
    for (let item = queue.shift(); item; item = queue.shift()) {
      ...
      if (value.name === 'NumberLiteral') {
      ...
      } else if (match && match instanceof VariableMatch) {
        ...
        } else if (match && (match instanceof ColorMatch || match instanceof ColorMixMatch)) {
...
export class GridTemplateRenderer implements MatchRenderer<GridTemplateMatch> {
  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: GridTemplateMatch): Node[] {
...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  private innerUpdateTitle(): void {
    ...
    const propertyRenderer = new StylesSidebarPropertyRenderer(this.name, this.value, [
      {
        type: VariableMatch,
        matcher: new VariableMatcher(this.matchedStylesInternal, this.style),
        renderer: new VariableRenderer(this),
      },
      {
        type: ColorMatch,
        matcher: new ColorMatcher(),
        renderer: new ColorRenderer(this),
      },
      {
        type: ColorMixMatch,
        matcher: new ColorMixMatcher(),
        renderer: new ColorMixRenderer(this.parentPaneInternal),
      },
      {
        type: URLMatch,
        matcher: new URLMatcher(),
        renderer: new URLRenderer(this.style.parentRule, this.node()),
      },
      {
        type: AngleMatch,
        matcher: new AngleMatcher(),
        renderer: new AngleRenderer(this),
      },
      {
        type: LinkableNameMatch,
        matcher: new LinkableNameMatcher(),
        renderer: new LinkableNameRenderer(this),
      },
      {
        type: BezierMatch,
        matcher: new BezierMatcher(),
        renderer: new BezierRenderer(this),
      },
      {
        type: StringMatch,
      ...
      {
        type: ShadowMatch,
        matcher: new ShadowMatcher(),
        renderer: new ShadowRenderer(this),
      },
      {
        type: FontMatch,
        matcher: new FontMatcher(),
        renderer: new FontRenderer(this),
      },
      {
        type: GridTemplateMatch,
        matcher: new GridTemplateMatcher(),
        renderer: new GridTemplateRenderer(),
      },
    ]);
