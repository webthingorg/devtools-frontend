...
export class AnchorFunctionRenderer implements MatchRenderer<AnchorFunctionMatch> {
  ...
  render(match: AnchorFunctionMatch, context: RenderingContext): Node[] {
    ...
    if (match.node.name === 'VariableName') {
      ...
      // The matcher passes an 'ArgList' node with a functionName ('anchor' or
      // 'anchor-size') if the arguments need to have an implicit anchor link
      // swatch rendered.
      ...
      const remainingArgs = ASTUtils.children(match.node).splice(1);
