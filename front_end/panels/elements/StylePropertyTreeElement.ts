...
export class VariableRenderer implements MatchRenderer<VariableMatch> {
  ...
  resolveVariable(name: string): SDK.CSSMatchedStyles.CSSVariableValue|null {
    return this.#matchedStyles.computeCSSVariable(this.#style, name);
  }

  fallbackValue(fallback: CodeMirror.SyntaxNode[], matching: BottomUpTreeMatching): string|null {
    if (fallback.length === 0 || fallback.some(node => matching.hasUnresolvedVars(node))) {
      return null;
    }
    return fallback.map(node => matching.getComputedText(node)).join(' ');
  }

  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: VariableMatch): Node[] {
    const renderedFallback = match.fallback.length > 0 ? Renderer.render(match.fallback, context) : undefined;

    const {declaration, value: variableValue} = this.resolveVariable(match.name) ?? {};
    const fromFallback = !variableValue;
    const computedValue = variableValue ?? this.fallbackValue(match.fallback, context.matchedResult);
...
export class ColorRenderer implements MatchRenderer<ColorMatch> {
  ...
  #getValueChild(node: CodeMirror.SyntaxNode, context: RenderingContext, match: ColorMatch):
      {valueChild: HTMLSpanElement, cssControls?: CSSControlMap} {
    const valueChild = document.createElement('span');
    if (node.name === 'ColorLiteral' || (node.name === 'ValueName' && Common.Color.Nicknames.has(match.text))) {
  ...
  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: ColorMatch): Node[] {
    const {valueChild, cssControls} = this.#getValueChild(node, context, match);
    const swatch = this.renderColorSwatch(context.matchedResult.getComputedText(node), valueChild);
...
export class ColorMixRenderer implements MatchRenderer<ColorMixMatch> {
  ...
  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: ColorMixMatch): Node[] {
...
export class URLRenderer implements MatchRenderer<URLMatch> {
  ...
  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: URLMatch): Node[] {
    ...
    if (this.rule && this.rule.resourceURL()) {
      hrefUrl = Common.ParsedURL.ParsedURL.completeURL(this.rule.resourceURL(), match.url);
    } else if (this.node) {
      hrefUrl = this.node.resolveURL(match.url);
    }
    const link = ImagePreviewPopover.setImageUrl(
        Components.Linkifier.Linkifier.linkifyURL(hrefUrl || match.url, {
          text: match.url,
          preventClick: false,
          // crbug.com/1027168
...
export class AngleRenderer implements MatchRenderer<AngleMatch> {
  ...
  render(_: unknown, context: RenderingContext, match: AngleMatch): Node[] {
...
export class LinkableNameRenderer implements MatchRenderer<LinkableNameMatch> {
  ...
  render(_: unknown, context: RenderingContext, match: LinkableNameMatch): Node[] {
...
export class BezierRenderer implements MatchRenderer<BezierMatch> {
  ...
  render(_: unknown, context: RenderingContext, match: BezierMatch): Node[] {
...
export class StringRenderer implements MatchRenderer<StringMatch> {
  render(node: unknown, context: unknown, match: StringMatch): Node[] {
...
export class ShadowRenderer implements MatchRenderer<ShadowMatch> {
  ...
  shadowModel(shadow: CodeMirror.SyntaxNode[], context: RenderingContext, shadowMatch: ShadowMatch): null|ShadowModel {
    ...
    for (let item = queue.shift(); item; item = queue.shift()) {
      ...
      if (value.name === 'NumberLiteral') {
        ...
        if (propertyType === undefined ||
            (propertyType === ShadowPropertyType.Spread && shadowMatch.shadowType === ShadowType.TextShadow)) {
          return null;
        }
        ...
        queue.unshift(...ASTUtils.siblings(ASTUtils.declValue(matches.ast.tree))
                          .map(matchedNode => ({
                                 ...
                                 expansionContext: new RenderingContext(computedValueAst, matches, context.rendererMap),
                               })));
      } else {
        ...
        if (value.name === 'ValueName' && text.toLowerCase() === 'inset') {
          if (shadowMatch.shadowType === ShadowType.TextShadow ||
    ...
    return new ShadowModel(shadowMatch.shadowType, properties, context);
  }

  render(node: CodeMirror.SyntaxNode, context: RenderingContext, match: ShadowMatch): Node[] {
    const shadows = ASTUtils.split(ASTUtils.siblings(ASTUtils.declValue(node)));
    const result: Node[] = [];

    for (const shadow of shadows) {
      const model = this.shadowModel(shadow, context, match);
...
export class FontRenderer implements MatchRenderer<FontMatch> {
  ...
  render(node: unknown, context: unknown, match: FontMatch): Node[] {
...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  private innerUpdateTitle(): void {
    ...
    const propertyRenderer = new StylesSidebarPropertyRenderer(
        ...
        {
          [MatchType.Var]: new VariableRenderer(this, this.style),
          [MatchType.Color]: new ColorRenderer(this),
          [MatchType.ColorMix]: new ColorMixRenderer(this.parentPane()),
          [MatchType.URL]: new URLRenderer(this.style.parentRule, this.node()),
          [MatchType.Angle]: new AngleRenderer(this),
          [MatchType.LinkableName]: new LinkableNameRenderer(this),
          [MatchType.Bezier]: new BezierRenderer(this),
          [MatchType.String]: new StringRenderer(),
          [MatchType.Shadow]: new ShadowRenderer(this),
          [MatchType.Font]: new FontRenderer(this),
          [MatchType.Text]: {
            render: (node: unknown, context: unknown, match: TextMatch) => {
              return [document.createTextNode(match.text)];
            },
          },
        });
