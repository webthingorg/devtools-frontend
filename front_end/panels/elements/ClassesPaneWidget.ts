...
export class ClassesPaneWidget extends UI.Widget.Widget {
  ...
  constructor() {
    ...
    this.contentElement.setAttribute('jslog', `${VisualLogging.pane().context('elements-classes-pane')}`);
...
export class ButtonProvider implements UI.Toolbar.Provider {
  ...
  private constructor() {
    ...
    this.button.element.setAttribute(
        'jslog', `${VisualLogging.toggleSubpane().track({click: true}).context('elements-classes-pane')}`);
