...
export abstract class TreeWalker {
  ...
  static walkExcludingSuccessors<T extends TreeWalker, ArgTs>(
...
export class Renderer extends TreeWalker {
  ...
  static render(node: CodeMirror.SyntaxNode, context: RenderingContext): Node[] {
    const renderer = Renderer.walkExcludingSuccessors(context.ast.subtree(node), context.matchedResult);
  ...
  protected override enter({node}: SyntaxNodeRef): boolean {
    ...
    if (match) {
      this.#output = mergeWithSpacing(this.#output, match.render(this.#context()));
      return false;
    }
...
class LegacyRegexMatch implements Match {
  ...
  constructor(text: string, processor: (text: string) => Node | null) {
    this.text = text;
