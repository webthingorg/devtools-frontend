...
export class SyntaxTree {
  constructor(
      readonly propertyValue: string, readonly rule: string, readonly tree: CodeMirror.SyntaxNode,
...
function requiresSpace(a: Node[], b: Node[]): boolean {
  ...
  const noSpaceBefore = ['', '(', ' ', '{', '}', ';'];
  const noSpaceAfter = ['', '(', ')', ',', ':', ' ', '*', '{', ';'];
  return !noSpaceBefore.includes(trailingChar) && !noSpaceAfter.includes(leadingChar);
}
...
export function parsePropertyValue(text: string, matchers: Matcher[]): Node[] {
  const ast = tokenizePropertyValue(text);
  if (!ast) {
    return [];
  }
  const matchedResult = BottomUpTreeMatching.walk(ast, true, matchers);
  ...
  return [...siblings(ast.tree), ...ast.trailingNodes]
      .map(node => Renderer.render(node, context))
