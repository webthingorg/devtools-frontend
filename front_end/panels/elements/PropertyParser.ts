...
export class Renderer extends TreeWalker {
  ...
  constructor(ast: SyntaxTree, matchedResult: BottomUpTreeMatching, cssControls: CSSControlMap, readonly: boolean) {
    super(ast);
    this.#matchedResult = matchedResult;
    this.#context = new RenderingContext(this.ast, this.#matchedResult, cssControls, readonly);
  }
  ...
  static render(nodeOrNodes: CodeMirror.SyntaxNode|CodeMirror.SyntaxNode[], context: RenderingContext):
    ...
    const renderers = nodeOrNodes.map(
        node => this.walkExcludingSuccessors(
            context.ast.subtree(node), context.matchedResult, cssControls, context.readonly));
...
class LegacyRegexMatch implements Match {
  ...
  render(_node: CodeMirror.SyntaxNode, context: RenderingContext): Node[] {
    const rendered = this.processor(this.#matchedText, context.readonly);
