...
export class Renderer extends TreeWalker {
  ...
  static render(nodeOrNodes: CodeMirror.SyntaxNode|CodeMirror.SyntaxNode[], context: RenderingContext):
      {nodes: Node[], nodeGroups: Node[][], cssControls: CSSControlMap} {
    ...
    const nodeGroups = renderers.map(node => node.#output);
    const nodes = nodeGroups.reduce(mergeWithSpacing);
    return {nodes, nodeGroups, cssControls};
  }
