...
export abstract class BezierMatch implements Match {
  ...
  constructor(
      readonly text: string,
      readonly model?: InlineEditor.CSSLinearEasingModel.CSSLinearEasingModel|UI.Geometry.CubicBezier) {
  }
  abstract render(node: CodeMirror.SyntaxNode, context: RenderingContext): Node[];
}
...
export class BezierMatcher extends MatcherBase<typeof BezierMatch> {
  ...
  override matches(node: CodeMirror.SyntaxNode, matching: BottomUpTreeMatching): Match|null {
    ...
    if (node.name === 'ValueName') {
      if (!UI.Geometry.CubicBezier.KeywordValues.has(text)) {
        return null;
      }
    ...
    const model =
        InlineEditor.CSSLinearEasingModel.CSSLinearEasingModel.parse(text) ?? UI.Geometry.CubicBezier.parse(text);
    if (!model) {
      return null;
    }
