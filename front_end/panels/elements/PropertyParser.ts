...
export class Renderer extends TreeWalker {
  ...
  #output: Node[] = [];
  #trace: RendererTrace|undefined;

  constructor(ast: SyntaxTree, matchedResult: BottomUpTreeMatching, trace?: RendererTrace) {
    super(ast);
    this.#matchedResult = matchedResult;
    this.#trace = trace;
  }

  static render(node: CodeMirror.SyntaxNode, context: RenderingContext, trace?: RendererTrace): Node[] {
    const renderer = Renderer.walkExcludingSuccessors(context.ast.subtree(node), context.matchedResult, trace);
  ...
  protected override enter({node}: SyntaxNodeRef): boolean {
    ...
    if (match) {
      ...
      if (output && this.#trace) {
        this.#trace.push({text: match.text, matchType: match.type});
      }
...
export function renderPropertyValue(value: string, matchers: Matcher[], trace?: RendererTrace): Node[] {
  ...
  return [...siblings(ast.tree), ...ast.trailingNodes]
      .map(node => Renderer.render(node, context, trace))
