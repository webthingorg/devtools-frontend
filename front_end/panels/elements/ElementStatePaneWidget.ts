...
export class ElementStatePaneWidget extends UI.Widget.Widget {
  ...
  constructor() {
    ...
    const createPageStateCheckbox =
        (state: string, setting: Common.Settings.Setting<boolean>, tooltip: string, metric: Host.UserMetrics.Action):
            Element => {
              const div = document.createElement('div');
              div.classList.add('page-state-checkbox');
              const label = UI.UIUtils.CheckboxLabel.create(state);
              UI.SettingsUI.bindCheckbox(label.checkboxElement, setting, metric);
              UI.Tooltip.Tooltip.install(label.textElement, tooltip);
    ...
    const keepPageFocusedCheckbox = createPageStateCheckbox(
        i18nString(UIStrings.emulateFocusedPage), Common.Settings.Settings.instance().moduleSetting('emulatePageFocus'),
        i18nString(UIStrings.emulatesAFocusedPage), Host.UserMetrics.Action.ToggleEmulateFocusedPageFromStylesPane);
