...
export class PositionTryFallbackMatch implements Match {
  constructor(readonly text: string, readonly matching: BottomUpTreeMatching, readonly node: CodeMirror.SyntaxNode) {
  }
}

// clang-format off
export class PositionTryFallbackMatcher extends matcherBase(PositionTryFallbackMatch) {
  override accepts(propertyName: string): boolean {
    return propertyName === 'position-try-fallbacks';
  }
