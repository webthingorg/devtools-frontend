...
function rendererMapWithTextRenderer() {
  const map: Elements.PropertyParser.RendererMap = new Map();
  map.set(Elements.PropertyParser.TextMatch, {
    render: (node: unknown, context: unknown, match: Elements.PropertyParser.TextMatch) => {
    ...
    if (matching.computedText.chunkCount === 0) {
      ...
      for (const [varName, value] of Object.entries(variables)) {
        ...
        for (let offset = matching.ast.rule.indexOf(varText); offset >= 0;
          ...
          matching.computedText.push({text: varText, computedText: () => value}, offset - propertyOffset);
        }
  ...
  sinon.stub(Elements.PropertyParser.BottomUpTreeMatching.prototype, 'getMatch')
      .callsFake(function(this: Elements.PropertyParser.BottomUpTreeMatching, node: CodeMirror.SyntaxNode):
                     Elements.PropertyParser.Match|undefined {
                       injectChunk(this);
                       return variableNames.has(this.ast.text(node)) ?
                           new Elements.PropertyParser.VariableMatch(
                               this.ast.text(node), this.ast.text(node), [], this,
...
describe('PropertyParser', () => {
  it('parses text', () => {
    assert.deepStrictEqual(
        textFragments(Elements.PropertyParser.renderPropertyValue('--p', 'var(--v)', [], new Map())),
        ['var', '(', '--v', ')']);

    assert.deepStrictEqual(
        textFragments(
            Elements.PropertyParser.renderPropertyValue('--p', '/* comments are text */ 1px solid 4', [], new Map())),
        ['/* comments are text */', ' ', '1px', ' ', 'solid', ' ', '4']);
    assert.deepStrictEqual(
        textFragments(Elements.PropertyParser.renderPropertyValue(
            '--p', '2px var(--double, var(--fallback, black)) #32a1ce rgb(124 125 21 0)', [], new Map())),
        [
  ...
  it('reproduces the input if nothing matched', () => {
    ...
    const context = new Elements.PropertyParser.RenderingContext(ast, matchedResult, rendererMapWithTextRenderer());
    assert.deepStrictEqual(
...
||| }`);
  ...
  it('renders trailing comments', () => {
    const property = '/* color: red */ blue /* color: red */';
    assert.strictEqual(
        textFragments(Elements.PropertyParser.renderPropertyValue('--p', property, [], new Map())).join(''), property);
  });
  ...
  it('parses colors', () => {
    for (const fail of ['red-blue', '#f', '#foobar', '', 'rgbz(1 2 2)', 'tan(45deg)']) {
      const {match, text} = matchSingleValue(
          'color', fail, Elements.PropertyParser.ColorMatch, new Elements.PropertyParser.ColorMatcher());
    ...
    for (const succeed
             of ['rgb(/* R */155, /* G */51, /* B */255)', 'red', 'rgb(0 0 0)', 'rgba(0 0 0)', '#fff', '#ffff',
                 '#ffffff', '#ffffffff']) {
      const {match, text} = matchSingleValue(
          'color', succeed, Elements.PropertyParser.ColorMatch, new Elements.PropertyParser.ColorMatcher());
    ...
    for (const fail
             of ['rgb(/* R */155, /* G */51, /* B */255)', 'red', 'rgb(0 0 0)', 'rgba(0 0 0)', '#fff', '#ffff',
                 '#ffffff', '#ffffffff']) {
      const {match, text} = matchSingleValue(
          'width', fail, Elements.PropertyParser.ColorMatch, new Elements.PropertyParser.ColorMatcher());
      assert.isNull(match, text);
    }
  });
  ...
  it('parses color-mix with vars', () => {
    ...
    });
    {
      const {ast, match, text} = matchSingleValue(
          ...
          Elements.PropertyParser.ColorMixMatch, new Elements.PropertyParser.ColorMixMatcher());
    ...
    for (const fail
      ...
      const {ast, match, text} = matchSingleValue(
          'color', fail, Elements.PropertyParser.ColorMixMatch, new Elements.PropertyParser.ColorMixMatcher());
      Platform.assertNotNullOrUndefined(ast, text);
      assert.isNull(match, text);
    }
  });
  ...
  it('parses vars correctly', () => {
    const cssMatchedStylesMock = sinon.createStubInstance(SDK.CSSMatchedStyles.CSSMatchedStyles);
    const styleMock = sinon.createStubInstance(SDK.CSSStyleDeclaration.CSSStyleDeclaration);
    for (const succeed
             of ['var(--a)', 'var(--a, 123)', 'var(--a, calc(1+1))', 'var(--a, var(--b))', 'var(--a, var(--b, 123))',
                 'var(--a, a b c)']) {
      const {ast, match, text} = matchSingleValue(
          'width', succeed, Elements.PropertyParser.VariableMatch,
          new Elements.PropertyParser.VariableMatcher(cssMatchedStylesMock, styleMock));
  ...
  it('parses URLs', () => {
    const url = 'http://example.com';
    {
      const {match, text} = matchSingleValue(
          ...
          new Elements.PropertyParser.URLMatcher());
      Platform.assertNotNullOrUndefined(match);
  ...
  it('parses angles correctly', () => {
    for (const succeed of ['45deg', '1.3rad', '-25grad', '2.3turn']) {
      const {ast, match, text} = matchSingleValue(
          'transform', succeed, Elements.PropertyParser.AngleMatch, new Elements.PropertyParser.AngleMatcher());
      Platform.assertNotNullOrUndefined(ast, succeed);
      Platform.assertNotNullOrUndefined(match, text);
      assert.strictEqual(match.text, succeed);
    }
    for (const fail of ['0DEG', '0', '123', '2em']) {
      const {match, text} = matchSingleValue(
          'transform', fail, Elements.PropertyParser.AngleMatch, new Elements.PropertyParser.AngleMatcher());
      assert.isNull(match, text);
    }
  });
  ...
  it('parses easing functions properly', () => {
    for (const succeed
      ...
      const {ast, match, text} = matchSingleValue(
          ...
          new Elements.PropertyParser.BezierMatcher());
      Platform.assertNotNullOrUndefined(ast, succeed);
      Platform.assertNotNullOrUndefined(match, text);
      assert.strictEqual(match.text, succeed);
    }
    ...
    const {ast, match, text} = matchSingleValue(
        'border', 'ease-in', Elements.PropertyParser.BezierMatch, new Elements.PropertyParser.BezierMatcher());
  ...
  it('parses shadows correctly', () => {
    const {match, text} = matchSingleValue(
        ...
        new Elements.PropertyParser.ShadowMatcher());
    Platform.assertNotNullOrUndefined(match, text);
  ...
  it('parses fonts correctly', () => {
    for (const fontSize of ['-.23', 'smaller', '17px', 'calc(17px + 17px)']) {
      const {ast, match, text} = matchSingleValue(
          'font-size', fontSize, Elements.PropertyParser.FontMatch, new Elements.PropertyParser.FontMatcher());
  ...
  it('parses grid templates correctly', () => {
    ...
    }
    {
      const {ast, match, text} = matchSingleValue(
          ...
          Elements.PropertyParser.GridTemplateMatch, new Elements.PropertyParser.GridTemplateMatcher());
      Platform.assertNotNullOrUndefined(ast, text);
      Platform.assertNotNullOrUndefined(match, text);
      assert.deepStrictEqual(
