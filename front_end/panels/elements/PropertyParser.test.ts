      ...
      for (const [varName, value] of Object.entries(variables)) {
        ...
        for (let offset = matching.ast.rule.indexOf(varText); offset >= 0;
          ...
          matching.computedText.push(
              {text: varText, type: Elements.PropertyParser.MatchType.Var, computedText: () => value},
...
describe('PropertyParser', () => {
  it('parses text', () => {
    assert.deepStrictEqual(
        textFragments(Elements.PropertyParser.renderPropertyValue('--p', 'var(--v)', [], {})),
        ['var', '(', '--v', ')']);

    assert.deepStrictEqual(
        textFragments(
            Elements.PropertyParser.renderPropertyValue('--p', '/* comments are text */ 1px solid 4', [], {})),
...
||| }`);
  ...
  class ComputedTextMatch implements Elements.PropertyParser.Match {
    type = 'computed-text-match' as Elements.PropertyParser.MatchType;
