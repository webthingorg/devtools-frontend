...
function matchSingleValue<T extends Elements.PropertyParser.Match>(
  ...
  return {
    ...
    text: Elements.PropertyParser.Printer.walk(ast).get(),
  };
}

function tokenizeDeclaration(name: string, value: string): Elements.PropertyParser.SyntaxTree {
  const ast = Elements.PropertyParser.tokenizeDeclaration(name, value);
  Platform.assertNotNullOrUndefined(ast, Elements.PropertyParser.Printer.rule(`*{${name}: ${value};}`));
  return ast;
}
...
describe('PropertyParser', () => {
  ...
  it('parses comments', () => {
    ...
    assert.strictEqual(
        Elements.PropertyParser.Printer.walk(ast.subtree(topNode)).get(),
