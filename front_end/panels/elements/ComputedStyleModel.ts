...
export class ComputedStyleModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  _onFrameResized(event: Common.EventTarget.EventTargetEvent): void {
  ...
  async fetchComputedStyle(): Promise<ComputedStyle | null> {
    ...
    function verifyOutdated(this: ComputedStyleModel, elementNode: SDK.DOMModel.DOMNode, style: Map<string, string> | null): ComputedStyle | null {
      return elementNode === this._elementNode() && style ? new ComputedStyle(elementNode, style) : null as ComputedStyle | null;
    }
  }
}
