...
import type {DOMNode} from './ElementsBreadcrumbs.js';
...
export class QueryContainer extends HTMLElement {
  ...
  private render(): void {
    ...
    const idToDisplay = this.queryName ? undefined : this.container.getAttribute('id');
    const classesToDisplay =
        this.queryName ? undefined : this.container.getAttribute('class')?.split(/\s+/).filter(Boolean);
