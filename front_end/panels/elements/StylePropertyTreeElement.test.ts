...
const variableName = (varExpression: string) => `${varExpression.substring('var('.length, varExpression.length - 1)}`;
...
describeWithRealConnection('StylePropertyTreeElement', () => {
  ...
  beforeEach(async () => {
    ...
    mockMatchedStyles.getVariableValue.callsFake((_, name) => {
      if (!mockVariableMap[name]) {
        return null;
      }
  ...
  describe('custom-properties', () => {
    it('linkifies var functions to declarations', async () => {
      ...
      mockMatchedStyles.getVariableValue.callsFake(
          (_, name) => name === '--prop' ?
    ...
    it('linkifies property definition to registrations', async () => {
      ...
      mockMatchedStyles.getVariableValue.returns(
  ...
  describe('LightDarkColorRenderer', () => {
    it('renders light-dark correctly', async () => {
      ...
      async function check(colorScheme: SDK.CSSModel.ColorScheme, lightText: string, darkText: string) {
        ...
        assert.strictEqual(
            swatch.getColor()?.getAuthoredText(),
            activeColor.startsWith('var') ? mockVariableMap[variableName(activeColor)] : activeColor);
