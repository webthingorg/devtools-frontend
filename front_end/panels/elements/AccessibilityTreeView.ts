...
export class AccessibilityTreeView extends UI.Widget.VBox {
  ...
  async refreshAccessibilityTree(): Promise<void> {
    ...
    this.rootNode = root;
    this.treeData = [sdkNodeToAXTreeNode(this.rootNode)];
  ...
  async loadSubTreeIntoAccessibilityModel(selectedNode: SDK.DOMModel.DOMNode): Promise<void> {
    ...
    // Create the TreeNode<..> for this SDK Node. Won't this break equality when TreeOutline does
    // it's traversal though?
    const treeNode = sdkNodeToAXTreeNode(inspectedAXNode);
