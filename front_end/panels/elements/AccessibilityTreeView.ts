...
export class AccessibilityTreeView extends UI.Widget.VBox implements
  ...
  async loadSubTreeIntoAccessibilityModel(selectedNode: SDK.DOMModel.DOMNode): Promise<void> {
    ...
    await this.accessibilityTreeComponent.focusNodeId(inspectedAXNode.getFrameId() + '#' + inspectedAXNode.id());
  }
  ...
  treeUpdated({data}: Common.EventTarget
    ...
    this.contentElement.removeChild(this.accessibilityTreeComponent);
    this.accessibilityTreeComponent = new TreeOutline.TreeOutline.TreeOutline<AccessibilityTreeUtils.AXTreeNodeData>();
    this.contentElement.appendChild(this.accessibilityTreeComponent);
