...
export class SourceFormatter {
  ...
  async discardFormattedUISourceCode(formattedUISourceCode: Workspace.UISourceCode.UISourceCode): Promise<Workspace.UISourceCode.UISourceCode | null> {
    ...
    const resultPromise = new Promise<SourceFormatData>(async (resolve) => {
      ...
      FormatterInterface.format(uiSourceCode.contentType(), uiSourceCode.mimeType(), content || '', async (formattedContent, formatterMapping) => {
        ...
        for (const decoration of uiSourceCode.allDecorations()) {
          ...
          formattedUISourceCode.addDecoration(new TextUtils.TextRange.TextRange(startLocation[0], startLocation[1], endLocation[0], endLocation[1]), (decoration.type() as string), decoration.data());
        }
...
class ScriptMapping implements Bindings.DebuggerWorkspaceBinding.DebuggerSourceMapping {
  ...
  uiLocationToRawLocations(uiSourceCode: Workspace.UISourceCode.UISourceCode, lineNumber: number, columnNumber: number): SDK.DebuggerModel.Location[] {
    ...
    if (formatData.originalSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
      ...
      if (debuggerModel) {
        ...
        const locations = (scripts.map(script => script.rawLocation(originalLine, originalColumn)).filter(l => Boolean(l)) as SDK.DebuggerModel.Location[]);
  ...
  _scriptsForUISourceCode(uiSourceCode: Workspace.UISourceCode.UISourceCode): SDK.Script.Script[] {
    ...
    if (uiSourceCode.contentType().isScript()) {
      ...
      return /** @type {!Array<!SDK.Script.Script>} */ rawLocations.map(location => location.script()).filter(script => Boolean(script)) as SDK.Script.Script[];
    }
    return [];
  }
}

const sourceCodeToHeaders = new WeakMap<Workspace.UISourceCode.UISourceCode, SDK.CSSStyleSheetHeader.CSSStyleSheetHeader[]>();
...
class StyleMapping implements Bindings.CSSWorkspaceBinding.SourceMapping {
  ...
  _headersForUISourceCode(uiSourceCode: Workspace.UISourceCode.UISourceCode): SDK.CSSStyleSheetHeader.CSSStyleSheetHeader[] {
    if (uiSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
      ...
      return /** @type {!Array<!SDK.CSSStyleSheetHeader.CSSStyleSheetHeader>} */ rawLocations.map(rawLocation => rawLocation.header()).filter(header => Boolean(header)) as SDK.CSSStyleSheetHeader.CSSStyleSheetHeader[];
    }
    return [];
  }
}
