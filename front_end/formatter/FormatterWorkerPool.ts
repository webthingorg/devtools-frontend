...
export class FormatterWorkerPool {
  ...
  _createWorker(): Common.Worker.WorkerWrapper {
    const worker = Common.Worker.WorkerWrapper.fromURL(new URL('../formatter_worker/formatter_worker-entrypoint.js', import.meta.url));
  ...
  format(mimeType: string, content: string, indentString: string): Promise<FormatResult> {
    ...
    return /** @type {!Promise<!FormatResult>} */ this._runTask(FormatterActions.FormatterActions.FORMAT, parameters) as Promise<FormatResult>;
  }
  ...
  findLastExpression(content: string): Promise<string | null> {
    return /** @type {!Promise<?string>} */ this._runTask(FormatterActions.FormatterActions.FIND_LAST_EXPRESSION, { content }) as Promise<string | null>;
  }
  ...
  findLastFunctionCall(content: string): Promise<{
    ...
    return /** @type {!Promise<?{baseExpression: string, receiver: string, argumentIndex: number, functionName: string}>} */ this._runTask(FormatterActions.FormatterActions.FIND_LAST_FUNCTION_CALL, { content }) as Promise<{
      baseExpression: string;
      receiver: string;
      argumentIndex: number;
      functionName: string;
  ...
  argumentsList(content: string): Promise<string[]> {
    return /** @type {!Promise<!Array<string>>} */ this._runTask(FormatterActions.FormatterActions.ARGUMENTS_LIST, { content }) as Promise<string[]>;
  }
}
