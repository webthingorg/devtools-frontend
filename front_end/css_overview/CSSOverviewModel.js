      ...
      for (let idx = 0; idx < layout.styles.length; idx++) {
        ...
        if (backgroundColor && textColor) {
          ...
          if (aaThreshold > contrastRatio) {
            const formatted = formatColor(textColor);
            if (!textColorContrastIssues.has(formatted)) {
              textColorContrastIssues.set(formatted, []);
            }
            textColorContrastIssues.get(formatted).push({
              nodeId,
              contrastRatio,
            });
          }
