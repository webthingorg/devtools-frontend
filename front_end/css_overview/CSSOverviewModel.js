    ...
    for (const {nodes, layout} of documents) {
      ...
      for (let idx = 0; idx < layout.styles.length; idx++) {
        ...
        if (backgroundColor && textColor) {
          ...
          if (aaThreshold > contrastRatio) {
            const formattedColor = formatColor(textColor);
            const issue = {
              nodeId,
              contrastRatio,
            };
            if (textColorContrastIssues.has(formattedColor)) {
              textColorContrastIssues.get(formattedColor).push(issue);
            } else {
              textColorContrastIssues.set(formattedColor, [issue]);
            }
          }
        }
