...
CssOverview.CSSOverviewCompletedView = class extends UI.PanelWithSidebar {
    ...
    <div class="vbox overview-completed-view">
      ...
      <div $="unused-declarations" class="results-section unused-declarations">
        ...
        unusedDeclarations.size > 0 ?
            this._unusedDeclarationsToFragment(unusedDeclarations) :
      ...
      <div $="media-queries" class="results-section media-queries">
        ...
        ${
        mediaQueries.size > 0 ?
            this._mediaQueriesToFragment(mediaQueries) :
  ...
  _createElementsView(evt) {
    ...
    if (!view) {
      view = new CssOverview.CSSOverviewCompletedView.ElementDetailsView(
          this._controller, this._domModel, this._cssModel, this._linkifier);
  ...
  _unusedDeclarationsToFragment(unusedDeclarations) {
    const type = 'unused-declarations';
    return UI.Fragment.build`<ul>
      ${Array.from(unusedDeclarations.entries()).map(([declaration, nodes]) => {
      const declarationButton = UI.createTextButton(ls`${nodes.length} items`, undefined, '', true /* primary */);
      declarationButton.dataset.type = type;
      declarationButton.dataset.declaration = declaration;
      return UI.Fragment.build`<li>${declarationButton} ${declaration}</li>`;
    })}
    </ul>`;
  }

  _mediaQueriesToFragment(mediaQueries) {
    const type = 'media-queries';
    return UI.Fragment.build`<ul>
      ${Array.from(mediaQueries.entries()).map(([text, entries]) => {
      const mediaQueryButton = UI.createTextButton(ls`${entries.length} items`, undefined, '', true /* primary */);
      mediaQueryButton.dataset.type = type;
      mediaQueryButton.dataset.text = text;
      return UI.Fragment.build`<li>${mediaQueryButton} ${text}</li>`;
    })}
    </ul>`;
  }
...
CssOverview.CSSOverviewCompletedView.DetailsView = class extends UI.VBox {
  ...
  appendTab(id, tabTitle, view, isCloseable) {
    if (this._tabbedPane.hasTab(id)) {
      this._tabbedPane.selectTab(id, true, true);
      return;
    }
...
CssOverview.CSSOverviewCompletedView.ElementDetailsView = class extends UI.Widget {
  ...
  async populateNodes(data) {
    ...
    if (visibility.nodeId) {
      // Grab the nodes from the frontend.
      const nodeIds = new Set(data.map(node => node.nodeId));
      relatedNodesMap = await this._domModel.pushNodesByBackendIdsToFrontend(nodeIds);
    }

    for (const item of data) {
      if (visibility.nodeId) {
        item.node = relatedNodesMap.get(item.nodeId);
      }

      const node = new CssOverview.CSSOverviewCompletedView.ElementNode(
          this._elementGrid, item, this._linkifier, this._cssModel);
