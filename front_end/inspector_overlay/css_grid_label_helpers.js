...
export function drawGridNumbers(config, bounds) {
  ...
  if (config.positiveColumnLineNumberOffsets) {
    let i = 1;
    for (const offset of config.positiveColumnLineNumberOffsets) {
      ...
      _placeColumnLabel(labelContainer, i.toString(), bounds.minX + offset, bounds.minY, isFirstColumn, isLastColumn);
      i += 1;
    }
  }
  if (config.positiveRowLineNumberOffsets) {
    let i = 1;
    for (const offset of config.positiveRowLineNumberOffsets) {
      ...
      _placeRowLabel(
          labelContainer, i.toString(), bounds.minX, bounds.minY + offset, isTopRow, isBottomRow, avoidColumnLabel);
      i += 1;
    }
  }

  if (config.negativeColumnLineNumberOffsets) {
    let i = config.negativeColumnLineNumberOffsets.length * -1;
    for (const offset of config.negativeColumnLineNumberOffsets) {
      ...
      _placeColumnLabel(
          labelContainer, i.toString(), bounds.minX + offset, bounds.maxY, isFirstColumn, isLastColumn, true);
      i += 1;
    }
  }
  if (config.negativeRowLineNumberOffsets) {
    let i = config.negativeRowLineNumberOffsets.length * -1;
    for (const offset of config.negativeRowLineNumberOffsets) {
      const isTopRow = offset === config.negativeRowLineNumberOffsets[0];
      const isBottomRow = offset === config.negativeRowLineNumberOffsets[config.negativeRowLineNumberOffsets.length];
      const avoidColumnLabel = bounds.minY < gridPageMargin;
      _placeRowLabel(
          labelContainer, i.toString(), bounds.maxX, bounds.minY + offset, isTopRow, isBottomRow, avoidColumnLabel,
          true);
      i += 1;
    }
  }
