...
export function drawGridTrackSizes(container, trackGap, trackSizes, bounds, direction) {
  let start = 0;
  for (const [i, breadth] of trackSizes.entries()) {
    if (i > 0) {
      start += trackGap;
    }
    const end = start + breadth;
    const element = _createLabelElement(container, Math.round(breadth) + 'px');
    const _placeLabel = direction === 'column' ? _placePositiveColumnLabel : _placePositiveRowLabel;
    _placeLabel(element, (start + end) / 2, {bounds});
    start = end;
  }
}
