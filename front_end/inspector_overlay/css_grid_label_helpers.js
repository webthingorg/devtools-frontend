...
export function drawGridNumbers(config, bounds) {
  ...
  if (config.columnLineNumberOffsets) {
    let i = 1;
    for (const offset of config.columnLineNumberOffsets) {
      const isFirstColumn = offset === config.columnLineNumberOffsets[0];
      const isLastColumn = offset === config.columnLineNumberOffsets[config.columnLineNumberOffsets.length];
      _placeColumnLabel(labelContainer, i.toString(), bounds.minX + offset, bounds.minY, isFirstColumn, isLastColumn);
      i += 1;
    }
  }
  if (config.rowLineNumberOffsets) {
    let i = 1;
    for (const offset of config.rowLineNumberOffsets) {
      const isTopRow = offset === config.rowLineNumberOffsets[0];
      const isBottomRow = offset === config.rowLineNumberOffsets[config.rowLineNumberOffsets.length];
