 ...
 * @param {string} coord - 'x' | 'y'
 */
function* positionIterator(positions, coord) {
  ...
  for (const [i, pos] of positions.entries()) {
    ...
    const isFarEnoughFromPrevious = pos[coord] - (lastEmittedPos ? lastEmittedPos[coord] : 0) > gridLabelDistance;
    // And if there is also some minium distance from the very last position.
    const isFarEnoughFromLast = !isLast && positions[positions.length - 1][coord] - pos[coord] > gridLabelDistance;
