...
/** @typedef {!{computedSize: number, x: number, y: number, bearing: number}} */
let TrackSize;  // eslint-disable-line no-unused-vars

/** @typedef {!{columnTrackSizes?: TrackSize[], rowTrackSizes?: TrackSize[], positiveRowLineNumberOffsets?: number[], negativeRowLineNumberOffsets?: number[], positiveColumnLineNumberOffsets?: number[], negativeColumnLineNumberOffsets?: number[]}} */
...
export function drawGridLabels(config, gridBounds, areaBounds) {
  ...
  if (config.gridHighlightConfig.showTrackSizes) {
    // Draw column sizes.
    drawGridTrackSizes(trackSizesContainer, config.columnTrackSizes, 'column');
    // Draw row sizes.
    drawGridTrackSizes(trackSizesContainer, config.rowTrackSizes, 'row');
  }
}
 ...
 */
export function drawGridTrackSizes(container, trackSizes, direction) {
  for (const {x, y, bearing, computedSize} of trackSizes) {
    ...
    // TODO (alexrudenko): Can it be simplified?
    // TODO (alexrudenko): Sometimes the rotation resutls in a blurry test.
    // TODO (alexrudenko): Flip labels when not enough space outside (angle + 180 should work).
    if (direction === 'column') {
      element.style['transform-origin'] = `50% calc(100% + ${gridArrowWidth}px)`;
    } else {
      element.style['transform-origin'] = `calc(100% + ${gridArrowWidth}px) 50%`;
    }
    const normalBearing = direction === 'column' ? 180 : 90;
    const angle = Math.round(bearing) - normalBearing;
