...
export class AXBreadcrumbsPane extends AccessibilitySubPane {
  ...
  setAXNode(axNode: SDK.AccessibilityModel.AccessibilityNode|null): void {
    ...
    let depth = 0;
    let breadcrumb: AXBreadcrumb|null = null;
  ...
  _onKeyDown(event: Event): void {
    ...
    if (!keyboardEvent.composedPath().some(
            (element: EventTarget): boolean => element === preselectedBreadcrumb.element())) {
      return;
    }
    ...
    let handled: boolean|false = false;
  ...
  _inspectDOMNode(axNode: SDK.AccessibilityModel.AccessibilityNode): boolean {
    ...
    if (deferredNode) {
      deferredNode.resolve((domNode: SDK.DOMModel.DOMNode|null): void => {
  ...
  _contextMenuEventFired(event: Event): void {
    ...
    contextMenu.viewSection().appendItem(ls`Scroll into view`, (): void => {
      const deferredNode = axNode.deferredDOMNode();
      if (!deferredNode) {
        return;
      }
      deferredNode.resolvePromise().then((domNode: SDK.DOMModel.DOMNode|null): void => {
        if (!domNode) {
          return;
        }
...
const elementsToAXBreadcrumb: WeakMap<Element, AXBreadcrumb> = new WeakMap();
...
export const RoleStyles: {[x: string]: string;} = {
