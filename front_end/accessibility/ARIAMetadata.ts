...
export class ARIAMetadata {
  ...
  _initialize(config: Config): void {
    ...
    /** @type {!Array<string>} */
    this._roleNames = config['roles'].map((roleConfig: RoleConfig): string => roleConfig.name);
  }
...
export function ariaMetadata(): ARIAMetadata {
  if (!_instance) {
    _instance = new ARIAMetadata(/** @type {!Config} */ (ARIAProperties.config as Config) || null);
  }
  return _instance;
}
