...
export class NetworkOverview extends PerfUI.TimelineOverviewPane.TimelineOverviewBase {
  ...
  update() {
    ...
    if (this._highlightedRequest) {
      ...
      context.fillStyle = UI.UIUtils.ThemeSupport.instance().getComputedValue('--selection-bg-color');
    ...
    if (this._selectedFilmStripTime !== -1) {
      context.lineWidth = 2;
      context.beginPath();
      context.strokeStyle = UI.UIUtils.ThemeSupport.instance().getComputedValue('--network-frame-divider-color');
