...
export class BlockedURLsPane extends UI.Widget.VBox implements UI.ListWidget.Delegate {
  ...
  constructor() {
    ...
    SDK.SDKModel.TargetManager.instance().addModelListener(SDK.NetworkManager.NetworkManager, SDK.NetworkManager.Events.RequestFinished, this._onRequestFinished, this);
  ...
  _createEditor(): UI.ListWidget.Editor<SDK.NetworkManager.BlockedPattern> {
    ...
    const validator = (item: SDK.NetworkManager.BlockedPattern, index: number, input: HTMLInputElement | HTMLSelectElement): {
      valid: boolean;
