...
export class EventSourceMessagesView extends UI.Widget.VBox {
  ...
  _sortItems(): void {
    ...
    const comparator = (Comparators[sortColumnId] as ((arg0: DataGrid.SortableDataGrid.SortableDataGridNode<EventSourceMessageNode>, arg1: DataGrid.SortableDataGrid.SortableDataGridNode<EventSourceMessageNode>) => number) | undefined);
    if (!comparator) {
      return;
    }
...
export function EventSourceMessageNodeComparator(fieldGetter: (arg0: SDK.NetworkRequest.EventSourceMessage) => (number | string), a: EventSourceMessageNode, b: EventSourceMessageNode): number {
