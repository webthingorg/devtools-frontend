...
export class NetworkSearchScope implements Search.SearchConfig.SearchScope {
  ...
  async performSearch(searchConfig: Search.SearchConfig.SearchConfig, progress: Common.Progress.Progress, searchResultCallback: (arg0: Search.SearchConfig.SearchResult) => void, searchFinishedCallback: (arg0: boolean) => void): Promise<void> {
    const promises = [];
    const requests = SDK.NetworkLog.NetworkLog.instance().requests().filter(request => searchConfig.filePathMatchesFileQuery(request.url()));
  ...
  async _searchRequest(searchConfig: Search.SearchConfig.SearchConfig, request: SDK.NetworkRequest.NetworkRequest, progress: Common.Progress.Progress): Promise<NetworkSearchResult | null> {
...
export class UIRequestLocation {
  ...
  isUrlMatch: boolean;
  constructor(request: SDK.NetworkRequest.NetworkRequest, requestHeader: SDK.NetworkRequest.NameValue | null, responseHeader: SDK.NetworkRequest.NameValue | null, searchMatch: TextUtils.ContentProvider.SearchMatch | null, urlMatch: boolean) {
...
export class NetworkSearchResult implements Search.SearchConfig.SearchResult {
  ...
  matchLineContent(index: number): string {
    ...
    return /** @type {!TextUtils.ContentProvider.SearchMatch} */ (location.searchMatch as TextUtils.ContentProvider.SearchMatch).lineContent;
  }
