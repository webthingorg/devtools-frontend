...
export class NetworkManageCustomHeadersView extends UI.Widget.VBox implements UI.ListWidget.Delegate {
  _list: UI.ListWidget.ListWidget<{
    header: string;
  }>;
  ...
  commitEdit(item: {
    header: string;
  }, editor: UI.ListWidget.Editor<any>, isNew: boolean): void {
  ...
  beginEdit(item: {
    header: string;
  }): UI.ListWidget.Editor<any> {
    const editor = this._createEditor();
    editor.control('header').value = item.header;
    return editor;
  }

  _createEditor(): UI.ListWidget.Editor<any> {
    if (this._editor) {
      return this._editor;
    }
    ...
    function validateHeader(this: NetworkManageCustomHeadersView, item: {
      header: string;
    }, index: number, input: HTMLInputElement | HTMLSelectElement): UI.ListWidget.ValidatorResult {
      let valid = true;
