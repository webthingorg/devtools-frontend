...
export class NetworkWaterfallColumn extends UI.Widget.VBox {
  ...
  static _buildRequestTimeRangeStyle(): Map<RequestTimeRangeNames, _LayerStyle> {
    ...
    const styleMap = new Map<RequestTimeRangeNames.Connecting, {
      fillStyle: string;
    }>();
  ...
  static _buildResourceTypeStyle(): Map<Common.ResourceType.ResourceType, _LayerStyle>[] {
    ...
    const waitingStyleMap = new Map<Common.ResourceType.ResourceType, {
      fillStyle: string | null;
      lineWidth: number;
      borderColor: string | null;
    }>();
    const downloadingStyleMap = new Map<Common.ResourceType.ResourceType, {
    ...
    for (const resourceType of Object.values(Common.ResourceType.resourceTypes)) {
      ...
      waitingStyleMap.set(resourceType, { fillStyle: toWaitingColor((color as string)), lineWidth: 1, borderColor: borderColor });
  ...
  _onMouseMove(/** @type {!MouseEvent} */ event: MouseEvent): void {
    this._setHoveredNode(this.getNodeFromPoint(event.offsetX, event.offsetY), event.shiftKey);
  }

  _onClick(/** @type {!MouseEvent} */ event: MouseEvent): void {
  ...
  _getPopoverRequest(event: MouseEvent): UI.PopoverHelper.PopoverRequest | null {
    ...
    return {
      ...
      show: (popover: UI.GlassPane.GlassPane): Promise<true> => {
        const content = RequestTimingView.createTimingTable((request as SDK.NetworkRequest.NetworkRequest), this._calculator);
