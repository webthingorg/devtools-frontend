...
export class RequestTrustTokensReport extends HTMLElement {
  ...
  private renderParameterSection() {
    ...
    return LitHtml.html`
      <devtools-report-section .data=${{sectionTitle: ls`Parameters`} as Components.ReportView.ReportSectionData}>
        ...
        ${RequestTrustTokensReport.renderRefreshPolicy(this.trustTokenData.params)}
        ${RequestTrustTokensReport.renderIssuers(this.trustTokenData.params)}
      </devtools-report-section>
    `;
    // clang-format on
  }

  private static renderRefreshPolicy(params: Protocol.Network.TrustTokenParams) {
  ...
  private static renderIssuers(params: Protocol.Network.TrustTokenParams) {
