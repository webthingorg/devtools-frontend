...
class RequestTrustTokensReport extends HTMLElement {
  ...
  private renderRefreshPolicy() {
    if (this.params!.type !== Protocol.Network.TrustTokenOperationType.Redemption) {
  ...
  private renderIssuers() {
    if (!this.params!.issuers || this.params!.issuers.length === 0) {
      return LitHtml.nothing;
    }
