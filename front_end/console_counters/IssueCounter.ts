...
export function getIssueKindIconData(issueKind: SDK.Issue.IssueKind): WebComponents.Icon.IconWithName {
  switch (issueKind) {
...
function toIconGroup({iconName, color, width, height}: WebComponents.Icon.IconWithName, defaultIconSize: boolean):
    WebComponents.IconButton.IconWithTextData {
  if (defaultIconSize) {
...
export class IssueCounter extends HTMLElement {
  ...
  private render(): void {
    ...
    const data: WebComponents.IconButton.IconButtonData = {
      groups: [
        {
    ...
    LitHtml.render(
        ...
        <icon-button .data=${data as WebComponents.IconButton.IconButtonData}></icon-button>
        `,
        this.shadow);
