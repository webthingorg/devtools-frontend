...
export class HTMLModel {
  ...
  _build(text) {
    ...
    while (true) {
      ...
      const element = this._stack[_stack.length - 1];
      if (!element) {
        break;
      }
  ...
  _onTagComplete(tag) {
    if (tag.isOpenTag) {
      const topElement = this._stack[_stack.length - 1];
      if (topElement) {
