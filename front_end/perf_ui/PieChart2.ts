...
// import {createShadowRootWithCoreStyles} from '../ui/utils/create-shadow-root-with-core-styles.js';
...
export class PieChart2 extends HTMLElement {
  // TODO: This is how you can get data-keyboard-focus focus/blur events (kind of).
  private readonly shadow = this.attachShadow({mode: 'open'});  // createShadowRootWithCoreStyles(this);
  private chartName = '';
  private size = 0;
  ...
  private sliceSelected: number = -1;
    ...
    const output = LitHtml.html`
      <style>
        ...
        .chart-root .slice.selected {
          /* TODO: No access to vars? Does not pick it up */
        ...
        <div class="chart-root" style="width: ${this.size}px; height: ${this.size}px;">
          ...
          <g transform="scale(${this.size / 2}) translate(1, 1) scale(0.99, 0.99)">
            ...
            ${this.slices.map((slice, index) => {
              ...
              // TODO: handle data-keyboard-focus properly
              return LitHtml.svg`<path class="slice ${selected ? 'selected' : ''}"
  ...
  private focusNextElement() {
    if (this.totalSelected) {
      ...
      this.selectSlice(++this.sliceSelected);
    }
  }

  private focusPreviousElement() {
    if (this.totalSelected) {
      this.selectSlice(this.slices.length - 1);
