...
import * as UI from '../ui/utils/utils.js';
...
export class PieChart2 extends HTMLElement {
  private readonly shadow = UI.createShadowRootWithKeyboardFocusShenanigans(this);
  private chartName = '';
  private size = 0;
          ...
          <g transform="scale(${this.size / 2}) translate(1, 1) scale(0.99, 0.99)">
            ...
            ${this.slices.map((slice, index) => {
              ...
              return svg`<path class="slice ${selected ? 'selected' : ''}"
                  @click=${this.selectClickedSlice(index)} tabIndex=${tabIndex}
                  fill=${slice.color} d=${this.getPathStringForSlice(slice)}
                  aria-label=${slice.title}></path>`;
            })}
            </g>
          </svg>
          `}
          ...
          ${this.slices.map((slice, index) => {
            ...
            return html`
              <div class="pie-chart-legend-row ${selected ? 'selected' : ''}"
                  @click=${this.selectClickedSlice(index)} tabIndex=${selected ? '0' : '-1'}>
  ...
  private selectClickedSlice(index: number) {
    return () => {
  ...
  private selectAndFocusTotal() {
    this.selectTotal();
    // TODO(petermarshall): In order to interact with data-keyboard-focus we
    ...
    // showing a focus ring when we click on something is not necessary. This
    // has a lot of interaction with the old UI system and relies on global
    // state there that tracks keyboard events, but to make this component work
    // the same as the old pie chart this is necessary for now. The same goes
    // for focusing slices below.
    const totalLegendRow = this.shadowRoot!.querySelector<HTMLDivElement>('.pie-chart-legend > :last-child');
    totalLegendRow!.focus();
  }

  private selectAndFocusSlice(index: number) {
    this.selectSlice(index);
    const sliceLegendRow =
        this.shadowRoot!.querySelector<HTMLDivElement>(`.pie-chart-legend > :nth-child(${index + 1})`);
