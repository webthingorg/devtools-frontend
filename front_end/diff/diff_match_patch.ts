...
(function (): void {
  function diff_match_patch(): void { this.Diff_Timeout = 1; this.Diff_EditCost = 4; this.Match_Threshold = 0.5; this.Match_Distance = 1E3; this.Patch_DeleteThreshold = 0.5; this.Patch_Margin = 4; this.Match_MaxBits = 32; }
  diff_match_patch.prototype.diff_main = function (a: any, b: any, c: any, d: any): any {
  ...
  diff_match_patch.prototype.diff_compute_ = function (a: any, b: any, c: any, d: any): any {
    if (!a)
      return [[1, b]];
    if (!b)
      return [[-1, a]];
  ...
  diff_match_patch.prototype.diff_lineMode_ = function (a: any, b: any, c: any): any {
  ...
  diff_match_patch.prototype.diff_bisect_ = function (a: any, b: any, c: any): any {
    ...
    for (var k = d - e, q = 0 != k % 2, r = 0, t = 0, p = 0, w = 0, v = 0; v < f && !((new Date).getTime() > c); v++) {
      for (var n: number = -v + r; n <= v - t; n += 2) {
        var l: number = g + n, m;
  ...
  diff_match_patch.prototype.diff_bisectSplit_ = function (a: any, b: any, c: any, d: any, e: any): any { var f = a.substring(0, c), g = b.substring(0, d); a = a.substring(c); b = b.substring(d); f = this.diff_main(f, g, !1, e); e = this.diff_main(a, b, !1, e); return f.concat(e); };
  diff_match_patch.prototype.diff_linesToChars_ = function (a: any, b: any): {
    chars1: string;
    chars2: string;
    lineArray: string[];
  } {
    function c(a: any): string {
  ...
  diff_match_patch.prototype.diff_charsToLines_ = function (a: any, b: any): void {
  ...
  diff_match_patch.prototype.diff_commonPrefix = function (a: any, b: any): number {
    if (!a || !b || a.charAt(0) != b.charAt(0))
      return 0;
    for (var c = 0, d = Math.min(a.length, b.length), e: number = d, f = 0; c < e;)
      a.substring(f, e) == b.substring(f, e) ? f = c = e : d = e, e = Math.floor((d - c) / 2 + c);
    return e;
  };
  diff_match_patch.prototype.diff_commonSuffix = function (a: any, b: any): number {
  ...
  diff_match_patch.prototype.diff_commonOverlap_ = function (a: any, b: any): any {
  ...
  diff_match_patch.prototype.diff_halfMatch_ = function (a: any, b: any): any[] | null {
    function c(a: any, b: any, c: any): any[] | null {
  ...
  diff_match_patch.prototype.diff_cleanupSemantic = function (a: any): void {
    for (var b = !1, c:  = [], d = 0, e: null = null, f = 0, g = 0, h = 0, j = 0, i = 0; f < a.length;)
  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a: any): void {
    function b(a: any, b: any): 0 | 1 | 2 | 4 | 3 | 6 | 5 {
      if (!a || !b)
        return 6;
    ...
    for (var c = 1; c < a.length - 1;) {
      if (0 == a[c - 1][0] && 0 == a[c + 1][0]) {
        ...
        for (var g = d, h = e, j = f, i: number = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0);) {
  ...
  diff_match_patch.prototype.diff_cleanupEfficiency = function (a: any): void {
    for (var b = !1, c = [], d = 0, e: null = null, f = 0, g = !1, h = !1, j = !1, i = !1; f < a.length;) {
      if (0 == a[f][0])
  ...
  diff_match_patch.prototype.diff_cleanupMerge = function (a: any): void {
    a.push([0, ""]);
  ...
  diff_match_patch.prototype.diff_xIndex = function (a: any, b: any): number {
  ...
  diff_match_patch.prototype.diff_prettyHtml = function (a: any): string {
  ...
  diff_match_patch.prototype.diff_text1 = function (a: any): string {
    for (var b = [], c = 0; c < a.length; c++)
      1 !== a[c][0] && (b[c] = a[c][1]);
    return b.join("");
  };
  diff_match_patch.prototype.diff_text2 = function (a: any): string {
    for (var b = [], c = 0; c < a.length; c++)
      -1 !== a[c][0] && (b[c] = a[c][1]);
    return b.join("");
  };
  diff_match_patch.prototype.diff_levenshtein = function (a: any): number {
  ...
  diff_match_patch.prototype.diff_toDelta = function (a: any): string {
    for (var b = [], c = 0; c < a.length; c++)
      switch (a[c][0]) {
        case 1:
          b[c] = "+" + encodeURI(a[c][1]);
  ...
  diff_match_patch.prototype.diff_fromDelta = function (a: any, b: any): any[][] {
  ...
  diff_match_patch.prototype.match_main = function (a: any, b: any, c: any): any {
  ...
  diff_match_patch.prototype.match_bitap_ = function (a: any, b: any, c: any): any {
    function d(a: any, d: any): number { var e = a / b.length, g = Math.abs(c - d); return !f.Match_Distance ? g ? 1 : e : e + g / f.Match_Distance; }
    if (b.length > this.Match_MaxBits)
      throw Error("Pattern too long for this browser.");
    var e = this.match_alphabet_(b), f = this, g: number = this.Match_Threshold, h = a.indexOf(b, c);
  ...
  diff_match_patch.prototype.match_alphabet_ = function (a: any): {} {
    for (var b = {}, c = 0; c < a.length; c++)
      b[a.charAt(c)] = 0;
  ...
  diff_match_patch.prototype.patch_addContext_ = function (a: any, b: any): void {
    if (0 != b.length) {
  ...
  diff_match_patch.prototype.patch_make = function (a: any, b: any, c: any): any {
    var d;
    ...
    for (var e = 0, f = 0, g = 0, h: string = d, j = 0; j < b.length; j++) {
      var i = b[j][0], k = b[j][1];
  ...
  diff_match_patch.prototype.patch_deepCopy = function (a: any): patch_obj[] {
    for (var b = [], c = 0; c < a.length; c++) {
  ...
  diff_match_patch.prototype.patch_apply = function (a: any, b: any): any[] {
    if (0 == a.length)
      return [b, []];
  ...
  diff_match_patch.prototype.patch_addPadding = function (a: any): string {
  ...
  diff_match_patch.prototype.patch_splitMax = function (a: any): void {
  ...
  diff_match_patch.prototype.patch_toText = function (a: any): string {
    for (var b = [], c = 0; c < a.length; c++)
      b[c] = a[c];
    return b.join("");
  };
  diff_match_patch.prototype.patch_fromText = function (a: any): any[] {
    var b = [];
    if (!a)
      return b;
    a = a.split("\n");
  ...
  diff_match_patch.patch_obj = function (): void { this.diffs = []; this.start2 = this.start1 = null; this.length2 = this.length1 = 0; };
  diff_match_patch.patch_obj.prototype.toString = function (): string {
    var a, b;
