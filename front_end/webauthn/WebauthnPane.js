 ...
 * @extends {DataGrid.DataGrid.DataGridNode<!DataGridNode>}
 */
export class EmptyDataGridNode extends DataGrid.DataGrid.DataGridNode {
  /**
   * @override
   */
  createCells(element) {
    ...
    message.textContent = ls`No credentials available yet.`;
...
export class WebauthnPaneImpl extends UI.Widget.VBox {
  ...
  _createCredentialsDataGrid(authenticatorId) {
    ...
    const dataGrid = new DataGrid.DataGrid.DataGridImpl({displayName: ls`Cookies`, columns});
   ...
   * @param {!Protocol.WebAuthn.AuthenticatorId} authenticatorId
   */
  async _updateCredentialDataGrid(authenticatorId) {
    ...
    let credentials = [];
    try {
      credentials = await this._model.getCredentials(authenticatorId);
    } catch (e) {
      return;
    }
    ...
    setTimeout(this._updateCredentialDataGrid.bind(this, authenticatorId), TIMEOUT);
  }
  ...
  async _addAuthenticatorSection(authenticatorId, options) {
    ...
    this._updateCredentialDataGrid(authenticatorId);
  }
  ...
  async _removeCredential(authenticatorId, credentialId) {
    ...
    // const node = dataGrid.rootNode().children.find(n => n.data.credentialId === credentialId);
    // node.remove();
