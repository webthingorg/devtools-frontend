...
export class WebauthnPaneImpl extends UI.Widget.VBox {
  ...
  _addAuthenticatorSection(authenticatorId, options) {
    ...
    this._setActiveAuthenticator(authenticatorId);
   ...
   * @param {!Protocol.WebAuthn.AuthenticatorId} authenticatorId
   */
  _setActiveAuthenticator(authenticatorId) {
    if (this._activeAuthId) {
      this._model.setAutomaticPresenceSimulation(this._activeAuthId, false);
    }
    this._activeAuthId = authenticatorId;
    this._model.setAutomaticPresenceSimulation(this._activeAuthId, true);
  }
}
