...
export class WebauthnPaneImpl extends UI.Widget.VBox {
  constructor() {
    ...
    this._enabled = false;
    this._authenticatorsToOptionsMap = new Map();
    this._authenticatorsToSectionMap = new Map();
  ...
  _updateVisibility(enabled) {
    if (enabled) {
      if (this._newAuthenticatorSection)
        {this._newAuthenticatorSection.style.display = 'block';}
    } else {
      if (this._newAuthenticatorSection)
        {this._newAuthenticatorSection.style.display = 'none';}
    }
  }
  ...
  _createNewAuthenticatorSection() {
    ...
    this._newAuthenticatorTitle = UI.UIUtils.createLabel(ls`New Authenticator`, 'new-authenticator-title');
    this._newAuthenticatorSection.appendChild(this._newAuthenticatorTitle);
    ...
    const u2fOption = new Option(Common.UIString.UIString('u2f'), Protocol.WebAuthn.AuthenticatorProtocol.U2f);
    const ctap2Option = new Option(Common.UIString.UIString('ctap2'), Protocol.WebAuthn.AuthenticatorProtocol.Ctap2);
    this._protocolSelect.appendChild(u2fOption);
    this._protocolSelect.appendChild(ctap2Option);
    ...
    const usbOption = new Option(Common.UIString.UIString('usb'), Protocol.WebAuthn.AuthenticatorTransport.Usb);
    const nfcOption = new Option(Common.UIString.UIString('nfc'), Protocol.WebAuthn.AuthenticatorTransport.Nfc);
    const bleOption = new Option(Common.UIString.UIString('ble'), Protocol.WebAuthn.AuthenticatorTransport.Ble);
    const cableOption = new Option(Common.UIString.UIString('ble'), Protocol.WebAuthn.AuthenticatorTransport.Cable);
    const internalOption =
        new Option(Common.UIString.UIString('internal'), Protocol.WebAuthn.AuthenticatorTransport.Internal);
    this._transportSelect.appendChild(usbOption);
    this._transportSelect.appendChild(nfcOption);
    this._transportSelect.appendChild(bleOption);
    this._transportSelect.appendChild(cableOption);
    this._transportSelect.appendChild(internalOption);
    this._transportSelect.selectedIndex = 0;

    this._residentKeyCheckboxLabel =
        UI.UIUtils.CheckboxLabel.create(Common.UIString.UIString('Supports Resident Keys'), false);
    ...
    this._userVerificationCheckboxLabel =
        UI.UIUtils.CheckboxLabel.create(Common.UIString.UIString('Supports User Verification'), false);
  ...
  async _handleAddAuthenticatorButton() {
    ...
    this._authenticatorsToOptionsMap.set(authenticatorId, options);
  ...
  _addAuthenticatorSection(authenticatorId, options) {
    const title = 'Authenticator ' + authenticatorId;
    const section = document.createElement('div');
    section.classList.add('authenticator-section');
    this._authenticatorsToSectionMap.set(authenticatorId, section);
    ...
    srkField.createChild('div', 'authenticator-field-value').textContent = options.hasResidentKey ? 'Yes' : 'No';
    suvField.createChild('div', 'authenticator-field-value').textContent = options.hasUserVerification ? 'Yes' : 'No';
  }

  /**
   * Removes the authenticator and its respective UI element
   * @param {!Protocol.WebAuthn.AuthenticatorId} authenticatorId
   */
  _removeAuthenticator(authenticatorId) {
    const sectionToRemove = this._authenticatorsToSectionMap.get(authenticatorId);
    sectionToRemove.remove();
    this._authenticatorsToSectionMap.delete(authenticatorId);

    this._model.removeAuthenticator(authenticatorId);
    this._authenticatorsToOptionsMap.delete(authenticatorId);
  }
  ...
  _createOptionsFromCurrentInputs() {
    return {
      protocol: this._protocolSelect.options[this._protocolSelect.selectedIndex].value,
          ...
          automaticPresenceSimulation: true,  // Currently always true. Optionality to be introduced in later patch
          isUserVerified: true,               // Currently always true. Optionality to be introduced in later patch
    };
  }
}
