...
export const enum Scheduling {
  // If the throttler has run another task recently (i.e. time since the last run is less then the throttling delay), schedule the task to be run
  // after throttling delay. Otherwise scheule the task after the next tick
  Default = 'Default',
  // Schedule the task to run at the next tick, even if the throttler has run
  // another task recently.
  AsSoonAsPossible = 'AsSoonAsPossible',
  // Schedule the task to run after throttling delay, even if the throttler has not run
  // any task recently.
  Delayed = 'Delayed',
}
