...
export async function reveal(revealable: unknown, omitFocus: boolean = false): Promise<void> {
  await revealWithOptions(revealable, {omitFocus});
}

export async function revealWithOptions(revealable: unknown, options: Partial<RevealableOptions>): Promise<void> {
  await RevealerRegistry.instance().reveal(revealable, options);
}
