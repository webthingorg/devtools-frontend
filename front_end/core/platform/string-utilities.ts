...
export const sprintf = (fmt: string, ...args: unknown[]): string => {
  let argIndex = 0, fmtIndex = 0, result = '';
  const re = new RegExp('%(?:(\\d+)\\$)?(?:\\.(\\d*))?([%dfs])', 'g');
  for (let match = re.exec(fmt); match !== null; match = re.exec(fmt)) {
    result += fmt.substring(fmtIndex, match.index);
    const [full, index, precision, specifier] = match;
    if (specifier === '%') {
      result += '%';
    } else {
      if (index !== undefined) {
      ...
      if (argIndex >= args.length) {
        throw new RangeError(
            `Expected at least ${argIndex + 1} format parameters, but only ${args.length} where given.`);
      }
      if (specifier === 's') {
        const argValue = String(args[argIndex++]);
        if (precision !== undefined) {
          result += argValue.substring(0, Number(precision));
        } else {
          result += argValue;
        }
      } else {
        ...
        if (specifier === 'd') {
          result += String(Math.floor(argValue)).padStart(Number(precision), '0');
        } else {
          result += precision === undefined ? argValue : argValue.toFixed(Number(precision));
        }
      }
    }
    fmtIndex = match.index + full.length;
  }
  result += fmt.substring(fmtIndex);
  return result;
};
