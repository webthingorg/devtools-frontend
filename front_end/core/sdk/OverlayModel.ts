...
export class OverlayModel extends SDKModel<EventTypes> implements ProtocolProxyApi.OverlayDispatcher {
  ...
  constructor(target: Target) {
    ...
    this.#wcoModeActiveInternal = false;
  }
  ...
  wcoModeActive(): boolean {
    return this.#wcoModeActiveInternal;
  }
  ...
  updateWCOPlatform(opt: OSType): void {
    this.#windowControlsOverlay.setWCOConfigPlatform(opt);
  }
  updateWCOThemeColor(themeColor: string): void {
    this.#windowControlsOverlay.setWCOConfigThemeColor(themeColor);
  }

  toggleWcoToolbar(show: boolean): void {
    this.#wcoModeActiveInternal = show;
    if (!this.#wcoModeActiveInternal) {
      // If the WCO overlay is not active, the css var checkbox should be unchecked and the config should be updated
      this.#windowControlsOverlay.setWCOConfigCSS(false);
    }
    const wcoConfigObj = show ? {WCOConfig: this.#windowControlsOverlay.getConfig()} : {};

    void this.overlayAgent.invoke_setShowWco(wcoConfigObj);

    void this.#windowControlsOverlay.updateStylesheet(show);
    // we can only display one overlay at a time,
    // disable the viewport size overlay so WCO overlay can stay when resizing the window
    this.setShowViewportSizeOnResize(!this.#wcoModeActiveInternal);
  }

  toggleWcoCss(show: boolean): void {
    this.#windowControlsOverlay.setWCOConfigCSS(show);
    if (show && !this.#wcoModeActiveInternal) {
      // if the WCO overlay is not active, we need to activate it and set the stylesheet
      // If the WCO overlay is already active, we don't want to set the stylesheet again
      this.#wcoModeActiveInternal = show;
      void this.#windowControlsOverlay.updateStylesheet(show);
    }
    void this.overlayAgent.invoke_setShowWco({WCOConfig: this.#windowControlsOverlay.getConfig()});
    // we can only display one overlay at a time,
    // disable the viewport size overlay so WCO overlay can stay when resizing the window
    this.setShowViewportSizeOnResize(!this.#wcoModeActiveInternal);
  }
  ...
  async hasStyleSheetTextForWCO(url: Platform.DevToolsPath.UrlString): Promise<boolean> {
    return this.#windowControlsOverlay.getStylesheetTextForWCO(url);
  }
}
...
export class WindowControlsOverlay {
  ...
  private stylesheetTextForWindows?: string;
  private stylesheetTextForMacOS?: string;
  private stylesheetTextForLinuxOS?: string;
  private stylesheetId?: Protocol.CSS.StyleSheetId;
  private wcoConfig:
      Protocol.Overlay.WCOConfig = {showCSS: false, selectedPlatform: OSType.WindowsOS, themeColor: '#ffffff'};
  constructor(cssModel: CSSModel) {
    this.#cssModel = cssModel;
  }

  getConfig(): Protocol.Overlay.WCOConfig {
    return this.wcoConfig;
  }

  setWCOConfigPlatform(opt: OSType): void {
    this.wcoConfig.selectedPlatform = opt;
  }

  setWCOConfigThemeColor(themeColor: string): void {
    this.wcoConfig.themeColor = themeColor;
  }

  setWCOConfigCSS(shouldShow: boolean): void {
    this.wcoConfig.showCSS = shouldShow;
  }

  async getStylesheetTextForWCO(url: Platform.DevToolsPath.UrlString): Promise<boolean> {
    // The stylesheet text is only fetched once
    if (this.originalStylesheetText) {
      return true;
    }
    let completeURL = url;
  ...
  private getStyleSheetForPlatform(): string {
    switch (this.wcoConfig.selectedPlatform) {
      case OSType.WindowsOS:
        if (!this.stylesheetTextForWindows) {
          // for Windows 11, the titlebar area hight is 33px
          // And the width is the viewport width - the width of the Window Controls Overlay(238px).
          this.stylesheetTextForWindows = this.parseStyleSheet(
              ': env(titlebar-area-x, 0);', ': env(titlebar-area-y, 0);',
              ': env(titlebar-area-width, calc(100% - 238px));', ': env(titlebar-area-height, 33px);');
        }
        return this.stylesheetTextForWindows;
      case OSType.MacOS:
        if (!this.stylesheetTextForMacOS) {
          // for MacOS, the titlebar area hight is 40px
          // And the width is the viewport width - the width of left overlay(85px) and right overlay(100px).
          this.stylesheetTextForMacOS = this.parseStyleSheet(
              ': env(titlebar-area-x, 85px);', ': env(titlebar-area-y, 0);',
              ': env(titlebar-area-width, calc(100% - 185px));', ': env(titlebar-area-height, 40px);');
        }
        return this.stylesheetTextForMacOS;
      case OSType.LinuxOS:
        if (!this.stylesheetTextForLinuxOS) {
          // for Ubuntu 22.04, the titlebar area hight is 34px
          // And the width is the viewport width - the width of the Window Controls Overlay(196px).
          this.stylesheetTextForLinuxOS = this.parseStyleSheet(
              ': env(titlebar-area-x, 0);', ': env(titlebar-area-y, 0);',
              ': env(titlebar-area-width, calc(100% - 196px));', ': env(titlebar-area-height, 34px);');
        }
        return this.stylesheetTextForLinuxOS;
      default:
        return this.originalStylesheetText;
    }
  }

  private parseStyleSheet(x: string, y: string, width: string, height: string): string {
    const stylesheetText = this.originalStylesheetText;
    let updatedStylesheet = stylesheetText.replace(/: env\(titlebar-area-x.*\);/g, x);
    updatedStylesheet = updatedStylesheet.replace(/: env\(titlebar-area-y.*\);/g, y);
    updatedStylesheet = updatedStylesheet.replace(/: env\(titlebar-area-width.*\);/g, width);
    updatedStylesheet = updatedStylesheet.replace(/: env\(titlebar-area-height.*\);/g, height);
