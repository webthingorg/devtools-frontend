...
export class CPUProfileDataModel extends ProfileTreeModel {
  ...
  private translateProfileTree(nodes: Protocol.Profiler.ProfileNode[]): CPUProfileNode {
    ...
    function buildHitCountFromSamples(nodes: Protocol.Profiler.ProfileNode[], samples: number[]|undefined): void {
      ...
      for (let i = 0; i < samples.length; ++i) {
        // TODO(crbug.com/1172300) Ignored during the jsdoc to ts migration
        // @ts-expect-error
        ++((protocolNodeById.get(samples[i]) as Protocol.Profiler.ProfileNode).hitCount);
      }
    }
    ...
    const root = nodes[0];
    const idToUse = new Map<number, number>([[root.id, root.id]]);
    ...
    while (sourceNodeStack.length) {
      ...
      idToUse.set(sourceNode.id, parentNode.id);
    ...
    if (this.samples) {
      this.samples = this.samples.map(id => idToUse.get(id) as number);
    }
    return resultRoot;
  }

  /**
   * Sorts the samples array in place by timestamp using the timestamps array
   * (there is a one to one matching by index between the two)
   */
  private sortSamples(): void {
    ...
    for (let i = 0; i < timestamps.length; ++i) {
      const orderedIndex: number = indices[i];
      if (orderedIndex === i) {
        continue;
      }
      ...
      while (orderedIndex !== i) {
        samples[currentIndex] = samples[orderedIndex];
        timestamps[currentIndex] = timestamps[orderedIndex];
