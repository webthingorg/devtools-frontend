  ...
  private frameDetached(event: Common.EventTarget.EventTargetEvent<{frame: ResourceTreeFrame, isSwap: boolean}>): void {
    ...
    if (isSwap && !this.#frames.get(frame.id)) {
      ...
      if (traceData.creationStackTrace) {
        cachedFrameAttributes.creationStackTrace = traceData.creationStackTrace;
        cachedFrameAttributes.creationStackTraceTarget = traceData.creationStackTraceTarget;
      }
      if (adScriptId) {
        cachedFrameAttributes.adScriptId = adScriptId;
      }
      if (debuggerId) {
        cachedFrameAttributes.debuggerId = debuggerId;
      }
