...
export class FrameManager extends Common.ObjectWrapper.ObjectWrapper implements SDKModelObserver<ResourceTreeModel> {
  ...
  _frameAdded(event: Common.EventTarget.EventTargetEvent): void {
    ...
    if (frameData) {
      ...
      if (frameData.frame._creationStackTrace) {
        frame._creationStackTrace = frameData.frame._creationStackTrace;
        frame._creationStackTraceTarget = frameData.frame.resourceTreeModel().target();
      }
