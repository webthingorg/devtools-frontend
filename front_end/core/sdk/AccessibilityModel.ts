...
export class AccessibilityNode {
  ...
  // Only the root node gets a frameId, so decendants have to walk up the tree to find their frameId.
...
export class AccessibilityModel extends SDKModel<void> {
  ...
  async requestAXChildren(nodeId: Protocol.Accessibility.AXNodeId, frameId?: Protocol.Page.FrameId|null):
    ...
    // @ts-ignore
    const {nodes} = await this.agent.invoke_getChildAXNodes({id: nodeId, frameId: frameId || undefined});
    if (!nodes) {
      return [];
    }
