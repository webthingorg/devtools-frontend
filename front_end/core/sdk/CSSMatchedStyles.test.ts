...
describe('CSSMatchedStyles', () => {
  ...
  describe('resolveGlobalKeyword', () => {
    ...
    function checkResolution(
      ...
      const ownProperties =
          new Map(matchedStyles.nodeStyles()[0].allProperties().map(property => [property.name, property]));

      for (const {name: propertyName, expectedValue} of properties) {
        const property = ownProperties.get(propertyName);
        assert.isOk(property);

        let resolvedValue: SDK.CSSMatchedStyles.CSSVariableDeclaration|null =
            new SDK.CSSMatchedStyles.CSSVariableDeclaration(property);
