...
export class TargetManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  setInspectedURL(_inspectedURL: string, name: string): void {
    this.dispatchEventToListeners(Events.InspectedURLChanged, this);
    if (!name) {
      this.dispatchEventToListeners(Events.NameChanged, this);
    }
  }
