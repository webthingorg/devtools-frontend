  ...
  private updateCacheNames(
    ...
    for (const cacheJson of cachesJson) {
      ...
      if (cacheJson.storageBucketId) {
        storageBucket = this.#storageBucketModel.getBucketById(cacheJson.storageBucketId);
      } else {
        storageBucket = this.#storageBucketModel.getDefaultBucketForStorageKey(cacheJson.storageKey);
      }
      if (!storageBucket) {
        continue;
      }
