...
export class ServiceWorkerCacheModel extends SDKModel<EventTypes> implements ProtocolProxyApi.StorageDispatcher {
  ...
  readonly #storageBucketsUpdated = new Set<Protocol.Storage.StorageBucketInfo>();
  ...
  enable(): void {
    ...
    for (const storageBucket of this.#storageBucketModel.getBuckets()) {
      this.addStorageBucket(storageBucket);
    }
    this.#enabled = true;
  }
  ...
  private async loadCacheNames(storageBucket: Protocol.Storage.StorageBucketInfo): Promise<void> {
  ...
  private updateCacheNames(
      storageBucket: Protocol.Storage.StorageBucketInfo, cachesJson: Protocol.CacheStorage.Cache[]): void {
    ...
    for (const cacheJson of cachesJson) {
      const storageBucket = this.#storageBucketModel.getBucketById(cacheJson.storageKey, cacheJson.storageBucketId);
      if (!storageBucket) {
        continue;
      }
  ...
  private storageBucketAdded({data: {bucket}}: Common.EventTarget.EventTargetEvent<BucketEvent>): void {
    this.addStorageBucket(bucket);
  }

  private storageBucketRemoved({data: {bucket}}: Common.EventTarget.EventTargetEvent<BucketEvent>): void {
  ...
  cacheStorageListUpdated({storageKey, bucketId}: Protocol.Storage.CacheStorageListUpdatedEvent): void {
  ...
  cacheStorageContentUpdated({storageKey, bucketId, cacheName}: Protocol.Storage.CacheStorageContentUpdatedEvent):
      void {
    const storageBucket = this.#storageBucketModel.getBucketById(storageKey, bucketId);
    if (storageBucket) {
...
export interface CacheStorageContentUpdatedEvent {
  storageBucket: Protocol.Storage.StorageBucketInfo;
  cacheName: string;
}
...
export class Cache {
  ...
  constructor(
      model: ServiceWorkerCacheModel, storageBucket: Protocol.Storage.StorageBucketInfo, cacheName: string,
      cacheId: Protocol.CacheStorage.CacheId) {
    this.#model = model;
