...
export class PreloadingModel extends SDKModel<EventTypes> {
  ...
  private isJustTargetAttached: boolean = true;
  private modelLastPrimary: PreloadingModel|null = null;
  ...
  constructor(target: Target) {
    ...
    if (targetInfo !== undefined && targetInfo.subtype === 'prerender') {
      this.modelLastPrimary = TargetManager.instance().primaryPageTarget()?.model(PreloadingModel) || null;
    }
  ...
  private onPrimaryPageChanged(
    ...
    if (this.modelLastPrimary === null && type === PrimaryPageChangeType.Activation) {
      return;
    }
    ...
    if (this.modelLastPrimary !== null && type === PrimaryPageChangeType.Activation) {
      ...
      this.loaderIds = this.modelLastPrimary.loaderIds;
      for (const [loaderId, prev] of this.modelLastPrimary.documents.entries()) {
        this.ensureDocumentPreloadingData(loaderId);
        this.documents.get(loaderId)?.mergePrevious(prev);
      }
    }

    this.modelLastPrimary = null;
  ...
  onRuleSetUpdated(event: Protocol.Preload.RuleSetUpdatedEvent): void {
    ...
    if (this.currentLoaderId() === null) {
      ...
      this.isJustTargetAttached = false;
    }
