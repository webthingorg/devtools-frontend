...
export class PreloadingModel extends SDKModel<EventTypes> {
  ...
  private currentDocument(): DocumentPreloadingData|null {
    ...
    if (this.loaderId === null) {
      const iterResult = this.documents.keys().next();
      if (iterResult.done) {
        return null;
      }
      // At almost all timing, we have at most one DocumentPreloadingData. We may have two ones
      // iff all of the following condition satisfied.
      //
      ...
      //
      // We don't expect this occurs. If occurred, following Page.frameNavigated triggers
      // rerenderering of PreloadingView. So, we return arbitrary one for that case.
      const loaderId = iterResult.value;
      return this.documents.get(loaderId) || null;
    }
...
class PreloadingAttemptRegistry {
  ...
  getAll(ruleSetId: Protocol.Preload.RuleSetId|null, sources: SourceRegistry):
    ...
    const iter =
        Array.from(this.map.entries()).map(([id, value]) => ({id, value: this.enrich(value, sources.getById(id))}));
    return ruleSetId === null ? iter : iter.filter(({value}) => value.ruleSetIds.includes(ruleSetId));
  }
...
class SourceRegistry {
  ...
  update(sources: Protocol.Preload.PreloadingAttemptSource[]): void {
    ...
    for (const source of sources) {
      const id = makePreloadingAttemptId(source.key);
      this.map.set(id, source);
    }
  }
}
