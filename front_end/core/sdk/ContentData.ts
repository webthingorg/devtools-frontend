...
export class ContentData {
  ...
  constructor(
      data: string, isBas64: boolean, resourceType: Common.ResourceType.ResourceType, mimeType: string,
      charset?: string) {
  ...
  get text(): string {
    ...
    if (!this.resourceType.isTextType()) {
      throw new Error('Attempting to interpret binary data as text!');
    }

    const charset = this.#charset || 'utf-8';
    const binString = window.atob(this.#contentAsBase64 as string);
