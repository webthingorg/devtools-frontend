...
import {DebuggerModel} from './sdk.js';
  ...
  async attachedToTarget({sessionId, targetInfo, waitingForDebugger}: Protocol.Target.AttachedToTargetEvent):
    ...
    if (waitingForDebugger) {
      ...
      if (target.type() === Type.SharedStorageWorklet) {
        const debuggerModel = target.model(DebuggerModel.DebuggerModel);
        if (debuggerModel) {
          if (!debuggerModel.isReadyToPause()) {
            await debuggerModel.once(DebuggerModel.Events.DebuggerIsReadyToPause);
          }
