...
export class DOMDebuggerModel extends SDKModel {
  ...
  private domBreakpointsInternal: DOMBreakpoint[];
  private readonly domBreakpointsSetting: Common.Settings
  ...
  private nodeRemoved(event: Common.EventTarget.EventTargetEvent): void {
    if (this.suspended) {
      return;
    }
...
export class EventListenerBreakpoint extends CategorizedBreakpoint {
  ...
  private static readonly instrumentation = 'instrumentation:';
}
...
export class DOMDebuggerManager implements SDKModelObserver<DOMDebuggerModel> {
  ...
  constructor() {
    ...
    this.createEventListenerBreakpoints(i18nString('Worker'), ['message', 'messageerror'], ['*']);
    this.createEventListenerBreakpoints(
        i18nString(UIStrings.xhr),
        ['readystatechange', 'load', 'loadstart', 'loadend', 'abort', 'error', 'progress', 'timeout'],
        ['xmlhttprequest', 'xmlhttprequestupload']);

    let breakpoint: EventListenerBreakpoint|null;
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:setTimeout.callback');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.setTimeoutOrIntervalFired, {PH1: 'setTimeout'}));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:setInterval.callback');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.setTimeoutOrIntervalFired, {PH1: 'setInterval'}));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:scriptFirstStatement');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.scriptFirstStatement));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:scriptBlockedByCSP');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.scriptBlockedByContentSecurity));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:requestAnimationFrame');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.requestAnimationFrame));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:cancelAnimationFrame');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.cancelAnimationFrame));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:requestAnimationFrame.callback');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.animationFrameFired));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:webglErrorFired');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.webglErrorFired));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:webglWarningFired');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.webglWarningFired));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:Element.setInnerHTML');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.setInnerhtml));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:canvasContextCreated');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.createCanvasContext));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:Geolocation.getCurrentPosition');
    if (breakpoint) {
      breakpoint.setTitle('getCurrentPosition');
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:Geolocation.watchPosition');
    if (breakpoint) {
      breakpoint.setTitle('watchPosition');
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:Notification.requestPermission');
    if (breakpoint) {
      breakpoint.setTitle('requestPermission');
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:DOMWindow.close');
    if (breakpoint) {
      breakpoint.setTitle('window.close');
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:Document.write');
    if (breakpoint) {
      breakpoint.setTitle('document.write');
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:audioContextCreated');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.createAudiocontext));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:audioContextClosed');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.closeAudiocontext));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:audioContextResumed');
    if (breakpoint) {
      breakpoint.setTitle(i18nString(UIStrings.resumeAudiocontext));
    }
    breakpoint = this.resolveEventListenerBreakpoint('instrumentation:audioContextSuspended');
    if (breakpoint) {
  ...
  private resolveEventListenerBreakpoint(eventName: string, eventTargetName?: string): EventListenerBreakpoint|null {
  ...
  resolveEventListenerBreakpointTitle(auxData: {
    ...
    const breakpoint = this.resolveEventListenerBreakpoint(id, auxData['targetName']);
    if (!breakpoint) {
      return '';
    }
  ...
  resolveEventListenerBreakpointFromAuxData(auxData: {
    eventName: string,
    targetName: string,
  }): EventListenerBreakpoint|null {
    return this.resolveEventListenerBreakpoint(auxData['eventName'], auxData['targetName']);
  }
