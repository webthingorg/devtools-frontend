...
export class DOMDebuggerManager implements SDKModelObserver<DOMDebuggerModel> {
  ...
  resolveEventListenerBreakpoint(eventName: string, eventTargetName?: string): DOMEventListenerBreakpoint|null {
    ...
    for (const breakpoint of this.#eventListenerBreakpointsInternal) {
      if (eventName && breakpoint.name === eventName && breakpoint.eventTargetNames.indexOf(eventTargetName) !== -1) {
        result = breakpoint;
      }
