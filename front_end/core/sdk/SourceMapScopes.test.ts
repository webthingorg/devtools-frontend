...
describe('decodeGeneratedRanges', () => {
  ...
  it('decodes original scope (definition) references', () => {
    ...
    assert.strictEqual(generatedRange.originalScope, originalScopes[0], 'range does not reference global scope');
    assert.strictEqual(
        generatedRange.children[0].originalScope, originalScopes[0].children[0],
  ...
  it('decodes original scope (definition) references across multiple original sources', () => {
    ...
    assert.strictEqual(generatedRange.children[0].originalScope, originalScopes[0].children[0]);
    assert.strictEqual(generatedRange.children[1].originalScope, originalScopes[1].children[0]);
  });
});
