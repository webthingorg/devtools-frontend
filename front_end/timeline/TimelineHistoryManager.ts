...
export class TimelineHistoryManager {
  ...
  constructor() {
    this._recordings = [];
    this._action = (UI.ActionRegistry.ActionRegistry.instance().action('timeline.show-history') as UI.ActionRegistration.Action);
  ...
  async showHistoryDropDown(): Promise<PerformanceModel | null> {
    ...
    const model = await DropDown.show(this._recordings, (this._lastActiveModel as PerformanceModel), this._button.element);
    if (!model) {
      return null;
    }
...
export class DropDown implements UI.ListControl.ListDelegate {
  ...
  _listControl: UI.ListControl.ListControl<any>;
  ...
  constructor(models: PerformanceModel[]) {
    ...
    const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(this._glassPane.contentElement, { cssFile: 'timeline/timelineHistoryManager.css', enableLegacyPatching: true, delegatesFocus: undefined });
  ...
  heightForItem(item: PerformanceModel): number {
    console.assert(false, 'Should not be called');
    return 0;
  }

  isItemSelectable(item: PerformanceModel): boolean {
    return true;
  }
