...
class MainWidget extends UI.Widget.VBox {
  constructor(tmp, model) {
    super();
    this.webVitals = new WebVitals(model);
...
export class TimelineFlameChartView extends UI.Widget.VBox {
  ...
  constructor(delegate) {
    ...
    this._mainWidget = new MainWidget(this._mainFlameChart);
  ...
  setModel(model) {
    ...
    this._webvitalsTimeline.data = {
      startTime: model.window().left - timelineModel.minimumRecordTime(),
      ...
      lcps: latestLcpEvents.map(e => e.startTime - timelineModel.minimumRecordTime()).map(t => ({timestamp: t})),
    ...
    this._mainWidget.webVitals._chartViewport.setBoundaries(
        model.window().left - timelineModel.minimumRecordTime(),
