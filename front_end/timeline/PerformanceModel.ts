...
export class PerformanceModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  _recordStartTime: number | undefined;
  constructor() {
    super();
  ...
  setTracingModel(model: SDK.TracingModel.TracingModel): void {
    ...
    for (const entry of this._extensionTracingModels) {
      entry.model.adjustTime(this._tracingModel.minimumRecordTime() + (entry.timeOffset / 1000) - (this._recordStartTime as number));
    }
  ...
  _autoWindowTimes(): void {
    ...
    function findLowUtilizationRegion(startIndex: number, stopIndex: number): number {
      const threshold = 0.1;
      let cutIndex: number = startIndex;
      let cutTime: number = (tasks[cutIndex].startTime + (tasks[cutIndex].endTime as number)) / 2;
      let usedTime = 0;
