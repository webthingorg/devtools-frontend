...
export class TimelineController {
  ...
  async startRecording(options, providers) {
    ...
    includedCategories.push(TimelineModel.TimelineModel.TimelineModelImpl.Category.LatencyInfo);

    if (Root.Runtime.experiments.isEnabled('timelineFlowEvents')) {
      includedCategories.push('devtools.timeline.async');
    }

    if (Root.Runtime.experiments.isEnabled('timelineV8RuntimeCallStats') && options.enableJSSampling) {
      includedCategories.push(disabledByDefault('v8.runtime_stats_sampling'));
    }
    if (!Root.Runtime.queryParam('timelineTracingJSProfileDisabled') && options.enableJSSampling) {
      includedCategories.push(disabledByDefault('v8.cpu_profiler'));
    }
    includedCategories.push(disabledByDefault('devtools.timeline.stack'));
    if (Root.Runtime.experiments.isEnabled('timelineInvalidationTracking')) {
      includedCategories.push(disabledByDefault('devtools.timeline.invalidationTracking'));
    }
    ...
    if (options.captureFilmStrip) {
      includedCategories.push(disabledByDefault('devtools.screenshot'));
    }
    ...
    const response =
        await this._startRecordingWithCategories(includedCategories, excludedCategories, options.enableJSSampling);
   ...
   */
  async _startRecordingWithCategories(included, excluded, enableJSSampling) {
    ...
    return this._tracingManager.start(this, included, excluded);
  }
