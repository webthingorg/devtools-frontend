...
export class TimelinePaintProfilerView extends UI.SplitWidget.SplitWidget {
  ...
  _update(): void {
    ...
    if (this._pendingSnapshot) {
      ...
      const picture = (TimelineModel.TimelineModel.TimelineData.forEvent(this._event).picture as SDK.TracingModel.ObjectSnapshot);
      snapshotPromise = picture.objectPromise()
        .then(data => (this._paintProfilerModel as SDK.PaintProfiler.PaintProfilerModel).loadSnapshot(data['skp64']))
    ...
    function onCommandLogDone(this: TimelinePaintProfilerView, snapshot: SDK.PaintProfiler.PaintProfilerSnapshot, clipRect: Protocol.DOM.Rect | null, log?: SDK.PaintProfiler.PaintProfilerLogItem[]): void {
...
export class TimelinePaintImageView extends UI.Widget.Widget {
  ...
  constructor() {
    ...
    this._transformController = new LayerViewer.TransformController.TransformController((this.contentElement as HTMLElement), true);
