...
import * as PuppeteerConnection from '../../models/puppeteer_connection/puppeteer_connection.js';
...
async function start(method: string, params: any): Promise<unknown> {
  ...
  try {
    ...
    const {page, browser} = await PuppeteerConnection.PuppeteerConnection.getPuppeteerConnection(rawConnection, mainFrameId, mainTargetId);

    // @ts-expect-error https://github.com/GoogleChrome/lighthouse/issues/11628
    const result = await self.lhNavigation({
      url,
      config,
      page,
    });
