...
import * as PuppeteerModel from '../../models/puppeteer_model/puppeteer_model.js';
...
async function start(method: string, params: any): Promise<unknown> {
  ...
  try {
    ...
    const {page, browser} = await PuppeteerModel.PuppeteerConnection.getPuppeteerConnection(rawConnection, mainFrameId, mainTargetId);
