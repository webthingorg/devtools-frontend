...
export class Transport implements puppeteer.ConnectionTransport {
  ...
  send(message: string): void {
    ...
    this.#connection.sendRawMessage(JSON.stringify({
      ...data,
      sessionId: data.sessionId === undefined ? this.#connection.getSessionId() : data.sessionId,
    }));
  }
...
export async function getPuppeteerConnection(
  ...
  const page = await browser.pages().then(pages => pages.find(p => p.mainFrame()._id === mainFrameId) || null);
  return {page, browser};
}
