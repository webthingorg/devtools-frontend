...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  _onFrameAddedToTarget(event) {
    const {frame} = /** @type {!{frame:!SDK.ResourceTreeModel.ResourceTreeFrame}} */ (event.data);
    // Determining third-party status usually requires the registerd domain of the top frame.
    // When DevTools is initially opened on an already navigated to page, we can receive issues
    // before the FrameTree is fully available. Thus, we trigger a recalcuation of third-party-ness
