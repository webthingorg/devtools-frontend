...
export class FrameManager extends Common.ObjectWrapper.ObjectWrapper {
   ...
   */
  _frameAdded(event) {
    const eventFrame = /** @type {!SDK.ResourceTreeModel.ResourceTreeFrame} */ (event.data);
    const frameData = this._frames.get(eventFrame.id);
    // If the frame is already in the map, increase its count, otherwise add it to the map.
    if (frameData) {
      this._frames.set(eventFrame.id, {frame: eventFrame, count: frameData.count + 1});
    } else {
      this._frames.set(eventFrame.id, {frame: eventFrame, count: 1});
    }

    // Add the frameId to the the targetId's set of frameIds
    const frameSet = this._framesForTarget.get(eventFrame.resourceTreeModel().target().id());
    if (frameSet) {
      frameSet.add(eventFrame.id);
    }
  }
  ...
  _frameDetached(event) {
    ...
    this.decreaseOrRemoveFrame(eventFrame.id);

    // Remove the frameId from the target's set of frameIds
    const frameSet = this._framesForTarget.get(eventFrame.resourceTreeModel().target().id());
    if (frameSet) {
      frameSet.delete(eventFrame.id);
    }
  }
