...
const timestampRenderer = (value: Components.DataGridUtils.CellValue): Platform.UIString.LocalizedString => {
...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    const dataGridInitialData: Components.DataGridController.DataGridControllerData = {
      ...
      rows: [],
      contextMenus: {
        bodyRow: (menu: UI.ContextMenu.ContextMenu, columns: readonly Components.DataGridUtils.Column[], row: Readonly<Components.DataGridUtils.Row>): void => {
    ...
    this._dataGridIntegrator.dataGrid.addEventListener('cell-focused', /**
     @param {!Event} event
     */ /**
     @param {!Event} event
     */ /**
    ...
    */ event: Event) => {
    ...
    this._dataGridIntegrator.addEventListener('new-user-filter-text', /**
     @param {*} event
    */ /**
     @param {*} event
    */ /**
  ...
  _targetToString(target: SDK.SDKModel.Target | null): string {
    ...
    return target.decorateLabel(`${target.name()} ${target === SDK.SDKModel.TargetManager.instance().mainTarget() ? '' : target.id()}`);
  }

  _messageReceived(message: any, target: ProtocolClient.InspectorBackend.TargetBase | null): void {
    if ('id' in message) {
