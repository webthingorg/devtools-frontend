...
class DataGridControllerIntegrator extends UI.Widget.VBox {
  /**
   *
   * @param {Components.DataGridController.DataGridControllerData} data
   */
  constructor(data) {
    super(true, true);
    this.dataGrid = new Components.DataGridController.DataGridController();
    this.dataGrid.data = data;
    this.contentElement.appendChild(this.dataGrid);
  }

  /**
   * @returns {Components.DataGridController.DataGridControllerData}
   */
  dataGridData() {
...
export class ProtocolMonitorImpl extends UI.Widget.VBox {
  constructor() {
    ...
    clearButton.addEventListener(UI.Toolbar.ToolbarButton.Events.Click, () => {
      this._dataGridIntegrator.update({...this._dataGridIntegrator.dataGridData(), rows: []});
    });
    ...
    this._dataGridIntegrator = new DataGridControllerIntegrator(dataGridInitialData);
    ...
    this._textFilterUI.addEventListener(UI.Toolbar.ToolbarInput.Event.TextChanged, event => {
      ...
      this._dataGridIntegrator.update({...this._dataGridIntegrator.dataGridData(), filters});
    });
  ...
  _messageReceived(message, target) {
    if ('id' in message) {
      ...
      const allExistingRows = this._dataGridIntegrator.dataGridData().rows;
      ...
      const newRowsArray = [...this._dataGridIntegrator.dataGridData().rows];
      ...
      this._dataGridIntegrator.update({
        ...this._dataGridIntegrator.dataGridData(),
        rows: newRowsArray,
      });
      return;
    }
    ...
    const newRow = {
      cells: [
        ...
        },
        {
          ...
          renderer: value => i18nString(UIStrings.sMs, {PH1: value})
        },
    ...
    this._dataGridIntegrator.update({
      ...this._dataGridIntegrator.dataGridData(),
      rows: this._dataGridIntegrator.dataGridData().rows.concat([newRow]),
    });
  }
...
// TODO: where is this ProtocolColumnConfig used?
