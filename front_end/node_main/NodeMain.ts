...
export class NodeMainImpl extends Common.ObjectWrapper.ObjectWrapper implements Common.Runnable.Runnable {
  async run(): Promise<void> {
    ...
    SDK.Connections.initMainConnection(async () => {
      const target = SDK.SDKModel.TargetManager.instance().createTarget('main', i18nString(UIStrings.main), SDK.SDKModel.Type.Browser, null);
...
export class NodeChildTargetManager extends SDK.SDKModel.SDKModel implements ProtocolProxyApi.TargetDispatcher {
  ...
  targetDestroyed(event: Protocol.Target.TargetDestroyedEvent): void {
  }
  ...
  attachedToTarget({ sessionId, targetInfo }: Protocol.Target.AttachedToTargetEvent): void {
    ...
    const target = this._targetManager.createTarget(targetInfo.targetId, name, SDK.SDKModel.Type.Node, this._parentTarget, undefined, undefined, connection);
  ...
  targetCrashed(event: Protocol.Target.TargetCrashedEvent): void {
  }
}
