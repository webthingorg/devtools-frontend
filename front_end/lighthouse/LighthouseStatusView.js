...
export class StatusView {
  ...
  show(dialogRenderElement) {
    ...
    this._dialog.show(/** @type {*} */ (dialogRenderElement));
  }
  ...
  updateStatus(message) {
    ...
    const currentPhaseIndex = this._currentPhase ? StatusPhases.indexOf(this._currentPhase) : 0;
  ...
  _getPhaseForMessage(message) {
    const phase = StatusPhases.find(phase => message.startsWith(phase.statusMessagePrefix));
    if (!phase) {
      throw new Error('Unable to locate phase');
    }
    return phase;
  }
