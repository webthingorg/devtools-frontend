...
export class QuickOpenImpl {
    ...
    const unionOfProviders = [
      ...Root.Runtime.Runtime.instance().extensions(Provider).map(extension => {
        return {
          ...
          provider: /** @type { function(): !Promise<!Provider>}*/ (extension.instance),
        };
      }),
  ...
  /**
   * @param {!{prefix: ?string, title: string, provider: function(): !Promise<!Provider>}} extension
   */
  _addProvider(extension) {
    ...
    this._providers.set(
        ...
        (extension.provider.bind(extension)));
  }
