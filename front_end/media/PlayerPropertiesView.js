...
Media.PlayerPropertiesView = class extends UI.VBox {
  ...
  formatFileSize(bytes) {
    ...
    return `${(bytes / Math.pow(1000, power)).toFixed(2)} ${suffix}`;
  }
  ...
  populateAttributesAndElements() {
    ...
    const decoderName = new Media.DefaultPropertyRenderer(ls`Decoder Name`, 'No Decoder');
    ...
    const audioDecoder = new Media.DefaultPropertyRenderer(ls`Decoder Name`, 'No Decoder');
