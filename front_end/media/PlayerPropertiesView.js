...
Media.PlayerPropertiesView = class extends UI.VBox {
  ...
  populateAttributesAndElements() {
    ...
    this._attribute_map['surface_layer_mode'] = surface_layer_mode;

    const video_buffering_state = new Media.PropertyRenderer('Buffering State');
    this._video_decoder_elements.push(video_buffering_state);
    this._attribute_map['video_buffering_state'] = video_buffering_state;

    const time_base = new Media.PropertyRenderer('Time Base');
    this._video_decoder_elements.push(time_base);
    this._attribute_map['time_base'] = time_base;

    const video_dds = new Media.PropertyRenderer('Decrypting Demuxer');
    this._video_decoder_elements.push(video_dds);
    this._attribute_map['video_dds'] = video_dds;

    /* Audio Decoder Properties */
    const has_audio = new Media.DefaultPropertyRenderer('Has Audio', 'false');
    this._audio_decoder_elements.push(has_audio);
    this._attribute_map['found_audio_stream'] = has_audio;

    const audio_decoder = new Media.PropertyRenderer('Decoder');
    this._audio_decoder_elements.push(audio_decoder);
    this._attribute_map['audio_decoder'] = audio_decoder;
    ...
    this._attribute_map['audio_samples_per_second'] = audio_samples_per_second;

    const audio_buffering_state = new Media.PropertyRenderer('Buffering State');
    this._audio_decoder_elements.push(audio_buffering_state);
    this._attribute_map['audio_buffering_state'] = audio_buffering_state;
  }
};
