 ...
 * @return {string}
 */
function formatMilisecondsToSeconds(ms, decimalPlaces) {
...
function calculateFontColor(backgroundColor) {
  ...
  const computedColor = window.getComputedStyle(colorDiv, null).getPropertyValue('background-color');
  document.body.removeChild(colorDiv);
  const colors = computedColor.match(/\d+/g);
...
/**
 * @typedef {{
 *     setLive: (!function(int):int),
 *     setComplete: (!function(int):undefined),
 *     updateMaxTime: (!function(int):undefined)
 * }}
 */
...
class Event {
  ...
  decorate(htmlElement) {
    ...
    const startTimeReadable = formatMilisecondsToSeconds(this.startTime, 2);
    if (this._live) {
      htmlElement.createChild('span').textContent = `Duration: ${startTimeReadable} - LIVE!`;
    } else if (this._duration !== NaN) {
      const durationReadable = formatMilisecondsToSeconds(this.duration + this.startTime, 2);
...
export class TickingFlameChart extends UI.Widget.VBox {
  ...
  startEvent(properties) {
    ...
    properties['duration'] = properties['duration'] || -1;
...
class TickingFlameChartDataProvider {
   ...
   */
  textColor(index) {
    return this._eventMap[index].fontColor;
  }
  ...
  formatValue(value, precision) {
    ...
    if (range < TwoDecimalLimit) {
      return formatMilisecondsToSeconds(value, 2);
    }
    if (range < OneDecimalLimit) {
      return formatMilisecondsToSeconds(value, 1);
    }
    return formatMilisecondsToSeconds(value, 0);
  }
