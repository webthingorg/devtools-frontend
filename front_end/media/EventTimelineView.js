...
export class PlayerEventsTimeline extends TickingFlameChart {
  ...
  _onPlaybackEvent(event, normalized_time) {
    switch (event.event) {
      case 'kPlay':
        ...
        this._playback_status_last_event.displayData = {'StartTime': normalized_time};
        break;
  ...
  _onBufferingStatus(event, normalized_time) {
    ...
    let video_state = null;

    switch (event.event) {
      case 'kBufferingStateChanged':
        audio_state = event.value['audio_buffering_state'];
        video_state = event.value['video_buffering_state'];
