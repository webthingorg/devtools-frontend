...
export class ConsolePrompt extends UI.Widget.Widget {
  ...
  async _requestPreview() {
    ...
    if (result && !('error' in result) && result.object && result.object.subtype === 'node') {
  ...
  async _wordsWithQuery(queryRange, substituteRange, force) {
    ...
    if (!this._defaultAutocompleteConfig || !this._defaultAutocompleteConfig.suggestionsCallback) {
      return [];
    }
