...
import {EdgeView} from './EdgeView.js'; // eslint-disable-line no-unused-vars
import {GraphView} from './GraphView.js'; // eslint-disable-line no-unused-vars
...
const findConnectionPoints = (graph, edge, route) => {
  ...
  const point1 = getPointByDistance(sourcePos, control1, GraphStyle.InputPortRadius);
  // Determine radius by destinationPort.type.
  const radius = destinationPort.type === WebAudio.GraphVisualizer.PortTypes.In ? GraphStyle.InputPortRadius :
                                                                                  GraphStyle.AudioParamRadius;
  const point2 = getPointByDistance(destinationPos, control2, radius);
  return {
    sourcePoint: roundPoint(point1),
    destinationPoint: roundPoint(point2),
  };
};
...
const createArrowHeadPath = (sourcePoint, destinationPoint, route) => {
  ...
  const angle = calculateAngleRadian(lastMiddlePoint, destinationPoint);
  const segments = [];

  let segment = createSegment('M', destinationPoint);
  segments.push(segment);

  segment = createSegment('L', {
    x: destinationPoint.x - GraphStyle.ArrowHeadSize * Math.cos(-angle - Math.PI / 6),
    y: destinationPoint.y - GraphStyle.ArrowHeadSize * Math.sin(-angle - Math.PI / 6),
  });
  segments.push(segment);
