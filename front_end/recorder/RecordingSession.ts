...
export class StepFrameContext {
  ...
  static getExpressionForTarget(target: string): "const targetPage = page;" | `const target = await browser.waitForTarget(p => p.url() === ${string});
...
export class ClickStep extends Step {
  ...
  toString(): `{
  ${string}
  const element = await frame.waitForSelector(${string});
  await element.click();
  }` {
    return `{
  ${this.context}
...
export class NavigationStep extends Step {
  ...
  toString(): `await page.goto(${string});` {
...
export class CloseStep extends Step {
  ...
  const target = await browser.waitForTarget(p => p.url() === ${string});
  const targetPage = await target.page();
  await targetPage.close();
  }` {
    return `{
...
export class EmulateNetworkConditions extends Step {
  ...
  toString(): `{
  // Simulated network throttling (${string})
...
export class RecordingSession {
  ...
  constructor(target: SDK.SDKModel.Target, uiSourceCode: Workspace.UISourceCode.UISourceCode) {
    ...
    this._axModel = target.model(SDK.AccessibilityModel.AccessibilityModel) as SDK.AccessibilityModel.AccessibilityModel;
    this._debuggerModel = target.model(SDK.DebuggerModel.DebuggerModel) as SDK.DebuggerModel.DebuggerModel;
    this._resourceTreeModel = target.model(SDK.ResourceTreeModel.ResourceTreeModel) as SDK.ResourceTreeModel.ResourceTreeModel;
  ...
  async isSubmitButton(targetId: string): Promise<any> {
  ...
  async evaluateInAllFrames(target: SDK.SDKModel.Target, expression: string): Promise<void> {
    const resourceTreeModel = target.model(SDK.ResourceTreeModel.ResourceTreeModel) as SDK.ResourceTreeModel.ResourceTreeModel;
