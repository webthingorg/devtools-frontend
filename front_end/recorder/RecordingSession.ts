...
export function assertNotNull<T>(instance: T|null): T {
  if (instance === null) {
    throw new TypeError('Instance expected to not be null');
  }
  return instance;
}

export class RecordingSession implements ProtocolProxyApi.DebuggerDispatcher {
  private _target: SDK.SDKModel.Target;
  private _uiSourceCode: Workspace.UISourceCode.UISourceCode;
  private _currentIndentation: number;

  private _debuggerAgent: ProtocolProxyApi.DebuggerApi;
  private _domDebuggerAgent: ProtocolProxyApi.DOMDebuggerApi;
  private _runtimeAgent: ProtocolProxyApi.RuntimeApi;
  private _pageAgent: ProtocolProxyApi.PageApi;

  private _domModel: SDK.DOMModel.DOMModel;
  private _axModel: Accessibility.AccessibilityModel.AccessibilityModel;
  private _debuggerModel: SDK.DebuggerModel.DebuggerModel;
  ...
  constructor(target: SDK.SDKModel.Target, uiSourceCode: Workspace.UISourceCode.UISourceCode) {
    ...
    this._domModel = assertNotNull(target.model(SDK.DOMModel.DOMModel));
    this._axModel = assertNotNull(target.model(Accessibility.AccessibilityModel.AccessibilityModel));
    this._debuggerModel = assertNotNull(target.model(SDK.DebuggerModel.DebuggerModel));
  ...
  async start() {
    const setupEventListeners = () => {
      window.addEventListener('click', _event => {}, true);
      window.addEventListener('submit', _event => {}, true);
      window.addEventListener('change', _event => {}, true);
    };

    const wrapFunctionSource = (fn: Function) => `(${fn.toString()})()`;

    await this._debuggerModel.suspendModel();
    await this._debuggerAgent.invoke_enable({});
    await this._pageAgent.invoke_addScriptToEvaluateOnNewDocument({source: wrapFunctionSource(setupEventListeners)});
    await this._runtimeAgent.invoke_evaluate({expression: wrapFunctionSource(setupEventListeners)});
  ...
  appendLineToScript(line: string) {
  ...
  async handleClickEvent(localFrame: Protocol.Runtime.PropertyDescriptor[]) {
  ...
  async handleSubmitEvent(localFrame: Protocol.Runtime.PropertyDescriptor[]) {
    const targetId = await this._findTargetId(localFrame, [
      'SubmitEvent',
    ]);
  ...
  async handleChangeEvent(localFrame: Protocol.Runtime.PropertyDescriptor[]) {
    const targetId = await this._findTargetId(localFrame, [
      'Event',
    ]);
  ...
  async _findTargetId(localFrame: Protocol.Runtime.PropertyDescriptor[], interestingClassNames: string[]) {
  ...
  async _getSelector(node: SDK.DOMModel.DOMNode) {
  ...
  async _getAriaSelector(node: SDK.DOMModel.DOMNode) {
