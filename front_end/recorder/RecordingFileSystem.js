...
class RecordingsFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  constructor() {
  ...
  async createFile(path, name) {
    ...
    const snippetName = ls`Recording #${nextId}`;
  ...
  async searchInPath(query, progress) {
    ...
    return matchedSnippets.map(snippet => `recording:///${escapeSnippetName(snippet.name)}`);
  }
...
Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance().addPlatformFileSystem(
    'recording://', new RecordingsFileSystem());
