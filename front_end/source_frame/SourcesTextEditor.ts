...
export class SourcesTextEditor extends TextEditor.CodeMirrorTextEditor.CodeMirrorTextEditor {
  ...
  static getForCodeMirror(codeMirrorEditor: any): SourcesTextEditor {
    return /** @type {!SourcesTextEditor} */ TextEditor.CodeMirrorTextEditor.CodeMirrorTextEditor.getForCodeMirror(codeMirrorEditor) as SourcesTextEditor;
  }
   ...
   */
  _gutterClick(instance: Object, lineNumber: number, gutterType: string, event: MouseEvent): void {
   ...
   * |instance| is actually a CodeMirror.Editor
   */
  _textAreaContextMenu(instance: Object, event: MouseEvent): void {
   ...
   * |instance| is actually a CodeMirror.Editor
   */
  _gutterContextMenu(instance: Object, lineNumber: number, gutterType: string, event: MouseEvent): void {
    const contextMenu = new UI.ContextMenu.ContextMenu(event);
    event.consume(true); // Consume event now to prevent document from handling the async menu
  ...
  _onUpdateEditorIndentation(): void {
    this._setEditorIndentation(TextEditor.CodeMirrorUtils.pullLines((this.codeMirror() as typeof import("/usr/local/google/home/janscheffler/dev/devtools/devtools-frontend/node_modules/@types/codemirror/index")), LinesToScanForIndentationGuessing));
  }
  ...
  _setEditorIndentation(lines: string[]): void {
    ...
    if (indent === '\t') {
       ...
       */
      extraKeys.Tab = function (codeMirror: any): any {
  ...
  _fireBeforeSelectionChanged(codeMirror: typeof CodeMirror, selection: any): void {
    ...
    this._reportJump(this.selection(), TextEditor.CodeMirrorUtils.toRange(primarySelection.anchor, primarySelection.head));
  }
  ...
  _allWhitespaceOverlayMode(mimeType: string): string {
     ...
     */
    function modeConstructor(config: Object, parserConfig: any): CodeMirror.Mode<any> {
 ...
 * @interface
 */
export interface SourcesTextEditorDelegate {
...
export class TokenHighlighter {
  ...
  _tokenHighlighter(token: string, selectionStart: any, stream: any): string | null {
    ...
    if (stream.match(token) &&
      (stream.eol() || !TextUtils.TextUtils.Utils.isWordChar((stream.peek() as string)))) {
