...
export class XMLViewNode extends UI.TreeOutline.TreeElement {
  ...
  _updateTitle() {
    ...
    if (node instanceof Element) {
      ...
      for (let i = 0; i < attributes.length; ++i) {
        ...
        titleItems.push(
            ...
            'shadow-xml-view-tag', attributeNode.value, 'shadow-xml-view-attribute-value', '"', 'shadow-xml-view-tag');
      }
      if (!this.expanded) {
        if (node.childElementCount) {
          ...
          titleItems.push(
              '>', 'shadow-xml-view-tag', node.textContent, 'shadow-xml-view-text', '</' + tag, 'shadow-xml-view-tag');
        } else {
    ...
    if (node instanceof CDATASection && node.nodeValue) {
      this._setTitle([
