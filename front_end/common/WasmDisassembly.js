...
export class WasmDisassembly {
   ...
   */
  * nonBreakableLineNumbers(functionBodyOffsets) {
    let lineNumber = 0;
    let functionIndex = 0;
    while (lineNumber < this.lineNumbers) {
      if (functionIndex < functionBodyOffsets.length) {
        const offset = this.lineNumberToBytecodeOffset(lineNumber);
        if (offset >= functionBodyOffsets[functionIndex].start) {
          lineNumber = this.bytecodeOffsetToLineNumber(functionBodyOffsets[functionIndex++].end);
          continue;
        }
      }
      yield lineNumber++;
