...
export class Runtime {
  ...
  static _experimentsSetting(): {
    ...
    try {
      return /** @type {!Object<string,boolean>} */ JSON.parse(self.localStorage && self.localStorage['experiments'] ? self.localStorage['experiments'] : '{}') as {
        [x: string]: boolean;
      };
    }
    catch (e) {
  ...
  loadAutoStartModules(moduleNames: string[]): Promise<any[]> {
    const promises = [];
...
export function loadResourcePromise(url: string): Promise<string> {
  ...
  function load(fulfill: (arg0: unknown) => void, reject: (arg0: any) => void): void {
