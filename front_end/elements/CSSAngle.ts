...
const roundAngleByUnit = (angle: number, unit: AngleUnit): number => {
  switch (unit) {
    case AngleUnit.Deg:
    case AngleUnit.Grad:
...
export class CSSAngle extends HTMLElement {
  ...
  set data(data: CSSAngleData) {
    ...
    this.onPopoverOpen = data.onPopoverOpen;
    this.onPopoverClosed = data.onPopoverClosed;
  ...
  private updateAngleFromMousePosition(mouseX: number, mouseY: number): void {
    ...
    this.render();
    this.onValueChanged(`${this.angle}${this.unit}`);
  }
  ...
  private renderPopover() {
    ...
    // clang-format off
    return html`
      <div class="popover popover-css-angle">
