...
export class LayoutSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
   ...
   */
  _mapGridNodesToElements(nodes) {
    return nodes.map(node => ({
                       id: node.id,
                       name: node.localName(),
                       extras: {id: node.getAttribute('id'), classes: (node.getAttribute('class') || '').split(/\s+/)},
  ...
  async doUpdate() {
    this._layoutPane.data = {
      gridElements: this._mapGridNodesToElements(await this._fetchGridNodes()),
  ...
  wasShown() {
    ...
    overlayModel.addEventListener(SDK.OverlayModel.Events.PersistentGridOverlayEnabled, this.update, this);
    overlayModel.addEventListener(SDK.OverlayModel.Events.PersistentGridOverlayDisabled, this.update, this);
    this.update();
  }
  ...
  willHide() {
    ...
    overlayModel.removeEventListener(SDK.OverlayModel.Events.PersistentGridOverlayEnabled, this.update, this);
    overlayModel.removeEventListener(SDK.OverlayModel.Events.PersistentGridOverlayDisabled, this.update, this);
  }
}
