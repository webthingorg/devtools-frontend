...
export class BezierPopoverIcon {
  ...
  _bezierEditor: (InlineEditor.BezierEditor.BezierEditor|undefined)|undefined;
  _scrollerElement: (Element|undefined)|undefined;
  _originalPropertyText: (string|null|undefined)|undefined;
  constructor(
    ...
    this._swatch.iconElement().addEventListener(
        ...
        (/** @param {!Event} event */ /** @param {!Event} event */ event: Event) => event.consume(),
        false);
  ...
  _bezierChanged(event: Common.EventTarget.EventTargetEvent) {
  ...
  _onPopoverHidden(commitEdit: boolean) {
    if (this._scrollerElement) {
...
export class ColorSwatchPopoverIcon {
  ...
  _boundOnScroll: (event: Event) => void;
  _spectrum: (ColorPicker.Spectrum.Spectrum|undefined)|undefined;
  _scrollerElement: (Element|undefined)|undefined;
  _originalPropertyText: (string|null|undefined)|undefined;
  constructor(
      treeElement: StylePropertyTreeElement, swatchPopoverHelper: InlineEditor.SwatchPopoverHelper.SwatchPopoverHelper,
  ...
  setContrastInfo(contrastInfo: ColorPicker.ContrastInfo.ContrastInfo) {
  ...
  showPopover() {
    ...
    let format: string|(string | null) = this._swatch.format;
    if (!color || !format) {
      return;
    }
  ...
  _spectrumResized(event: Common.EventTarget.EventTargetEvent) {
    this._swatchPopoverHelper.reposition();
  }

  _spectrumChanged(event: Common.EventTarget.EventTargetEvent) {
...
export class ShadowSwatchPopoverHelper {
  ...
  _boundOnScroll: (event: Event) => void;
  _cssShadowEditor: (InlineEditor.CSSShadowEditor.CSSShadowEditor|undefined)|undefined;
  _scrollerElement: (Element|undefined)|undefined;
  _originalPropertyText: (string|null|undefined)|undefined;
  constructor(
      treeElement: StylePropertyTreeElement, swatchPopoverHelper: InlineEditor.SwatchPopoverHelper.SwatchPopoverHelper,
    ...
    this._iconElement.addEventListener('mousedown', (event: MouseEvent) => event.consume(), false);
  ...
  _shadowChanged(event: Common.EventTarget.EventTargetEvent) {
...
export class FontEditorSectionManager {
  ...
  _fontChanged(event: Common.EventTarget.EventTargetEvent) {
  ...
  async _updateFontProperty(propertyName: string, value: string, treeElement?: StylePropertyTreeElement|undefined) {
    if (treeElement && treeElement.treeOutline && treeElement.valueElement && treeElement.property.parsedOk &&
        treeElement.property.range) {
      let elementRemoved: true|false = false;
  ...
  _createPropertyValueMap(): Map<string, string> {
    ...
    for (const fontProperty of this._treeElementMap) {
      const propertyName: string = (fontProperty[0] as string);
  ...
  async showPopover(iconElement: Element, parentPane: StylesSidebarPane) {
    if (this._swatchPopoverHelper.isShowing()) {
      this._swatchPopoverHelper.hide(true);
      return;
    }
