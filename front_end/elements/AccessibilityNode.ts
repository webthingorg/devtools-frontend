...
export interface AccessibilityNodeData {
  axNode: AXNode, axTree: AccessibilityTree|null, isSelected: boolean, indexInParent: number,
}
...
export class AccessibilityNode extends HTMLElement {
  ...
  upArrowPress(): void {
    ...
    if (parent) {
      ...
      // Move focus up to the previous visible node
  ...
  downArrowPress(): void {
    ...
    // Move focus down to next visible node
    const parent = this.getParentNode();
    if (parent) {
