...
export class FontEditorSectionManager {
  ...
  async _fontChanged(event) {
    const {propertyName, value, repopulate} = event.data;
    let willRepopulate = repopulate ? true : false;
  ...
  async _removeAddedProperties() {
    for (const property of this._treeElementMap.keys()) {
      if (!this._propertyMap.has(property)) {
        const treeElement = this._treeElementMap.get(property);
        const propertyName = treeElement.property.name;
  ...
  registerFontProperty(treeElement) {
    ...
    this._treeElementMap.set(treeElement.property.name, treeElement);
