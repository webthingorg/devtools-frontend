...
function flexAlignContentIcon(iconName) {
  ...
  function getIcon(computedStyles) {
    const directions = getPhysicalFlexDirections(computedStyles);
    /**
     * @type {!Object.<string, !PhysicalFlexDirection>}
     */
    const flexDirectionToPhysicalDirection = {
      column: directions.row,
      row: directions.column,
      'column-reverse': directions['row-reverse'],
      'row-reverse': directions['column-reverse'],
    };
    return rotateAlignContentIcon(
        iconName, flexDirectionToPhysicalDirection[computedStyles.get('flex-direction') || 'row']);
  }
  return getIcon;
}
