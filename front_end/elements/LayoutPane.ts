...
import {BooleanSetting, EnumSetting, GridElement, Setting, SettingType} from './LayoutPaneUtils.js';
...
export class LayoutPane extends HTMLElement {
  ...
  private gridElements: Readonly<GridElement[]> = [];
  ...
  set data(data: {settings: Setting[], gridElements: GridElement[]}) {
  ...
  private onElementToggle(element: GridElement, event: HTMLInputElementEvent) {
    event.preventDefault();
    this.dispatchEvent(new OverlayChangedEvent(element.id, event.target.checked));
  }

  private onElementClick(element: GridElement, event: HTMLInputElementEvent) {
    event.preventDefault();
    this.dispatchEvent(new ElementClickedEvent(element.id));
  }

  private renderElement(element: GridElement) {
  ...
  private buildElementName(element: GridElement) {
