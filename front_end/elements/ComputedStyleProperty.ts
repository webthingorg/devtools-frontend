...
export class ComputedStyleProperty extends HTMLElement {
  ...
  private onToggleDetails() {
    const detailsElement: HTMLDetailsElement = this.shadow.querySelector('details')!;
    setTimeout(() => {
      this.expanded = detailsElement.open;
    });
  }
  ...
  private render() {
    ...
    // clang-format off
    if (this.inherited) { // TODO: figure out the exact condition
      render(html`
        ${this.renderStyle()}
        ${this.renderProperty()}
      `, this.shadow, {
      ...
      render(html`
        ...
        <details ?open=${this.expanded}>
          <summary @click=${this.onToggleDetails}>
            ${this.renderProperty()}
          </summary>
