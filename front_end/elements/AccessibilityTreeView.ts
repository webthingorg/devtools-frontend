...
export class AccessibilityTree extends HTMLElement {
  ...
  appendNodeElement(parent: HTMLUListElement, node: SDK.AccessibilityModel.AccessibilityNode): void {
    const role = node.role();
    const name = node.name();
    if (role && name) {
      const nodeElement = new AccessibilityNode(parent, node);
      if (nodeElement) {
        // FIXME(annabelzhou): empty for now to pass presubmit checks
      }
    }
  }
  ...
  private render(): void {
    ...
    <style>
      ...
      .ax-node:not(.no-children)::before {
...
export class AccessibilityNode {
  ...
  constructor(parent: HTMLUListElement, node: SDK.AccessibilityModel.AccessibilityNode) {
    this.parent = parent;
    this.axNode = node;
    ...
    const childIds = this.axNode._childIds;
