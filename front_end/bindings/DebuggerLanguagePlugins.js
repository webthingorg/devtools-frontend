...
export class DebuggerLanguagePluginManager {
  ...
  modelRemoved(debuggerModel) {
    ...
    this._pluginForScript.forEach((pluginPromise, script, pluginForScript) => {
      if (script.debuggerModel !== debuggerModel) {
        return;
      }
      pluginForScript.delete(script);
      pluginPromise.then(plugin => plugin && plugin.removeRawModule(script.sourceURL)).catch(error => {
  ...
  addPlugin(plugin) {
    ...
    for (const debuggerModel of this._debuggerModelToData.keys()) {
      for (const script of debuggerModel.scripts()) {
        if (this._pluginForScript.has(script)) {
          continue;
        }
  ...
  _getPluginForScript(script) {
    return this._pluginForScript.get(script) || Promise.resolve(null);
  }
  ...
  uiLocationToRawLocationRanges(uiSourceCode, lineNumber, columnNumber) {
    ...
    this._pluginForScript.forEach((pluginPromise, script) => {
      const modelData = this._debuggerModelToData.get(script.debuggerModel);
      if (!modelData) {
        return;
      }
      ...
      locationPromises.push(pluginPromise.then(plugin => plugin ? getLocations(plugin, script) : []));
    });
