...
export class DebuggerLanguagePluginManager {
  ...
  constructor(targetManager, workspace, debuggerWorkspaceBinding) {
    ...
    /** @type {!Map<!SDK.Script.Script, !Promise<!DebuggerLanguagePlugin>>} */
    this._scriptToPluginPromise = new Map();
  }
  ...
  modelRemoved(debuggerModel) {
    ...
    this._scriptToPluginPromise.forEach((pluginPromise, script, pluginForScript) => {
      if (script.debuggerModel !== debuggerModel) {
        return;
      }
      pluginForScript.delete(script);
      pluginPromise
          .then(plugin => {
            plugin.removeRawModule(script.sourceURL);
          })
          .catch(error => {
  ...
  addPlugin(plugin) {
    ...
    for (const debuggerModel of this._debuggerModelToData.keys()) {
      for (const script of debuggerModel.scripts()) {
        if (this._scriptToPluginPromise.has(script)) {
          continue;
        }
        ...
        this._scriptToPluginPromise.set(script, this._loadScript(plugin, script));
      }
    }
  }
  ...
  hasPluginForScript(script) {
    return this._scriptToPluginPromise.has(script);
  }
  ...
  async rawLocationToUILocation(rawLocation) {
    ...
    const plugin = await this._scriptToPluginPromise.get(script);
    if (!plugin) {
      return null;
    }
  ...
  uiLocationToRawLocationRanges(uiSourceCode, lineNumber, columnNumber) {
    ...
    this.scriptsForUISourceCode(uiSourceCode).forEach(script => {
      const pluginPromise = this._scriptToPluginPromise.get(script);
      if (!pluginPromise) {
        return;
      }
      locationPromises.push(pluginPromise.then(plugin => {
        if (!plugin) {
          return [];
        }
        return getLocations(plugin, script);
      }));
    });
     ...
     */
    async function getLocations(plugin, script) {
      const pluginLocation =
          {rawModuleId: script.sourceURL, sourceFileURL: uiSourceCode.url(), lineNumber, columnNumber};
  ...
  async _loadScript(plugin, script) {
    ...
    try {
      const sourceFileURLs = await plugin.addRawModule(url, symbolsUrl, {url, code});
      for (const sourceFileURL of sourceFileURLs) {
        modelData._addUISourceCode(script, sourceFileURL);
      }
...
class ModelData {
  ...
  constructor(debuggerModel, workspace) {
    ...
    /** @type {!Map<!Workspace.UISourceCode.UISourceCode, !Set<!SDK.Script.Script>>} */
  ...
  _addUISourceCode(script, url) {
    ...
    if (!uiSourceCode) {
      ...
      const contentProvider = new SDK.CompilerSourceMappingContentProvider.CompilerSourceMappingContentProvider(
          url, Common.ResourceType.resourceTypes.SourceMapScript, script.createPageResourceLoadInitiator());
    ...
    // Bind the uiSourceCode to the script. The same
    // uiSourceCode can be provided by different scripts.
    let scripts = this._uiSourceCodeToScripts.get(uiSourceCode);
    if (!scripts) {
      scripts = new Set();
