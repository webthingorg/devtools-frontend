 ...
 */
class DebuggerLanguageExtensionPlugin {
  /**
   * @param {Extensions.ExtensionServer.LanguageExtensionEndpoint} extension
   */
  ...
  handleScript(script) {
    ...
    return !!language && !!script.debugSymbols && this._extension.supportsScript(language, script.debugSymbols.type);
  }
  ...
  */
  async addRawModule(rawModuleId, symbolsURL, rawModule) {
    try {
      return await this._extension.sendRequest('addRawModule', {rawModuleId, symbolsURL, rawModule});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
  ...
  async removeRawModule(rawModuleId) {
    try {
      return await this._extension.sendRequest('removeRawModule', {rawModuleId});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
  ...
  */
  async sourceLocationToRawLocation(sourceLocation) {
    try {
      return await this._extension.sendRequest('sourceLocationToRawLocation', {sourceLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
  ...
  */
  async rawLocationToSourceLocation(rawLocation) {
    try {
      return await this._extension.sendRequest('rawLocationToSourceLocation', {rawLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
  ...
  */
  async listVariablesInScope(rawLocation) {
    try {
      return await this._extension.sendRequest('listVariablesInScope', {rawLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
  ...
  */
  async evaluateVariable(name, location) {
    try {
      return await this._extension.sendRequest('evaluateVariable', {name, location});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error.message);
    }
  }
...
export class DebuggerLanguagePluginManager {
  ...
  constructor(debuggerModel, workspace, debuggerWorkspaceBinding) {
    ...
    for (const extension of self.Extensions.extensionServer.languageExtensionEndpoints) {
      this._plugins.push(new DebuggerLanguageExtensionPlugin(extension));
    }
  ...
  addPlugin(plugin) {
    ...
    scripts.forEach(script => this._newScriptSource(script));
  }

  _languageExtensionPluginAdded(event) {
    this.addPlugin(new DebuggerLanguageExtensionPlugin(event.data));
  }
  ...
  async rawLocationToUILocation(rawLocation) {
    ...
    try {
      ...
      const uiSourceCode = this._project.uiSourceCodeForURL(sourceLocation.sourceFile);
      if (!uiSourceCode) {
        return null;
      }
  ...
  async uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    async function getLocations(debuggerModel, plugin, sourceFile, script) {
      const pluginLocation =
          {rawModuleId: script.sourceURL, sourceFile: sourceFile, lineNumber: lineNumber, columnNumber: columnNumber};
  ...
  _bindUISourceCode(uiSourceCode, script, sourceFile) {
    ...
    if (entries) {
      entries.push([sourceFile, script]);
    } else {
  ...
  _unbindUISourceCode(uiSourceCode, deletedScript) {
    const remainingEntries =
        this._uiSourceCodes.get(uiSourceCode).filter(([sourceFile, script]) => script !== deletedScript);
  ...
  async _newScriptSource(script) {
    ...
    for (const plugin of this._plugins) {
      if (plugin.handleScript(script)) {
        this._pluginForScriptId.set(
            script.scriptId, (async () => {
              ...
              const sourceFiles = await plugin.addRawModule(script.sourceURL, symbolsUrl, rawModule);
              if (!sourceFiles) {
                return null;  // TODO(bmeurer): Throw an error here! Or handle above before putting into the Map.
              }

              for (const sourceFile of sourceFiles) {
                this._getOrCreateUISourceCode(sourceFile, script);
              }
