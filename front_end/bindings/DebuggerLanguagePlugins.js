...
export class DebuggerLanguagePluginManager {
  ...
  /* async */ rawLocationToUILocation(rawLocation) {
    ...
    const sourceFileURL =
        DebuggerLanguagePluginManager._makeUISourceFileURL(mapping[0].sourceFile, new URL(script.sourceURL).origin)
            .toString();
  ...
  async _newSourceMap(event) {
    ...
    for (const sourceFile of sourceFiles) {
      const sourceFileURL =
          DebuggerLanguagePluginManager._makeUISourceFileURL(sourceFile, new URL(script.sourceURL).origin).toString();
      this._getOrCreateUISourceCode(sourceFile, script, sourceFileURL);
    }
