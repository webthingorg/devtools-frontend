...
class EvalNode extends SDK.RemoteObject.RemoteObjectImpl {
  ...
  static async evaluate(callFrame, plugin, sourceType, base, field) {
    // TODO(bmeurer): This is a layering violation? Handle errors gracefully.
...
/** Front-end side handle to a raw module in a language plugin.
 * @typedef {{
 *            rawModuleId: string,
 ...
 */
// @ts-ignore typedef
let RawModuleHandle;  // eslint-disable-line no-unused-vars
...
export class DebuggerLanguagePluginManager {
  ...
  constructor(targetManager, workspace, debuggerWorkspaceBinding) {
    ...
    /** @type {!Map<string, !RawModuleHandle>} */
    ...
    for (const plugin of this._plugins) {
      ...
      if (!rawModuleHandle) {
        const sourceFileURLsPromise = (async () => {
          ...
          if (symbolsUrl) {
            console.log(ls`Loading debug symbols for ${url} (via ${symbolsUrl})...`);
          } else {
            console.log(ls`Loading debug symbols for ${url}...`);
          }
          try {
            ...
            console.log(ls`Loaded debug symbols for ${url} (${sourceFileURLs.length} source files)`);
            return sourceFileURLs;
          } catch (error) {
            console.error(ls`Failed to load debug symbols for ${url} (${error.message})`);
            this._rawModuleHandles.delete(rawModuleId);
            return [];
          }
        })();
