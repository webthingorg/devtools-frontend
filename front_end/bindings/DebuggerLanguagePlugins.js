...
export class DebuggerLanguagePluginManager {
  ...
  constructor(debuggerModel, workspace, debuggerWorkspaceBinding) {
    ...
    this._pluginPromiseForScriptId = new Map();
   ...
   */
  async _waitForPluginForScript(script) {
    if (this._pluginPromiseForScriptId.has(script.scriptId)) {
      return await this._pluginPromiseForScriptId.get(script.scriptId);
    }
   ...
   * @return {!Promise<?DebuggerLanguagePlugin>}
   */
  async _findOrWaitForPluginForScript(script) {
    if (this._pluginPromiseForScriptId.has(script.scriptid)) {
      return await this._pluginPromiseForScriptId.get(script.scriptId);
    }

    const pluginPromise = this._findPlugin(script);
    this._pluginPromiseForScriptId.set(script.scriptId, pluginPromise);
    const plugin = await pluginPromise;
    if (!plugin) {
      this._pluginPromiseForScriptId.delete(script.scriptId);
    }
    return plugin;
  }
  ...
  async rawLocationToUILocation(rawLocation) {
    ...
    const plugin = await this._waitForPluginForScript(script);
    if (!plugin) {
      return null;
    }
  ...
  async _newScriptSource(script) {
    if (!await this._findOrWaitForPluginForScript(script)) {
      return;
    }
  ...
  dispose() {
    ...
    this._pluginPromiseForScriptId.clear();
