...
class DebuggerLanguagePluginExtension {
  ...
  */
  async addRawModule(rawModuleId, symbolsURL, rawModule) {
    try {
      return await self.Extensions.extensionServer.sendLanguagePluginRequest(
          this._pluginId, 'addRawModule', {rawModuleId, symbolsURL, rawModule});
    } catch (error) {
  ...
  */
  async sourceLocationToRawLocation(sourceLocation) {
    try {
      return await self.Extensions.extensionServer.sendLanguagePluginRequest(
          this._pluginId, 'sourceLocationToRawLocation', {sourceLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error);
    }
  }
  ...
  */
  async rawLocationToSourceLocation(rawLocation) {
    try {
      return await self.Extensions.extensionServer.sendLanguagePluginRequest(
          this._pluginId, 'rawLocationToSourceLocation', {rawLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error);
    }
  }
  ...
  */
  async listVariablesInScope(rawLocation) {
    try {
      return await self.Extensions.extensionServer.sendLanguagePluginRequest(
          this._pluginId, 'listVariablesInScope', {rawLocation});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error);
    }
  }
  ...
  */
  async evaluateVariable(name, location) {
    try {
      return await self.Extensions.extensionServer.sendLanguagePluginRequest(
          this._pluginId, 'evaluateVariable', {name, location});
    } catch (error) {
      throw new DebuggerLanguagePluginError('EXTENSION_ERROR', error);
    }
  }
...
export class DebuggerLanguagePluginManager {
  ...
  async _getSourceFiles(script) {
    ...
    if (plugin) {
      ...
      console.error(`rawModule: ${JSON.stringify(rawModule)}`);
      if (!rawModule) {
        return null;
      }
      const symbolsFile = script.sourceMapURL || script.debugSymbols.externalURL || '';
      const sourceFiles = await plugin.addRawModule(script.scriptId, symbolsFile, rawModule);
      return sourceFiles;
    }
    return null;
  }
