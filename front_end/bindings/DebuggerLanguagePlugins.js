...
class SourceScopeRemoteObject extends SDK.RemoteObject.RemoteObjectImpl {
  ...
  async doGetProperties(ownProperties, accessorPropertiesOnly, generatePreview) {
    ...
    const properties = [];
    /** @type {!Map<string, !SDK.RemoteObject.RemoteObject>} */
    ...
    for (const variable of this.variables) {
      ...
      if (variable.nestedName && variable.nestedName.length > 1) {
        ...
        for (const nestedName of variable.nestedName) {
          let namespace = parent.get(nestedName);
          if (!namespace) {
            namespace = new NamespaceObject({});
            parent.set(nestedName, namespace);
          }
          parent = namespace.value;
        }
        const name = variable.nestedName[variable.nestedName.length - 1];
        parent.set(name, sourceVar);
      } else {
    ...
    for (const namespace of namespaces.keys()) {
      properties.push(
          makeProperty(namespace, /** @type {!SDK.RemoteObject.RemoteObject} */ (namespaces.get(namespace))));
    }
