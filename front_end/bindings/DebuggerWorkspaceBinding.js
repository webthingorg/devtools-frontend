...
import {DebuggerLanguagePlugins} from './DebuggerLanguagePlugins.js';
...
class ModelData {
  ...
  constructor(debuggerModel, debuggerWorkspaceBinding) {
    ...
    if (Root.Runtime.experiments.isEnabled('wasmDWARFDebugging')) {
      this._plugin = new DebuggerLanguagePlugins(debuggerModel, workspace, debuggerWorkspaceBinding);
      this._plugin.addPlugin(new CXXDWARFLanguagePlugin());
    }
  ...
  _rawLocationToUILocation(rawLocation) {
    let uiLocation = null;
    if (Root.Runtime.experiments.isEnabled('wasmDWARFDebugging')) {
      uiLocation = this._plugin.rawLocationToUILocation(rawLocation);
    }
  ...
  _uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    if (Root.Runtime.experiments.isEnabled('wasmDWARFDebugging')) {
      rawLocations = this._plugin.uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber);
    }
