...
class LinearMemoryInspectorView extends UI.Widget.VBox {
  static MEMORY_TRANSFER_MIN_CHUNK_SIZE = 1000;
  ...
  constructor(memoryWrapper: LazyUint8Array, address: number) {
    ...
    this.inspector.addEventListener('memory-request', ((event: MemoryRequestEvent) => {
    ...
    const memoryChunkStart = Math.max(0, address - LinearMemoryInspectorView.MEMORY_TRANSFER_MIN_CHUNK_SIZE / 2);
    const memoryChunkEnd = memoryChunkStart + LinearMemoryInspectorView.MEMORY_TRANSFER_MIN_CHUNK_SIZE;
  ...
  private memoryRequested(e: MemoryRequestEvent) {
    ...
    const chunkEnd = Math.max(end, start + LinearMemoryInspectorView.MEMORY_TRANSFER_MIN_CHUNK_SIZE);
