...
export class LinearMemoryNavigator extends HTMLElement {
    ...
    const result = html`
      ...
      <div class="navigator">
        <div class="navigator-item">
          ${this.createButton('ic_undo_16x16_icon', i18nString(UIStrings.goBackInAddressHistory), new HistoryNavigationEvent(Navigation.Backward), this.canGoBackInHistory)}
          ${this.createButton('ic_redo_16x16_icon', i18nString(UIStrings.goForwardInAddressHistory), new HistoryNavigationEvent(Navigation.Forward), this.canGoForwardInHistory)}
        </div>
        <div class="navigator-item">
  ...
  private createButton(name: string, title: string, event: Event, enabled = true): LitHtml.TemplateResult {
    const color = enabled ? 'var(--color-text-secondary)' : 'var(--color-background-highlight)';
    return html`
      <button class="navigator-button" ?disabled=${!enabled}
        data-button=${event.type} title=${title}
        @click=${this.dispatchEvent.bind(this, event)}>
        <devtools-icon .data=${{iconName: name, color, width: '14px'} as Components.Icon.IconWithName}>
        </devtools-icon>
      </button>`;
  }
}
