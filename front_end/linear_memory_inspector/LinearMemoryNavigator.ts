...
export class LinearMemoryNavigator extends HTMLElement {
    ...
    const result = html`
      ...
      <div class="navigator">
        <div class="navigator-item">
          ${this.createButton({name: 'ic_undo_16x16_icon', title: i18nString(UIStrings.goBackInAddressHistory), event: new HistoryNavigationEvent(Navigation.Backward), enabled: this.canGoBackInHistory})}
          ${this.createButton({name: 'ic_redo_16x16_icon', title: i18nString(UIStrings.goForwardInAddressHistory), event: new HistoryNavigationEvent(Navigation.Forward), enabled: this.canGoForwardInHistory})}
        </div>
        <div class="navigator-item">
          ${this.createButton({name: 'ic_page_prev_16x16_icon', title: i18nString(UIStrings.previousPage), event: new PageNavigationEvent(Navigation.Backward), enabled: true})}
          ${this.createAddressInput()}
          ${this.createButton({name: 'ic_page_next_16x16_icon', title: i18nString(UIStrings.nextPage), event: new PageNavigationEvent(Navigation.Forward), enabled: true})}
        </div>
        ${this.createButton({name: 'refresh_12x12_icon', title: i18nString(UIStrings.refresh), event: new RefreshRequestedEvent(), enabled: true})}
      </div>
      `;
  ...
  private createButton(data: {name: string, title: string, event: Event, enabled: boolean}): LitHtml.TemplateResult {
    ...
    return html`
      <button class="navigator-button" ?disabled=${!data.enabled}
        ...
        <devtools-icon .data=${{iconName: data.name, color, width: '14px'} as Components.Icon.IconWithName}>
        </devtools-icon>
      </button>`;
  }
}
