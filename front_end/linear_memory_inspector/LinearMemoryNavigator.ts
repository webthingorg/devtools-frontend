...
export class LinearMemoryNavigator extends HTMLElement {
    ...
    render(html`
      ...
      <div class="navigator">
        ...
        <div>
          ...
          <input contenteditable="true" value="0x${toHexString(this.address, 8)}"/>
  ...
  private createButton(name: string, event: Event) {
    return html`
      <button class="navigator-button" @click=${this.dispatchEvent.bind(this, event)}>
        <devtools-icon .data=${
        {iconName: name, color: 'rgb(110 110 110)', width: '16px'} as Elements.Icon.IconWithName}></devtools-icon>
      </button>`;
  }
}

customElements.define('devtools-linear_memory_inspector-navigator', LinearMemoryNavigator);

declare global {
  interface HTMLElementTagNameMap {
    'devtools-linear_memory_inspector-navigator': LinearMemoryNavigator;
  }
}
