...
import {GROUP_TO_TYPES, ValueTypeGroup} from './ValueInterpreterSettingsUtils.js';
      ...
      ${types.map(type => {
        return html`
          <label class="type-label" title=${type}>
            <input data-input="true" type="checkbox" .checked=${this.valueTypes.has(type)} @change=${(e: HTMLInputElementEvent) => this.onTypeToggle(type, e)}>
            <span data-title="true">${type}</span>
          </label>
     `;})}`;
  }

  private onTypeToggle(type: ValueType, e: HTMLInputElementEvent) {
    this.dispatchEvent(new TypeToggleEvent(type, e.target.checked));
  }
}
