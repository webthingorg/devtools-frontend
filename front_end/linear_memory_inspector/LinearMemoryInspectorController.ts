...
export class RemoteArrayWrapper implements LazyUint8Array {
  private _remoteArray: SDK.RemoteObject.RemoteArray;

  constructor(array: SDK.RemoteObject.RemoteArray) {
    this._remoteArray = array;
  }
   ...
   */
  length() {
    return this._remoteArray.length();
  }

  async getRange(start: number, end: number) {
    const newEnd = Math.min(end, this._remoteArray.length());
    if (start < 0 || newEnd >= this._remoteArray.length() || start > newEnd) {
  ...
  private async extractByteArray(start: number, end: number) {
    const promises = [];
    for (let i = start; i < end && i < this._remoteArray.length(); ++i) {
      // TODO: encode requested range in base64 string.
      promises.push(this._remoteArray.at(i).then(x => x.value));
    }
...
export class LinearMemoryInspectorController extends SDK.SDKModel.SDKModelObserver<SDK.RuntimeModel.RuntimeModel> {
  ...
  async openInspectorView(obj: SDK.RemoteObject.RemoteObject, address: number) {
    ...
    const object = await this.getObjectWithExtendedLifetime(obj);
    this.scriptIdToRemoteObject.set(scriptId, object);
