...
export class LinearMemoryViewer extends HTMLElement {
  ...
  private recomputeSize() {
    ...
    // Depending on that initially plotted row we can determine how many rows and bytes per rows we can fit:
  ...
  private render() {
    ...
    render(html`
      <style>
        ...
        .memory-inspector-byte-group-margin {
          margin-left: ${LinearMemoryViewer.BYTE_GROUP_MARGIN}px;
        }

        .memory-inspector-text-cell {
          min-width: 14px;
          color: var(--selected-cell-color);
        }
