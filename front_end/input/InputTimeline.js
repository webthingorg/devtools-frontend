...
import {TimelineLoader} from '../timeline/TimelineLoader.js';
import * as UI from '../ui/ui.js';

import {InputModel} from './InputModel.js';

/**
 * @implements {TimelineLoader.Client}
 * @unrestricted
 */
export class InputTimeline extends UI.Widget.VBox {
  ...
  _updateControls() {
    ...
    this._startReplayAction.setEnabled(this._isAvailableState(this._state) && !!this._tracingModel);
    ...
    this._loadButton.setEnabled(this._isAvailableState(this._state));
    this._saveButton.setEnabled(this._isAvailableState(this._state) && !!this._tracingModel);
  }
  ...
  async _saveToFile() {
    ...
    const fileName = `Profile-${new Date().toISO8601Compact()}.json`;
