...
export class InputModel extends SDK.SDKModel.SDKModel {
  ...
  _processThreadEvents(tracingModel: SDK.TracingModel.TracingModel, thread: SDK.TracingModel.Thread): void {
  ...
  _isValidInputEvent(eventData: EventData): boolean {
    return this._isMouseEvent(eventData as MouseEventData) ||
      this._isKeyboardEvent(eventData as KeyboardEventData);
  }
