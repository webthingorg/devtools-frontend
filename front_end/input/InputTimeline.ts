...
export class InputTimeline extends UI.Widget.VBox implements Timeline.TimelineLoader.Client {
  ...
  constructor() {
    ...
    this._toggleRecordAction = UI.ActionRegistry.ActionRegistry.instance().action('input.toggle-recording') as UI.ActionRegistration.Action;
    this._startReplayAction = UI.ActionRegistry.ActionRegistry.instance().action('input.start-replaying') as UI.ActionRegistration.Action;
    this._togglePauseAction = UI.ActionRegistry.ActionRegistry.instance().action('input.toggle-pause') as UI.ActionRegistration.Action;
  ...
  async _startRecording(): Promise<void> {
    ...
    this._tracingClient = new TracingClient(SDK.SDKModel.TargetManager.instance().mainTarget() as SDK.SDKModel.Target, this);
  ...
  loadingComplete(tracingModel: SDK.TracingModel.TracingModel | null): void {
    ...
    this._inputModel =
      new InputModel(SDK.SDKModel.TargetManager.instance().mainTarget() as SDK.SDKModel.Target);
