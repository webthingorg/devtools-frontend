...
const validPlaceholderPattern = /\{([a-zA-Z][a-zA-Z0-9]*)\}/g;
...
export function substitutePlaceholders(markdown: string, substitutions?: Map<string, string>): string {
  ...
  return markdown.replace(validPlaceholderPattern, (match, placeholder) => {
    const replacement = substitutions.get(placeholder);
