...
export function createIssueDescriptionFromMarkdown(description) {
  const rawMarkdown = self.Runtime.cachedResources[description.file];
  if (!rawMarkdown) {
    throw new Error(`File ${description.file} not found. Declare it as a resource in the module.json file`);
  }
