...
class ComboBoxOfCheckBoxes extends UI.Toolbar.ToolbarButton {
  ...
  constructor(title) {
    ...
    /** @type {!Array<!{title: string, func: () => void}>} */
    this.headers = [];
  }
   ...
   * @param {() => void} func
   */
  addHeader(headerName, func) {
    this.headers.push({title: headerName, func: func});
  }
  ...
  _showLevelContextMenu(event) {
    ...
    for (const {title, func} of this.headers) {
      this._contextMenu.headerSection().appendCheckboxItem(title, () => func());
    }
