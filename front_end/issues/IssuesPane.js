...
class AffectedMixedContentView extends AffectedResourcesView {
  ...
  _appendAffectedMixedContents(mixedContents) {
    ...
    for (const mixedContent of mixedContents) {
      if (mixedContent.request && self.SDK.networkLog.requestsForId(mixedContent.request.requestId).length) {
        self.SDK.networkLog.requestsForId(mixedContent.request.requestId).forEach(networkRequest => {
          this.appendAffectedMixedContent(
              /** @type{!{type:string,status:string,url:string,initiator:string}} */ (mixedContent), networkRequest);
          count++;
        });
      } else {
        this.appendAffectedMixedContent(
   ...
   */
  appendAffectedMixedContent(mixedContent, request = null) {
