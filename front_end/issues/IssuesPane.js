...
class AffectedResourcesView extends UI.TreeOutline.TreeElement {
  ...
  /**
   * This function resolves a frameId to ResourceTreeFrame. If the frameId does not resolve, a listener is installed
   * that takes care of updating the view if the frame is added. This is useful if the issue is added before the frame
   * gets reported.
  ...
  _createFrameCell(frameId) {
    ...
    frameUrl.classList.add('affected-resource-cell');
    if (frame) {
