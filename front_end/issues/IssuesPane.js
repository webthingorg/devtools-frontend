...
class IssueView extends UI.Widget.Widget {
  ...
  /**
   * @param {(boolean|undefined)=} uncollapse - Uncollapses the issue if `true`, collapses if `false`, toggles collapse if undefined
   */
  toggle(uncollapse) {
    if (uncollapse === undefined) {
      this.contentElement.classList.toggle('collapsed');
    } else {
      this.contentElement.classList.toggle('collapsed', !uncollapse);
    }
  }
...
export class IssuesPaneImpl extends UI.Widget.VBox {
   ...
   */
  handleSelect(issueView, force) {
    issueView.toggle(force);
  }
...
export class IssueRevealer {
  ...
  async reveal(issue) {
    if (!(issue instanceof SDK.Issue.Issue)) {
      return Promise.reject(new Error('Internal error: not a issue'));
    }
