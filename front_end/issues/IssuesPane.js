...
function CoepFrameResourceNeedsCoepHeaderMessage() {
  const message = createElementWithClass('div', 'message');
  message.textContent = ls
  `An iframe was embeded on a site which enables the cross-origin embedder policy, but the response headers for the iframe's document did not specify a cross-origin embedder policy, which causes the iframe to get blocked.
