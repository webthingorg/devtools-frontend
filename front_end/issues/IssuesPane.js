...
class AffectedResourcesView extends UI.TreeOutline.TreeElement {
  ...
  _createFrameCell(frameId, issue) {
    ...
    if (frame) {
      ...
      icon.classList.add('elements-panel');
      icon.onclick = async () => {
...
class IssueView extends UI.TreeOutline.TreeElement {
  ...
  _appendHeader() {
    ...
    header.classList.add('header');
    const icon = Elements.Icon.createIcon();
    icon.data = {iconName: 'breaking_change_icon', color: '', width: '14px', height: '15px'};
...
export class IssuesPaneImpl extends UI.Widget.VBox {
  ...
  _createToolbars() {
    ...
    breakingChangeIcon.data = {iconName: 'breaking_change_icon', color: '', width: '14px', height: '15px'};
