...
import {Conditions, MobileThrottlingConditionsGroup, NetworkThrottlingConditionsGroup, PlaceholderConditions, ThrottlingPresets} from './ThrottlingPresets.js';  // eslint-disable-line no-unused-vars
...
export class ThrottlingManager extends Common.ObjectWrapper.ObjectWrapper implements
  ...
  createMobileThrottlingButton(): UI.Toolbar.ToolbarMenuButton {
    ...
    const options: (Conditions|PlaceholderConditions)[] = [];
    let selectedIndex = -1;
    ...
    function populate(groups: MobileThrottlingConditionsGroup[]): (Conditions|PlaceholderConditions|null)[] {
      const options: (Conditions|PlaceholderConditions|null)[] = [];
