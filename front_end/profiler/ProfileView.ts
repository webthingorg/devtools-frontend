...
export class ProfileView extends UI.View.SimpleView implements UI.SearchableView.Searchable {
  ...
  initialize(nodeFormatter: Formatter): void {
    ...
    const options = new Map(viewTypes.map(type => [type, this.viewSelectComboBox.createOption((optionNames.get(type) as string), type)]));
  ...
  refreshVisibleData(): void {
    let child: (DataGrid.DataGrid.DataGridNode<any> | null) = this.dataGrid.rootNode().children[0];
    while (child) {
      child.refresh();
