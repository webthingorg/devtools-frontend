...
liveHeapProfileViewInstance: LiveHeapProfileView;
...
export class LiveHeapProfileView extends UI.Widget.VBox {
  ...
  // Transformation: updateAccessDeclarations
  private constructor() {
    super(true);
    ...
    this._toggleRecordAction = (UI.ActionRegistry.ActionRegistry.instance().action('live-heap-profile.toggle-recording') as UI.ActionRegistration.Action);
    this._toggleRecordButton = (UI.Toolbar.Toolbar.createActionButton(this._toggleRecordAction) as UI.Toolbar.ToolbarToggle);
    ...
    if (mainTarget && mainTarget.model(SDK.ResourceTreeModel.ResourceTreeModel)) {
      const startWithReloadAction = (UI.ActionRegistry.ActionRegistry.instance().action('live-heap-profile.start-with-reload') as UI.ActionRegistration.Action);
  ...
  _createDataGrid(): DataGrid.SortableDataGrid.SortableDataGrid<GridNode> {
    const 
    // Transformation: updateVariableDeclarations
    defaultColumnConfig: DataGrid.DataGrid.ColumnDescriptor = {
      id: '',
  ...
  async _poll(): Promise<void> {
    ...
    do {
      ...
      const profiles = await Promise.all(isolates.map(
      // Transformation: updateParameters
      isolate => {
  ...
  _update(isolates: SDK.IsolateManager.Isolate[], profiles: (Protocol.HeapProfiler.SamplingHeapProfile | null)[]): void {
    ...
    const 
    // Transformation: updateMissingGenericsInSets
    exisitingNodes = new Set<GridNode>();
    ...
    // Transformation: updateReturnType
    // Transformation: updateParameters
    function processNodeTree(isolate: SDK.IsolateManager.Isolate, parentUrl: string, node: Protocol.HeapProfiler.SamplingHeapProfileNode): void {
  ...
  _sortingChanged(): void {
    ...
    // Transformation: updateReturnType
    // Transformation: updateParameters
    function sortByUrl(a: DataGrid.SortableDataGrid.SortableDataGridNode<GridNode>, b: DataGrid.SortableDataGrid.SortableDataGridNode<GridNode>): number {
      return (b as GridNode)._url.localeCompare((a as GridNode)._url);
    }

    // Transformation: updateReturnType
    // Transformation: updateParameters
    function sortBySize(a: DataGrid.SortableDataGrid.SortableDataGridNode<GridNode>, b: DataGrid.SortableDataGrid.SortableDataGridNode<GridNode>): number {
  ...
  _startRecording(reload?: boolean): void {
    ...
    const resourceTreeModel = (mainTarget.model(SDK.ResourceTreeModel.ResourceTreeModel) as SDK.ResourceTreeModel.ResourceTreeModel | null);
...
let 
// Transformation: updateVariableDeclarations
profilerActionDelegateInstance: ActionDelegate;
