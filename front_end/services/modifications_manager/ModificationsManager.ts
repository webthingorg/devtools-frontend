...
export class ModificationsManager {
  ...
  #traceParsedData: TraceEngine.Handlers.Types.TraceParseData;
  #eventsSerializer: EventsSerializer.EventsSerializer.EventsSerializer;
  ...
  private constructor({traceParsedData, traceBounds, modifications}: ModificationsManagerData) {
    ...
    this.#eventsSerializer = new EventsSerializer.EventsSerializer.EventsSerializer();
  }
  ...
  applyModificationsIfPresent(): void {
    ...
    const expandableEntries = modifications.entriesModifications.expandableEntries.map(
        key => key.split('-').map(item => isNaN(parseInt(item, 10)) ? item : parseInt(item, 10)));
    if (!hiddenEntries.every(EventsSerializer.EventsSerializer.EventsSerializer.isTraceEventSerializableKey) ||
        !expandableEntries.every(EventsSerializer.EventsSerializer.EventsSerializer.isTraceEventSerializableKey)) {
