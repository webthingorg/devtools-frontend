...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper<EventTypes> implements
  ...
  #headerOverridesMap: Map<string, HeaderOverride[]> = new Map();
  ...
  async generateHeaderPatterns(uiSourceCode: Workspace.UISourceCode.UISourceCode, pattern: string):
    ...
    for (const headerOverride of headerOverrides) {
      ...
      // For patterns which explicitly match a folder's default index page, we
    ...
    this.#headerOverridesMap.set(patternPath, headerOverrides);
    return result;
  }
  ...
  doesUrlMatchPattern(queryUrl: string, patternUrl: string): boolean {
    ...
    // If the query ends with '/' or '/' directly followed by a query, insert
    // the default filename 'index.html' and check if this creates a match.
  ...
  splitUrlIntoSegments(url: string): string[] {
    const path = this.encodedPathFromUrl(url);
    const segments = path.split('/');
    // The empty string will match global overrides
    // (= overrides which are not domain-specific).
    segments.unshift('');
    return segments;
  }

  async handleHeaderInterception(interceptedRequest: SDK.NetworkManager.InterceptedRequest):
    ...
    // e.g. 'http?://', 'http?://www.example.com/', 'http?://www.example.com/path/', ...
    for (const segment of urlSegments) {
      path += segment + '/';
      const headerOverrides = this.#headerOverridesMap.get('http?:/' + path);
      if (headerOverrides) {
        for (const headerOverride of headerOverrides) {
          if (this.doesUrlMatchPattern(requestUrl, path + headerOverride.applyTo)) {
  ...
  private async interceptionHandler(interceptedRequest: SDK.NetworkManager.InterceptedRequest): Promise<void> {
    ...
    if (Root.Runtime.experiments.isEnabled(Root.Runtime.ExperimentName.HEADER_OVERRIDES)) {
      responseHeaders = await this.handleHeaderInterception(interceptedRequest);
    }
