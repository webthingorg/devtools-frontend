...
describeWithLocale('ContextMenuProvider', () => {
  ...
  it('passes along the "isEncoded" flag to the FileManager for "Save as"', async () => {
    ...
    menuProvider.appendApplicableItems(new Event('click'), contextMenu, contentProvider);
    ...
    await new Promise(r => setTimeout(r, 0));  // Tick once for ContentProvider.requestContent.

    assert.isTrue(saveStub.calledOnceWithExactly(
