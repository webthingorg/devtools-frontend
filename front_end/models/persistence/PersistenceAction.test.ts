...
describeWithLocale('ContextMenuProvider', () => {
  ...
  beforeEach(() => {
    ...
    sandbox.stub(Persistence.NetworkPersistenceManager.NetworkPersistenceManager, 'instance')
        .returns(sandbox.createStubInstance(Persistence.NetworkPersistenceManager.NetworkPersistenceManager));
    sandbox.stub(Host.InspectorFrontendHost.InspectorFrontendHostInstance, 'isHostedMode').returns(false);
    sandbox.stub(Host.InspectorFrontendHost.InspectorFrontendHostInstance, 'showContextMenuAtPoint');
  });
  ...
  it('passes along the "isEncoded" flag to the FileManager for "Save as"', async () => {
    ...
    sandbox.stub(event, 'target').value(document);
    ...
    const saveStub = sandbox.stub(Workspace.FileManager.FileManager.instance(), 'save');
