...
export class ContextMenuProvider implements UI.ContextMenu.Provider {
  ...
  private async redirectOverrideToDeployedUiSourceCode(
    ...
    const deployedText = Bindings.ResourceUtils.displayNameForURL(deployedUrl);

    const oriUrl = originalUiSourceCode.url();
    const oriText = Bindings.ResourceUtils.displayNameForURL(oriUrl);

    const warningMsg = i18nString(UIStrings.overrideSourceMappedFileWarning, {PH1: deployedText}) + '\n' +
        i18nString(UIStrings.overrideSourceMappedFileExplanation, {PH1: oriText});

    const shouldJumpToMinFile = await UI.UIUtils.ConfirmDialog.show(warningMsg);

    if (shouldJumpToMinFile) {
  ...
  private getDeployedUiSourceCode(uiSourceCode: Workspace.UISourceCode.UISourceCode):
    ...
    // Get the deployed source script from this source mapped file
    for (const script of debuggerWorkspaceBinding.scriptsForUISourceCode(uiSourceCode)) {
      const deployedUiSourceCode = debuggerWorkspaceBinding.uiSourceCodeForScript(script);
      if (!deployedUiSourceCode) {
        continue;
      }
    ...
    // Get the deployed source styles from this source mapped file
    for (const originURL of Bindings.SASSSourceMapping.SASSSourceMapping.uiSourceOrigin(uiSourceCode)) {
      const deployedUiSourceCode = Workspace.Workspace.WorkspaceImpl.instance().uiSourceCodeForURL(originURL) ||
          Workspace.Workspace.WorkspaceImpl.instance().uiSourceCodeForURL(
              Common.ParsedURL.ParsedURL.urlWithoutHash(originURL) as Platform.DevToolsPath.UrlString);
      return deployedUiSourceCode;
    }
