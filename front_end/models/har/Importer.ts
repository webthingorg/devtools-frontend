...
export class Importer {
  ...
  static setupTiming(
    ...
    // `blocked_queueing` should be excluded from `lastEntry` (`timings.blocked`) here because it should be taken
    // into account by `timing.requestTime`, and other subsequent timings are calculated based on the accumulated
    // `lastEntry`. Otherwise the total duration would be miscalculated by the time in `blocked_queueing`.
