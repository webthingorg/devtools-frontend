...
export class Importer {
  ...
  static setupTiming(
    ...
    // `blocked_queueing` should be excluded from `lastEntry` (`timings.blocked`) here because the queued time is taken into account in `timing.requestTime`, and other subsequent timings are calculated based on the accumulated `lastEntry`. Otherwise the total duration would be miscalculated by |blocked_queueing|.
