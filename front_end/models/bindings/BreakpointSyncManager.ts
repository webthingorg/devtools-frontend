...
export class BreakpointSyncManager implements SDK.TargetManager.SDKModelObserver<SDK.DebuggerModel.DebuggerModel> {
  ...
  modelAdded(debuggerModel: SDK.DebuggerModel.DebuggerModel): void {
    ...
    debuggerModel.setInstrumentationBreakCallback(this.onInstrumentationBreakpointHit.bind(this));
  }
  ...
  async onInstrumentationBreakpointHit(pausedDetails: SDK.DebuggerModel.DebuggerPausedDetails): Promise<void> {
    const {callFrames} = pausedDetails;
    const script = callFrames[0].script;
    const debuggerModel = script.debuggerModel;
    ...
    if (missingFiles && missingFiles.size > 0) {
      this.#debuggerModelToCallback.set(
          debuggerModel, this.#notifyInstrumentationBreakpointSyncFinished.bind(this, debuggerModel, pausedDetails));
      if (script.sourceMapURL) {
        this.#addSourceMapListeners(debuggerModel);
      }
    } else {
      return this.#notifyInstrumentationBreakpointSyncFinished(debuggerModel, pausedDetails);
    }
  }

  async #notifyInstrumentationBreakpointSyncFinished(
      debuggerModel: SDK.DebuggerModel.DebuggerModel,
      debuggerPausedDetails: SDK.DebuggerModel.DebuggerPausedDetails): Promise<void> {
    ...
    debuggerModel.breakpointsInitalizedOnInstrumentationBreak(debuggerPausedDetails, isAlsoRegularBreak);
  }
