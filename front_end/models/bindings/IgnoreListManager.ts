...
export class IgnoreListManager implements SDK.TargetManager.SDKModelObserver<SDK.DebuggerModel.DebuggerModel> {
  ...
  isIgnoreListedUISourceCode(
    ...
    return Boolean(url ? this.isUserIgnoreListedURL(url) || sourceMap?.hasIgnoreListHint(url) : false);
  }

  isUserOrSourceMapIgnoreListedURL(url: Platform.DevToolsPath.UrlString, options: {
    isContentScript?: boolean,
    sourceMap?: SDK.SourceMap.SourceMap|null,
  }): boolean {
    const userIgnoreListed = this.isUserIgnoreListedURL(url, options.isContentScript);
    const sourceMapIgnoreListed = options.sourceMap?.hasIgnoreListHint(url) ?? false;
