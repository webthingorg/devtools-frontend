...
export class DebuggerWorkspaceBinding implements SDK.TargetManager.SDKModelObserver<SDK.DebuggerModel.DebuggerModel> {
  ...
  private async beforePaused(
    ...
    if (!autoStep || debuggerPausedDetails.reason !== Protocol.Debugger.PausedEventReason.Step || !functionLocation ||
        !this.pluginManager || !frame.script.isWasm() || !Common.Settings.moduleSetting('wasmAutoStepping').get()) {
      return true;
    }
