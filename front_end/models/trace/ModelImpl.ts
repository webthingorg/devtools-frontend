...
export class Model extends EventTarget {
  ...
  async parse(traceEvents: readonly Types.Events.Event[], config?: ParseConfig): Promise<void> {
    ...
    try {
      ...
      this.#storeParsedFileData(file, this.#processor.parsedTrace, this.#processor.insightSets);
  ...
  #storeParsedFileData(
      file: ParsedTraceFile, data: Handlers.Types.ParsedTrace|null, insights: Insights.Types.InsightSets|null): void {
  ...
  traceInsights(index: number = this.#traces.length - 1): Insights.Types.InsightSets|null {
    if (!this.#traces[index]) {
      return null;
    }
...
export type ParsedTraceFile = Types.File.TraceFile&{
  ...
  traceInsights: Insights.Types.InsightSets | null,
};
