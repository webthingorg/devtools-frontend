 ...
 **/
export class Model<ModelHandlers extends {[key: string]: Handlers.Types.TraceEventHandler}> extends EventTarget {
  readonly #traces: ParsedTraceFile<ModelHandlers>[] = [];
  readonly #nextNumberByDomain = new Map<string, number>();

  readonly #recordingsAvailable: string[] = [];
  #lastRecordingIndex = 0;
  #processor: TraceProcessor<ModelHandlers>;

  static createWithAllHandlers(): Model<typeof Handlers.ModelHandlers> {
    return new Model(Handlers.ModelHandlers);
  }

  constructor(handlers: ModelHandlers) {
    super();
    this.#processor = new TraceProcessor({
      // We require that the Meta Handler is run in order to generate a name
      // for the trace, so let's ensure that we always run the Meta Handler.
  ...
  async parse(traceEvents: readonly Types.TraceEvents.TraceEventData[], config?: ParseConfig): Promise<void> {
    ...
    const file: ParsedTraceFile<ModelHandlers> = {
      traceEvents,
      metadata,
  ...
  #parsingComplete(file: ParsedTraceFile<ModelHandlers>, data: Handlers.Types.HandlerData<ModelHandlers>|null): void {
   ...
   */
  traceParsedData(index: number = this.#traces.length - 1): Handlers.Types.HandlerData<ModelHandlers>|null {
...
export type ParsedTraceFile<Handlers extends {[key: string]: Handlers.Types.TraceEventHandler}> = TraceFile&{
  traceParsedData: Handlers.Types.HandlerData<Handlers>| null,
};
