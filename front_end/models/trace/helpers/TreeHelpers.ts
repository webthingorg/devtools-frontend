...
export const makeEmptyTraceEntryNode =
    (entry: Types.TraceEvents.RenderererEntry, id: TraceEntryNodeId): TraceEntryNode => ({
      entry,
      id,
      parentId: null,
      children: new Set(),
...
export interface TraceEntryNode {
  entry: Types.TraceEvents.RenderererEntry;
  depth: number;
  id: TraceEntryNodeId;
 ...
 */
export function treify(entries: Types.TraceEvents.RenderererEntry[], options?: {
  filter: {has: (name: Types.TraceEvents.KnownEventName) => boolean},
}): {tree: TraceEntryTree, entryToNode: Map<Types.TraceEvents.RenderererEntry, TraceEntryNode>} {
  ...
  const entryToNode = new Map<Types.TraceEvents.RenderererEntry, TraceEntryNode>();
...
export function walkTreeFromEntry(
    entryToNode: Map<Types.TraceEvents.RenderererEntry, TraceEntryNode>,
    rootEntry: Types.TraceEvents.RenderererEntry,
    onEntryStart: (entry: Types.TraceEvents.RenderererEntry) => void,
    onEntryEnd: (entry: Types.TraceEvents.RenderererEntry) => void,
    ): void {
