...
type RootCauseProtocolInterface = {
  getInitiatorForRequest(url: string): Protocol.Network.Initiator|null,
  pushNodesByBackendIdsToFrontend(backendNodeIds: Protocol.DOM.BackendNodeId[]): Promise<Protocol.DOM.Node[]>,
...
export interface LayoutShiftRootCauses {
...
const rootCauseCacheMap = new Map<TraceEngine.TraceEvents.TraceEventLayoutShift, LayoutShiftRootCauses>();
...
*/
export async function rootCausesForEvent(
    providedProtocolInterface: RootCauseProtocolInterface, modelData: TraceParseData,
    event: TraceEngine.TraceEvents.TraceEventLayoutShift): Promise<Readonly<LayoutShiftRootCauses>|null> {
  const cachedResult = rootCauseCacheMap.get(event);
  if (cachedResult) {
    return cachedResult;
  }

  // Set a global variable for communicating with the page.
  // In general such a global would be problematic, but
  // 1) we do not expect concurrent calls and
  // 2) if concurrent calls do occur, it is fully expected they would be the same value for providedProtocolInterface
  // So let's save on passing this around everywhere by using a global.
  protocolInterface = providedProtocolInterface;
  ...
  allLayoutShifts.forEach(setDefaultValue);
  ...
  const resultForEvent = rootCauseCacheMap.get(event);
  if (!resultForEvent) {
...
function setDefaultValue(shift: TraceEngine.TraceEvents.TraceEventLayoutShift): void {
  Platform.MapUtilities.getWithDefault(rootCauseCacheMap, shift, () => {
    return {
      unsizedMedia: [],
      iframes: [],
      fontChanges: [],
 ...
 */
async function blameShifts(
    layoutShifts: TraceEngine.TraceEvents.TraceEventLayoutShift[],
    modelData: TraceParseData,
    ): Promise<void> {
  await linkShiftsToLayoutInvalidations(layoutShifts, modelData);
 ...
 * to layout shifts.
 */
async function linkShiftsToLayoutInvalidations(
  ...
  const nodes = await protocolInterface.pushNodesByBackendIdsToFrontend(backendNodeIds);
  const nodesByBackendIdMap = new Map<Protocol.DOM.BackendNodeId, Protocol.DOM.Node>();
  for (let i = 0; i < backendNodeIds.length; i++) {
    nodesByBackendIdMap.set(backendNodeIds[i], nodes[i]);
  }
  ...
  for (const layoutInvalidation of liEvents) {
    ...
    const fontChangeRootCause = getFontChangeRootCause(layoutInvalidation, nextPrePaint, modelData);
    const renderBlockRootCause = getRenderBlockRootCause(layoutInvalidation, nextPrePaint, modelData);
    const layoutInvalidationDOMNode = nodesByBackendIdMap.get(layoutInvalidation.args.data.nodeId);
    let unsizedMediaRootCause: UnsizedMedia|null = null;
    let iframeRootCause: InjectedIframe|null = null;
    if (layoutInvalidationDOMNode) {
      unsizedMediaRootCause =
          await getUnsizedMediaRootCause(layoutInvalidation.args.data.reason, layoutInvalidationDOMNode);
      iframeRootCause = await getIframeRootCause(layoutInvalidation.args.data.reason, layoutInvalidationDOMNode);
    }
    ...
    for (const shift of subsequentShifts) {
      const rootCausesForShift = Platform.MapUtilities.getWithDefault(rootCauseCacheMap, shift, () => {
        return {
          unsizedMedia: [],
          iframes: [],
          fontChanges: [],
 ...
 */
function linkShiftsToLayoutEvents(
    layoutShifts: TraceEngine.TraceEvents.TraceEventLayoutShift[], modelData: TraceParseData): void {
 ...
 */
async function getUnsizedMediaRootCause(
    reason: TraceEngine.TraceEvents.LayoutInvalidationReason,
    layoutInvalidationDOMNode: Protocol.DOM.Node): Promise<UnsizedMedia|null> {
  // Filter events to resizes only.
  if (reason !== TraceEngine.TraceEvents.LayoutInvalidationReason.SIZE_CHANGED) {
    return null;
  }
  const computedStylesList = await protocolInterface.getComputedStyleForNode(layoutInvalidationDOMNode.nodeId);
  const computedStyles = new Map(computedStylesList.map(item => [item.name, item.value]));
  if (computedStyles && !(await nodeIsUnfixedMedia(layoutInvalidationDOMNode, computedStyles))) {
    return null;
  }
  const authoredDimensions = await getNodeAuthoredDimensions(layoutInvalidationDOMNode);
  ...
  return {node: layoutInvalidationDOMNode, authoredDimensions, computedDimensions};
}
 ...
 */
function requestsInInvalidationWindow(
  ...
  for (let i = lastRequestIndex; i > -1; i--) {
    ...
    if (layoutInvalidation.ts - previousRequestEndTime < MAX_DELTA_FOR_FONT_REQUEST) {
      ...
      const initiator =
          protocolInterface.getInitiatorForRequest(previousRequest.args.data.url as Platform.DevToolsPath.UrlString);
 ...
 * returned instead.
 */
function getFontChangeRootCause(
    layoutInvalidation: TraceEngine.TraceEvents.TraceEventLayoutInvalidation,
  ...
  const fontRequestsInThisPrepaint =
      getFontRequestsInInvalidationWindow(requestsInInvalidationWindow(layoutInvalidation, modelData));
 ...
 * requests of them.
 */
function getFontRequestsInInvalidationWindow(requestsInInvalidationWindow: RootCauseRequest[]): FontChange[] {
  ...
  for (let i = 0; i < requestsInInvalidationWindow.length; i++) {
    ...
    const fontFace = protocolInterface.fontFaceForSource(fontRequest.request.args.data.url);
 ...
 *  returned instead.
 */
function getRenderBlockRootCause(
    layoutInvalidation: TraceEngine.TraceEvents.TraceEventLayoutInvalidation,
    nextPrePaint: TraceEngine.TraceEvents.TraceEventPrePaint, modelData: TraceParseData): RenderBlockingRequest[]|null {
  // Prevent computing the result of this function multiple times per PrePaint event.
  ...
  const renderBlocksInThisPrepaint =
      getRenderBlockRequestsInInvalidationWindow(requestsInInvalidationWindow(layoutInvalidation, modelData));
...
export function firstIframeInDOMTree(root: Protocol.DOM.Node): Protocol.DOM.Node|null {
 ...
 */
async function nodeMatchedStylesPropertyGetter(node: Protocol.DOM.Node):
    Promise<((property: string) => string | null)> {
  const response = await protocolInterface.getMatchedStylesForNode(node.nodeId);
 ...
 */
async function getNodeAuthoredDimensions(node: Protocol.DOM.Node): Promise<CSSDimensions> {
  const authoredDimensions: CSSDimensions = {};

  const cssMatchedRulesGetter = await nodeMatchedStylesPropertyGetter(node);
