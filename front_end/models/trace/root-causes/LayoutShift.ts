...
async function linkShiftsToLayoutInvalidations(
  ...
  for (const layoutInvalidation of liEvents) {
    ...
    const layoutInvalidationNode =
        layoutInvalidationNodeId ? await protocolInterface.getNode(layoutInvalidationNodeId) : null;
    ...
    if (layoutInvalidationNode) {
      unsizedMediaRootCause =
          await getUnsizedMediaRootCause(layoutInvalidation.args.data.reason, layoutInvalidationNode);
      iframeRootCause = await getIframeRootCause(layoutInvalidation.args.data.reason, layoutInvalidationNode);
    }
...
async function getUnsizedMediaRootCause(
  ...
  const computedStylesList = await protocolInterface.getComputedStyleForNode(layoutInvalidationNode.nodeId);
  ...
  const authoredDimensions = await getNodeAuthoredDimensions(layoutInvalidationNode);
