...
function getNonCompositedAnimations(animations: readonly Types.TraceEvents.SyntheticAnimationPair[]):
  ...
  for (const event of animations) {
    ...
    const statusEvents = event.args.data.statusEvents;
    if (statusEvents) {
      /**
       * Animation events containing composite information are ASYNC_NESTABLE_INSTANT ('n').
       * Since a trace can contain multiple 'n' events, we only want those with useful composite data.
       */
      const compositeEvents = statusEvents.filter(event => {
        return Boolean(event.args?.data.unsupportedProperties);
      });
      for (const event of compositeEvents) {
...
export function generateInsight(traceParsedData: RequiredData<typeof deps>, context: NavigationInsightContext):
    InsightResult<{animationFailures?: NoncompositedAnimationFailure[]}> {
