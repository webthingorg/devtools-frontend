...
function getNonCompositedFailureRootCauses(
    animationEvent: Types.Events.SyntheticAnimationPair,
  ...
  rootCausesByShift: Map<Types.Events.LayoutShift, LayoutShiftRootCausesData>,
  failures: NoncompositedAnimationFailure[],
} {
  /**
   * Animation events containing composite information are ASYNC_NESTABLE_INSTANT ('n').
   * An animation may also contain multiple 'n' events, so we look through those with useful non-composited data.
   */
  const failures = getNonCompositedFailure(animationEvent);
  if (!failures) {
    return {rootCausesByShift, failures: []};
  }

  const nextPrePaint = getNextPrePaintEvent(prePaintEvents, animationEvent);
  ...
  if (!isInInvalidationWindow(animationEvent, nextPrePaint)) {
    return {rootCausesByShift, failures};
  }
...
export function generateInsight(parsedTrace: RequiredData<typeof deps>, context: InsightSetContext): CLSInsightResult {
  ...
  compositeAnimationEvents.map(event => {
    const {failures} = getNonCompositedFailureRootCauses(event, prePaintEvents, shiftsByPrePaint, rootCausesByShift);
    allAnimationFailures.push(...failures);
  });

  // Sort by cumulative score, since for insights we interpret these for their "bad" scores.
  clusters?.sort((a, b) => b.clusterCumulativeScore - a.clusterCumulativeScore);

  return {
    animationFailures: allAnimationFailures,
