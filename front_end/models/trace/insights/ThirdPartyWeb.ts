...
function getSelfTimeByUrl(
    traceParsedData: RequiredData<typeof deps>, context: NavigationInsightContext): Map<string, number> {
  // TODO: improve api, we should already have this. context.navigation instead of context.navigationId
  const navigation = traceParsedData.Meta.navigationsByNavigationId.get(context.navigationId);
  if (!navigation) {
  ...
  for (const process of traceParsedData.Renderer.processes.values()) {
    ...
    for (const thread of process.threads.values()) {
      if (thread.name === 'CrRendererMain') {
        ...
        Helpers.TreeHelpers.walkEntireTree(traceParsedData.Renderer.entryToNode, thread.tree, event => {
          const node = traceParsedData.Renderer.entryToNode.get(event);
          if (!node || !node.selfTime) {
            return;
          }

          const url = Extras.URLForEntry.get(traceParsedData as Handlers.Types.TraceParseData, event);
          if (!url) {
            return;
          }
...
export function generateInsight(
  ...
  const selfTimeByUrl = getSelfTimeByUrl(traceParsedData, context);
