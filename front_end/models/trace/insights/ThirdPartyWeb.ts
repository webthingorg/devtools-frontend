    ...
    for (const thread of process.threads.values()) {
      if (thread.name === 'CrRendererMain') {
        ...
        for (const event of thread.entries) {
          if (!Helpers.Timing.eventIsInBounds(event, context.window)) {
            continue;
          }
...
export function generateInsight(
  ...
  const navigationUrl = context.type === 'with-navigation' ? context.navigation.args.data?.url : null;
  const firstPartyUrl = navigationUrl ?? traceData.Meta.mainFrameURL;
