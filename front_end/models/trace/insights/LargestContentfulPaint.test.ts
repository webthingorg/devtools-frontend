...
describe('LargestContentfulPaint', function() {
  ...
  it('image lcp attributes', async () => {
    ...
    const {imageLoadingAttr, imagePreloaded, imageFetchPriorityHint} =
        TraceModel.Insights.InsightRunners.LargestContentfulPaint.generateInsight(data, context);

    assert.strictEqual(imageLoadingAttr, '');
    assert.strictEqual(imagePreloaded, false);
    assert.strictEqual(imageFetchPriorityHint, 'auto');
  });
