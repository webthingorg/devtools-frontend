...
describe('ThirdPartyWeb', function() {
  it('categorizes third party web requests (simple)', async () => {
    ...
    assert.deepEqual(summaryResult, [
      ['localhost', {transferSize: 751}],
      ['Google Fonts', {transferSize: 0}],
    ]);
  });
  ...
  it('categorizes third party web requests (complex)', async () => {
    ...
    assert.deepEqual(summaryResult, [
      ['paulirish.com', {transferSize: 157130}],
      ['Google Tag Manager', {transferSize: 95375}],
      ['Google Fonts', {transferSize: 80003}],
      ['Google Analytics', {transferSize: 20865}],
      ['Disqus', {transferSize: 1551}],
      ['Firebase', {transferSize: 2847}],
    ]);
  });
});
