...
describe('RenderBlockingRequests', function() {
  ...
  it('finds render blocking requests', async () => {
    ...
    const {renderBlockingRequests} = TraceModel.Insights.InsightRunners.RenderBlockingRequests.generateInsight(context);
    assert.strictEqual(renderBlockingRequests.length, 2);
