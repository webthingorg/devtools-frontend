...
import {type NavigationInsightContext, type RequiredData, type SCSInsightResult} from './types.js';
...
export function generateInsight(
    traceParsedData: RequiredData<typeof deps>, context: NavigationInsightContext): SCSInsightResult {
  ...
  const selectorTimingsSortByElapsedMs = selectorTimings.toSorted((a, b) => {
    return b[SelectorTimingsKey.Elapsed] - a[SelectorTimingsKey.Elapsed];
  });

  // sort by match attempts
  const selectorTimingsSortByMatchAttempts = selectorTimings.toSorted((a, b) => {
  ...
  return {
    ...
    topElapsedMs: [...selectorTimingsSortByElapsedMs.slice(0, 3)],
    topMatchAttempts: [...selectorTimingsSortByMatchAttempts.slice(0, 3)],
  };
}
