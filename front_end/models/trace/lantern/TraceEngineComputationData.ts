...
type MetricName = TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName;
type SyntheticNetworkRequest = TraceModel.Types.TraceEvents.SyntheticNetworkRequest;
type TraceParseData = TraceModel.Handlers.Types.TraceParseData;

function createProcessedNavigation(traceEngineData: TraceParseData): Lantern.Simulation.ProcessedNavigation {
  ...
  const getTimestampOrUndefined = (metric: MetricName): TraceModel.Types.Timing.MicroSeconds|undefined => {
  ...
  const getTimestamp = (metric: MetricName): TraceModel.Types.Timing.MicroSeconds => {
    const metricScore = scores.get(metric);
    if (!metricScore?.event) {
  ...
  const FCP = TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.FCP;
  const LCP = TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MetricName.LCP;
  return {
    timestamps: {
...
function createLanternRequest(
    traceEngineData: TraceParseData, workerThreads: Map<number, number[]>,
    request: SyntheticNetworkRequest): Lantern.NetworkRequest|undefined {
 ...
 */
function chooseInitiatorRequest(request: Lantern.NetworkRequest, requestsByURL: Map<string, Lantern.NetworkRequest[]>):
    Lantern.NetworkRequest|null {
...
function linkInitiators(lanternRequests: Lantern.NetworkRequest[]): void {
  const requestsByURL: Map<string, Lantern.NetworkRequest[]> = new Map();
...
function createNetworkRequests(trace: Lantern.Trace, traceEngineData: TraceParseData): Lantern.NetworkRequest[] {
  const workerThreads = findWorkerThreads(trace);

  const lanternRequests: Lantern.NetworkRequest[] = [];
...
function collectMainThreadEvents(trace: Lantern.Trace, traceEngineData: TraceParseData): Lantern.TraceEvent[] {
  const Meta = traceEngineData.Meta;
...
function createGraph(
    requests: Lantern.NetworkRequest[], trace: Lantern.Trace, traceEngineData: TraceParseData,
    URL?: Lantern.Simulation.URL): TraceModel.Lantern.Node {
