...
interface FirstPaintBasedGraphOpts<T> {
   ...
   * render-blocking.
   */
  treatNodeAsRenderBlocking: (node: Lantern.NetworkNode<T>) => boolean;
  /**
   ...
   */
  additionalCpuNodesToTreatAsRenderBlocking?: (node: Lantern.CPUNode) => boolean;
}
...
class FirstContentfulPaint extends Lantern.Metric {
  ...
  static getRenderBlockingNodeData<T = unknown>(
    ...
    const scriptUrlToNodeMap = new Map<string, Lantern.CPUNode>();

    const cpuNodes: Lantern.CPUNode[] = [];
    graph.traverse(node => {
  ...
  static override getOptimisticGraph<T>(
      dependencyGraph: Lantern.Node<T>, processedNavigation: Lantern.Simulation.ProcessedNavigation): Lantern.Node<T> {
