...
import {type Node} from '../graph/BaseNode.js';
...
class LargestContentfulPaint extends Metric {
  ...
  static override getOptimisticGraph(dependencyGraph: Node, processedNavigation: Types.Simulation.ProcessedNavigation):
      Node {
  ...
  static override getPessimisticGraph(dependencyGraph: Node, processedNavigation: Types.Simulation.ProcessedNavigation):
      Node {
    const lcp = processedNavigation.timestamps.largestContentfulPaint;
    if (!lcp) {
      throw new Core.LanternError('NO_LCP');
