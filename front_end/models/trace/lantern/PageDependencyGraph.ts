...
type Node<T = any> = import('./BaseNode.js').Node<T>;
...
class PageDependencyGraph {
  ...
  static getNetworkNodeOutput(networkRequests: Lantern.NetworkRequest[]): NetworkNodeOutput {
    ...
    networkRequests.forEach(request => {
      ...
      if (request.frameId && request.resourceType === Lantern.NetworkRequestTypes.Document &&
  ...
  static createGraph(
    ...
    const rootRequest = Lantern.Simulation.NetworkAnalyzer.findResourceForUrl(networkRequests, requestedUrl);
    if (!rootRequest) {
