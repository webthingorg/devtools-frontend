...
type NetworkRequest = Lantern.NetworkRequest<TraceEvents.SyntheticNetworkRequest>;
...
function createProcessedNavigation(traceEngineData: TraceParseData): Lantern.Simulation.ProcessedNavigation {
  ...
  const getTimestampOrUndefined = (metric: MetricName): MicroSeconds|undefined => {
  ...
  const getTimestamp = (metric: MetricName): MicroSeconds => {
    const metricScore = scores.get(metric);
    if (!metricScore?.event) {
  ...
  return {
    timestamps: {
      firstContentfulPaint: getTimestamp(MetricName.FCP),
      largestContentfulPaint: getTimestampOrUndefined(MetricName.LCP),
    },
  };
}
...
function createLanternRequest(
    ...
    request: TraceEvents.SyntheticNetworkRequest): NetworkRequest|undefined {
...
function createGraph(
    ...
    url?: Lantern.Simulation.URL): Lantern.Node {
