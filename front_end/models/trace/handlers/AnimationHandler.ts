...
import {Phase} from '../types/TraceEvents.js';
import * as Types from '../types/types.js';

import {HandlerState} from './types.js';

const animations: Types.TraceEvents.TraceEventAnimation[] = [];
const animationsSynteticEvents: Types.TraceEvents.TraceEventSyntheticNestableAsyncEvent[] = [];

export interface AnimationData {
  animations: readonly Types.TraceEvents.TraceEventAnimation[];
  animationsSynteticEvents: readonly Types.TraceEvents.TraceEventSyntheticNestableAsyncEvent[];
}
let handlerState = HandlerState.UNINITIALIZED;

export function reset(): void {
  animations.length = 0;
  animationsSynteticEvents.length = 0;
}
...
export async function finalize(): Promise<void> {
  const matchedEvents = matchEndAndBegginingEvents();

  createSortedAnimationsSynteticEvents(matchedEvents);

  handlerState = HandlerState.FINALIZED;
}

function matchEndAndBegginingEvents(): Map<string, {
  ...
  for (const event of animations) {
    // not sure which id to use here
    const id = event.id2 || event.args.id;
...
function createSortedAnimationsSynteticEvents(matchedEvents: Map<string, {
  begin: Types.TraceEvents.TraceEventNestableAsyncBegin | null,
  end: Types.TraceEvents.TraceEventNestableAsyncEnd | null,
}>): void {
  for (const [id, eventsPair] of matchedEvents.entries()) {
    ...
    animationsSynteticEvents.push(event);
  }

  animationsSynteticEvents.sort((event1, event2) => {
