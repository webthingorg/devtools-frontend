...
export function handleEvent(event: Types.TraceEvents.TraceEventData): void {
  if (Types.TraceEvents.isTraceEventAnimation(event)) {
     ...
     * and ASYNC_NESTABLE_INSTANT ('n'). Since a trace can contain multiple 'n' events, we only
     * want those with useful composite data. We filter for these in order to create the
     * synthetic pairs appropriately in finalize().
     *
    ...
    const isComposite = event.ph === Types.TraceEvents.Phase.ASYNC_NESTABLE_INSTANT && event.args.data.compositeFailed;
    if (isStartAnimationEvent || isComposite) {
...
export function data(): AnimationData {
  ...
  return {
    ...
    compositeAnimations: compositeAnimationsSyntheticEvents,
  };
}
