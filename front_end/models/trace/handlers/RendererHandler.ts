...
export function makeCompleteEvent(event: Types.TraceEvents.TraceEventBegin|Types.TraceEvents.TraceEventEnd):
  ...
  if (Types.TraceEvents.isTraceEventEnd(event)) {
    ...
    if (!completeEventStack.length) {
      return null;
    }
    const top = completeEventStack.pop();
    if (!top) {
      return null;
    }
    if (top.name !== event.name || top.cat !== event.cat) {
      console.error(
          'B/E events mismatch at ' + top.ts + ' (' + top.name + ') vs. ' + event.ts + ' (' + event.name + ')');
    }
    top.dur = Types.Timing.MicroSeconds(event.ts - top.ts);
    return null;
  }
