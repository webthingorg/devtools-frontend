...
describe('InitiatorsHandler', () => {
  ...
  it('for a PostMessage Handler event the initiator is the PostMessage Dispatch event', async function() {
    ...
    const postMessageDispatchEvent = traceEvents.find(TraceModel.Types.TraceEvents.isTraceEventPostMessageDispatch);
    if (!postMessageDispatchEvent) {
      throw new Error('Could not find PostMessageDispatch event');
    }

    const postMessageHandlerEvent = traceEvents.find(TraceModel.Types.TraceEvents.isTraceEventPostMessageHandler);
    if (!postMessageHandlerEvent) {
      throw new Error('Could not find PostMessageHandler event');
    }

    assert.strictEqual(data.eventToInitiator.get(postMessageHandlerEvent), postMessageDispatchEvent);
    assert.deepEqual(data.initiatorToEvents.get(postMessageDispatchEvent), [postMessageHandlerEvent]);
  });
});
