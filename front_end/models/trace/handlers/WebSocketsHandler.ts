...
function createSyntheticWebSocketEvent(
    ...
    firstRecordedEvent: Types.TraceEvents.WebSocketEvent): Types.TraceEvents.SyntheticWebSocketEvent {
...
export async function finalize(): Promise<void> {
  ...
  webSocketData.forEach(data => {
    ...
    if (data.events.length > 0 && Types.TraceEvents.isSyntheticWebSocketEvent(data.events[0])) {
    ...
    const syntheticWebSocketEvent = createSyntheticWebSocketEvent(startEvent, endEvent, data.events[0]);
