...
const postMessageDispatchEventByTraceId: Map<number, Types.TraceEvents.TraceEventPostMessageDispatch> = new Map();
...
export function handleEvent(event: Types.TraceEvents.TraceEventData): void {
  if (Types.TraceEvents.isTraceEventScheduleStyleRecalculation(event)) {
    ...
    const linkifier = event.args.data?.traceId;
    if (linkifier) {
      postMessageDispatchEventByTraceId.set(linkifier, event);
    }
