...
export function handleEvent(event: Types.TraceEvents.TraceEventData): void {
  ...
  if (Types.TraceEvents.isTraceEventProfile(event)) {
    ...
    cdpProfile.startTime = Helpers.Timing.microSecondsToMilliseconds(event.ts);
    return;
  }
...
export interface SamplesHandlerData {
  profiles: Map<Types.TraceEvents.ProfileID, Partial<SamplesProfile>>;
  processes: Map<Types.TraceEvents.ProcessID, SamplesProcess>;
  // This is a redundant map which we'll have in the intermin state
  // as we finish crbug.com/1457230. In the future we will have this
  // map alone.
