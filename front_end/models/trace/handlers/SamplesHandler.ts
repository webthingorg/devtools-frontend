...
const eventByProfile = new Map<CPUProfile.CPUProfileDataModel.ExtendedProfile, Types.TraceEvents.TraceEventData>();
...
export function buildProfileCalls(): void {
  for (const [processId, profiles] of profilesInProcess) {
    for (const [profileId, profileData] of profiles) {
      const profileById = profilesInProcess.get(processId);
      const cdpProfile = profileById?.get(profileId);
      if (!profileById || !cdpProfile) {
        continue;
      }
...
export function data(): SamplesHandlerData {
  ...
  return {
    ...
    eventByProfile: new Map(eventByProfile),
  };
}
...
export interface SamplesHandlerData {
  ...
  eventByProfile: typeof eventByProfile;
}
