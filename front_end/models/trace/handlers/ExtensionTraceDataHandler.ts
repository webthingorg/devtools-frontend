...
export function extractExtensionEntries(
  ...
  for (const timing of timings) {
    ...
    if (!extensionName) {
      console.error('Extension track entry did not have an extension name', timing);
      continue;
    }
    const extensionSyntheticEntry = {
      ...
      dur: 0 as Types.Timing.MicroSeconds,
...
export function extensionDataInTiming(timing: Types.TraceEvents.SyntheticUserTimingPair|
  ...
  const timingDetail = Types.TraceEvents.isTraceEventPerformanceMark(timing) ? timing.args.detail :
