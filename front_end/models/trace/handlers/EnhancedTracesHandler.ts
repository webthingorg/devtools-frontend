...
function getEnhancedTracesScriptIdentifier(isolate: string, scriptId: number): string {
...
export function handleEvent(event: Types.TraceEvents.TraceEventData): void {
  ...
  if (Types.TraceEvents.isTraceEventTargetRundown(event)) {
    ...
    scriptToV8Context.set(getEnhancedTracesScriptIdentifier(data.isolate, data.scriptId), data.v8context);
    // Add target
    ...
    const data = event.args.data;
    const scriptIdentifier = getEnhancedTracesScriptIdentifier(data.isolate, data.scriptId);
    if (data.sourceText) {
      scriptToScriptSource.set(scriptIdentifier, data.sourceText);
    }
    if (data.length) {
...
export async function finalize(): Promise<void> {
  ...
  scriptRundownEvents.forEach(scriptRundownEvent => {
    ...
    const v8Context = scriptToV8Context.get(getEnhancedTracesScriptIdentifier(data.isolate, data.scriptId));
    if (v8Context) {
  ...
  scripts.forEach(script => {
    const scriptIdentifier = getEnhancedTracesScriptIdentifier(script.isolate, script.scriptId);
    script.sourceText = scriptToScriptSource.get(scriptIdentifier);
