...
describe('SelectorStatsHandler', () => {
  it('does nothing if the experiment is not enabled', async function() {
    // By default the experiment is disabled so we don't expect the handler to
    // populate the map of events.
    ...
    assert.strictEqual(data.dataForUpdateLayoutEvent.size, 0);
  });
  ...
  it('associates timings and stats with each UpdateLayoutEvent', async function() {
    ...
    config.experiments.captureStyleSelectorStats = true;
    TraceEngine.Handlers.ModelHandlers.SelectorStats.handleUserConfig(config);
