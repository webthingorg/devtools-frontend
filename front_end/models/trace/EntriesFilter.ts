...
export class EntriesFilter {
  ...
  #newActions: UserApplyFilterAction[] = [];
  ...
  /**
   * Adds an action (filter) to newActions or removes entries
  ...
  /**
   * The set of entries that are invisible given the set of applied actions. If no new actions are to be applied,
   * this will return the previously calculated invisibleEntries.
   *
  ...
  #calculateInvisibleEntries(): Types.TraceEvents.TraceEventData[] {
    // We apply each new user action in turn to the set of all entries, and mark
    ...
    for (const action of this.#newActions) {
      switch (action.type) {
    ...
    // Clear the list of new Actions - this invalidates the cache and
    // ensures that new invisible entries will be added to the invisible
    // list, which we have to do now we added another action.
    this.#newActions = [];
    this.#invisibleEntries = [...this.#invisibleEntries, ...entriesToHide];
