...
export class EntriesFilter {
  ...
  revealParent(index: number): boolean {
    // Check if the index is in hidden
    const entry = this.#invisibleEntries.find(entry => {
      return this.#entryToNode.get(entry)?.id === index;
    });
    if (entry) {
