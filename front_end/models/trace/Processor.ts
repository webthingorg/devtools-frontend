...
export class TraceProcessor<EnabledModelHandlers extends {[key: string]: Handlers.Types.TraceEventHandler}> extends
  ...
  async parse(traceEvents: readonly Types.TraceEvents.TraceEventData[], freshRecording = false): Promise<void> {
    ...
    // NOTE: `this.traceParsedData` is needed to compute insights, and that getter requires
    // `Status.FINISHED_PARSING`. Consider doing what `this.#computeTraceParsedData` does (sets #data)
  ...
  #createLanternContext(
    ...
    if (!traceEngineDataNarrowed.NetworkRequests.byTime.length) {
      throw new Error('no network requests found in trace');
    }
    ...
    const metrics = {
      ...
      totalBlockingTime: Lantern.Metrics.TotalBlockingTime.compute(computeData, {fcpResult, interactiveResult}),
    };
  ...
  #computeInsights(
    ...
    const enabledInsightRunners = this.#getEnabledInsightRunners(traceParsedData);
