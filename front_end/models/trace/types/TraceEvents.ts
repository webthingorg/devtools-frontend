...
export function objectIsTraceEventCallFrame(object: {}): object is CallFrame {
 ...
 * trace engine.
 */
export interface SyntheticCpuProfile extends Instant, SyntheticBasedEvent<Phase.INSTANT> {
  name: 'CpuProfile';
  args: Args&{
    data: ArgsData & {
...
export interface SyntheticNetworkRequest extends Complete, SyntheticBasedEvent<Phase.COMPLETE> {
...
export interface SyntheticWebSocketConnection extends Complete, SyntheticBasedEvent<Phase.COMPLETE> {
...
export interface SyntheticAuctionWorklet extends Instant, SyntheticBasedEvent<Phase.INSTANT> {
  rawSourceEvent: Event;
...
export interface SyntheticScreenshot extends Event, SyntheticBasedEvent {
...
export interface SyntheticLayoutShift extends LayoutShift, SyntheticBasedEvent<Phase.INSTANT> {
  name: 'LayoutShift';
...
export interface TargetRundown extends Event {
...
export function isTargetRundown(event: Event): event is TargetRundown {
  if (event.cat !== 'disabled-by-default-devtools.target-rundown') {
    return false;
  }
...
export interface ScriptRundown extends Event {
...
export function isScriptRundown(event: Event): event is ScriptRundown {
  if (event.cat !== 'disabled-by-default-devtools.v8-source-rundown') {
    return false;
  }
  const data = event.args?.data;
  if (!data) {
...
export interface ScriptRundownSource extends Event {
...
export function isScriptRundownSource(event: Event): event is ScriptRundownSource {
  if (event.cat !== 'disabled-by-default-devtools.v8-source-rundown-sources') {
    return false;
  }
  const data = event.args?.data;
  if (!data) {
...
export interface SyntheticBasedEvent<Ph extends Phase = Phase> extends Event {
  ph: Ph;
  rawSourceEvent: Event;
  _tag: 'SyntheticEntryTag';
}

export function isSyntheticBasedEvent(event: Event): event is SyntheticBasedEvent {
...
export interface SyntheticEventPair<T extends PairableAsync = PairableAsync> extends SyntheticBasedEvent {
  rawSourceEvent: Event;
  name: T['name'];
  cat: T['cat'];
  id?: string;
