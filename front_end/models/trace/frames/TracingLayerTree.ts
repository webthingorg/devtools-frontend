...
import {type LayerPaintEvent, type LayerPaintEventPicture} from './TimelineFrameModel.js';
...
export class TracingLayerTree extends SDK.LayerTreeBase.LayerTreeBase {
  ...
  setTiles(tiles: Types.TraceEvents.TracingLayerTile[]): void {
    this.tileById = new Map();
...
export class TracingLayer implements SDK.LayerTreeBase.Layer {
  ...
  async pictureForRect(targetRect: number[]): Promise<SDK.PaintProfiler.SnapshotWithRect|null> {
    const pictures = this.paints.map(paint => paint.picture());
    const filteredPictures = pictures
                                 .filter(function(pictureOrNull):
                                             pictureOrNull is LayerPaintEventPicture {
                                               return pictureOrNull !== null;
                                             })
                                 .filter(picture => {
                                   return rectsOverlap(picture.rect, targetRect);
                                 });

    const fragments =
        filteredPictures.map(picture => ({x: picture.rect[0], y: picture.rect[1], picture: picture.serializedPicture}));
