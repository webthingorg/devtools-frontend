...
// TODO: this event should likely be renamed to TraceParseProgressEvent as it
// does not represent the 'done' event now. The ModelImpl does that. We can
// keep the ModelImpl TraceParseEvent alone as that can still be emitted from
// the model, but we should change the Processor event to make it clear that
// it's just the update event.
processor.addEventListener(Processor.TraceParseEvent.eventName, (event: Event) => {
  const updateEvent = event as Processor.TraceParseEvent;
