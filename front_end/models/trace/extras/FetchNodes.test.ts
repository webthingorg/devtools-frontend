...
describeWithMockConnection('FetchNodes', function() {
  ...
  describe('DOMNodeLookup', function() {
    it('returns the DOM Node for the given node ID', async function() {
      ...
      const {traceParsedData: modelData} = await TraceLoader.traceEngine(this, 'cls-single-frame.json.gz');
  ...
  describe('nodeIdsForEvent', () => {
    it('identifies node ids for a Layout event', async function() {
      const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'web-dev-with-commit.json.gz');
    ...
    it('identifies node ids for a LayoutShift event', async function() {
      const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
    ...
    it('identifies node ids for a ScrollLayer event', async function() {
      ...
      const {traceParsedData: traceData} = await TraceLoader.traceEngine(this, 'extension-tracks-and-marks.json.gz');
