...
describeWithEnvironment('FilmStrip', function() {
  it('identifies the frames from a trace', async function() {
    ...
    const filmStrip = Trace.Extras.FilmStrip.fromTraceData(parsedTrace);
    assert.lengthOf(filmStrip.frames, 5);
  });

  it('caches the film strip based on the trace data and the zero time', async function() {
    const {parsedTrace} = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
    const filmStrip1 = Trace.Extras.FilmStrip.fromTraceData(parsedTrace);
    const filmStrip2 = Trace.Extras.FilmStrip.fromTraceData(parsedTrace);
    // It is from cache so you get back the exact same object.
    assert.strictEqual(filmStrip1, filmStrip2);

    const filmStrip3 = Trace.Extras.FilmStrip.fromTraceData(parsedTrace, Trace.Types.Timing.MicroSeconds(0));
    const filmStrip4 = Trace.Extras.FilmStrip.fromTraceData(parsedTrace, Trace.Types.Timing.MicroSeconds(5));
  ...
  it('can use a custom zero time to filter out screenshots', async function() {
    ...
    const newFilmStrip = Trace.Extras.FilmStrip.fromTraceData(parsedTrace, newCustomZeroTime);
