...
export class RecordingSession {
  ...
  bindingCalled(event: Common.EventTarget.EventTargetEvent): void {
    this._eventQueue.push({type: 'bindingCalled', event});
    this._processEvent();
  }
  ...
  async attachToTarget(target: SDK.SDKModel.Target): Promise<void> {
    ...
    const setupEventListeners =
        setupRecordingClient.toString() + ';setupRecordingClient({getAccessibleName, getAccessibleRole}, true);';

    const {identifier} = await pageAgent.invoke_addScriptToEvaluateOnNewDocument(
        {source: setupEventListeners, worldName: RECORDER_ISOLATED_WORLD_NAME});
  ...
  receiveWindowOpened(event: Common.EventTarget.EventTargetEvent): void {
    this._eventQueue.push({type: 'windowOpened', event});
    this._processEvent();
  }

  receiveWindowClosed(event: Common.EventTarget.EventTargetEvent): void {
    this._eventQueue.push({type: 'windowClosed', event});
    this._processEvent();
  }

  receiveNavigation(event: Common.EventTarget.EventTargetEvent): void {
    this._eventQueue.push({type: 'navigation', event});
    this._processEvent();
  }

  async _processEvent(): Promise<void> {
