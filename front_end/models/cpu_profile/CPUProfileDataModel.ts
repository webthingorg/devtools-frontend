...
export class CPUProfileDataModel extends ProfileTreeModel {
  ...
  forEachFrame(
    ...
    for (sampleIndex = startIndex; sampleIndex < samplesCount; sampleIndex++) {
      ...
      // If `prevNode` differs from `node`, the current sample indicates
      // was taken after a change in the call stack,
      // meaning that frames in the path of `prevNode` that differ from
      // those in the path of `node` can be closed.
      // So go down to the lowest common ancestor and close current intervals.
      //
      // For example:
      //
      // prevNode  node
