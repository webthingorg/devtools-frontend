...
export class SearchConfig implements ProjectSearchConfig {
  ...
  filePathMatchesFileQuery(filePath: Platform.DevToolsPath.RawPathString|
    ...
    return this.#fileRegexQueries.every(({regex, isNegative}) => (Boolean(filePath.match(regex)) !== isNegative));
  }
  ...
  static #parse(query: string, ignoreCase: boolean, isRegex: boolean):
    ...
    for (const queryPart of queryParts) {
      ...
      if (fileQuery) {
        ...
        fileRegexQueries.push({regex, isNegative: fileQuery.isNegative});
      } else if (isRegex) {
