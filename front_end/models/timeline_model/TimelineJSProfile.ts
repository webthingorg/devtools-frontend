...
export class TimelineJSProfileProcessor {
  ...
  static generateConstructedEventsFromCpuProfileDataModel(
    ...
    // Adds call stacks to fake trace events the tree in CPUProfileDataModel
   ...
   *
   * Because JavaScript profiling information comes in the shape of
   * samples, JS function call durations are deduced using their first
   * ancestor trace event duration, the timings of subsequent equal
   * samples
   * events,
   *
   * @param events merged ordered array of trace events and synthetic
   * "instant" events created for samples.
