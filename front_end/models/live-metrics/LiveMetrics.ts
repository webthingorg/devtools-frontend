...
export class LiveMetrics extends Common.ObjectWrapper.ObjectWrapper<EventTypes> implements SDK.TargetManager.Observer {
  ...
  private async getIsolatedContextId(): Promise<Protocol.Runtime.ExecutionContextId|null> {
    if (!this.target) {
      return null;
    }

    const resourceTreeModel = this.target.model(
  ...
  private async resolveDomNode(index: number): Promise<SDK.DOMModel.DOMNode|null> {
    if (!this.target) {
      return null;
    }
    ...
    const runtimeModel = (this.target.model(SDK.RuntimeModel.RuntimeModel) as SDK.RuntimeModel.RuntimeModel);
    const domModel = (this.target.model(SDK.DOMModel.DOMModel) as SDK.DOMModel.DOMModel);

    const remoteObject = runtimeModel.createRemoteObject(result);
    return domModel.pushObjectAsNodeToFrontend(remoteObject);
  }

  private async onBindingCalled(event: {data: Protocol.Runtime.BindingCalledEvent}): Promise<void> {
    const {data} = event;
