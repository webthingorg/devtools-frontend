...
export class StreamingContentData extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  private constructor(mimeType: string, charset?: string, initialContent?: ContentData) {
    ...
    this.#isComplete = initialContent !== undefined;
   ...
   */
  static createStreaming(mimeType: string, charset?: string): StreamingContentData {
    return new StreamingContentData(mimeType, charset);
  }

  /**
   * Creates a new StringContentData from an existing ContentData instance. Calling `addChunk`
   * on the resulting `StreamingContentData` causes an error.
   *
   * Useful when we aready have the complete data but a "StreamingContentData" is expected.
   *
   * The reason is that JavaScript TextEncoder only supports UTF-8. We can't convert text with
   * arbitrary encoding back to base64 for concatenation.
   */
  static createComplete(content: ContentData): StreamingContentData {
  ...
  addChunk(chunk: string): void {
    if (this.#isComplete) {
      throw new Error('Cannot add more data to already complete ContentData.');
    }
