...
export async function getFunctionNameViaExtensionOrCache(
    sourceMap: SDK.SourceMap.SourceMap, mappingEntry: SDK.SourceMap.SourceMapEntry,
    content: string): Promise<string|undefined> {
  let resolvedName;
  const extensionMangerInstance = await loadExtensionManager();

  if (extensionMangerInstance && mappingEntry.sourceURL) {
    ...
    let parsed = functionDescriptorsViaSourceUrlMap.get(mappingEntry.sourceURL);
    if (!parsed) {
      const extensions = extensionMangerInstance?.findFunctionParserExtensionsForFile(mappingEntry.sourceURL);
      if (extensions && extensions.length) {
        for (const extension of extensions) {
          parsed = await extension.getFunctionRanges(mappingEntry.sourceURL, content);

          if (parsed && parsed.length) {
            functionDescriptorsViaSourceUrlMap.set(mappingEntry.sourceURL, parsed);
            break;
          }
        }
      }
    ...
    if (parsed) {
      const best =
          sourceMap.findBestCandidate(parsed, mappingEntry.sourceLineNumber, mappingEntry.sourceColumnNumber) as
          Chrome.DevTools.FunctionDescriptor;
      if (best) {
