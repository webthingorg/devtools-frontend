...
const tryParseScope =
    async function(scopeText: string): Promise<[number, Formatter.FormatterWorkerPool.ScopeTreeNode]|null> {
  const prefixSuffixToTry = [
    ...
    // Then try async generator, this handles functions yield, await keywords.
  ...
  for (const {prefix, suffix} of prefixSuffixToTry) {
    ...
    if (scopeTree) {
      return [prefix.length, scopeTree];
    }
  }
  return null;
};
