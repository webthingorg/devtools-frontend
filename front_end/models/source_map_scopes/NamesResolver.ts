    ...
    const identifiersPromise =
        (async(): Promise<{variableMapping: Map<string, string>, thisMapping: string | null}> => {
          ...
          const resolveEntry = (id: IdentifierPositions, handler: (sourceName: string) => void): void => {
            // First see if we have a source map entry with a name for th eidentifier.
            ...
            async function resolvePosition(): Promise<void> {
              ...
              for (let i = 0; i < id.positions.length; ++i) {
                const sourceName = await resolveSourceName(script, sourceMap, id.name, id.positions[i], textCache);
                if (sourceName) {
                  handler(sourceName);
                  return;
                }
