...
interface ExternalExtensionManager {
  ...
  findBestCandidate(
      choices: Chrome.DevTools.FunctionDescriptor[], resolvedLineNumber: number, resolvedColumnNumber: number): Object
      |null;
}
    ...
    if (!parsed) {
      ...
      if (extensions && extensions.length) {
        for (const extension of extensions) {
          parsed = await extension.getFunctionRanges(
              script.sourceURL,
              content,
              mappingEntry,
              /** TODO: UnminificationMode */);
