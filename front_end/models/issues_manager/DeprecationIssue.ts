...
const
   ...
   */
           ...
           *    `supportsSession`, which returns a promise that resolves if the
           *    XR session can be supported and rejects if not, they should use
           *    `isSessionSupported` which will return a promise which resolves
           *    to a boolean indicating if the XR session can be supported or
           *    not, but may reject to throw an exception.
           */
          xrSupportsSession:
