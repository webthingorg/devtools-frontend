...
let formIssuesCheckTriggerInstance: FormIssuesCheckTrigger|null = null;

export class FormIssuesCheckTrigger {
  ...
  static instance({forceNew}: {forceNew: boolean} = {forceNew: false}): FormIssuesCheckTrigger {
    if (!formIssuesCheckTriggerInstance || forceNew) {
      formIssuesCheckTriggerInstance = new FormIssuesCheckTrigger();
    }

    return formIssuesCheckTriggerInstance;
  }
  ...
  #checkFormIssues(resourceTreeModel: SDK.ResourceTreeModel.ResourceTreeModel): void {
    // TODO(crbug.com/): Drop current issues in favor of the new one.
