...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper implements
  ...
  constructor(
    ...
    if (Root.Runtime.experiments.isEnabled('hideIssuesFeature')) {
      // We listen for events instead of changes to HideIssueSetting because the hideIssue function
      // can change the value of HideIssueSetting which leads to the change listener calling
      // the hideFilteredIssues function multiple times.
  ...
  private hideIssue(issue: Issue): void {
    ...
    // In case of hiding category or kind of issues, first we make sure the code of issue
    // being considered isn't already present in "code" section of our setting and it has the
    // IssueStatus.Hidden value. If not we check whether the category or kind of the issue being
    // considered has a IssueStatus.Hidden value. If it does then the code of the issue is added
    // to the "code" section of our setting and the issue gets hidden.
