...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper implements
  ...
  private hideIssue(issue: Issue): void {
    ...
    // All issues are hidden via their code despite of being hidden by either category, kind or code.
    // For hiding we check whether the issue code is present and has a value of IssueStatus.Hidden assosciated with it.
    // If all these conditions are met the issue is hidden. IssueStatus is set in hidden issues menu.
    // In case a user wants to hide a specific issue, the issue code is added to "code" section of our setting
    // and its value is set to IssueStatus.Hidden. Then issue then gets hidden.
    // In case of hiding category or kind of issues, first we make sure whether the issue being considered isn't already
    // added to "code" section of our setting and it has the IssueStatus.Hidden value. If not we check whether
    // the category or code of the issue being considered has a IssueStatus.Hidden value. If it does then the code
    // of the issue is added to the "code" section of our setting and the issue is hidden.
