...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper implements
  ...
  private updateHiddenIssuesCount(kind: IssueKind, increment: boolean): void {
    ...
    if (count && count <= 0) {
      this.hiddenIssuesCount.set(kind, 0);
      return;
    }
    this.hiddenIssuesCount.set(kind, count - 1);
  }
  ...
  private hideIssue(issue: Issue): void {
    ...
    if (values && values['code'][code]) {
      if (values['code'][code] === IssueStatus.Hidden) {
        ...
        this.updateHiddenIssuesCount(kind, true);
        return;
      }
