...
import {defaultHideIssueFilterSetting, defaultHideIssueMenuSetting, HideIssue} from './Issue.js';
...
import type {Issue, IssueKind, HideIssueFilterSetting, HideIssueMenuSetting} from './Issue.js';
...
export interface IssuesManagerCreationOptions {
  ...
  hideIssueMenuSetting?: Common.Settings.Setting<HideIssueMenuSetting>;
  hideIssueFilterSetting?: Common.Settings.Setting<HideIssueFilterSetting>;
}
...
export class IssuesManager extends Common.ObjectWrapper.ObjectWrapper implements
  ...
  private hiddenIssuesCount: number = 0;
  constructor(
      private readonly showThirdPartyIssuesSetting?: Common.Settings.Setting<boolean>,
      private readonly hideIssueMenuSetting?: Common.Settings.Setting<HideIssueMenuSetting>,
      private readonly hideIssueFilterSetting?: Common.Settings.Setting<HideIssueFilterSetting>) {
    super();
    ...
    if (Root.Runtime.experiments.isEnabled('hideIssuesFeature')) {
      this.hideIssueMenuSetting?.addChangeListener(() => this.hideFilteredIssues());
    }
  ...
  static instance(opts: IssuesManagerCreationOptions = {
    ...
    if (!issuesManagerInstance || opts.forceNew) {
      issuesManagerInstance =
          new IssuesManager(opts.showThirdPartyIssuesSetting, opts.hideIssueMenuSetting, opts.hideIssueFilterSetting);
    }
  ...
  addIssue(issuesModel: SDK.IssuesModel.IssuesModel, issue: Issue): void {
    ...
    if (this.issueFilter(issue)) {
      ...
      if (issue.isHidden()) {
        this.dispatchEventToListeners(Events.HideAddedIssue, {issuesModel, issue});
      } else {
  ...
  private assignHiddenIssueStatus(entry: string, issue: Issue): void {
    const statusSetting = this.hideIssueFilterSetting;
    const status = statusSetting?.get();
    const hideIssues = this.hideIssueMenuSetting?.get();
    const issueStatus = hideIssues ? hideIssues[entry] : undefined;
    if (!issueStatus) {
      return;
    }
    if (status && statusSetting) {
      if (issueStatus === HideIssue.Hide) {
        status[entry] = HideIssue.Hide;
        statusSetting.set(status);
        issue.setHidden(true);
        this.hiddenIssuesCount++;
        return;
      }
      // if (issueStatus === HideIssue.Unhide) {
      //   status[entry] = HideIssue.Unhide;
      //   statusSetting.set(status);
  ...
  private hideIssue(issue: Issue): void {
    if (Root.Runtime.experiments.isEnabled('hideIssuesFeature')) {
      ...
      const statusSetting = this.hideIssueFilterSetting;
      const status = statusSetting?.get();
      // Extra check for status is only because of this feature being experimental.
      if (status && status[code] && status[code] === HideIssue.Hide) {
        issue.setHidden(true);
        this.hiddenIssuesCount++;
        return;
      }
      // This is here only because of this feature being experimental.
      if (hideIssues) {
        this.assignHiddenIssueStatus(code, issue);
        this.assignHiddenIssueStatus(category, issue);
        this.assignHiddenIssueStatus(kind, issue);
      }
    }
  }
  ...
  showAllIssues(): void {
    this.hiddenIssuesCount = 0;
    this.hideIssueFilterSetting?.set(defaultHideIssueFilterSetting());
    this.hideIssueMenuSetting?.set(defaultHideIssueMenuSetting());
