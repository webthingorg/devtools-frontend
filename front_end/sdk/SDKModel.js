...
export class TargetManager extends Common.ObjectWrapper.ObjectWrapper {
  ...
  suspendAllTargets(reason) {
    ...
    return Promise.all(Array.from(this._targets.values(), target => target.suspend(reason)));
  }
  ...
  resumeAllTargets() {
    ...
    this._isSuspended = false;
    this.dispatchEventToListeners(Events.SuspendStateChanged);
    return Promise.all(Array.from(this._targets.values(), target => target.resume()));
  }
