...
export class OverlayModel extends SDKModel {
  ...
  async setInspectMode(mode, showDetailedTooltip = true) {
    ...
    if (this._inspectModeEnabled) {
      ...
      this.dispatchEventToListeners(Events.PersistentOverlayCleared);
    }
  }
...
export const Events = {
  ...
  PersistentOverlayCleared: Symbol('PersistentOverlayCleared'),
};
...
class DefaultPersistentGridHighlighter {
  ...
  highlightInOverlay(nodeId, config) {
    ...
    gridHighlights.set(nodeId, config);
  ...
  hideInOverlay(nodeId) {
    const gridHighlights = this._gridHighlights;
    if (gridHighlights.has(nodeId)) {
      gridHighlights.delete(nodeId);
      if (gridHighlights.size === 0) {
