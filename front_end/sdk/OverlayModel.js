...
export class OverlayModel extends SDKModel {
  ...
  // Used to ensure grid settings telemetry is only registered once
  static gridTelemetryRegistered = false;

  _registerGridSettingsTelemetry() {
    if (OverlayModel.gridTelemetryRegistered) {
      return;
    }
    ...
    OverlayModel.gridTelemetryRegistered = true;
  }
  ...
  _recordGridSettingChange(setting) {
    ...
    Host.userMetrics.cssGridSettingChanged(`${setting.name}.${setting.get()}`);
  }
