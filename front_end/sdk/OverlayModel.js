...
export class OverlayModel extends SDKModel {
  ...
  constructor(target) {
    ...
    if (this._gridFeaturesExperimentEnabled) {
      ...
      this._domModel.addEventListener(DOMModelEvents.NodeRemoved, event => {
        this._persistentGridHighlighter.hideInOverlay(/** @type {!SDK.DOMModel.DOMNode} */ (event.data.node));
      });
    }
  }
  ...
  highlightInOverlay(data, mode, showInfo) {
    if (this._isPersistentGridModeOn) {
      // Currently the backend doesn't support normal highlights
      // when persistent highlight is turned on.
      return;
    }
  ...
  highlightGridInPersistentOverlay(node) {
    this._persistentGridHighlighter.highlightInOverlay(node, this._buildGridHighlightConfig());
  }

  hideGridInPersistentOverlay(node) {
    this._persistentGridHighlighter.hideInOverlay(node);
  }
...
export class PersistentGridHighlighter {
   ...
   */
  highlightInOverlay(node, config) {
  }
...
class DefaultPersistentGridHighlighter {
   ...
   */
  highlightInOverlay(node, config) {
    const gridHighlights = this._gridHighlights;
    if (gridHighlights.size === 0) {
      this._model.setPersistentGridMode(true /* persistent mode on */);
    }
    gridHighlights.set(node, config);
  ...
  hideInOverlay(node) {
    const gridHighlights = this._gridHighlights;
    if (gridHighlights.has(node)) {
      gridHighlights.delete(node);
      if (gridHighlights.size === 0) {
        this._model.setPersistentGridMode(false /* persistent mode off */);
      }
      this._updateHighlightsInOverlay();
    }
  }
  ...
  _updateHighlightsInOverlay() {
    ...
    const hasGridToHighlight = this._gridHighlights.size > 0;

    overlayModel.setShowViewportSizeOnResize(!hasGridToHighlight);

    const gridNodeHighlightConfigs = [];
    for (const [node, gridHighlightConfig] of this._gridHighlights.entries()) {
      gridNodeHighlightConfigs.push({nodeId: node.id, gridHighlightConfig});
    }
