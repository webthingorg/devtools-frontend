...
export class EmulationModel extends SDKModel {
  ...
  constructor(target) {
    ...
    const colorVisionDeficiencySetting = self.Common.settings.moduleSetting('emulatedColorVisionDeficiency');
    colorVisionDeficiencySetting.addChangeListener(
        () => this._emulateColorVisionDeficiency(colorVisionDeficiencySetting.get()));
    if (colorVisionDeficiencySetting.get()) {
      this._emulateColorVisionDeficiency(colorVisionDeficiencySetting.get());
    }
   ...
   * @param {string} type
   */
  async _emulateColorVisionDeficiency(type) {
