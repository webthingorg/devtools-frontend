...
export class CSSProperty {
  ...
  static _formatStyle(styleText, indentation, endIndentation, tokenizerFactory) {
    ...
    function processToken(token, tokenType, column, newColumn) {
      ...
      if (token === '}' || token === ';') {
        // While it's tempting to trim `propertyText` unconditionally,
        // doing so breaks valid CSS declarations such as `--foo: ;`
        // which would then produce `--foo:;` which is invalid.
        // https://crbug.com/1071296
        result = result.trimRight() + indentation +
            (Platform.StringUtilities.isCssWhitespace(propertyText) ? propertyText : propertyText.trim()) + ';';
        needsSemi = false;
