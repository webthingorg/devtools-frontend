...
function formatFloat32(n) {
  ...
  const data = view.getInt32(0, true);
  const payload = data & 0x7FFFFF;
...
function formatFloat64(n) {
  ...
  const data1 = view.getUint32(0, true);
  const data2 = view.getInt32(4, true);
  const payload = data1 + (data2 & 0xFFFFF) * 4294967296;
...
let operatorCodeNamesCache = null;
function getOperatorName(code) {
  if (!operatorCodeNamesCache) {
    ...
    Object.keys(OperatorCodeNames).forEach(key => {
      const value = OperatorCodeNames[key];
...
class LineBuffer {
  ...
  get length() {
    switch (this._count) {
      ...
      default:
        return this._firstPart.length + this._secondPart.length + this._thirdPart.length;
    }
  }
  append(part) {
    switch (this._count) {
      ...
      default:
        this._count = 1;
        this._firstPart = this._firstPart + this._secondPart + this._thirdPart + part;
        break;
    }
  }
  finalize() {
...
export class WasmDisassembler {
  ...
  printFuncType(typeIndex) {
    const type = this._types[typeIndex];
  ...
  useLabel(depth) {
    ...
    const i = this._backrefLabels.length - depth - 1;
    if (i < 0) {
      return '' + depth;
    }
    const backrefLabel = this._backrefLabels[i];
    if (!backrefLabel.useLabel) {
      backrefLabel.useLabel = true;
      backrefLabel.label = this._nameResolver.getLabel(this._labelIndex);
      const line = this._lines[backrefLabel.line];
      this._lines[backrefLabel.line] =
          line.substring(0, backrefLabel.position) + ' ' + backrefLabel.label + line.substring(backrefLabel.position);
      this._labelIndex++;
    }
  ...
  printOperator(operator) {
    ...
    switch (code) {
      ...
      case 13 /* br_if */:
        ...
        break;
      case 14 /* br_table */:
        for (let i = 0; i < operator.brTable.length; i++) {
          this.appendBuffer(' ');
      ...
      case 16 /* call */:
        var funcName =
            this._nameResolver.getFunctionName(operator.funcIndex, operator.funcIndex < this._importCount, true);
  ...
  disassemble(reader) {
    ...
    if (this._addOffsets) {
      lines = lines.map((line, index) => {
        const position = formatHex(this._offsets[index], 4);
    ...
    while (true) {
      ...
      switch (reader.state) {
        ...
        case 14 /* TABLE_SECTION_ENTRY */:
          ...
          this.appendBuffer(
              `  (table ${tableName} ${limitsToString(tableInfo.limits)} ${typeToString(tableInfo.elementType)})`);
          this.newLine();
          break;
        case 17 /* EXPORT_SECTION_ENTRY */:
          ...
          switch (exportInfo.kind) {
            case 0 /* Function */:
              var funcName =
                  this._nameResolver.getFunctionName(exportInfo.index, exportInfo.index < this._importCount, true);
        ...
        case 12 /* IMPORT_SECTION_ENTRY */:
          ...
          switch (importInfo.kind) {
            ...
            case 1 /* Table */:
              ...
              var tableName = this._nameResolver.getTableName(this._tableCount++, false);
              this.appendBuffer(` (table ${tableName} ${limitsToString(tableImportInfo.limits)} ${
                  typeToString(tableImportInfo.elementType)})`);
              break;
            case 2 /* Memory */:
        ...
        case 22 /* START_SECTION_ENTRY */:
          ...
          var funcName =
              this._nameResolver.getFunctionName(startEntry.index, startEntry.index < this._importCount, true);
...
export class NameSectionReader {
  ...
  read(reader) {
    ...
    while (true) {
      ...
      switch (reader.state) {
        ...
        case 3 /* BEGIN_SECTION */:
          var sectionInfo = reader.result;
          if (sectionInfo.id === 0 /* Custom */ && bytesToString(sectionInfo.name) === 'name') {
            break;
          }
  ...
  getNameResolver() {
    ...
    for (let i = 0; i < functionNames.length; i++) {
      ...
      const goodName = !(name in usedNameAt) && !/[^0-9A-Za-z!#$%&'*+.:<=>?@^_`|~\/\-]/.test(name) &&
...
/** @constructor */
SDK.WasmDis = {};
SDK.WasmDis.WasmDisassembler = WasmDisassembler;
