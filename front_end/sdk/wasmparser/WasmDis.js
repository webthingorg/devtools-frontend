...
function getOperatorName(code) {
    if (!operatorCodeNamesCache) {
        ...
        Object.keys(OperatorCodeNames).forEach(function (key) {
            var value = OperatorCodeNames[key];
        ...
        while (true) {
            ...
            switch (reader.state) {
                ...
                case 34 /* ELEMENT_SECTION_ENTRY_BODY */:
                    ...
                    if (elementSegmentBody.elementType != 0) {
                        var typeName = typeToString(elementSegmentBody.elementType);
                        this.appendBuffer(" " + typeName);
                    }
                    elementSegmentBody.elements.forEach(function (funcIndex) {
                        ...
                        if (elementSegmentBody.asElements) {
                            if (funcIndex == NULL_FUNCTION_INDEX) {
                                ...
                                _this.appendBuffer(" (ref.func " + funcName + ")");
                            }
                        }
                        else {
            ...
            switch (reader.state) {
                ...
                case 19 /* NAME_SECTION_ENTRY */:
                    ...
                    functionNameInfo.names.forEach(function (naming) {
                        _this._functionNames[naming.index] = bytesToString(naming.name);
                    });
                    this._hasNames = true;
                    break;
                default:
