...
export class OverlayPersistentHighlighter {
   ...
   */
  colorOfFlex(nodeId) {
    let color = this._flexColors.get(nodeId);
    if (!color) {
      color = this._flexColorGenerator.next();
      this._flexColors.set(nodeId, color);
    }
  ...
  refreshHighlights() {
    const gridsNeedUpdate = this._checkDOMNodes(this._gridHighlights);
    const flexboxesNeedUpdate = this._checkDOMNodes(this._flexHighlights);
