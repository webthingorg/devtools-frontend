...
function textMessageWithResolutions(text, resolveMessage, resolutions) {
   ...
   */
  const styleCode = message => {
    return message.replace(/|(.*?)|/g, replacer);
  };
  const message = document.createElement('div');
  message.classList.add('message');
  const messageContent = document.createElement('p');
  messageContent.innerHTML = styleCode(text);
  ...
  if (resolutions.length > 0) {
    ...
    resolutionParagraphTextContent.innerHTML = styleCode(resolveMessage);
    ...
    for (const resolution of resolutions) {
      ...
      // We need a wrapper span to ensure correct spacing after the list icon.
      const wrapper = document.createElement('span');
      wrapper.innerHTML = styleCode(resolution);
...
const resolutionsSet = [
  ls`Specify |SameSite=None| and |Secure| if the cookie should be set in a cross-site context. Note that only cookies sent over HTTPS may use the |Secure| attribute.`,
...
const sameSiteUnspecifiedErrorSet = {
  title:
      ls`Indicate whether a cookie is inteded to be set by in a cross-site context by specifying its SameSite attribute`,
  message: () => textMessageWithResolutions(
...
const sameSiteUnspecifiedWarnSet = {
  title:
      ls`Indicate whether a cookie is inteded to be set by in cross-site context by specifying its SameSite attribute`,
  message: () => textMessageWithResolutions(
