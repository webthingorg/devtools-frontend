...
const schemefulSameSiteArticle = {
  link: ls`TODO`,
  linkTitle: ls`Schemeful SameSite explained`
};
...
function sameSiteWarnStrictLaxDowngradeStrict(isSecure) {
  ...
  return {
    ...
    message: () => textMessageWithResolutions(
        ...
        resolveBySentenceForDowngrade, resolutionsSet),
    issueKind: IssueKind.BreakingChange,
    links: [schemefulSameSiteArticle],
  };
}
...
function sameSiteWarnCrossDowngradeSet(isSecure) {
  ...
  return {
    ...
    message: () => textMessageWithResolutions(
        ls`A cookie is being set by ${origin} origin from ${
