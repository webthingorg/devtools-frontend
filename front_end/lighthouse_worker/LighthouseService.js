...
class LighthouseService {  // eslint-disable-line
  ...
  async fetchLocaleData(locales) {
    ...
    try {
      ...
      const localeDataTextPromise = Root.Runtime.loadResourcePromise(localeUrl).catch(() => {
        // Attempt to load a locale file from the local file system. CQ will always hit this code path.
        console.error(`Could not fetch locale at ${localeUrl}. Trying relative path.`);
        return Root.Runtime.loadResourcePromise(`../third_party/lighthouse/locales/${locale}.json`);
      });
