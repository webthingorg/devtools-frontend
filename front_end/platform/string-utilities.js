...
export const format = function(formatString, substitutions, formatters, initialValue, append, tokenizedFormat) {
  ...
  for (let i = 0; i < tokens.length; ++i) {
    ...
    if (!token.value && token.substitutionIndex && token.precision &&
    ...
    if (!token.value && token.substitutionIndex) {
      usedSubstitutionIndexes[token.substitutionIndex] = true;
    }

    if (!token.specifier || !(token.specifier in formatters)) {
      // Encountered an unsupported format character, treat as a string.
      warn('unsupported format character \u201C' + token.specifier + '\u201D. Treating as a string.');
      result =
          append(result, (token.value || !token.substitutionIndex) ? '' : actualSubstitutions[token.substitutionIndex]);
      continue;
    }
    ...
    result = append(
        ...
        formatters[token.specifier](
            token.value || token.substitutionIndex && actualSubstitutions[token.substitutionIndex], token));
  }
...
export const standardFormatters = {
  ...
  f: function(substitution, token) {
    if (substitution && token.precision && token.precision > -1) {
      substitution = substitution.toFixed(token.precision);
    }
    return !isNaN(substitution) ? substitution :
                                  (token.precision && token.precision > -1 ? Number(0).toFixed(token.precision) : 0);
  },
