...
class PuppeteerRunnerExtension extends RunnerExtension {
    ...
    async runStepInFrame(step, mainPage, targetPageOrFrame, localFrame, timeout) {
        ...
        switch (step.type) {
            case exports.StepType.DoubleClick:
                await puppeteer.Locator.race(step.selectors.map((selector) => {
                    return targetPageOrFrame.locator(selectorToPElementSelector(selector));
                }))
                    .setTimeout(timeout)
                    .on(puppeteer.LocatorEmittedEvents.Action, () => startWaitingForEvents())
                    .click({
                    count: 2,
...
function isNavigationStep(step) {
    ...
    return Boolean(step.type === exports.StepType.Navigate ||
        ((_a = step.assertedEvents) === null || _a === void 0 ? void 0 : _a.some((event) => event.type === exports.AssertedEventType.Navigation)));
}
