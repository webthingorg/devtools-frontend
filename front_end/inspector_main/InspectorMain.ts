...
export class InspectorMainImpl extends Common.ObjectWrapper.ObjectWrapper implements Common.Runnable.Runnable {
  ...
  async run(): Promise<void> {
    ...
    await SDK.Connections.initMainConnection(async () => {
      ...
      const waitForDebuggerInPage = type === SDK.SDKModel.Type.Frame && Root.Runtime.Runtime.queryParam('panel') === 'sources';
      const target = SDK.SDKModel.TargetManager.instance().createTarget('main', i18nString(UIStrings.main), type, null, undefined, waitForDebuggerInPage);
...
export class NodeIndicator implements UI.Toolbar.Provider {
  ...
  private constructor() {
    ...
    const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(element, { cssFile: 'inspector_main/nodeIcon.css', enableLegacyPatching: false, delegatesFocus: undefined });
    ...
    SDK.SDKModel.TargetManager.instance().addEventListener(SDK.SDKModel.Events.AvailableTargetsChanged, event => this._update((event.data as Protocol.Target.TargetInfo[])));
...
export class BackendSettingsSync implements SDK.SDKModel.Observer {
  _autoAttachSetting: Common.Settings.Setting<any>;
  _adBlockEnabledSetting: Common.Settings.Setting<any>;
  _emulatePageFocusSetting: Common.Settings.Setting<any>;
  constructor() {
