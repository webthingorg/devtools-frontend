...
export class Automapping {
  ...
  _onUISourceCodeRemoved(uiSourceCode) {
    if (uiSourceCode.project().type() === Workspace.Workspace.projectTypes.FileSystem) {
      ...
      const status = sourceCodeToAutoMappingStatusMap.get(uiSourceCode);
      if (status) {
        this._clearNetworkStatus(status.network);
      }
  ...
  _computeNetworkStatus(networkSourceCode) {
    if (sourceCodeToProcessingPromiseMap.has(networkSourceCode) ||
        sourceCodeToAutoMappingStatusMap.has(networkSourceCode)) {
      return;
    }
    ...
    sourceCodeToProcessingPromiseMap.set(networkSourceCode, createBindingPromise);
    ...
    async function validateStatus(status) {
      ...
      if (sourceCodeToProcessingPromiseMap.get(networkSourceCode) !== createBindingPromise) {
        return null;
      }
    ...
    function onStatus(status) {
      ...
      sourceCodeToProcessingPromiseMap.delete(networkSourceCode);
      if (!status) {
      ...
      if (sourceCodeToAutoMappingStatusMap.has(status.network) ||
          sourceCodeToAutoMappingStatusMap.has(status.fileSystem)) {
        return;
      }

      this._statuses.add(status);
      sourceCodeToAutoMappingStatusMap.set(status.network, status);
      sourceCodeToAutoMappingStatusMap.set(status.fileSystem, status);
  ...
  _clearNetworkStatus(networkSourceCode) {
    ...
    const status = sourceCodeToAutoMappingStatusMap.get(networkSourceCode);
    if (!status) {
      return;
    }

    this._statuses.delete(status);
    sourceCodeToAutoMappingStatusMap.delete(status.network);
    sourceCodeToAutoMappingStatusMap.delete(status.fileSystem);
    if (status.exactMatch) {
      const projectFolder = this._projectFoldersIndex.closestParentFolder(status.fileSystem.url());
      if (projectFolder) {
  ...
  _createBinding(networkSourceCode) {
    ...
    function onMetadatas() {
      ...
      const networkMetadata = sourceCodeToMetadataMap.get(networkSourceCode);
  ...
  _pullMetadatas(uiSourceCodes) {
    return Promise.all(uiSourceCodes.map(async file => {
      sourceCodeToMetadataMap.set(file, await file.requestMetadata());
    }));
  }
  ...
  _filterWithMetadata(files, networkMetadata) {
    return files.filter(file => {
      const fileMetadata = sourceCodeToMetadataMap.get(file);
      if (!fileMetadata) {
        return false;
      }
...
/** @type {!Map<!Workspace.UISourceCode.UISourceCode, !Promise<void>>} */
const sourceCodeToProcessingPromiseMap = new Map();

/** @type {!Map<!Workspace.UISourceCode.UISourceCode, !AutomappingStatus>} */
const sourceCodeToAutoMappingStatusMap = new Map();
