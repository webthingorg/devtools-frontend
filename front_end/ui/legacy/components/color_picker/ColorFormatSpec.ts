...
// The labels except `HEX` should always be 4 characters
// It represents each of the inputs. For example, we have
// 4 inputs for rgba colors: r, g, b, a and each character from "RGBA"
// represents one of the inputs.
type Label = 'HEX'|'RGBA'|'HSLA'|'HWBA'|'lchA'|'labA'|'xyzA';
...
export type SpectrumColorFormat = Exclude<
    Common.Color.Format,
    Common.Color.Format.Nickname|Common.Color.Format.RGBA|Common.Color.Format.HSLA|Common.Color.Format.HWBA|
    Common.Color.Format.HEXA|Common.Color.Format.ShortHEXA>;

const roundAndStringify = (arr: [number, number, number, number]): [string, string, string, string] =>
    arr.map(el => (Math.round(el * 100) / 100).toString()) as [string, string, string, string];
...
export const colorFormatSpec: Record<
    Exclude<SpectrumColorFormat, Common.Color.Format.HEX|Common.Color.Format.ShortHEX>, ColorFormatSpec> = {
  ...
  [Common.Color.Format.HSL]: {
    ...
    fromValues:
        function(values: [string, string, string, string]): Common.Color.Color |
        null {
