...
import * as CSSMetadata from '../../../../core/sdk/CSSMetadata.js';
...
export class CSSVarSwatch extends HTMLElement {
  ...
  private parseVariableFunctionParts(): ParsedVariableFunction|null {
    ...
    // editing is clicked.
    const result = this.text.replace(/\s{2,}/g, ' ').match(CSSMetadata.VariableRegex);
    if (!result) {
      return null;
    }
  ...
  private get variableName(): string {
    const match = this.text.match(CSSMetadata.VariableRegex);
    if (match) {
      return match[2];
    }
    return '';
