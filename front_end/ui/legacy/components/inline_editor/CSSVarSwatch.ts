...
export class CSSVarSwatch extends HTMLElement {
  ...
  private renderLink(variableName: string): {html: LitHtml.TemplateResult, title: string} {
    ...
    return {
      html: html`<span class=${classes} title=${title} @mousedown=${onActivate} @keydown=${
  ...
  private render(): void {
    ...
    // clang-format off
    render(
      html`<span title=${this.computedValue || ''}>${functionParts.pre}${variableNameLink.html}${fallbackIncludeComma}${functionParts.post}</span>`,
      this.shadow, { host: this });
    // clang-format on

    // Render Popover
    const cssVarLink = this.shadow.querySelector('.css-var-link') as HTMLElement;
    this.popoverHelper = new UI.PopoverHelper.PopoverHelper(cssVarLink, event => {
      ...
      return {
        ...
        show: async(popover: UI.GlassPane.GlassPane): Promise<boolean> => {
          ...
          popupElement.innerText = variableNameLink.title;
    ...
    this.popoverHelper.setTimeout(200, 110);
