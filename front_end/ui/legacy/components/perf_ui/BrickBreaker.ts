...
const colorPallettes: ColorPalette[] = [
  ...
  // pinks
  {
    light: 'rgb(230, 230, 250)',
    mediumLighter: 'rgb(238,190,236)',
    mediumDarker: 'rgb(230,159,227)',
    dark: 'rgb(253,77,246)',
  },
  // purples
  {
...
export class BrickBreaker extends HTMLElement {
  ...
  #drawPaddle(): void {
    ...
    // Shadow (offset slightly to the bottom left for top-right light)
    this.#ctx.beginPath();
    this.#ctx.rect(
        this.#paddleX - 3,                          // Offset slightly to the left
        this.offsetHeight - PADDLE_HEIGHT / 2 + 3,  // Offset downwards
        5,
        PADDLE_HEIGHT / 2,
    );
    this.#ctx.fill();
  ...
  #party(): void {
    ...
    const drawConfetti = (): void => {
      for (let i = 0; i < columnCount * rowCount; i++) {
        ...
        setTimeout(() => this.append(confettiContainerElement), Math.random() * 100);
        setTimeout(() => {
      ...
      window.alert('CONGRATULATIONS, YOU WIN!\n PS: You can also open the game by typing \'fixperf\'');
      this.#closeGame();
    };
    drawConfetti();
  }
