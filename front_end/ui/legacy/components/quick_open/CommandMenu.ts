...
export class CommandMenu {
  ...
  static createSettingCommand<V>(setting: Common.Settings.Setting<V>, title: Common.UIString.LocalizedString, value: V):
    ...
    const settingsThatNeedMetrics: {[key: string]: Host.UserMetrics.Action} = {
      'emulatePageFocus': Host.UserMetrics.Action.ToggleKeepPageFocusFromCommandMenu,
    };
    ...
    return CommandMenu.createCommand({
      ...
      executeHandler: (): void => {
        ...
        if (settingsThatNeedMetrics[setting.name]) {
          Host.userMetrics.actionTaken(Host.UserMetrics.Action.ToggleKeepPageFocusFromCommandMenu);
        }
