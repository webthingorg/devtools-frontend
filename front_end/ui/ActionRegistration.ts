...
export interface ActionRegistration {
  ...
  /**
   * Returns the classes that represent the 'context flavors' under which the action is available for triggering. The
   * context of the application is described in 'flavors' that are usually views added and removed to the context
   ...
   * In the case that an action has context types it depends on, and they haven't been loaded yet, the function
   * should be defined and return empty array, this will make the action unavailable as its flavors haven't been added
   * to the application context.
   *
   * The common pattern for implementing this function is relying on having the module with the corresponding context
   * types loaded and stored when the related 'view' extension is loaded asynchronously.
   */
