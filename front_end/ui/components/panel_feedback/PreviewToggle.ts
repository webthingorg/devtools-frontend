...
export class PreviewToggle extends HTMLElement {
  ...
  private name = '';
  private helperText: string|null = null;
  private feedbackURL: string|null = null;
  ...
  private render(): void {
    ...
    render(
      ...
      <div class="helper">
        ${
            this.helperText && this.feedbackURL ? html`<p>${this.helperText} <x-link href=${this.feedbackURL}>${
                                                      i18nString(UIStrings.previewTextFeedbackLink)}</x-link></p>` :
                                                  nothing}
      </div>
    `,
        this.shadow,
