...
import {calculateColumnWidth, Column, getRowEntryForColumnId, Row, SortState, Cell} from './DataGridUtils.js';
...
export class CellFocusEvent extends Event {
  data: {
   cell: Cell,
   position: { columnIndex: number, rowIndex: number}
  }

  constructor(cell: Cell, position: { columnIndex: number, rowIndex: number}) {
    super('cellFocus');
    this.data = {
      cell,
      position,
...
export class DataGrid extends HTMLElement {
  ...
  set data(data: DataGridData) {
    ...
    if (this.columns.some(col => col.sortable === true) && !this.hasRenderedAtLeastOnce) {
      // It's the first render and we just learned that at least one column is
      // sortable, so set the first focusable cell to be [0,0], which is the
      // first cell of the header.
      this.focusableCell = [0, 0];
    }
    this.render();
  }
  ...
  private getCurrentlyFocusableCell() {
    ...
    const cell = this.shadow.querySelector<HTMLTableCellElement>(`[data-row-index="${rowIndex}"][data-col-index="${columnIndex}"]`);
    return cell;
  ...
  private onTableKeyDown(event: KeyboardEvent) {
    const key = event.key;
    // console.log('key', key, event.key, key === '', key === ' ');
    ...
    if (key === 'ArrowLeft') {
      ...
      let newRowIndex = Math.min(selectedRowIndex + 1, this.rows.length);
      while (newRowIndex < this.rows.length) {
        const rowBelow = this.rows[newRowIndex];
        if (rowBelow && !rowBelow.hidden) {
          break;
        }
        newRowIndex++;
      }
      if (newRowIndex === this.rows.length + 1) {
        return;
      }
      this.focusCell(selectedColIndex, newRowIndex);
    }
          ...
          <tr>
            ${this.columns.map((col, columnIndex) => {
              ...
              return LitHtml.html`<th class=${thClasses}
                ...
                @focus=${() => {
                  // TODO: dispatchEvent's type needs to change - Cell | Column?
                  // this.dispatchEvent(new CellFocusEvent(entryForRow, {
                  //   columnIndex,
