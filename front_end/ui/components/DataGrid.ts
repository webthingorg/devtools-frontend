...
export class DataGrid extends HTMLElement {
  ...
  private render() {
    ...
    <style>
      ...
      /* TODO: this breaks if you hide the first column. Might have to apply a
      JS class to the first visible cells, and use that here. */
          ...
          <tr>
            ${this.columns.map((col, index) => {
              const thClasses = LitHtml.Directives.classMap({
                ...
                firstVisibleColumn: index === indexOfFirstVisibleColumn,
              });
              return LitHtml.html`<th class=${thClasses}
                data-grid-header-cell=${col.id}
                @click=${this.onColumnHeaderClick(col, index)}
                aria-sort=${this.ariaSortForHeader(col)}
