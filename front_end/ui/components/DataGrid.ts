...
export class DataGrid extends HTMLElement {
  ...
  private onTableKeyDown(event: KeyboardEvent) {
    ...
    if (key === 'ArrowLeft') {
      ...
      // ArrowDown needs more work: if the selectedRowIndex = 0, that means the
      // user is on the column headers, so down will move them onto row 1, which
      // is the body.
      let newRowIndexIncludingColumnRow = selectedRowIndex + 1;
      while (newRowIndexIncludingColumnRow < this.rows.length) {
        // Subtract one because this.rows contains only the body rows, and
        // newRowIndexIncludingColumnRow accounts for the column headers row as row 0.
