...
// TODO: pass this into css as a variable
const ROW_HEIGHT = 18;
...
export class DataGrid extends HTMLElement {
  ...
  private calculateTopAndBottomRowIndexes(): {topVisibleRow: number, bottomVisibleRow: number} {
    ...
    const padding = ROW_HEIGHT * PADDING_ROWS_COUNT;
    let topVisibleRow = Math.floor((scrollTop - padding) / ROW_HEIGHT);
    let bottomVisibleRow = Math.ceil((scrollTop + clientHeight + padding) / ROW_HEIGHT);
    ...
    this.scheduledRenderId = requestAnimationFrame(() => {
      ...
      <div class="wrapping-container" @scroll=${this.onScroll} @wheel=${this.onWheel}>
        ...
        >
          ...
          <tbody>
            <tr class="filler-row-top" style=${LitHtml.Directives.styleMap({
              height: `${topVisibleRow * ROW_HEIGHT}px`,
            })}></tr>
            ...
            <!-- TODO: fix empty table rendering -->
          <!--${this.renderFillerRow()}-->
            <tr class="filler-row-bottom" style=${LitHtml.Directives.styleMap({
              height: `${(this.rows.length - bottomVisibleRow) * ROW_HEIGHT}px`,
            })}></tr>
          </tbody>
        </table>
      </div>
      ...
      if (this.userHasFocused) {
        // TODO(jacktfranklin): move to a separate frame.
