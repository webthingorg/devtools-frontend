...
export type Row = RowEntry[]
    ...
    export function
    nextRowIndex(totalRows: number, currentIndex: number|null, key: 'ArrowDown'|'ArrowUp'):
        number|null {
          if (totalRows === 0) {
            return null;
          }
...
export function
getRowEntryForColumnId(row: Row, id: string):
    RowEntry {
      const rowEntry = row.find(r => r.columnId === id);
...
export function
calculateColumnWidth(allColumns: ReadonlyArray<Column>, columnId: string):
    number {
