...
import type {MemoryObjectInfo} from './LinearMemoryInspector.js';
...
export class LinearMemoryViewer extends HTMLElement {
  ...
  #renderByteValues(startIndex: number, endIndex: number): LitHtml.TemplateResult {
    ...
    for (let i = startIndex; i < endIndex; ++i) {
      ...
      const belongsToMemoryObject = this.#memoryObjectInfo.startAddress <= actualIndex
        && actualIndex < this.#memoryObjectInfo.startAddress + this.#memoryObjectInfo.size;
      const classMap = {
        'cell': true,
        'byte-cell': true,
        'byte-group-margin': addMargin,
        selected,
        belongsToMemoryObject,
      };
