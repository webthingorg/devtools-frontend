...
export class TreeOutline extends HTMLElement {
  ...
  /**
   * Recursively expands the tree from the root nodes, to a max depth.
   * @param maxDepth the depth at which to expand the tree to. Defaults to 2.
   */
  ...
  /**
   *
   * @param domNode the <li> element from the DOM.
   * @returns
   */
  ...
  private async processArrowKeyNavigation(key: Platform.KeyboardUtilities.ArrowKey, currentDOMNode: HTMLLIElement):
    ...
    const domNode = await coordinator.read<HTMLLIElement>(() => {
      return findNextNodeForTreeOutlineKeyboardNavigation({
        currentDOMNode,
        currentTreeNode,
        direction: key,
