...
import {findNextNodeForTreeOutlineKeyboardNavigation, getNodeChildren, getPathToTreeNode, isExpandableNode, listenToNodeBeingRendered, trackDOMNodeToTreeNode, TreeNode, TreeNodeWithChildren} from './TreeOutlineUtils.js';
...
export class TreeOutline<TreeNodeDataType> extends HTMLElement {
  ...
  // TODO: get rid of the Promise<> here this has to use regular Promise.then
  // rather than await so it stays fully async
  private renderNode(node: TreeNode<TreeNodeDataType>, {depth, setSize, positionInSet}: {
    ...
    return LitHtml.html`
      <li role="treeitem"
        ...
        listen-to-tree-node-being-rendered=${listenToNodeBeingRendered(node === this.nodePendingFocus, domNode => {
