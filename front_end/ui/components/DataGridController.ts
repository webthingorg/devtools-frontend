...
import {SortState, Column, nextRowIndex, Row, getRowEntryForColumnId} from './DataGridUtils.js';
...
export class DataGridController extends HTMLElement {
  ...
  private cloneAndFilterRows(rows: Row[], columns: Column[], filterText?: string): Row[] {
    ...
    if (filterTextIsColumnValue) {
      ...
      // TODO: fix this with the new hidden field
      if (!column) {
        // If we couldn't find a column, the filter is invalid, so let's return nothing.
        return rows.map(row => ({...row, hidden: true}));
      }
  ...
  private filterRowsByColumnValue(rows: Row[], column: Column, value: string): Row[] {
    return rows.map(row => {
      ...
      const rowMatchesFilter = entryForColumn && entryForColumn.value.toLowerCase().includes(value.toLowerCase());
      return {
        ...row,
        hidden: !rowMatchesFilter,
      };
    });
  }
