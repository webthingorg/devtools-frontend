...
export class Linkifier extends HTMLElement {
  ...
  async #render(): Promise<void> {
    ...
    // eslint-disable-next-line rulesdir/ban_a_tags_in_lit_html
    const withoutLinkText = LitHtml.html`<a class="link" href=${this.#url} @click=${this.#onLinkActivation}><slot>${
        LinkifierUtils.linkText(this.#url, this.#lineNumber)}</slot></a>`;
    // Disabled until https://crbug.com/1079231 is fixed.
    await coordinator.write(() => {
      LitHtml.render(this.#linkText ? withLinkText : withoutLinkText, this.#shadow, {host: this});
    });
  }
}
