...
export class TextEditor extends HTMLElement {
  ...
  #lastScrollPos: CodeMirror.StateEffect<unknown>|undefined;
  #resizeTimeout = -1;
  #resizeListener = (): void => {
  ...
  #createEditor(): CodeMirror.EditorView {
    this.#activeEditor = new CodeMirror.EditorView({
      ...
      scrollTo: this.#lastScrollPos,
    });

    this.#activeEditor.scrollDOM.addEventListener('scroll', () => {
      if (this.#activeEditor) {
        this.#lastScrollPos = this.#activeEditor.scrollSnapshot();
  ...
  connectedCallback(): void {
    if (!this.#activeEditor) {
      this.#createEditor();
    } else if (this.#lastScrollPos) {
      this.#activeEditor.dispatch({effects: this.#lastScrollPos});
    }
  }
