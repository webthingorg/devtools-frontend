...
export class TextEditor extends HTMLElement {
  ...
  #saveScrollPosition(editor: CodeMirror.EditorView, {scrollLeft, scrollTop}: {scrollLeft: number, scrollTop: number}):
    ...
    this.scrollEventHandledToSaveScrollPositionToRestoreForTest();
  }

  scrollEventHandledToSaveScrollPositionToRestoreForTest(): void {
  }

  connectedCallback(): void {
    if (!this.#activeEditor) {
      this.#createEditor();
    } else if (this.#options.restoreScrollPosition) {
      this.#restoreScrollPosition(this.#activeEditor);
    }
  }
