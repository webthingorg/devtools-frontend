...
export class TextEditor extends HTMLElement {
  ...
  private activeSettingListeners: [Common.Settings.Setting<any>, (event: {data: any}) => void][] = [];
  ...
  disconnectedCallback(): void {
    ...
    for (const [setting, handler] of this.activeSettingListeners) {
      setting.removeChangeListener(handler);
    }
  }
  ...
  private registerSettingHandlers(): void {
    ...
    for (const opt of this.state.facet(dynamicSetting)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const handler = ({data}: {data: any}): void => {
