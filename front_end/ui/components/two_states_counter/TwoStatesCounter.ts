...
export class TwoStatesCounter extends HTMLElement {
  ...
  #width: Readonly<string> = '14px';
  #height: Readonly<string> = '14px';
  ...
  #renderPart(count: number, classInfo: ClassInfo, title?: string): unknown {
    const styles = {
      width: this.#width,
      height: this.#height,
    };
    ...
    return count > 0 ? LitHtml.html`
       <div class=${LitHtml.Directives.classMap(classInfo)} style=${LitHtml.Directives.styleMap(styles)} title=${title}>
        <span>
          ${count}
        </span>
       </div>
