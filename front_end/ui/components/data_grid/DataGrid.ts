...
export class DataGrid extends HTMLElement {
  ...
  private onScroll(event: Event): void {
    ...
    // We track if the user last scrolled to the bottom so that we can resume
    // auto-scrolling when new data is added if their last scroll took them to
    // the bottom of the grid.
    const isAtBottom = element.scrollHeight - Math.abs(element.scrollTop) === element.clientHeight;
    this.userLastScrolledToBottom = isAtBottom;
    this.render();
  }
