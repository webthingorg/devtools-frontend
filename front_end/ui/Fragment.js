...
export class Fragment {
    ...
    while (walker.nextNode()) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE && node.hasAttributes()) {
        if (node.hasAttribute('$')) {
          ...
          binds.push({replaceNodeIndex: undefined, attr: undefined, elementId: node.getAttribute('$') || undefined});
  ...
  static _render(template, values) {
    ...
    for (let bindIndex = 0; bindIndex < template.binds.length; bindIndex++) {
      ...
      if ('elementId' in bind && bind.elementId !== undefined) {
        result._elementsById.set(bind.elementId, element);
      } else if ('replaceNodeIndex' in bind && bind.replaceNodeIndex !== undefined) {
      ...
      } else if ('attr' in bind && bind.attr !== undefined) {
