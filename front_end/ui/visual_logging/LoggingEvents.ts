...
export async function logClick(event: Event, doubleClick: boolean = false): Promise<void> {
  if (!(event instanceof MouseEvent)) {
    return;
  }
  const loggingState = getLoggingState(event.currentTarget as Element);
  const clickEvent:
      Host.InspectorFrontendHostAPI.ClickEvent = {veid: loggingState.veid, mouseButton: event.button, doubleClick};
