...
export function processEventForDebugging(name: string, config?: LoggingConfig, context?: string): void {
  if (!veDebuggingEnabled && !veDebugLoggingEnabled) {
    return;
  }
  const event = `${name}: ${config ? debugString(config) : ''}; ${context ? 'context: ' + context : ''}`;
