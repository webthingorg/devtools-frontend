...
let domProcessingThrottler: Common.Throttler.Throttler|null;
...
export function stopLogging(): void {
  ...
  documents.length = 0;
  domProcessingThrottler = null;
}

export function scheduleProcessing(): void {
  if (!domProcessingThrottler) {
    return;
  }
...
async function process(): Promise<void> {
  ...
  for (const {loggable, config, parent} of getNonDomState().loggables) {
    ...
    loggingState.impressionLogged = true;
    unregisterLoggable(loggable);  // No need to track loggable as soon as we've logged the impression
  }
