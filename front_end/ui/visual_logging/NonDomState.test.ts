...
describe('NonDomState', () => {
  it('registers, unregisters and returns loggables', () => {
    ...
    assert.sameDeepMembers(VisualLogging.NonDomState.getNonDomState(undefined).loggables, [
    ...
    VisualLogging.NonDomState.unregisterLoggables(undefined);
    assert.sameDeepMembers(VisualLogging.NonDomState.getNonDomState(undefined).loggables, []);
  });
  ...
  it('prevents external changes to the registry', () => {
    ...
    const loggables = VisualLogging.NonDomState.getNonDomState(undefined).loggables;
    loggables.pop();
