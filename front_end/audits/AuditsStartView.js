...
Audits.StartView = class extends UI.Widget {
  ...
  _populateRuntimeSettingAsToolbarCheckbox(settingName, toolbar) {
    ...
    if (runtimeSetting.disabled) {
      control.setEnabled(false);
      control.setChecked(runtimeSetting.setting._defaultValue, true);
    }
  ...
  _populateFormControls(fragment) {
    ...
    // UI.ARIAUtils.setAccessibleName(pluginFormElements, ls`Community Plugins`);
  }
  ...
  _render() {
    ...
    this._startButton = UI.createTextButton(
        ...
        true /* primary */);
    // Our button is special, and shouldn't have to fight all the default styles.
    this._startButton.classList.remove('text-button');
        ...
        <form>
          <div class="audits-form-section">
            <div class="audits-form-section-label">
              <div class="audits-icon-label">${ls`Categories`}</div>
            </div>
            <div class="audits-form-elements" $="categories-form-elements"></div>
          </div>
          <div style="display: none" class="audits-form-section">
            <div class="audits-form-section-label">
