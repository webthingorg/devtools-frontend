...
class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode<GridNode> {
    ...
    switch (columnId) {
      ...
      case 'size': {
        ...
        if (size !== null) {
          ...
          const sizeAccessibleName = (size === 1) ? i18nString(UIStrings.Byte) : i18nString(UIStrings.sBytes, { PH1: size });
  ...
  static sortFunctionForColumn(columnId: string): ((arg0: GridNode, arg1: GridNode) => number) | null {
    const nullToNegative = (x: any): number => x === null ? -1 : Number(x);
    switch (columnId) {
      case 'url':
      ...
      case 'initiator':
        return (a: GridNode, b: GridNode): number => (a.item.initiator.initiatorUrl || '').localeCompare(b.item.initiator.initiatorUrl || '');
      case 'errorMessage':
        return (a: GridNode, b: GridNode): number => (a.item.errorMessage || '').localeCompare(b.item.errorMessage || '');
      default:
