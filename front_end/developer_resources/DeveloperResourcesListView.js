...
export class DeveloperResourcesListView extends UI.Widget.VBox {
  /**
   * @param {function(!SDK.PageResourceLoader.PageResource):boolean} filterCallback
   */
  constructor(filterCallback) {
    ...
    const columns = [
      {id: 'status', title: ls`Status`, width: '20px', fixedWidth: true, sortable: true},
      {id: 'url', title: ls`URL`, width: '250px', fixedWidth: false, sortable: true},
      {id: 'frame', title: ls`Frame`, width: '40px', fixedWidth: false, sortable: true}, {
        id: 'size',
  ...
  update(items) {
    ...
    for (const item of items) {
      ...
      if (node) {
        if (this._filterCallback(node.item)) {
  ...
  updateFilterAndHighlight(highlightRegExp) {
    ...
    for (const node of this._nodeForItem.values()) {
      const shouldBeVisible = this._filterCallback(node.item);
    ...
    switch (columnId) {
      ...
      case 'status': {
        if (this.item.success !== null) {
          cell.textContent = this.item.success ? '✓' : '🗙';
        }
        break;
      }
      case 'size': {
