...
export class CoverageDecorationManager {
  ...
  async usageByLine(uiSourceCode) {
    ...
    for (let line = 0; line < sourceText.lineCount(); ++line) {
      ...
      const startLocations = await this._rawLocationsForSourceLocation(uiSourceCode, line, 0);
      const endLocations = await this._rawLocationsForSourceLocation(uiSourceCode, line, lineLength);
      let used = undefined;
