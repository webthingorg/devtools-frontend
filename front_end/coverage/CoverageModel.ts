...
export class CoverageModel extends SDK.SDKModel.SDKModel {
  ...
  _jsBacklog: {
    rawCoverageData: Array<Protocol.Profiler.ScriptCoverage>;
    stamp: number;
  }[];
  _cssBacklog: {
    rawCoverageData: Array<Protocol.CSS.RuleUsage>;
    stamp: number;
  }[];
  ...
  async _pollAndCallback(): Promise<void> {
    ...
    console.assert(this._suspensionState !== SuspensionState.Suspended || Boolean(this._performanceTraceRecording), 'CoverageModel was suspended while polling.');
    if (updates.length) {
  ...
  async suspendModel(reason?: string): Promise<void> {
  ...
  async _backlogOrProcessJSCoverage(freshRawCoverageData: Protocol.Profiler.ScriptCoverage[], freshTimestamp: number): Promise<CoverageInfo[]> {
    ...
    const ascendingByTimestamp = (x: {
      stamp: number;
    }, y: {
      stamp: number;
    }): number => x.stamp - y.stamp;
    const results = [];
  ...
  _processJSCoverage(scriptsCoverage: Protocol.Profiler.ScriptCoverage[], stamp: number): CoverageInfo[] {
    ...
    for (const entry of scriptsCoverage) {
      ...
      const subentry = this._addCoverage(script, script.contentLength, script.lineOffset, script.columnOffset, ranges, type as CoverageType, stamp);
      if (subentry) {
  ...
  async _backlogOrProcessCSSCoverage(freshRawCoverageData: Protocol.CSS.RuleUsage[], freshTimestamp: number): Promise<CoverageInfo[]> {
    if (freshRawCoverageData.length > 0) {
  ...
  _processCSSCoverage(ruleUsageList: Protocol.CSS.RuleUsage[], stamp: number): CoverageInfo[] {
    ...
    for (const entry of rulesByStyleSheet) {
      ...
      const subentry = this._addCoverage(styleSheetHeader, styleSheetHeader.contentLength, styleSheetHeader.startLine, styleSheetHeader.startColumn, ranges, CoverageType.CSS, stamp);
      if (subentry) {
        updatedEntries.push(subentry);
      }
    }
  ...
  _addStyleSheetToCSSCoverage(styleSheetHeader: SDK.CSSStyleSheetHeader.CSSStyleSheetHeader): void {
    this._addCoverage(styleSheetHeader, styleSheetHeader.contentLength, styleSheetHeader.startLine, styleSheetHeader.startColumn, [], CoverageType.CSS, Date.now());
  }

  _addCoverage(contentProvider: TextUtils.ContentProvider.ContentProvider, contentLength: number, startLine: number, startColumn: number, ranges: RangeUseCount[], type: CoverageType, stamp: number): CoverageInfo | null {
...
export class URLCoverageInfo extends Common.ObjectWrapper.ObjectWrapper {
  ...
  _ensureEntry(contentProvider: TextUtils.ContentProvider.ContentProvider, contentLength: number, lineOffset: number, columnOffset: number, type: CoverageType): CoverageInfo {
