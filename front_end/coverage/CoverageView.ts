...
export class CoverageView extends UI.Widget.VBox {
  ...
  _coverageTypeComboBoxSetting: Common.Settings.LegacySetting<any>;
  ...
  _showContentScriptsSetting: Common.Settings.LegacySetting<any>;
  ...
  private constructor() {
    ...
    this._coverageTypeComboBox = new UI.Toolbar.ToolbarComboBox(this._onCoverageTypeComboBoxSelectionChanged.bind(this), i18nString(UIStrings.chooseCoverageGranularityPer));
    const coverageTypes = [
      {
    ...
    this._toggleRecordAction = UI.ActionRegistry.ActionRegistry.instance().action('coverage.toggle-recording') as UI.ActionRegistration.Action;
    ...
    if (mainTargetSupportsRecordOnReload) {
      ...
      const startWithReloadAction = UI.ActionRegistry.ActionRegistry.instance().action('coverage.start-with-reload') as UI.ActionRegistration.Action;
    ...
    this._filterByTypeComboBox = new UI.Toolbar.ToolbarComboBox(this._onFilterByTypeChanged.bind(this), i18nString(UIStrings.filterCoverageByType));
    const options = [
      {
    ...
    this._contentScriptsCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(this._showContentScriptsSetting, i18nString(UIStrings.includeExtensionContentScripts), i18nString(UIStrings.contentScripts));
  ...
  _buildLandingPage(): UI.Widget.VBox {
    ...
    if (this._startWithReloadButton) {
      ...
      message = i18n.i18n.getFormatLocalizedString(str_, UIStrings.clickTheReloadButtonSToReloadAnd, { PH1: this._inlineReloadButton });
    }
    else {
  ...
  async _startRecording(options: {
    ...
    this._resourceTreeModel = mainTarget.model(SDK.ResourceTreeModel.ResourceTreeModel) as SDK.ResourceTreeModel.ResourceTreeModel | null;
    if (this._resourceTreeModel) {
      this._resourceTreeModel.addEventListener(SDK.ResourceTreeModel.Events.MainFrameNavigated, this._onMainFrameNavigated, this);
    }
  ...
  async stopRecording(): Promise<void> {
    if (this._resourceTreeModel) {
      this._resourceTreeModel.removeEventListener(SDK.ResourceTreeModel.Events.MainFrameNavigated, this._onMainFrameNavigated, this);
      this._resourceTreeModel = null;
    }
    if (this.hasFocus()) {
      this._listView.focus();
...
export class LineDecorator implements SourceFrame.SourceFrame.LineDecorator {
  ...
  makeGutterClickHandler(url: string): (arg0: Common.EventTarget.EventTargetEvent) => any {
