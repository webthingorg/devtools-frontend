...
export const UIStrings = {
  ...
  *@example {record button icon} RELOADBUTTON
  */
  clickTheReloadButtonSToReloadAnd: 'Click the reload button {RELOADBUTTON} to reload and start capturing coverage.',
  /**
  *@description Message in Coverage View of the Coverage tab
  *@example {record button icon} RECORD_BUTTON_ICON
  */
  clickTheRecordButtonSToStart: 'Click the record button {RECORD_BUTTON_ICON} to start capturing coverage.',
  /**
  *@description Footer message in Coverage View of the Coverage tab
  *@example {300k used, 600k unused} FORMATSTAT_FILTERED_
  *@example {500k used, 800k unused} FORMATSTAT_ALL_
  */
  filteredSTotalS: 'Filtered: {FORMATSTAT_FILTERED_}  Total: {FORMATSTAT_ALL_}',
  /**
  *@description Footer message in Coverage View of the Coverage tab
  *@example {1.5 MB} PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_USED_
  *@example {2.1 MB} PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_TOTAL_
  *@example {71%} PERCENTUSED
  *@example {29%} PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_UNUSED_
  */
  sOfSSUsedSoFarSUnused:
      '{PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_USED_} of {PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_TOTAL_} ({PERCENTUSED}%) used so far,\n        {PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_UNUSED_} unused.',
};
...
export class CoverageView extends UI.Widget.VBox {
  constructor() {
    ...
    this._coverageTypeComboBox = new UI.Toolbar.ToolbarComboBox(
        ...
        i18n.i18n.getLocalizedString(str_, UIStrings.chooseCoverageGranularityPer));
    const coverageTypes = [
      {
      ...
      {
        label: i18n.i18n.getLocalizedString(str_, UIStrings.perBlock),
    ...
    this._clearButton =
        new UI.Toolbar.ToolbarButton(i18n.i18n.getLocalizedString(str_, UIStrings.clearAll), 'largeicon-clear');
    ...
    this._saveButton =
        new UI.Toolbar.ToolbarButton(i18n.i18n.getLocalizedString(str_, UIStrings.export), 'largeicon-download');
    ...
    toolbar.appendSeparator();
    this._filterInput =
        new UI.Toolbar.ToolbarInput(i18n.i18n.getLocalizedString(str_, UIStrings.urlFilter), '', 0.4, 1);
    ...
    this._filterByTypeComboBox = new UI.Toolbar.ToolbarComboBox(
        this._onFilterByTypeChanged.bind(this), i18n.i18n.getLocalizedString(str_, UIStrings.filterCoverageByType));
    const options = [
      {
        label: i18n.i18n.getLocalizedString(str_, UIStrings.all),
        value: '',
      },
      {
        label: i18n.i18n.getLocalizedString(str_, UIStrings.css),
        value: CoverageType.CSS,
      },
      {
        label: i18n.i18n.getLocalizedString(str_, UIStrings.javascript),
        value: CoverageType.JavaScript | CoverageType.JavaScriptPerFunction,
      },
    ];
    ...
    const contentScriptsCheckbox = new UI.Toolbar.ToolbarSettingCheckbox(
        this._showContentScriptsSetting, i18n.i18n.getLocalizedString(str_, UIStrings.includeExtensionContentScripts),
        i18n.i18n.getLocalizedString(str_, UIStrings.contentScripts));
  ...
  _buildLandingPage() {
    ...
    if (this._startWithReloadButton) {
      ...
      message = i18n.i18n.getFormatLocalizedString(
          str_, UIStrings.clickTheReloadButtonSToReloadAnd, {RELOADBUTTON: this._inlineReloadButton});
    } else {
      const recordButton =
          UI.UIUtils.createInlineButton(UI.Toolbar.Toolbar.createActionButton(this._toggleRecordAction));
      message = i18n.i18n.getFormatLocalizedString(
          str_, UIStrings.clickTheRecordButtonSToStart, {RECORD_BUTTON_ICON: recordButton});
    }
  ...
  _updateStats() {
    ...
    this._statusMessageElement.textContent = filterApplied ?
        i18n.i18n.getLocalizedString(
            str_, UIStrings.filteredSTotalS,
            {FORMATSTAT_FILTERED_: formatStat(filtered), FORMATSTAT_ALL_: formatStat(all)}) :
        formatStat(all);
    ...
    function formatStat({total, unused}) {
      ...
      return i18n.i18n.getLocalizedString(str_, UIStrings.sOfSSUsedSoFarSUnused, {
        PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_USED_: Platform.NumberUtilities.bytesToString(used),
        PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_TOTAL_: Platform.NumberUtilities.bytesToString(total),
        PERCENTUSED: percentUsed,
        PLATFORM_NUMBERUTILITIES_BYTESTOSTRING_UNUSED_: Platform.NumberUtilities.bytesToString(unused)
      });
    }
  }
