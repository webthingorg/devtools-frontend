    ...
    switch (columnId) {
      ...
      case 'unusedSize': {
        ...
        const unusedPercentFormatted =
            i18nString(UIStrings.s, {PH1: this._percentageString(this._coverageInfo.unusedPercentage())});
      ...
      case 'bars': {
        ...
        if (this._coverageInfo.unusedSize() > 0) {
          ...
          if (this._coverageInfo.type() & CoverageType.JavaScriptPerFunction) {
            unusedSizeBar.title =
                i18nString(UIStrings.sBytesSBelongToFunctionsThatHave, {PH1: this._coverageInfo.unusedSize()});
          } else if (this._coverageInfo.type() & CoverageType.JavaScript) {
            unusedSizeBar.title = i18nString(UIStrings.sBytesSBelongToBlocksOf, {PH1: this._coverageInfo.unusedSize()});
          }
        }
        if (this._coverageInfo.usedSize() > 0) {
          ...
          if (this._coverageInfo.type() & CoverageType.JavaScriptPerFunction) {
            usedSizeBar.title =
                i18nString(UIStrings.sBytesSBelongToFunctionsThatHaveExecuted, {PH1: this._coverageInfo.usedSize()});
          } else if (this._coverageInfo.type() & CoverageType.JavaScript) {
            usedSizeBar.title =
                i18nString(UIStrings.sBytesSBelongToBlocksOfJavascript, {PH1: this._coverageInfo.usedSize()});
          }
        }
