...
class SnippetFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  ...
  initialFilePaths() {
    ...
    return savedSnippets.map(snippet => encodeURI(snippet.name));
  }
  ...
  async deleteFile(path) {
    const name = decodeURI(path.substring(1));
  ...
  async searchInPath(query, progress) {
    ...
    return snippets.map(snippet => encodeURI(snippet.name));
  }
   ...
   */
  tooltipForURL(url) {
    return ls`Linked to ${decodeURI(url.substring(this.path().length))}`;
  }
