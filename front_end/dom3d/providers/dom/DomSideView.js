...
Dom3d.DOMSideView = class extends UI.Widget {
  ...
  _createBodyContainer() {
    ...
    const nestingLevelContainer = this._getNestingLevelContent();
    const colorContainer = this._getColorContent();
    const isolateElementContainer = this._getIsolateElementContent();
    ...
    bodyContainer.appendChild(isolateElementContainer);
    bodyContainer.appendChild(showAllElementsContainer);
    bodyContainer.appendChild(nestingLevelContainer);
 ...
 */
  ...
  */
  ...
  _getIsolateElementContent() {
    const includeChildrenCheckbox = UI.CheckboxLabel.create(ls`Include children`, true);
    this._includeChildrenCheckbox = includeChildrenCheckbox.checkboxElement;

    const includeParentsCheckbox = UI.CheckboxLabel.create(ls`Include parents`, true);
  ...
  _onColorRadioButtonChanged(type) {
    if (this._rendererModel) {
      this._rendererModel.getDomSceneModel().makeHeatmap(type);
    }
  }
