...
Dom3d.Scene = class extends Dom3d.BaseScene {
  constructor(engine, canvas, contentElement, domModel, updateNestingLevelParentFunc, retakeSnapshotParentFunc) {
    ...
    this._boxTree = null;        // { name: "", children: [], parent:{} }
  ...
  */
  initializeScene(content) {
    ...
    this._boxTree = this.createNameTree(this._content);
  ...
  initializeTexture(imageData) {
    ...
    if (imageData) {
      ...
      this._texture = BABYLON.Texture.CreateFromBase64String(imageData, name, this.scene, false);
  ...
  makeHeatmap(colorSchemeType) {
    ...
    } else if (colorSchemeType === Dom3d.ColorSchemeTypes.Purple) {
      colorIteratorForHeatMap = this.colorHelper.purpleToWhiteToPink();
    } else if (colorSchemeType === Dom3d.ColorSchemeTypes.Gradient) {
  ...
  isElementAllowedInScene(element) {
    return this.isAValidNumericUnit(element.width) && this.isAValidNumericUnit(element.height) &&
