...
Dom3d.ZIndexSideView = class extends UI.Widget {
  ...
  /**
   * Creates the ui for the toolbar
   *
   * @returns {!Element}
   */
  _createToolbarContainer() {
    const toolbarContainer = createElementWithClass('div', 'toolbar');
    const toolbar = new UI.Toolbar('', toolbarContainer);
    const refreshButton = new UI.ToolbarButton(ls`Take new snapshot`, 'largeicon-refresh');
    refreshButton.addEventListener(UI.ToolbarButton.Events.Click, this._onRefreshButtonClicked, this);
    const resetButton = new UI.ToolbarButton(ls`Reset elements in view and center camera`, 'largeicon-rotate');
    resetButton.addEventListener(UI.ToolbarButton.Events.Click, this._onResetButtonClicked, this);
    toolbar.appendToolbarItem(refreshButton);
    toolbar.appendSeparator();
    toolbar.appendToolbarItem(resetButton);
  ...
  _createBodyContainer() {
    ...
    bodyContainer.appendChild(this._getHighlightStackingCtxContent());
    bodyContainer.appendChild(this._getShowAllElementsContent());
    bodyContainer.appendChild(this._getShowOnlyElementsWithStackingCtxContent());
    bodyContainer.appendChild(this._getIsolateElementContent());
    bodyContainer.appendChild(this._getColorContent());
    bodyContainer.appendChild(this._getHideChildrenWithSamePaintOrderContent());
   ...
   * @returns {?Element}
   */
  _getIsolateElementContent() {
    const isolateCheckboxShowParents = UI.CheckboxLabel.create(ls`Show all parents`, true);
   ...
   * @returns {?Element}
   */
  _getShowAllElementsContent() {
    const showAllElementsButton =
        UI.createTextButton(ls`Show all elements`, this._onShowAllButtonClicked.bind(this), 'Dom3d-show-all', false);
   ...
   * @returns {?Element}
   */
  _getShowOnlyElementsWithStackingCtxContent() {
    const showOnlyCtxButton = UI.createTextButton(
        ls`Show only elements with stacking context`, this._onShowOnlyStackingCtxButtonClicked.bind(this),
        'Dom3d-show-ctx', false);
    const showOnlyCtxContainer = createElementWithClass('div', 'Dom3d-section');
    showOnlyCtxContainer.appendChild(showOnlyCtxButton);

    return showOnlyCtxContainer;
  }

  /**
   * @returns {?Element}
   */
  _getHighlightStackingCtxContent() {
    const highlightCheckbox = UI.CheckboxLabel.create(ls`Highlight elements with stacking context`, true);
    ...
    const highlightContainer = createElementWithClass('div', 'Dom3d-section');
    highlightContainer.appendChild(ch1div);
   ...
   * @returns {?Element}
   */
  _getHideChildrenWithSamePaintOrderContent() {
    const hideCheckbox = UI.CheckboxLabel.create(ls`Hide elements with the same paint order as their parent.`, false);
