...
Dom3d.ZIndexScene = class extends Dom3d.BaseScene {
  ...
  highlightNode(nodeId) {
    if (nodeId) {
      const m = this.scene.getMeshByName(nodeId);
      if (m) {
        if (m.name !== this._currentSelectedMeshName) {
          m.previousMaterial = m.material;
          m.material = this.getMultiMaterialHighlighted();
          this._setSelectedMesh(m);
        }
      } else {
  ...
  changeColorScheme(type) {
    ...
    let colorRainbowGradient = null;
    if (type === Dom3d.ColorSchemeTypes.Blue) {
      colorIteratorForHeatMap = this.colorHelper.blueToYellow();
    } else if (type === Dom3d.ColorSchemeTypes.Purple) {
      colorIteratorForHeatMap = this.colorHelper.purpleToWhiteToPink();
    } else if (type === Dom3d.ColorSchemeTypes.Gradient) {
      colorRainbowGradient = this.colorHelper.makeRainbowGradient(this._maxNestingLevel + 1);
    }
   ...
   * @param {boolean} [value=false]
   */
  hideElementsWithSamePaintOrder(value = false) {
  ...
  isElementAllowedInScene(element, depthLevel, parentDepthLevel) {
    const hasValidDimensions = this.isAValidNumericUnit(element.width) && this.isAValidNumericUnit(element.height) &&
        element.y < this._maxHeight;
  ...
  createBoxForDepth(element, depthLevel = 0, parentDepthLevel = -1) {
    ...
    if (element.children && element.children.length > 0) {
      // let nd = depthLevel;  // keep all consecutive children on same level
  ...
  */
  ...
  isolateCurrentSelectedElement() {
    ...
    if (this._currentSelectedMeshName) {
      ...
      const m = this.scene.getMeshByName(this._currentSelectedMeshName);
      if (m) {
        this.scene.activeCamera.setTarget(m.position.clone());
      }
    }
    this.scene.freezeActiveMeshes();
  }
