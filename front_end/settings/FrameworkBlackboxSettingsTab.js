...
export class FrameworkBlackboxSettingsTab extends UI.Widget.VBox {
  constructor() {
    ...
    header.textContent = Common.i18n.getLocalizedString(str_, UIStrings.frameworkBlackboxing);
    UI.ARIAUtils.markAsHeading(header, 1);
    this.contentElement.createChild('div', 'intro').textContent =
        Common.i18n.getLocalizedString(str_, UIStrings.debuggerWillSkipThroughThe);
    ...
    blackboxContentScripts.appendChild(UI.SettingsUI.createSettingCheckbox(
        Common.i18n.getLocalizedString(str_, UIStrings.blackboxContentScripts),
        Common.Settings.Settings.instance().moduleSetting('skipContentScripts'), true));
    blackboxContentScripts.title = Common.i18n.getLocalizedString(str_, UIStrings.blackboxContentScriptsExtension);

    this._blackboxLabel = Common.i18n.getLocalizedString(str_, UIStrings.blackbox);
    this._disabledLabel = Common.i18n.getLocalizedString(str_, UIStrings.disabled);
    ...
    placeholder.textContent = Common.i18n.getLocalizedString(str_, UIStrings.noBlackboxedPatterns);
    this._list.setEmptyPlaceholder(placeholder);
    this._list.show(this.contentElement);
    const addPatternButton = UI.UIUtils.createTextButton(
        Common.i18n.getLocalizedString(str_, UIStrings.addPattern), this._addButtonClicked.bind(this), 'add-button');
  ...
  renderItem(item, editable) {
    ...
    pattern.title =
        Common.i18n.getLocalizedString(str_, UIStrings.blackboxScriptsWhoseNamesMatchS, {PH1: item.pattern});
  ...
  _createEditor() {
    ...
    titles.createChild('div', 'blackbox-pattern').textContent = Common.i18n.getLocalizedString(str_, UIStrings.pattern);
    ...
    titles.createChild('div', 'blackbox-behavior').textContent =
        Common.i18n.getLocalizedString(str_, UIStrings.behavior);
    ...
    UI.ARIAUtils.setAccessibleName(pattern, Common.i18n.getLocalizedString(str_, UIStrings.pattern));
    ...
    UI.ARIAUtils.setAccessibleName(behavior, Common.i18n.getLocalizedString(str_, UIStrings.behavior));
    ...
    function patternValidator(item, index, input) {
      ...
      if (!pattern.length) {
        return {valid: false, errorMessage: Common.i18n.getLocalizedString(str_, UIStrings.patternCannotBeEmpty)};
      }

      for (let i = 0; i < patterns.length; ++i) {
        if (i !== index && patterns[i].pattern === pattern) {
          return {valid: false, errorMessage: Common.i18n.getLocalizedString(str_, UIStrings.patternAlreadyExists)};
        }
      }
      ...
      if (!regex) {
        return {valid: false, errorMessage: Common.i18n.getLocalizedString(str_, UIStrings.patternMustBeAValidRegular)};
      }
      return {valid: true};
    }
