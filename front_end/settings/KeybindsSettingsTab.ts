...
export class KeybindsSettingsTab extends UI.Widget.VBox implements UI.ListControl.ListDelegate {
  ...
  constructor() {
    ...
    const docsLink = UI.UIUtils.createDocumentationLink('iterate/inspect-styles/shortcuts', i18nString(UIStrings.FullListOfDevtoolsKeyboard));
  ...
  commitChanges(item: UI.ActionRegistration.Action, editedShortcuts: Map<UI.KeyboardShortcut.KeyboardShortcut, UI.KeyboardShortcut.Descriptor[] | null>): void {
    for (const [originalShortcut, newDescriptors] of editedShortcuts) {
      ...
      if (newDescriptors) {
        UI.ShortcutRegistry.ShortcutRegistry.instance().registerUserShortcut(originalShortcut.changeKeys(newDescriptors as UI.KeyboardShortcut.Descriptor[])
   ...
   */
  heightForItem(item: KeybindsItem): number {
    return 0;
  }
  ...
  onEscapeKeyPressed(event: Event): void {
    ...
    for (const [shortcut, shortcutInput] of this._shortcutInputs.entries()) {
      if (activeElement === shortcutInput) {
        this._onShortcutInputKeyDown(shortcut as UI.KeyboardShortcut.KeyboardShortcut, shortcutInput as HTMLInputElement, event as KeyboardEvent);
      }
    }
  }
