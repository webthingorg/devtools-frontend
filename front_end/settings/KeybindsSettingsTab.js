...
export class ShortcutListItem {
  ...
  constructor(item, settingsTab, isEditing) {
    ...
    if (this._isEditing) {
      ...
      this._confirmButton.addEventListener('click', () => settingsTab.commitChanges(item, this._editedShortcuts));
      ...
      self.onInvokeElement(addShortcutLink, () => {
        const shortcut =
            new UI.KeyboardShortcut.KeyboardShortcut([], item.id(), UI.KeyboardShortcut.Type.UnsetShortcut);
  ...
  _validateInputs() {
    ...
    this._shortcutInputs.forEach((shortcutInput, shortcut) => {
      ...
      if (userDescriptors) {
        const conflicts = UI.ShortcutRegistry.ShortcutRegistry.instance().actionsForDescriptors(userDescriptors);
        if (UI.KeyboardShortcut.KeyboardShortcut.isModifier(userDescriptors[0].key)) {
          this._confirmButton.disabled = true;
          shortcutInput.classList.add('keybinds-invalid');
          ...
          shortcutInput.classList.remove('keybinds-invalid');
        }
      }
    });
  }
