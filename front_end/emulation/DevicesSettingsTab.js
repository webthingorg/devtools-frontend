...
export class DevicesSettingsTab extends UI.Widget.VBox {
  ...
  static _serializeBrandsList(brands) {
    ...
    const vParamName =
        /** @type {StructuredHeaders.ParamName}*/ ({kind: StructuredHeaders.ResultKind.ParamName, value: 'v'});
  ...
  _createEditor() {
    ...
    const brands = editor.createInput('brands', 'text', ls`UA brands list`, brandListValidator);
    addToTree(brands);

    const fullVersion = editor.createInput('full-version', 'text', ls`Full version`, chStringValidator);
    addToTree(fullVersion);

    // ### autocomplete hints?
    const platform = editor.createInput('platform', 'text', ls`Platform`, chStringValidator);
    addToTree(platform);
    ...
    const arch = editor.createInput('arch', 'text', ls`Architecture`, chStringValidator);
    addToTree(arch);
