...
export class DevicesSettingsTab extends UI.Widget.VBox {
  ...
  _createEditor() {
    ...
    const iconToolTipText = document.createTextNode(
        ls`User agent client hints are an alternative to the user agent string that identify the browser and the device in a more structured way with better privacy accounting. `);

    const iconToolTipLink = document.createElement('a');
    iconToolTipLink.textContent = ls`Learn more.`;
    iconToolTipLink.href = 'http://https://web.dev/user-agent-client-hints/';

    const iconSpan = document.createElement('span');
    iconSpan.appendChild(iconToolTipText);
    iconSpan.appendChild(iconToolTipLink);
    UI.Tooltip.Tooltip.install(icon, iconSpan);
    uaChFields.appendChild(icon);
