...
export class DevicesSettingsTab extends UI.Widget.VBox {
  ...
  _createEditor() {
    ...
    function brandListValidator(item, index, input) {
      const syntaxError = ls`Unable to parse as structured headers list`;
      const structError = ls`Brand list must consist of strings, each with a v parameter with a string value`;
