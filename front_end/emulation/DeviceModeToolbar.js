...
export class DeviceModeToolbar {
  ...
  _fillMainToolbar(toolbar) {
    ...
    UI.Tooltip.Tooltip.install(widthInput, i18nString(UIStrings.width));
    this._updateWidthInput =
    ...
    UI.Tooltip.Tooltip.install(heightInput, i18nString(UIStrings.heightLeaveEmptyForFull));
  ...
  update() {
    ...
    if (this._model.scale() !== this._cachedScale) {
      this._scaleItem.setText(`${this._model.scale() * 100}%`);
    ...
    if (deviceScale !== this._cachedDeviceScale) {
      this._deviceScaleItem.setText(`DPR: ${deviceScale}`);
