...
export class TopDownNode extends Node {
  ...
  _buildChildren() {
    ...
    function onStartEvent(e) {
      ...
      const duration =
          Math.min(endTime, e.endTime !== undefined ? e.endTime : endTime) - Math.max(startTime, e.startTime);
      if (duration < 0) {
...
export class BottomUpRootNode extends Node {
  ...
  _ungrouppedTopNodes() {
    ...
    function onStartEvent(e) {
      const duration =
          Math.min(e.endTime !== undefined ? e.endTime : endTime, endTime) - Math.max(e.startTime, startTime);
...
export class BottomUpNode extends Node {
  ...
  children() {
    ...
    function onEndEvent(e) {
      ...
      const totalTime =
          Math.min(e.endTime !== undefined ? e.endTime : endTime, endTime) - Math.max(e.startTime, lastTimeMarker);
      node.selfTime += selfTime || 0;
      node.totalTime += totalTime;
      lastTimeMarker = Math.min(e.endTime !== undefined ? e.endTime : endTime, endTime);
    }
...
export function eventURL(event) {
  ...
  while (frame) {
    ...
    // TODO: Runtime.CallFrame does not seem to have .parent at all.
