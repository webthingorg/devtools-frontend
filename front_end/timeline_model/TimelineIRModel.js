...
export class TimelineIRModel {
  ...
  _processInputLatencies(events) {
    ...
    function canMerge(threshold, first, second) {
      const actualEndTime = first.endTime !== undefined ? first.endTime : Number.MAX_SAFE_INTEGER;
      return actualEndTime < second.startTime && second.startTime < actualEndTime + threshold;
    }
  }
