...
export class TimelineIRModel {
  ...
  _processInputLatencies(events: SDK.TracingModel.AsyncEvent[]): void {
    ...
    function canMerge(threshold: number, first: SDK.TracingModel.AsyncEvent, second: SDK.TracingModel.AsyncEvent): boolean {
  ...
  reset(): void {
    ...
    function merge(threshold: number, first: Common.SegmentedRange.Segment, second: Common.SegmentedRange.Segment): Common.SegmentedRange.Segment | null {
  ...
  _inputEventType(eventName: string): InputEvents | null {
    ...
    if (!eventName.startsWith(prefix)) {
      if (eventName === InputEvents.ImplSideFling) {
        return /** @type {!InputEvents} */ eventName as InputEvents;
      }
      console.error('Unrecognized input latency event: ' + eventName);
      return null;
    }
    return /** @type {!InputEvents} */ eventName.substr(prefix.length) as InputEvents;
  }
}
