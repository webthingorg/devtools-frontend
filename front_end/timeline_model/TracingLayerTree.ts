...
export class TracingLayerTree extends SDK.LayerTreeBase.LayerTreeBase {
  ...
  layersById?: Map<string | number, SDK.LayerTreeBase.Layer>;
  ...
  async setLayers(root: TracingLayerPayload | null, layers: TracingLayerPayload[] | null, paints: LayerPaintEvent[]): Promise<void> {
  ...
  pictureForRasterTile(tileId: string): Promise<SDK.PaintProfiler.SnapshotWithRect | null> {
    ...
    if (!tile) {
      ...
      return /** @type {!Promise<?SDK.PaintProfiler.SnapshotWithRect>} */ Promise.resolve(null) as Promise<SDK.PaintProfiler.SnapshotWithRect | null>;
    }
  ...
  _innerSetLayers(oldLayersById: Map<string | number, SDK.LayerTreeBase.Layer>, payload: TracingLayerPayload): TracingLayer {
    let layer: TracingLayer | (TracingLayer | null) = (oldLayersById.get(payload.layer_id) as TracingLayer | null);
    if (layer) {
...
export class TracingLayer implements SDK.LayerTreeBase.Layer {
  ...
  _scrollRectsFromParams(params: number[], type: string): Object {
  ...
  _createScrollRects(payload: TracingLayerPayload): void {
    const nonPayloadScrollRects: any[] = [];
    if (payload.non_fast_scrollable_region) {
      nonPayloadScrollRects.push(this._scrollRectsFromParams(payload.non_fast_scrollable_region, SDK.LayerTreeBase.Layer.ScrollRectType.NonFastScrollable));
    }
    if (payload.touch_event_handler_region) {
      nonPayloadScrollRects.push(this._scrollRectsFromParams(payload.touch_event_handler_region, SDK.LayerTreeBase.Layer.ScrollRectType.TouchEventHandler));
    }
    if (payload.wheel_event_handler_region) {
      nonPayloadScrollRects.push(this._scrollRectsFromParams(payload.wheel_event_handler_region, SDK.LayerTreeBase.Layer.ScrollRectType.WheelEventHandler));
    }
    if (payload.scroll_event_handler_region) {
      nonPayloadScrollRects.push(this._scrollRectsFromParams(payload.scroll_event_handler_region, SDK.LayerTreeBase.Layer.ScrollRectType.RepaintsOnScroll));
    }
