...
export function registerUIStrings(path: string, stringStructure: Object): (id: string, values: Object) =>
   ...
   * */
  const str = (id: string, value: Object) => {
    try {
...
export function getFormatLocalizedString(
    str_: (id: string, values: Object) => StringUtilities.LocalizedString, stringId: string, placeholders: any) {
  if (!registeredLocale) {
    throw new Error(`Unsupported locale '${registeredLocale}'`);
  }
...
export function formatLocalized(formattedString: string, args: any) {
  const substitution: (substitution: string) => string = substitution => {
    return substitution;
  };

  function append(a: Element, b: any) {
...
export function serializeUIString(string: string, values: any = []) {
  const serializedMessage = {string, values};
  return JSON.stringify(serializedMessage);
}

export function deserializeUIString(serializedMessage: string) {
