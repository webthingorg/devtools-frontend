...
export class GutterDiffPlugin extends Plugin {
  ...
  _decorationsSetForTest(decorations: Map<number, {
    lineNumber: number;
    type: SourceFrame.SourceCodeDiff.EditType;
  }>): void {
  }
  ...
  async populateTextAreaContextMenu(contextMenu: UI.ContextMenu.ContextMenu, lineNumber: number, columnNumber: number): Promise<void> {
    GutterDiffPlugin._appendRevealDiffContextMenu(contextMenu, this._uiSourceCode);
  }
...
export class GutterDecoration {
  ...
  _className: string;
  type: symbol;
  constructor(textEditor: SourceFrame.SourcesTextEditor.SourcesTextEditor, lineNumber: number, type: symbol) {
    this._textEditor = textEditor;
