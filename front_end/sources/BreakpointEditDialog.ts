...
const str_ = i18n.i18n.registerUIStrings('sources/BreakpointEditDialog.ts', UIStrings);
...
export class BreakpointEditDialog extends UI.Widget.Widget {
  ...
  constructor(editorLineNumber: number, oldCondition: string, preferLogpoint: boolean, onFinish: (arg0: {
    ...
    if (extension) {
      extension.instance().then(factory => {
        ...
        this._editor = (factory as UI.TextEditor.TextEditorFactory).createEditor(editorOptions);
  ...
  _updatePlaceholder(): void {
    ...
    if (selectedValue === BreakpointType.Conditional) {
      ...
      /** @type {!HTMLSpanElement} */ UI.Tooltip.Tooltip.install((this._typeSelector.element), i18nString(UIStrings.pauseOnlyWhenTheConditionIsTrue));
    }
    else if (selectedValue === BreakpointType.Logpoint) {
      this._editor.setPlaceholder(i18nString(UIStrings.logMessageEgXIsX));
      /** @type {!HTMLSpanElement} */ UI.Tooltip.Tooltip.install((this._typeSelector.element), i18nString(UIStrings.logAMessageToConsoleDoNotBreak));
    }
  }
