...
export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
  ...
  /**
   * @returns {Promise<Workspace.UISourceCode.UILocation | null>}
   */
  ...
  async _getContent(locations) {
    ...
    return await Promise.all(locations.map(async ([breakpointLocation]) => {
  ...
  createElementForItem(item) {
    ...
    if (lineNumber < item.text.lineCount()) {
...
class BreakpointItem {
   ...
   */
  isSimilar(other) {
    return this.locations.length === other.locations.length && this.text.value() === other.text.value() &&
