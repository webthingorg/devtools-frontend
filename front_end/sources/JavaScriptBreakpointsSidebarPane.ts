...
const str_ = i18n.i18n.registerUIStrings('sources/JavaScriptBreakpointsSidebarPane.ts', UIStrings);
const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);
let javaScriptBreakpointsSidebarPaneInstance: JavaScriptBreakpointsSidebarPane;

export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget implements UI.ContextFlavorListener.ContextFlavorListener, UI.ListControl.ListDelegate {
  ...
  createElementForItem(item: BreakpointItem): Element {
    ...
    let checkedDescription: Common.UIString.LocalizedString = hasEnabled ? i18nString(UIStrings.checked) : i18nString(UIStrings.unchecked);
  ...
  heightForItem(item: BreakpointItem): number {
    return 0;
  }

  isItemSelectable(item: BreakpointItem): boolean {
    return true;
  }

  selectedItemChanged(from: BreakpointItem | null, to: BreakpointItem | null, fromElement: HTMLElement | null, toElement: HTMLElement | null): void {
    if (fromElement) {
