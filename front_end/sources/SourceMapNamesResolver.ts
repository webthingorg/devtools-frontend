...
export const resolveScopeChain = async function (callFrame: SDK.DebuggerModel.CallFrame | null): Promise<SDK.DebuggerModel.ScopeChainEntry[] | null> {
  if (!callFrame) {
    return null;
  }
...
export const resolveScope = async (scope: SDK.DebuggerModel.ScopeChainEntry): Promise<Map<string, string>> => {
  ...
  async function resolveSourceName(script: SDK.Script.Script, sourceMap: SDK.SourceMap.SourceMap, id: Identifier, textCache: Map<string, TextUtils.Text.Text>): Promise<string | null> {
...
export const resolveExpression = async (callFrame: SDK.DebuggerModel.CallFrame, originalText: string, uiSourceCode: Workspace.UISourceCode.UISourceCode, lineNumber: number, startColumnNumber: number, endColumnNumber: number): Promise<string> => {
  ...
  const sourceMap = (Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().sourceMapForScript(script) as SDK.SourceMap.TextSourceMap);
  if (!sourceMap) {
    return '';
  }
  const { content } = await script.requestContent();
...
export const resolveThisObject = async (callFrame: SDK.DebuggerModel.CallFrame | null): Promise<SDK.RemoteObject.RemoteObject | null> => {
  if (!callFrame) {
    return null;
  }
  const scopeChain = callFrame.scopeChain();
...
export class RemoteObject extends SDK.RemoteObject.RemoteObject {
  ...
  async getAllProperties(accessorPropertiesOnly: boolean, generatePreview: boolean): Promise<SDK.RemoteObject.GetPropertiesResult> {
   ...
   * @template T
   */
  callFunction(functionDeclaration: (this: Object, ...arg1: unknown[]) => T, args?: Protocol.Runtime.CallArgument[]): Promise<SDK.RemoteObject.CallFunctionResult> {
   ...
   * @template T
   */
  callFunctionJSON(functionDeclaration: (this: Object, ...arg1: unknown[]) => T, args: Protocol.Runtime.CallArgument[] | undefined): Promise<T> {
