...
Sources.CoveragePlugin = class extends Sources.UISourceCodeFrame.Plugin {
  ...
  constructor(textEditor, uiSourceCode) {
    ...
    const originalSourceCode = Sources.sourceFormatter.getOriginalUISourceCode(this._uiSourceCode);

    this._text = new UI.ToolbarText('Enable Coverage');

    const mainTarget = SDK.targetManager.mainTarget();
    if (mainTarget) {
      this._model = this._model = mainTarget.model(Coverage.CoverageModel);
      this._model.addEventListener(Coverage.CoverageModel.Events.CoverageReset, this._handleReset, this);

      this._coverage = this._model.getCoverageForUrl(originalSourceCode.url());
      if (this._coverage) {
  ...
  _updateStats() {
    if (this._coverage) {
      this._text.setText(this._coverage.usedPercentage().toFixed(1) + ' %');
    } else {
      this._text.setText('No Coverage Data available');
    }
  }
