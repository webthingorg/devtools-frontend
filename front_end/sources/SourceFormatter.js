...
Sources.SourceFormatter.ScriptMapping = class {
  ...
  rawLocationToUILocation(rawLocation) {
    ...
    if (script.isInlineScriptWithSourceURL()) {
      ...
      // but `rawLocation` will always use locations wrt. the containing document, because that is what the back-end is sending.
      // This is a hack, because what we are really doing here is deciding the location based on /how/ the script is displayed,
      // which is really something this layer cannot and should not have to decide: The SourceFormatter should not have to
      // know wether a script is displayed inline (in its containing document) or stand-alone.
    ...
    // Here we either have an inline script without a #sourceURL= or a stand-alone script. For stand-alone scripts,
    // no translation must be applied. For inline scripts, also no translation must be applied, because the line-endings
  ...
  uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    ...
    if (formatData.originalSourceCode.contentType().isScript()) {
      ...
      // either a stand-alone script or an inline script with a #sourceURL= and in both cases we can just forward
      // the question to the original (unformatted) source code.
    ...
    if (formatData.originalSourceCode.contentType() === Common.resourceTypes.Document) {
      ...
      if (debuggerModel) {
        ...
        // Here we have an inline script, which was formatted together with the containing document, so we must
        // not translate locations as they are relative to the start of the document.
