...
const str_ = i18n.i18n.registerUIStrings('sources/DebuggerPausedMessage.ts', UIStrings);
...
export class DebuggerPausedMessage {
  ...
  async render(details: SDK.DebuggerModel.DebuggerPausedDetails | null, debuggerWorkspaceBinding: Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding, breakpointManager: Bindings.BreakpointManager.BreakpointManager): Promise<void> {
    ...
    if (details.reason === Protocol.Debugger.PausedEventReason.DOM) {
      ...
      let eventNameForUI = '';
      if (details.auxData) {
        eventNameForUI = SDK.DOMDebuggerModel.DOMDebuggerManager.instance().resolveEventListenerBreakpointTitle((details.auxData as {
