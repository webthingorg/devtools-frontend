...
export class CSPViolationBreakpointsSidebarPane extends UI.Widget.VBox {
  constructor() {
    ...
    const categories = this._cspViolationsToBreakOn.map(breakpoint => breakpoint.category());
    categories.sort();
    ...
    /** @type {!Map<!CSPViolationBreakpoint, !Item>} */
    this._breakpoints = new Map();
    for (const breakpoint of this._cspViolationsToBreakOn) {
  ...
  /**
   * @param {!CSPViolationBreakpoint} breakpoint
   */
  ...
  _update() {
    ...
    /** @type {!CSPViolationBreakpoint | undefined}*/
    const breakpoint = this._cspViolationsToBreakOn.find(x => x.type() === breakpointType);
    if (!breakpoint) {
      return;
    }
  ...
  _breakpointCheckboxClicked(breakpoint) {
    ...
    SDK.DOMDebuggerModel.DOMDebuggerManager.instance().updateCSPViolationTypes(violationTypes);
