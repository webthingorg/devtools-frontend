...
export class FontEditor extends UI.Widget.VBox {
  ...
  _onFontSelectorChanged() {
    ...
    this._updatePropertyValue('font-family', value, /** repopulate= */ true);
  }

  /**
   * @param {string} propertyName
   * @param {string} value
   * @param {boolean} repopulate
   */
  _updatePropertyValue(propertyName, value, repopulate) {
    this.dispatchEventToListeners(Events.FontChanged, {propertyName, value, repopulate});
  }
...
class FontPropertyInputs {
  ...
  _createSliderInput(field, label) {
    ...
    slider.addEventListener(
        ...
        /** @param {!Event} event */
        event => {
          this._onSliderInput(event, /** repopulate= */ true);
        });
    slider.addEventListener(
        'keydown',
        /** @param {!Event} event */
  ...
  _createTextInput(field) {
    ...
    const min = this._range[0];
    const max = this._range[1];
    textInput.min = min.toString();
    textInput.max = max.toString();
  ...
  _onSelectorInput(event) {
    if (event.currentTarget) {
      ...
      this._boundUpdateCallback(this._propertyName, value, /** repopulate= */ true);
    }
  }
  ...
  _onTextInput(event) {
    if (event.currentTarget) {
      ...
      this._selectorInput.value = '';
      this._boundUpdateCallback(this._propertyName, value + units, /** repopulate= */ true);
    }
  }
  ...
  async _onUnitInput(event) {
    ...
    if (this._textInput.value) {
      this._boundUpdateCallback(this._propertyName, this._textInput.value + newUnit, /** repopulate= */ true);
    }
    this._units = newUnit;
  ...
  _setInputUnits(multiplier, newUnit) {
    ...
    this._textInput.min = (parseFloat(this._textInput.min) * multiplier).toFixed(roundingPrecision);
    this._textInput.max = (parseFloat(this._textInput.max) * multiplier).toFixed(roundingPrecision);
    this._sliderInput.sliderElement.step = newStep;
    if (this._textInput.value) {
      const newValue = parseFloat((parseFloat(this._textInput.value) * multiplier).toFixed(roundingPrecision));
