...
let computedArrayFontSizeIndex = 6;

/** @type {FontPropertyInputStaticParams} */
...
export class FontEditor extends UI.Widget.VBox {
  ...
  constructor(propertyMap) {
    ...
    new FontPropertyInputs(
        'font-size', ls`Font Size`, cssPropertySection, fontSizePropertyInfo, fontSizeStaticParams,
        this._updatePropertyValue.bind(this), this._resizePopout.bind(this), this._model, true);
    new FontPropertyInputs(
        'line-height', ls`Line Height`, cssPropertySection, lineHeightPropertyInfo, lineHeightStaticParams,
        this._updatePropertyValue.bind(this), this._resizePopout.bind(this), this._model, true);
    new FontPropertyInputs(
        'font-weight', ls`Font Weight`, cssPropertySection, fontWeightPropertyInfo, fontWeightStaticParams,
        this._updatePropertyValue.bind(this), this._resizePopout.bind(this), this._model, false);
    new FontPropertyInputs(
        'letter-spacing', ls`Spacing`, cssPropertySection, letterSpacingPropertyInfo, letterSpacingStaticParams,
        this._updatePropertyValue.bind(this), this._resizePopout.bind(this), this._model, true);
  }
...
class FontPropertyInputs {
  ...
  async _getNumConversionMultiplier(baseMultiplier, prevUnit, newUnit) {
    ...
    */
    function findFontSizeValue(computedObject) {
      ...
      if (computedArray[computedArrayFontSizeIndex].name &&
          computedArray[computedArrayFontSizeIndex].name !== 'font-size') {
  ...
  _setInputUnits(multiplier, newUnit) {
    let rangeObject = this._staticParams.rangeMap.get(this._units);
    const newRangeMap = this._staticParams.rangeMap.get(newUnit);
    const newStep = newRangeMap ? newRangeMap.step : 1;
    let hasValue = false;
    if (!rangeObject) {
      rangeObject = this._staticParams.rangeMap.get('px');
    }
