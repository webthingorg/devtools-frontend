...
export class FontEditor extends UI.Widget.VBox {
  ...
  async _createFontSelector(value, label) {
    ...
    selectorField.appendChild(deleteButton.element);
  ...
  _onSelectorChanged() {
    ...
    for (let i = 0; i < this._fontSelectors.length; i++) {
      ...
      if (fontSelectorInput.value !== '') {
        ...
        this._fontSelectorSection.removeChild(fontSelectorInput.parentNode);
        if (i === 0) {
          this._fontSelectors[1].label.textContent = 'Font Family';
        }
        this._fontSelectors.splice(i, 1);
        for (let j = i; j < this._fontSelectors.length; j++) {
          if (j !== 0) {
            this._fontSelectors[j].label.textContent = ls`Fallback ${j}`;
          }
        }
        i -= 1;
        this._resizePopout();
...
class FontPropertyInputs {
  constructor(propertyName, label, field, propertyInfo, rangeMap, updateCallback, model, hasUnits) {
    ...
    this._addedUnit = this._rangeMap.get(this._units) ? undefined : this._units;
  ...
  async _getNumConversionMultiplier(baseMultiplier, prevUnit, newUnit, isFontInput) {
    ...
    if (prevUnit === this._addedUnit) {
      prevUnit = 'px';
    }
    if (newUnit === this._addedUnit) {
      newUnit = 'px';
    }
    if (prevUnit === 'px') {
      if (newUnit === 'px') {
