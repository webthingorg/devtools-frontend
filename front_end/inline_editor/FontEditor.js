...
export class FontEditor extends UI.Widget.VBox {
  ...
  async _createFontSelectorSection(propertyValue) {
    if (propertyValue) {
      // propertyValue will not be split correctly for font family names that contain commas.
  ...
  async _createFontSelector(value, isPrimary) {
    // Custom font family names that use single/double quotes in the font family name will not be handled correctly.
  ...
  _deleteFontSelector(index, isGlobalValue) {
    const fontSelectorObject = this._fontSelectors[index];
  ...
  _updateFontSelectorList() {
    for (let i = 0; i < this._fontSelectors.length; i++) {
      ...
      if (i === 0) {
        fontSelectorObject.label.textContent = ls`Font Family`;
        UI.ARIAUtils.setAccessibleName(fontSelectorObject.input, ls`Font Family`);
        fontSelectorObject.deleteButton.setTitle(ls`Delete Font Family`);
      } else {
        const label = ls`Fallback ${i}`;
...
/**
 * @typedef {{regex: !RegExp, unitsArray: ?Array<string>, keyValuesArray: !Array<string>, rangeMap: !Map<string, FontEditor.PropertyRange>}}
 */
...
class FontPropertyInputs {
  ...
  _checkSelectorValueAndToggle() {
    if (this._staticParams.keyValuesArray && this._propertyInfo.value !== null &&
        (this._staticParams.keyValuesArray.indexOf(this._propertyInfo.value) !== -1)) {
  ...
  _createSelectorInput(field) {
    /** @type {!HTMLSelectElement} */
    const selectInput =
        UI.UIUtils.createSelect(ls`${this._propertyName} Key Value Selector`, this._staticParams.keyValuesArray);
