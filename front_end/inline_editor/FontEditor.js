...
const lineHeightRangeMap = new Map([
  ...
  // Extra Units
  ['rem', {min: 0, max: 2, step: .1}], ['vh', {min: 0, max: 4.5, step: .1}],
  ['vw', {min: 0, max: 4.5, step: .1}], ['vmin', {min: 0, max: 4.5, step: .1}],
  ['vmax', {min: 0, max: 4.5, step: .1}], ['cm', {min: 0, max: 1, step: .1}],
  ['mm', {min: 0, max: 8.5, step: .1}], ['in', {min: 0, max: .5, step: .1}],
  ['pt', {min: 0, max: 24, step: 1}], ['pc', {min: 0, max: 2, step: .1}]
]);
...
export class FontEditor extends UI.Widget.VBox {
  ...
  async updateRenderedFontList() {
    ...
    this._selectedNode.domModel().cssModel().removeEventListener(SDK.CSSModel.Events.ComputedStyleUpdated, this._boundUpdateRenderedList, false);
  ...
  async _createFontsList() {
    ...
    systemMap.set('System Fonts', [
      ...
      'Helvetica', 'Impact', '\'Times New Roman\'', 'Verdana'
    ]);
  ...
  async _createFontSelector(value, label) {
    ...
    label = label ? label : ls`Fallback ${this._fontSelectors.length}`;
    const fontSelectorObject = this._createSelector(selectorField, label, fontsList, value.trim());

      const deleteToolbar = new UI.Toolbar.Toolbar('', selectorField);
      const deleteButton = new UI.Toolbar.ToolbarButton(ls`Remove`, 'largeicon-trash-bin');
  ...
  _onSelectorChanged() {
    ...
    if (this._fontSelectors[this._fontSelectors.length - 1].input.value !== '' && this._fontSelectors.length < 10) {
      this._createFontSelector(ls`Fallback`, '');
      this._resizePopout();
    }
    this._selectedNode.domModel().cssModel().addEventListener(SDK.CSSModel.Events.ComputedStyleUpdated, this._boundUpdateRenderedList, false);
...
class FontPropertyInputs {
  ...
  async _getNumConversionMultiplier(baseMultiplier, prevUnit, newUnit, isFontInput) {
    ...
    if (prevUnit === 'vmin') {
      ...
      return await this._getNumConversionMultiplier(vminToPxValue, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'vmax') {
      const pageLayout = await this._selectedNode.domModel().target().pageAgent().invoke_getLayoutMetrics({});
      const viewportWidth = pageLayout.visualViewport.clientWidth;
      ...
      return await this._getNumConversionMultiplier(vmaxToPxValue, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'cm') {
      const cmToPxMultiplier = 37.795;
      return await this._getNumConversionMultiplier(cmToPxMultiplier, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'mm') {
      const mmToPxMultiplier = 3.7795;
      return await this._getNumConversionMultiplier(mmToPxMultiplier, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'in') {
      const inToPxMultiplier = 96;
      return await this._getNumConversionMultiplier(inToPxMultiplier, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'pt') {
      const ptToPxMultiplier = 1.333;
      return await this._getNumConversionMultiplier(ptToPxMultiplier, 'px', newUnit, isFontInput);
    }
    if (prevUnit === 'pc') {
      const pcToPxMultiplier = 16;
      return await this._getNumConversionMultiplier(pcToPxMultiplier, 'px', newUnit, isFontInput);
    }
    return 1;
  }
