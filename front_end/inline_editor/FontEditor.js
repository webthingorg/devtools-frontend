...
/** @enum {symbol} */
export const Events = {
  UnitWarningMessage: Symbol('UnitWarningMessage'),
...
class FontPropertyInputs {
  constructor(propertyName, label, field, propertyInfo, rangeMap, updateCallback, model, hasUnits) {
    ...
    this._unitWarning = this._rangeMap.get(this._units) ? undefined : this._units;
    this._range = this._getUnitRange();

    this._propertyName = propertyName;
    this._boundUpdateCallback = updateCallback;
    //this._boundWarningMessageCallback = warningMessageCallback;
    this._selectedNode = self.UI.context.flavor(SDK.DOMModel.DOMNode);
  ...
  _createUnitInput(field) {
    ...
    if (this._hasUnits) {
      ...
      if (this._unitWarning) {
        unitInput.classList.add('unit-warning');
  ...
  async _onUnitInput(event) {
    ...
    this._units = newUnit;
    if (newUnit === this._unitWarning) {
      this._unitInput.classList.add('unit-warning');
      this._addUnitWarningMessage();
    } else {
      this._unitInput.classList.remove('unit-warning');
      this._removeUnitWarningMessage();
    }
  }

  _addUnitWarningMessage() {
    this.dispatchEventToListener(Events.UnitWarningMessage, true);
  }

  _removeUnitWarningMessage() {
    this.dispatchEventToListener(Events.UnitWarningMessage, false);
  }
