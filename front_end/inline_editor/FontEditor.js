...
import * as FontEditorConstants from './FontEditorConstants.js';
...
export class FontEditor extends UI.Widget.VBox {
  ...
  constructor(propertyMap) {
    ...
    const fontSizePropertyInfo = this._getPropertyInfo('font-size', FontEditorConstants.FontSizeStaticParams.regex);

    const lineHeightPropertyInfo =
        this._getPropertyInfo('line-height', FontEditorConstants.LineHeightStaticParams.regex);

    const fontWeightPropertyInfo =
        this._getPropertyInfo('font-weight', FontEditorConstants.FontWeightStaticParams.regex);

    const letterSpacingPropertyInfo =
        this._getPropertyInfo('letter-spacing', FontEditorConstants.LetterSpacingStaticParams.regex);

    new FontPropertyInputs(
        'font-size', ls`Font Size`, cssPropertySection, fontSizePropertyInfo, FontEditorConstants.FontSizeStaticParams,
    ...
    new FontPropertyInputs(
        ...
        FontEditorConstants.LineHeightStaticParams, this._updatePropertyValue.bind(this), this._resizePopout.bind(this),
        this._model, true);
    new FontPropertyInputs(
        'font-weight', ls`Font Weight`, cssPropertySection, fontWeightPropertyInfo,
        FontEditorConstants.FontWeightStaticParams, this._updatePropertyValue.bind(this), this._resizePopout.bind(this),
        this._model, false);
    new FontPropertyInputs(
        'letter-spacing', ls`Spacing`, cssPropertySection, letterSpacingPropertyInfo,
        FontEditorConstants.LetterSpacingStaticParams, this._updatePropertyValue.bind(this),
        this._resizePopout.bind(this), this._model, true);
  }
  ...
  async _createFontSelectorSection(propertyValue) {
    if (propertyValue) {
      ...
      if (!FontEditorConstants.GlobalValues.includes(splitValue[0])) {
  ...
  async _createFontsList() {
    ...
    systemMap.set('System Fonts', FontEditorConstants.SystemFonts);
    systemMap.set('Generic Families', FontEditorConstants.GenericFonts);
  ...
  async _createFontSelector(value, isPrimary) {
    ...
    if (isPrimary) {
      ...
      const globalValuesMap = new Map([['Global Values', FontEditorConstants.GlobalValues]]);
  ...
  _onFontSelectorChanged() {
    let value = '';
    const isGlobalValue = FontEditorConstants.GlobalValues.includes(this._fontSelectors[0].input.value);
...
/**
 * @typedef {{regex: !RegExp, unitsArray: Array<string> | undefined, valueArray: !Array<string>, rangeMap: !Map<string, FontEditor.PropertyRange>}}
 */
...
class FontPropertyInputs {
   ...
   */
  constructor(propertyName, label, field, propertyInfo, staticParams, updateCallback, resizeCallback, model, hasUnits) {
  ...
  _createTextInput(field) {
    ...
    textInput.step = this._initialRange.step;
  ...
  _onTextInput(event) {
    if (event.currentTarget) {
      ...
      if (this._staticParams.regex.test(valueString) ||
          (value === '' && !event.currentTarget.validationMessage.length)) {
  ...
  async _getNumConversionMultiplier(baseMultiplier, prevUnit, newUnit) {
    ...
    if (prevUnit === 'px') {
      ...
      if (newUnit === 'em' || (this._propertyName === 'line-height' && newUnit === '')) {
        let currentFontSize;
        if (this._selectedNode && this._selectedNode.nodeName() !== 'HTML') {
          const fontSizeNodeId = isFontSizeProperty ? this._selectedNode.parentNode.id : this._selectedNode.id;
          currentFontSize = await this._model.getComputedStyleForNode(fontSizeNodeId).then(findFontSizeValue);
        } else {
      ...
      if (newUnit === 'rem') {
        ...
        let rootFontSize = await this._model.getComputedStyleForNode(htmlNode.id).then(findFontSizeValue);
        rootFontSize = rootFontSize.replace(/[a-z]/g, '');
        return baseMultiplier / rootFontSize;
      }
      if (newUnit === '%') {
        const fontSizeNodeId = isFontSizeProperty ? this._selectedNode.parentNode.id : this._selectedNode.id;
        let currentFontSize = await this._model.getComputedStyleForNode(fontSizeNodeId).then(findFontSizeValue);
        currentFontSize = currentFontSize.replace(/[a-z]/g, '');
        return baseMultiplier / currentFontSize * 100;
      }
      if (newUnit === 'vh') {
        const viewportObject = await this._getViewportObject();
    ...
    /**
     * @param {!Object} computedObject
    */
    function findFontSizeValue(computedObject) {
      /** @type {!Array<Object<string, string>>} */
      const computedArray = computedObject.computedStyle;
      if (computedArray[FontEditorConstants.computedArrayFontSizeIndex].name &&
          computedArray[FontEditorConstants.computedArrayFontSizeIndex].name !== 'font-size') {
        for (let i = 0; i < computedArray.length; i++) {
          if (computedArray[i].name === 'font-size') {
            FontEditorConstants.computedArrayFontSizeIndex = i;
            break;
          }
        }
      }
      return computedArray[FontEditorConstants.computedArrayFontSizeIndex].value;
    }
  }
  ...
  _setInputUnits(multiplier, newUnit) {
    ...
    this._sliderInput.sliderElement.min = Math.min(newValue, newMin);
    this._sliderInput.sliderElement.max = Math.max(newValue, newMax);
  ...
  async _getViewportObject() {
    ...
    if (!currentExecutionContext) {
      ...
      const widthObject = await currentExecutionContext.evaluate({expression: 'window.innerWidth'});
      const heightObject = await currentExecutionContext.evaluate({expression: 'window.innerHeight'});
