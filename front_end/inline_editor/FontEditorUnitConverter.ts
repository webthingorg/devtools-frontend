...
const unitConversionMap = new Map<'px', () => number>();
...
export async function getUnitConversionMultiplier(prevUnit: string, newUnit: string, isFontSize?: boolean): Promise<number> {
  if (prevUnit === '') {
    prevUnit = 'em';
  }
  if (newUnit === '') {
