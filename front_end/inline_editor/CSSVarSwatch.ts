...
export class CSSVarSwatch extends HTMLElement {
  ...
  private onIconClick: () => void = () => undefined;

  constructor() {
    super();
    this.shadow.adoptedStyleSheets = [
      ...getStyleSheets('inline_editor/cssVarSwatch.css', {patchThemeSupport: false}),
  ...
  set data(data: SwatchRenderData) {
    ...
    this.onIconClick = data.onIconClick;
  ...
  private renderLink(variableName: string) {
    ...
    const classes = isDefined ? 'css-var-link' : 'css-var-link undefined';
    const title = isDefined ? ls`Jump to definition` : ls`${variableName} is not defined`;
    const onClick = isDefined ? this.onIconClick.bind(this, this.variableName) : null;
  ...
  private render() {
    ...
    // clang-format off
    render(
      html`<span title="${this.computedValue || ''}">${colorSwatch}${functionParts.pre}${link}${functionParts.post}</span>`,
      this.shadow, { eventContext: this });
    // clang-format on
  }
}

customElements.define('css-var-swatch', CSSVarSwatch);

declare global {
  interface HTMLElementTagNameMap {
    'css-var-swatch': CSSVarSwatch;
  }
}
