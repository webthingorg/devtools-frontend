...
export class CSSVarSwatch extends HTMLElement {
  ...
  private onLinkActivate: (varialeName: string, event: MouseEvent) => void = () => undefined;
  ...
  set data(data: SwatchRenderData) {
    ...
    this.onLinkActivate = (variableName: string, event: MouseEvent): void => {
  ...
  private renderLink(variableName: string): LitHtml.TemplateResult {
    ...
    const onClick = isDefined ? this.onLinkActivate.bind(this, this.variableName.trim()) : null;
