...
// Parsing the value of CSS var(), must parse the space before '--'.
...
export class CSSVarSwatch extends HTMLElement {
  ...
  private parseVariableFunctionParts(): ParsedVariableFunction|null {
    // When the value of CSS var() is greater than two spaces, only one is always displayed, and the actual number of spaces is displayed when editing is clicked.
    const result = this.text.replace(/\s\s+/g, ' ').match(VARIABLE_FUNCTION_REGEX);
    if (!result) {
      return null;
    }
