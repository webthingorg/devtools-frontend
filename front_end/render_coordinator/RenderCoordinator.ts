...
export class RenderCoordinator extends EventTarget {
  ...
  async read<T extends unknown>(callback: CoordinatorCallback, label = UNNAMED_READ): Promise<T> {
    return this.enqueueHandler<T>(callback, ACTION.READ, label);
  }

  async write<T extends unknown>(callback: CoordinatorCallback, label = UNNAMED_WRITE): Promise<T> {
    return this.enqueueHandler<T>(callback, ACTION.WRITE, label);
  }
   ...
   */
  async scroll<T extends unknown>(callback: CoordinatorCallback, label = UNNAMED_SCROLL): Promise<T> {
    return this.read(callback, label);
  }
  ...
  private logIfEnabled(value: string|undefined): void {
    ...
    while (this.logInternal.length > this.maxRecordSize) {
