...
const str_ = i18n.i18n.registerUIStrings('resources/StorageView.js', UIStrings);
...
export class StorageView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  private settings: Map<Protocol.Storage.StorageType, Common.Settings.LegacySetting<boolean>>;
  private includeThirdPartyCookiesSetting: Common.Settings.LegacySetting<boolean>;
  ...
  constructor() {
    ...
    this.reportView.registerRequiredCSS('resources/clearStorageView.css', {enableLegacyPatching: false});
  ...
  async doUpdate(): Promise<void> {
    ...
    this.usageUpdatedForTest(response.usage, response.quota, response.usageBreakdown);
    this.update();
  }
  ...
  usageUpdatedForTest(_usage: number, _quota: number, _usageBreakdown: Protocol.Storage.UsageForType[]): void {
  }
}
