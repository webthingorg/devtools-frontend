...
export class ClearStorageView extends UI.ThrottledWidget.ThrottledWidget {
  constructor() {
    ...
    /** @type {?number} */
    this._previousOverrideSize = null;
    ...
    this._quotaOverrideEditor = (quotaOverrideControlRow.createChild('input', 'quota-override-notification-editor'));
    ...
    this._quotaOverrideEditor.addEventListener('keyup', event => {
      if (isEnterKey(event)) {
        this._quotaOverrideButtonClicked(event);
        return;
      }
    });
    ...
    this._quotaOverrideControlRow = quotaOverrideControlRow;
    this._quotaOverrideButton = quotaOverrideButton;
    this._quotaOverrideErrorMessage = errorMessage;
  ...
  _originChanged(event) {
    ...
    if (this._quotaOverridden && this._target && this._securityOrigin) {
      this._onClickCheckbox(this._target, this._securityOrigin);
      this._previousOverrideSize = null;
    }
   ...
   * @param {!SDK.SDKModel.Target} target
   * @param {?string} origin
   * @param {?number} quotaInBytes
   */
  async _overrideQuotaForOrigin(target, origin, quotaInBytes) {
    if (!origin) {
      return;
    }

    let quotaSize;
    if (quotaInBytes && quotaInBytes > -1) {
    ...
    const overrideQuotaParams = {origin, quotaSize};
    await target.storageAgent().invoke_overrideQuotaForOrigin(overrideQuotaParams);
  }
  ...
  async _onClickCheckbox(target, origin) {
    if (this._quotaOverrideControlRow.classList.contains('hidden')) {
      ...
      this._quotaOverrideEditor.value = this._previousOverrideSize ? this._previousOverrideSize.toString() : '';
      await this._quotaOverrideButtonClicked(new Event('clicked'));
    } else {
  ...
  async doUpdate() {
    ...
    this._quotaOverridden = response.overrideActive;
    ...
    const quota = this._quotaOverridden ? UI.Fragment.Fragment.build`<b>${quotaAsString} (custom)</b>` : quotaAsString;
    const fragment = UI.Fragment.Fragment.build`${usageAsString} used out of&nbsp;${quota}&nbsp;storage quota.`;
    this._quotaRow.appendChild(fragment.element());
    ...
    if (response.overrideActive) {
      this._previousOverrideSize = response.quota;
    }
