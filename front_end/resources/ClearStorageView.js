...
export class ClearStorageView extends UI.ThrottledWidget.ThrottledWidget {
  constructor() {
    ...
    this._quotaOverrideEditor = /** @type {!HTMLInputElement} */ (quotaOverrideEditor);
  ...
  async _onClickCheckbox(target, origin) {
    if (this._quotaOverrideControlRow.classList.contains('hidden')) {
      ...
      if (!this._quotaOverridden) {
        this._previousOverrideSize = undefined;
      }
      this._quotaOverridden = false;
      this._quotaOverrideErrorMessage.textContent = '';
    }
  ...
  async doUpdate() {
    ...
    const fragment = UI.Fragment.Fragment.build(
        [`${Platform.NumberUtilities.bytesToString(response.usage)} used out ` +
         `of${this._quotaOverridden ? '&nbsp;<b>' : ''} ` +
