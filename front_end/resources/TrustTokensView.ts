...
class TrustTokensViewWidgetWrapper extends UI.Widget.VBox {
  ...
  async wasShown(): Promise<void> {
    ...
    const {tokens} = await mainTarget.storageAgent().invoke_getStoredTrustTokens();
    this.trustTokensView.data = {tokens};
  }
}

interface TrustTokensViewData {
  tokens: Protocol.Storage.StoredTrustTokens[];
}

class TrustTokensView extends HTMLElement {
  private readonly shadow = this.attachShadow({mode: 'open'});
  private tokens: Protocol.Storage.StoredTrustTokens[] = [];
