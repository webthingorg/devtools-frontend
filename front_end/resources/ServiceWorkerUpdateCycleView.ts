...
export class ServiceWorkerUpdateCycleView {
  ...
  private updateTimingTable(timeRanges: Array<ServiceWorkerUpdateRange>): void {
    ...
    for (const range of timeRangeArray) {
      ...
      const tr = this.tableElement.createChild('tr');
      this.rows.push(tr);
  ...
  private constructUpdateDetails(tr: HTMLElement, range: ServiceWorkerUpdateRange): void {
    const detailsElement = this.tableElement.createChild('tr', 'service-worker-update-timing-bar-details');
  ...
  private toggle(detailsRow: Element, target: Element, expanded: boolean): void {
  ...
  private onFocus(event: Event): void {
    ...
    const tr: HTMLElement = target.parentElement as HTMLElement;
    if (!tr) {
      return;
    }
  ...
  private onKeydown(event: Event, detailsRow: HTMLElement): void {
    if (!event.target) {
      return;
    }
    const target: HTMLElement = event.target as HTMLElement;
  ...
  private focusRow(row: HTMLElement): void {
    ...
    if (tableRow) {
      tableRow.cells[0].focus();
    }
  }

  private blurRow(row: HTMLElement): void {
    const tableRow = row as HTMLTableRowElement;
    if (tableRow) {
      tableRow.cells[0].focus();
    }
  ...
  private onClick(event: Event, detailsRow: Element): void {
    ...
    this.toggle(detailsRow, tr, expanded);
    event.preventDefault();
  }
