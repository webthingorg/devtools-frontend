...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  async doUpdate() {
    ...
    maybeShowLinkToElementsPanel(this._ownerFieldValue, this._frame);
 ...
 * @param {!SDK.ResourceTreeModel.ResourceTreeFrame} openerFrame
 * @return {!Promise<?Element>}
 */
async function maybeShowLinkToElementsPanel(parent, openerFrame) {
...
export class OpenedWindowDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor(targetInfo, isWindowClosed) {
    ...
    const labelForDOMAccessibility = this._openerElementField.createChild('span', 'inline-name');
    ...
    this._valueForDOMAccessibility = this._openerElementField.createChild('span');
    this.update();
  }
  ...
  async maybeDisplayOpenerFrame() {
    ...
    if (openerFrameId) {
      const openerFrame = SDK.FrameManager.FrameManager.instance().getFrame(openerFrameId);
      if (openerFrame) {
        const elementLink = await maybeShowLinkToElementsPanel(this._openerElementField, openerFrame);
        if (elementLink) {
