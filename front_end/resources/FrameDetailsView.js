...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor(frame) {
    ...
    this._ApiDocumentDomain = this._apiAvailability.appendField('document.domain');
  ...
  _updateApiAvailability() {
    ...
    if (sabAvailable) {
      ...
      if (!this._frame.isCrossOriginIsolated()) {
        ...
        reasonHint.textContent = ls`⚠️ requires cross-origin isolated context in the future`;
      }
    } else {
    ...
    const documentDomainImmutable = features.includes(Protocol.Page.GatedAPIFeatures.DocumentDomainImmutable);
    // This is deprecated, only show line if it is immutable to inform users of the change.
    this._apiAvailability.setFieldVisible('document.domain', documentDomainImmutable);
    if (documentDomainImmutable) {
      this._ApiDocumentDomain.textContent = ls`immutable`;
    }
  }
