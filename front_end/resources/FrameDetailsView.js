...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
   ...
   * @param {any} condition
   */
  static maybeSetText(section, title, text, condition = text) {
    section.setFieldVisible(title, !!condition);
    const field = section.fieldValue(title);
  ...
  static fillCrossOriginPolicy(field, isEnabled, info) {
    ...
    if (endpoint) {
      const reportingEndpointPrefix = field.createChild('span', 'only-on-hover inline-name');
      reportingEndpointPrefix.textContent = 'Reporting endpoint: ';
      const reportingEndpointName = field.createChild('span', 'only-on-hover');
