...
export class AppManifestView extends UI.Widget.VBox {
  ...
  async _renderManifest(url, data, errors, installabilityErrors, manifestIcons) {
    ...
    for (const icon of icons) {
      ...
      imageErrors = [...iconErrors];
    }

    let shortcutIndex = 1;
    for (const shortcut of shortcuts) {
      const shortcutSection = this._reportView.appendSection(Common.UIString.UIString(`Shortcut #${shortcutIndex}`));
      ...
      for (const shortcutIcon of shortcut.icons) {
        ...
        imageErrors = [...shortcutIconErrors];
      }
      shortcutIndex++;
    }
