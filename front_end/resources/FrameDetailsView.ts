...
const UIStrings = {
  ...
  */
  ...
  /**
  *@description Label for a list of features which are not allowed according to the current Permissions policy
  *(a mechanism that allows developers to enable/disable browser features and APIs (e.g. camera, geolocation, autoplay))
  */
  disallowedFeatures: 'Disallowed Features',
};
...
export class FrameDetailsReportView extends HTMLElement {
  ...
  private showPermissionsPolicyDetails = false;
  ...
  private async renderPermissionPolicy(): Promise<LitHtml.TemplateResult|{}> {
    ...
    const toggleShowPermissionsPolicyDetails = (): void => {
      this.showPermissionsPolicyDetails = !this.showPermissionsPolicyDetails;
      this.render();
    };
    ...
    const renderDisallowed = async(): Promise<LitHtml.TemplateResult|{}> => {
      ...
      if (!this.showPermissionsPolicyDetails) {
        return LitHtml.html`
          <devtools-report-key>${i18nString(UIStrings.disallowedFeatures)}</devtools-report-key>
          <devtools-report-value>
      ...
      const featureRows = await Promise.all(disallowed.map(async policy => {
        ...
        const linkTargetDOMNode = await (
            policy.locator?.blockReason === Protocol.Page.PermissionsPolicyBlockReason.IframeAttribute && frame &&
            frame.getOwnerDOMNodeOrDocument());
        const resource = frame && frame.resourceForURL(frame.url);
        const linkTargetRequest = policy.locator?.blockReason === Protocol.Page.PermissionsPolicyBlockReason.Header &&
    ...
    return LitHtml.html`
      ...
      <div class="span-cols">
        <button class="link" @click=${(): void => toggleShowPermissionsPolicyDetails()}>
          ${this.showPermissionsPolicyDetails ? i18nString(UIStrings.hideDetails) : i18nString(UIStrings.showDetails)}
        </button>
      </div>
      ${renderAllowed()}
