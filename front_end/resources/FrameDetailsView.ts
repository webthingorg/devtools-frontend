...
export class FrameDetailsReportView extends HTMLElement {
  ...
  private async render(): Promise<void> {
    if (!this.frame) {
      throw new Error('Trying to render a Frame Details report without providing data');
    }
    ...
    LitHtml.render(LitHtml.html`
      ...
      <devtools-report .data=${{reportTitle: this.frame.displayName()} as Components.ReportView.ReportData}>
        ${await this.renderDocumentSection()}
        ${await this.renderIsolationSection()}
  ...
  private async renderDocumentSection(): Promise<LitHtml.TemplateResult|{}> {
    if (!this.frame) {
  ...
  private async renderIsolationSection(): Promise<LitHtml.TemplateResult|{}> {
    ...
    return LitHtml.html`
      ...
      ${await this.renderCoopCoepStatus()}
      <devtools-report-divider></devtools-report-divider>
    `;
  }
  ...
  private renderCrossOriginStatus(
    ...
    return LitHtml.html`
      ...
      <devtools-report-value>
        ...
        ${isReportOnly ? LitHtml.html`<span class="inline-comment">${ls`report-only`}</span>` : ''}
  ...
  private renderApiAvailabilitySection(): LitHtml.TemplateResult|{} {
    ...
    return LitHtml.html`
      ...
      <div class="span-cols">
        ...
        <a class="link" href="https://web.dev/why-coop-coep/" target="_blank">${ls`Learn more`}</a>
      </div>
  ...
  private renderMeasureMemoryAvailability(): LitHtml.TemplateResult|{} {
    if (this.frame) {
      ...
      const availabilityText = measureMemoryAvailable ?
          ls`available, but might require experimental web platform features` :
          ls`unavailable`;
      ...
      return LitHtml.html`
        ...
        <devtools-report-value title=${tooltipText}>
          ${availabilityText}
          <a class="link" href="https://web.dev/monitor-total-page-memory-usage/" target="_blank">${ls`Learn more`}</a>
        </devtools-report-value>
      `;
    }
    return LitHtml.nothing;
