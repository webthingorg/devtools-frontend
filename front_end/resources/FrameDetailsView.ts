...
export class FrameDetailsView extends UI.ThrottledWidget.ThrottledWidget {
  ...
  constructor(frame: SDK.ResourceTreeModel.ResourceTreeFrame) {
    ...
    if (this._protocolMonitorExperimentEnabled) {
      this._additionalInfo = this._reportView.appendSection(i18nString(UIStrings.additionalInformation));
      this._additionalInfo.setTitle(
          i18nString(UIStrings.additionalInformation), i18nString(UIStrings.thisAdditionalDebugging));
      const frameIDField = this._additionalInfo.appendField(i18nString(UIStrings.frameId));
