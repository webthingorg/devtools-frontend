...
export class SidebarFrameTree {
  ...
  _expandFrame(frame) {
    ...
    let treeElement = this._treeElementForFrameId.get(frame.globalId);
    if (!treeElement && !this._expandFrame(SidebarFrameTree._getParentFrame(frame))) {
      return false;
    }
    treeElement = this._treeElementForFrameId.get(frame.globalId);
    if (!treeElement) {
      return false;
    }
    treeElement.expand();
...
export class FrameDetailsView extends UI.Widget.VBox {
  ...
  constructor(frame) {
    ...
    paragraf.textContent = `url: ${frame.url}`;
