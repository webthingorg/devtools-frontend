...
```
Please refer from including submodules directly because that might break assumptions in the build system.

The primary view in a bottom drawer tab usually extends `UI.Widget.VBox`:
...
export class FeatureViewImpl extends UI.Widget.VBox {
  constructor() {
    ...
    this.registerRequiredCSS('feature/featureView.css');
...
```javascript
// The id 'featureView' has to match the one in the module.json file
...
```
  "front_end/feature/FeatureView.js",
```
