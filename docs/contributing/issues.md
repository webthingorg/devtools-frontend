...
![ACLs via Groups in Buganizer](./images/issues-acls.png "ACLs via Groups in Buganizer")
...
The daily runner performs the following tasks (for the
`Chromium>Platform>DevTools` component tree):

1.  Removes all assigned or accepted issues from the `DevTools-Backlog` hotlist,
    because by its very nature the backlog must not contain anything that is
    actively being worked on.
2.  Ensures that the `Needs-Feedback` and `DevTools-Backlog` hotlists are
    mutually exclusive, and removes all issues that are found on both from the
...
3.  Automatically closes `new` issues on the `Needs-Feedback` hotlist if they
    haven't been updated for 90 days.
