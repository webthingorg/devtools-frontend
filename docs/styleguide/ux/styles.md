...
## Colors
We define three set of color token types, **reference/palette tokens**, **system tokens**, and **application tokens**.
...
In the following we present general guidelines and examples on color choices by category (background, state layer, text, icon, and syntax highlighting).
...
##### State layer colors
This color needs to live in a state layer and is added to the elements' base color
...
* [Color definitions in the CDT Figma](https://www.figma.com/design/A5iQBBNAe5zPFpJvUzUgW8/CDT-design-kit?node-id=337-5217&m=dev)
