...
(async function() {
  TestRunner.addResult('Tests that the sidebar uses the correct styling for mixed content subresources.\n');
  ...
  TestRunner.mainTarget.model(Security.SecurityModel)
      .dispatchEventToListeners(Security.SecurityModel.Events.VisibleSecurityStateChanged, pageVisibleSecurityState);

  const passive = SDK.NetworkRequest.create(0, 'http://foo.test', 'https://foo.test', 0, 0, null);
  passive.mixedContentType = 'optionally-blockable';
  SecurityTestRunner.dispatchRequestFinished(passive);

  const active = SDK.NetworkRequest.create(0, 'http://bar.test', 'https://bar.test', 0, 0, null);
