...
(async function() {
  TestRunner.addResult('Tests that the panel shows the correct text for non-cryptographic secure origins\n');
  await TestRunner.loadTestModule('security_test_runner');
  await TestRunner.showPanel('security');

  const request1 = SDK.NetworkRequest.create(0, 'chrome-test://test', 'chrome-test://test', 0, 0, null);
