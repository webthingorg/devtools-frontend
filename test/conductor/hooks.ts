...
async function loadTargetPageAndFrontend(testServerPort: number) {
   ...
   */
  const needToLoadDevTools = process.env.TEST_SERVER_TYPE === 'hosted-mode';
  ...
  if (process.env.TEST_SERVER_TYPE === 'component-docs') {
    /**
...
export async function resetPages() {
  ...
  if (process.env.TEST_SERVER_TYPE === 'hosted-mode') {
