...
async function loadTargetPageAndFrontend(testServerPort: number) {
  ...
  if (TEST_SERVER_TYPE === 'hosted-mode') {
    ...
    frontendTargetPool = await FrontendTargetPool.create(poolOptions);
    setBrowser(browser);
...
export async function resetBrowserAndPagesFromPool(): Promise<void> {
...
export async function resetPagesBetweenTests() {
  ...
  if (TEST_SERVER_TYPE === 'hosted-mode') {
    await resetBrowserAndPagesFromPool();
  } else if (TEST_SERVER_TYPE === 'component-docs') {
...
async function throttleCPUIfRequired(): Promise<void> {
  ...
  if (envThrottleRate !== 1) {
    ...
    const client = await frontendTab.page.target().createCDPSession();
