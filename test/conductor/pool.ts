 ...
 *
 * Tab pairs are not intended to be reused and its the consumers responsiblity
...
export class FrontendTargetPool {
  ...
  /** Returns a pool with `FrontendTargetPool.POOL_SIZE` tab pairs ready to go. */
  static async create(browser: puppeteer.Browser, testServerPort: number): Promise<FrontendTargetPool> {
    const tabPool = new FrontendTargetPool(browser, testServerPort);
    const promises = [];
    for (let i = 0; i < FrontendTargetPool.POOL_SIZE; ++i) {
      promises.push(tabPool.addTabPairToPool());
    }
  ...
  async takeTabPair(): Promise<FrontendTargetTabs> {
    ...
    // We take one, so lets put a fresh one back.
