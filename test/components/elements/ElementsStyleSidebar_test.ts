...
import {$, activeElementTextContent, click, pasteText, selectTextFromNodeToNode, tabBackward, tabForward, typeText, waitForElementWithTextContent} from '../../shared/helper.js';

declare global {
  export namespace globalThis {
    function reset(): void;
    // eslint-disable-next-line no-var
    var injection: boolean|undefined;
  }
}
...
describe('Keyboard navigation', () => {
  ...
  describe('after clicking on a property name', () => {
    it('can tab into its value', async () => {
      await click(waitForElementWithTextContent('color'));
      await tabForward();
    ...
    it('can alter the value', async () => {
      await click(waitForElementWithTextContent('red'));
    ...
    it('can cancel and later commit a name change', async () => {
      ...
      await typeText('new-color-name');

      const textContent = await activeElementTextContent();

      assert.strictEqual(textContent, 'new-color-name');
    });
    ...
    it('can cancel, commit and then reset name change', async () => {
      ...
      await click(waitForElementWithTextContent('Reset'));
  ...
  describe('after clicking on the selector', () => {
    it('can hit enter to commit', async () => {
      ...
      await click(waitForElementWithTextContent('html'));
    ...
    it('can cancel, commit and then add a new selector', async () => {
      ...
      await click(waitForElementWithTextContent('Add new rule'));
  ...
  describe('can copy-paste CSS representation', () => {
    it('can select full text', async () => {
      const textSelection = await selectTextFromNodeToNode(
          waitForElementWithTextContent('}'), waitForElementWithTextContent('html'), 'up');
...
`);
    ...
    it('can select full text when expanded', async () => {
      const {target} = getBrowserAndPages();
      await click(waitForElementWithTextContent('padding'));
