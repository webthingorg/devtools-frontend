...
describe('GPU track', () => {
  ...
  itScreenshot('renders the GPU track correctly (expanded)', async () => {
    ...
    await assertElementScreenshotUnchanged(flameChart, 'performance/gpu_track_expanded.png', 0);
  });

  itScreenshot('renders the GPU track correctly (collapsed)', async () => {
    await loadComponentDocExample(`${urlForTest}&expanded=false`);
    const flameChart = await waitFor('.flame-chart-main-pane');
    await assertElementScreenshotUnchanged(flameChart, 'performance/gpu_track_collapsed.png', 0);
  });
  ...
  itScreenshot('renders the track (dark mode and expanded)', async () => {
    ...
    const flameChart = await waitFor('.flame-chart-main-pane');
    await assertElementScreenshotUnchanged(flameChart, 'performance/gpu_track_expanded_dark_mode.png', 0);
  });
  ...
  itScreenshot('renders the track (dark mode and collapsed)', async () => {
    ...
    const flameChart = await waitFor('.flame-chart-main-pane');
    await assertElementScreenshotUnchanged(flameChart, 'performance/gpu_track_collapsed_dark_mode.png', 0);
  });
});
