...
- `LATE_PROMISES=true|N` - Delays all promises in the frontend by `N` number of milliseconds. Defaults to 50ms if set to `true`.
...
git cl try -B devtools-frontend/try \
  ...
  -p e2e_env="{ \
    \"TEST_PATTERNS\": \"$(git diff-tree --no-commit-id --name-only HEAD -r | tr "\n" ",")_\", \
    \"STRESS\": true, \
    \"LATE_PROMISES\": true, \
    \"ITERATIONS\": 20 \
  }"
