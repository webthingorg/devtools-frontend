...
* `--test-file=FILE_PATTERN` — run tests in selected test files only. The patterns define paths relative to the test/e2e/ directory. To run all sources panel tests, for example, use `--test-file=sources/*`. File extensions are optional, e.g. `--test-file=console/console-clear_test`.
