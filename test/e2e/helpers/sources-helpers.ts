...
export async function checkBreakpointIsActive(lineNumber: number) {
  await step(`check that the breakpoint is still active at line ${lineNumber}`, async () => {
    const codeLineClasses = await (await $((await sourceLineSelector(lineNumber))))!.evaluate(element => {
      return element.className;
    });
    assert.deepInclude(codeLineClasses, 'cm-breakpoint');
    assert.notDeepInclude(codeLineClasses, 'cm-breakpoint-disabled');
    assert.notDeepInclude(codeLineClasses, 'cm-breakpoint-unbound');
  });
}
