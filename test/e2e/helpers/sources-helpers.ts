...
export async function getValuesForScope(scope: string, expandCount = 0, expectedNodes = 0) {
  ...
  if (expectedNodes > 0) {
    await waitForFunction(async () => (await $$(valueSelector)).length >= expectedNodes);
  }
