...
export class WasmLocationLabels {
  #mappings: Map<string, LabelMapping[]>;
  #source: string;
  #wasm: string;
  constructor(source: string, wasm: string, mappings: Map<string, Array<LabelMapping>>) {
  ...
  static load(source: string, wasm: string): WasmLocationLabels {
    ...
    const mappings = new Map<string, Array<LabelMapping>>();
  ...
  async runToLabelInSource(label: string, script: string) {
  ...
  async runToLabelInWasm(label: string, script: string) {
    const {target} = getBrowserAndPages();
  ...
  async continueToLabel(label: string) {
    await click(RESUME_BUTTON);
    await this.checkLocationForLabel(label);
  }

  getMappingsForPlugin(): Array<LabelMapping> {
