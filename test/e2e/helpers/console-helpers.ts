...
export async function typeIntoConsoleAndWaitForResult(
    frontend: puppeteer.Page, message: string, leastExpectedMessages = 1) {
  // Get the current number of console results so we can check we increased it.
  const originalLength = await frontend.evaluate(() => {
    return document.querySelectorAll('.console-message').length;
  });

  await typeIntoConsole(frontend, message);

  await new AsyncScope().exec(
      () => frontend.waitForFunction((originalLength: number, leastExpectedMessages: number) => {
        return document.querySelectorAll('.console-message').length >= originalLength + leastExpectedMessages;
      }, {timeout: 0}, originalLength, leastExpectedMessages));
}
