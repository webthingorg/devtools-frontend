...
export async function getTextFromHeadersRow(row: puppeteer.ElementHandle<Element>) {
  ...
  if (headerValueText === '') {
    const headerValueEditableSpanElement = await waitFor('.header-value devtools-editable-span', row);
    const editableSpan = await waitFor('.editable', headerValueEditableSpanElement);
