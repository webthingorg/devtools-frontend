...
const COMPUTED_STYLES_PANEL_SELECTOR = 'aria/Computed panel';
const COMPUTED_STYLES_SHOW_ALL_SELECTOR = 'aria/Show all';
...
export const expandSelectedNodeRecursively = async () => {
  const EXPAND_RECURSIVELY = 'aria/Expand recursively';
...
export const forcePseudoState = async (pseudoState: string) => {
  ...
  await click('aria/Toggle Element State');
...
export const getDisplayedCSSPropertyNames = async (propertiesSection: puppeteer.JSHandle<any>) => {
  ...
  const propertyNamesText =
      (await Promise.all(cssPropertyNames.map(async node => await node.evaluate(n => n.textContent)))).filter(c => !!c);
...
export const getBreadcrumbsTextContent = async () => {
  ...
  const crumbsAsText: string[] =
      await Promise.all(crumbs.map(async node => await node.evaluate(node => node.textContent as string)));
  return crumbsAsText;
};
...
export const clickOnFirstLinkInStylesPanel = async () => {
  ...
  await click('div.styles-section-subtitle span.devtools-link', {
    root: stylesPane || undefined,
  });
};
