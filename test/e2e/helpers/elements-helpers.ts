...
export const typeInClassesPaneInput = async (text: string, commitWithKey: string = 'Enter') => {
  const {frontend} = getBrowserAndPages();
  ...
  await frontend.keyboard.press(commitWithKey);

  if (initialValue === 'Enter') {
    // When Escape is used to commit the change, the class is removed from the element, and the selected node goes back
    // to what it was.
