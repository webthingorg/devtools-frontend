...
export const waitForSelectedNodeChange = async (maxTotalTimeout = 1000) => {
  ...
  // Helper to get the text content of the currently selected element.
  ...
  const initialNodeContent = getContentOfSelectedNode();
  do {
    await timeout(30);
    if (getContentOfSelectedNode() !== initialNodeContent) {
      return true;
    }
