...
export async function filterDataGrid(filter: string) {
  const element = await $('.toolbar-input-prompt') as puppeteer.ElementHandle;
  await element.type(filter);
}

export async function clearDataGridFilter() {
  await click('.toolbar-input-clear-button');
}

export async function clearDataGrid() {
  // Clear all cookies
