...
import {$, getTextContent, platform, waitForElementWithTextContent} from '../../shared/helper.js';
import {
  $$,
  click,
  clickElement,
...
export async function findSearchResult(searchResult: string, pollIntrerval: number = 100) {
  const match = await waitFor('#profile-views table.data');
  await waitForFunction(async () => {
    const selectedTextBefore = await getTextContent('#profile-views table.data tr.selected');
    await click('[aria-label="Search next"]');
    await waitForFunction(async () => {
      const selectedTextAfter = await getTextContent('#profile-views table.data tr.selected');
      return selectedTextAfter !== selectedTextBefore;
    });
