 ...
 */
export async function waitForSomeRequestsToAppear(numberOfRequests: number, selector = '.name-column') {
  const {frontend} = getBrowserAndPages();

  await frontend.waitForFunction((numberOfRequests: number, selector: string) => {
    return document.querySelectorAll(selector).length >= numberOfRequests;
  }, {}, numberOfRequests, REQUEST_LIST_SELECTOR + ' ' + selector);
}
