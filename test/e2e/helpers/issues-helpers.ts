...
export async function assertBlockedStatus() {
  const issueMessageElement = await waitFor(BLOCKED_STATUS);
  const selectedIssueMessage = await issueMessageElement.evaluate(node => node.textContent);
  assert.strictEqual(selectedIssueMessage, 'blocked');
}

export async function assertReportOnlyStatus() {
  const issueMessageElement = await waitFor(REPORT_ONLY_STATUS);
  const selectedIssueMessage = await issueMessageElement.evaluate(node => node.textContent);
  assert.strictEqual(selectedIssueMessage, 'report-only');
}
