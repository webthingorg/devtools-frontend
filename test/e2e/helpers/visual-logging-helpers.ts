...
function formatEvents(events: string[]) {
  ...
  for (const event of events.sort()) {
    while (!event.startsWith(lastEvent)) {
      lastEvent = lastEvent.substr(0, lastEvent.lastIndexOf(' > '));
    }
    result.push(' '.repeat(lastEvent.length) + event.substr(lastEvent.length));
    lastEvent = event;
  }
  return result.join('\n');
}
      ...
      if (actualEvents.length <= i) {
        bestError ||= {
          ...
          description: 'interaction' in expectedEvent ?
              ...
              'Missing VE impressions:\n' + formatEvents(expectedEvent.impressions),
        };
