...
export function veImpressionForMainToolbar(options?: {
  ...
  const regularPanels = ['elements', 'console', 'sources', 'network', 'timeline', 'heap-profiler'];
  if (!options?.dockable) {
...
export async function expectVeImpressions(expectedImpressions: string[]) {
  ...
  let actualImpressions: string[]|null = null;
  for (let i = actualEvents.length - 1; i >= 0; --i) {
    const event = actualEvents[i];
    if ('impressions' in event) {
      actualImpressions = event.impressions;
      break;
    }
  }
  ...
  assert.isTrue(match, description);
}
