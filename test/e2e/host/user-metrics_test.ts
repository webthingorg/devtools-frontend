...
declare global {
  interface Window {
    ...
    __cssGridSettings: (evt: Event) => void;
...
async function beginCatchEvents(frontend: puppeteer.Page) {
  await frontend.evaluate(() => {
    ...
    window.__cssGridSettings = (evt: Event) => {
      const customEvt = evt as CustomEvent;
      window.__caughtEvents.push({name: 'DevTools.CSSGridSettings', value: customEvt.detail.value});
    };
    ...
    window.__beginCatchEvents = () => {
      ...
      window.addEventListener('DevTools.CSSGridSettings', window.__cssGridSettings);
    };
    ...
    window.__endCatchEvents = () => {
      ...
      window.removeEventListener('DevTools.CSSGridSettings', window.__cssGridSettings);
    };
