...
describe('The Extension API', async () => {
  ...
  it('handles resource paths correctly', async () => {
    ...
    await extension.evaluate(async () => {
      await new Promise(r => window.chrome.devtools.panels.create('extension-tab-title', '', 'http://example.com', r));
    });
    ...
    {
      // absolute URL paths work
      await extension.evaluate(async () => {
        await new Promise(r => window.chrome.devtools.panels.create('extension-tab-title', '', '/blank.html', r));
      });
