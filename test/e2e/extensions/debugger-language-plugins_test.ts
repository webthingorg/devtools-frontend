...
describe('The Debugger Language Plugins', async () => {
  ...
  it('shows a warning when no debug info is present', async () => {
    ...
    await extension.evaluate(() => {
      class MissingInfoPlugin {
        ...
        async addRawModule() {
          return {missingDebugFiles: ['test.wasm']};
        }
      }
  ...
  it('shows warnings when function info not present', async () => {
    ...
    await extension.evaluate(() => {
      class MissingInfoPlugin {
        ...
        async addRawModule(rawModuleId: string, symbols: string, rawModule: Chrome.DevTools.RawModule) {
          ...
          chrome.devtools.languageServices.reportResourceLoad(
              new URL('test.dwo', rawModule.url || symbols).href, {success: false, errorMessage: '404'});
          return [sourceFileURL];
        }
        ...
        async getFunctionInfo() {
          return {missingDebugFiles: ['test.dwo']};
        }
    ...
    assert.deepEqual(cellContents.filter(f => f !== ''), [
      'failure',
      `${getResourcesPath()}/sources/wasm/test.dwo`,
      `https://${getDevToolsFrontendHostname()}:${getTestServerPort()}`,
      '404',
    ]);
  });
