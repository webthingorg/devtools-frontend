...
describe('The Debugger Language Plugins', async () => {
  ...
  it('reports loaded files to the developer resources panel', async () => {
    ...
    await extension.evaluate(() => {
      class MissingInfoPlugin {
        async addRawModule(rawModuleId: string, symbols: string, rawModule: Chrome.DevTools.RawModule) {
          ...
          chrome.devtools.languageServices.reportResourceLoad(missingDwoName, {success: false, errorMessage: '404'});
    ...
    const moduleName = 'unreachable.wasm';
    const initiatorUrl = `https://${getDevToolsFrontendHostname()}:${getTestServerPort()}`;
    const reportedModuleRequest = allRequestContents.find(row => row[1]?.includes(moduleName));
    const reportedDWORequest = allRequestContents.find(row => row[1]?.includes(missingDwoName));
    assert.deepEqual(reportedModuleRequest?.filter(c => Boolean(c)), [
      'success',
      `${getResourcesPath()}/sources/wasm/${moduleName}`,
      initiatorUrl,
      '20',
    ]);
