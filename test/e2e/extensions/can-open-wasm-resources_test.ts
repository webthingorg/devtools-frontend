...
describe('The Extension API', async () => {
  ...
  it('can open page resources with column numbers', async () => {
    ...
    // Accepts a column number, but the column number is ignored
    await extension.evaluate(resource => window.chrome.devtools.panels.openResource(resource, 1, 2000), resource);
