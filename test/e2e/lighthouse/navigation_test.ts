...
describe.skipOnParallel('Navigation', async function() {
  ...
  for (const mode of modes) {
    describe(`in ${mode} mode`, () => {
      // for debugging
      it.skip('[crbug.com/1418288]: successfully returns a Lighthouse report', async () => {
      ...
      it.only('successfully returns a Lighthouse report with DevTools throttling', async () => {
        await navigateToLighthouseTab('lighthouse/hello.html');
      ...
      // for debugging
      it.skip('[crbug.com/1418288]: successfully returns a Lighthouse report when settings changed', async () => {
