...
import {getAllRequestNames, getSelectedRequestName, navigateToNetworkTab, selectRequest, togglePersistLog, waitForSelectedRequestChange, waitForSomeRequestsToAppear} from '../helpers/network-helper.js';
...
describe('The Network Tab', async () => {
  ...
  it('can persist requests', async () => {
    ...
    // Wait for all the requets to be displayed + 1 to account for the page itself, and get their names.
    await waitForSomeRequestsToAppear(SIMPLE_PAGE_REQUEST_NUMBER + 1);
