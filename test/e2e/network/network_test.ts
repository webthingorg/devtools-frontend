...
import {getAllRequestNames, getSelectedRequestName, navigateToNetworkTab, selectRequest, togglePersistLog, waitForSelectedRequestChange, waitForSomeRequestsToAppear} from '../helpers/network-helpers.js';
...
describe('The Network Tab', async () => {
  ...
  it('can select requests', async () => {
    ...
    // Select the request at index 0. This will select the test HTML page itself.
    ...
    // In order to select the last request in the list, select at index SIMPLE_PAGE_REQUEST_NUMBER.
    await selectRequest(SIMPLE_PAGE_REQUEST_NUMBER);
    await waitForSelectedRequestChange(selected);

    selected = await getSelectedRequestName();
    assert.strictEqual(
        selected, `image.svg?id=${SIMPLE_PAGE_REQUEST_NUMBER - 1}`, 'Selecting the last request should work');
  });
  ...
  it('can persist requests', async () => {
    ...
    assert.deepStrictEqual(firstPageRequestNames, secondPageRequestNames, 'The requests were persisted');
  });
});
