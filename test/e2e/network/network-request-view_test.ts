...
describe('The Network Request view', async () => {
  ...
  function assertOutlineTextMatches(expectedPatterns: string[], outlineText: string[]) {
    const regexpSpecialChars = /[-\/\\^$*+?.()|[\]{}]/g;
    for (const item of outlineText) {
  ...
  it('shows request headers and payload', async () => {
    ...
    for (const sectionContent of expectedHeadersContent) {
      ...
      assertOutlineTextMatches(sectionContent.rows, rowsText);
    }
    ...
    assertOutlineTextMatches(expectedPayloadContent, payloadOutlineText);
  });
  ...
  it('shows raw headers', async () => {
    ...
    assertOutlineTextMatches([expectedRawHeadersContent], [rawHeadersText]);
