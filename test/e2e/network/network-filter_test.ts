...
import {navigateToNetworkTab, setCacheDisabled, setPersistLog} from '../helpers/network-helpers.js';
...
describe('The Network Tab', async function() {
  ...
  it('can filter by cache status in the log view', async () => {
    await setCacheDisabled(true);
    await navigateToNetworkTab('requests.html?num=5');
    await setPersistLog(true);
    await setCacheDisabled(false);
    await navigateToNetworkTab(`requests.html?num=3&nocache=${Math.random()}`);
