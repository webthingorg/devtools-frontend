...
describe('The Network Tab', async () => {
  ...
  it.repeat(100, '[crbug.com/1066813] shows Last-Modified', async () => {
    ...
    await step('Check that the "Last-Modified" column has the expected values', async () => {
      waitForFunction(async () => {
        const lastModifiedColumnValues =
            await frontend.$$eval('pierce/.last-modified-column', cells => cells.map(element => element.textContent))
                .catch(() => []);
        return lastModifiedColumnValues === ['Last-Modified', '', 'Sun, 26 Sep 2010 22:04:35 GMT'] || null;
      });
    });
  });
