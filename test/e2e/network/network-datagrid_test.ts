...
describe('The Network Tab', async function() {
  ...
  it('shows the main service worker request as complete', async () => {
    ...
    await waitForFunction(async () => {
      const {status} = await getRequestRowInfo(frontend, 'service-worker.html/test/e2e/resources/network');
      return status === '200OK';
    });
    ...
    await waitForFunction(async () => {
      const {status} = await getRequestRowInfo(frontend, '⚙ service-worker.js/test/e2e/resources/network');
      return status === '200OK';
    });
