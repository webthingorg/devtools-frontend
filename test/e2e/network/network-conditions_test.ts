...
describe('The Network Tab', async function() {
  ...
  it('can override userAgentMetadata', async () => {
    ...
    const noMetadataVersionUserAgentMetadataExpected = {'brands': [], mobile: false};

    const getUserAgentMetaData = '({brands: navigator.userAgentData.brands, mobile: navigator.userAgentData.mobile})';
    ...
    await uaDropdown.click();
    const fixedVersionUserAgentMetadata = await target.evaluate(getUserAgentMetaData);
    ...
    await uaDropdown.click();
    const dynamicVersionUserAgentMetadata = await target.evaluate(getUserAgentMetaData);
    ...
    await uaDropdown.click();
    const noMetadataVersionUserAgentMetadata = await target.evaluate(getUserAgentMetaData);
