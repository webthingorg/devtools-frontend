...
describe('The Network Tab', async function() {
  ...
  it('can override userAgentMetadata', async () => {
    ...
    const fixedVersionUserAgentMetadata1Expected = JSON.stringify({
      'brands': [
    ...
    const dynamicVersionUserAgentMetadata2Expected = JSON.stringify({
      'brands': [
        {'brand': 'Not A;Brand', 'version': '99'},
        {'brand': 'Chromium', 'version': version},
        {'brand': 'Google Chrome', 'version': version},
    ...
    const noMetadataVersionUserAgentMetadata3Expected = JSON.stringify({'brands': [], mobile: false});
    ...
    assert.strictEqual(userAgentMetadata1, fixedVersionUserAgentMetadata1Expected);

    await uaDropdown.click();
    await uaDropdown.select(dynamicVersionUAValue);
    await uaDropdown.click();
    const userAgentMetadata2 = await target.evaluate(getUserAgentMetaData);
    assert.strictEqual(userAgentMetadata2, dynamicVersionUserAgentMetadata2Expected);

    await uaDropdown.click();
    await uaDropdown.select(noMetadataVersionUAValue);
    await uaDropdown.click();
    const userAgentMetadata3 = await target.evaluate(getUserAgentMetaData);
    assert.strictEqual(userAgentMetadata3, noMetadataVersionUserAgentMetadata3Expected);
  });
});
