...
describe('The Network Tab', async () => {
  ...
  it('shows Last-Modified', async () => {
    ...
    const lastModifiedColumnValues = await networkTabPageObject.retrieveColumnValues(networkTabPageObject.lastModifiedColumn);
  ...
  it('shows correct MimeType when resources came from HTTP cache', async () => {
    ...
    let obtainNetworkRequestSize = (await networkTabPageObject.retrieveColumnValues(networkTabPageObject.sizeColumn)).slice(1, 3);
    let obtainNetworkRequestMimeTypes = (await networkTabPageObject.retrieveColumnValues(networkTabPageObject.typeColumn)).slice(1, 3);

    const computeByteSize = (value: number) => {
      return `${value}\xA0B`;
    };

    assert.deepEqual(obtainNetworkRequestSize, [
      computeByteSize(378) + computeByteSize(258),
      computeByteSize(362) + computeByteSize(28),
    ]);
    assert.deepEqual(obtainNetworkRequestMimeTypes, [
      `document`,
      `script`,
    ]);
    ...
    obtainNetworkRequestSize = (await networkTabPageObject.retrieveColumnValues(networkTabPageObject.sizeColumn)).slice(1, 3);
    obtainNetworkRequestMimeTypes = (await networkTabPageObject.retrieveColumnValues(networkTabPageObject.typeColumn)).slice(1, 3);
