...
import {$$, click, enableExperiment, goToResource, typeText, waitFor, waitForFunction} from '../../shared/helper.js';
import {describe, it} from '../../shared/mocha-extensions.js';
import {openPanelViaMoreTools} from '../helpers/settings-helpers.js';

async function getDatagridRows(nRows: number, root?: ElementHandle<Element>): Promise<ElementHandle<Element>[][]> {
  ...
  const rowsHandler = await waitForFunction(async () => {
    ...
    return (rows.length === nRows) ? rows : undefined;
  });
