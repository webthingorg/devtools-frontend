...
describe('CORS issues', async () => {
  ...
  it('should display CORS ACAO mismatches with the correct affected resources', async () => {
    ...
    await target.evaluate(async () => {
      ...
      try {
        const url2 = new URL('./issues/acao-mismatch.rawresponse', document.location.toString())
                         .toString()
                         .replace('localhost', 'devtools.oopif.test');
        await fetch(url2);
      } catch (e) {
  ...
  it('should display invalid CORS ACAC values with the correct affected resources', async () => {
    ...
    await target.evaluate(async () => {
      ...
      try {
        const url2 = new URL('./issues/acac-invalid.rawresponse', document.location.toString())
                         .toString()
                         .replace('localhost', 'devtools.oopif.test');
        await fetch(url2, {credentials: 'include'});
      } catch (e) {
      }
    });
    await navigateToIssuesTab();
    ...
    if (issueElement) {
      ...
      if (table) {
        ...
        assert.deepEqual(table[0], [
          ...
          'Status',
          'Preflight Request (if problematic)',
          'Allow Credentials Value from Header',
        ]);
        matchArray(table[1], [
