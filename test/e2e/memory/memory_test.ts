...
describe('The Memory Panel', async function() {
  ...
  it('shows object source links in snapshot', async () => {
    ...
    for (const entry of expectedEntries) {
      ...
      for (const r of rows) {
        const constructorName = await waitForFunction(async () => {
          return await r.evaluate(e => e.firstChild?.textContent);
        });
      ...
      const linkText = await waitForFunction(async () => {
        return await element?.evaluate(e => e.querySelector('.devtools-link')?.textContent);
      });
