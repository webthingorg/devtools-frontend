...
describe('The Performance panel', function() {
  ...
  it('CSS selector stats performance test', async () => {
    // set a tentative threshold of 1%
    const timeDiffPercentageMax = 0.01;

    await navigateToPerformanceTab('selectorStats/page-with-style');
    await disableCSSSelectorStats();
    await reloadAndRecord();
    const recordingTimeWithSelectorStatsDisabled = await getTotalTimeFromSummary();

    await enableCSSSelectorStats();
    await reloadAndRecord();
    const recordingTimeWithSelectorStatsEnabled = await getTotalTimeFromSummary();
