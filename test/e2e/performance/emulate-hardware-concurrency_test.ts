...
import {nonNull, waitFor, waitForAria, waitForFunction} from '../../shared/helper.js';
...
describe('The Performance panel', () => {
  it('can emulate navigator.hardwareConcurrency', async () => {
    ...
    await waitForFunction(async () => await warning.evaluate(e => e.style.visibility) === 'visible');

    // Check that resetting the value works:
    const button = nonNull(await waitForAria('Reset to the default value'));
    await button.click();
