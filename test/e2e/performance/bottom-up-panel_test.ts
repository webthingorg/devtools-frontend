...
describe('The Performance tool, Bottom-up panel', async function() {
  ...
  beforeEach(async () => {
    await step('navigate to the Performance tab and upload performance profile', async () => {
      ...
      await uploadProfileHandle.uploadFile('test/e2e/resources/performance/bottom-up/bottom-up-test-trace.json');
    });
  });
  ...
  it('simple filter is working as expected', async () => {
    const expectedActivities = ['h2', 'h2_with_suffix'];
  ...
  it('filtered results keep context', async () => {
    ...
    const expectedActivities = ['h2', 'container1', 'Function Call', 'Timer Fired', 'Parse HTML', 'Function Call'];
