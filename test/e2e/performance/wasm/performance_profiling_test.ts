...
import {$$, dragAndDrop, getBrowserAndPages, step, waitFor, waitForFunction} from '../../../shared/helper.js';
...
async function expandActivityTree(frontend: puppeteer.Page, numExpectedActivities: number) {
  ...
  await waitForFunction(async () => {
    const expandedActivities = await $$('.data-grid-data-grid-node.parent.revealed.expanded');
    return expandedActivities.length === (numExpectedActivities - 1);
  });
}
...
describe('The Performance panel', async function() {
  ...
  it('is able to inspect the call stack for a wasm function from the bottom up', async () => {
    ...
    await step('increase the size of the activities panel', async () => {
      await dragAndDrop(frontend, 577, 407, 577, 407 + 100);
      await dragAndDrop(frontend, 550, 281, 550, 281 - 100);
    });
