...
import {$, debuggerStatement, getBrowserAndPages, step, waitFor, waitForElementWithTextContent, waitForFunction} from '../../../shared/helper.js';
import {describe, it} from '../../../shared/mocha-extensions.js';
import {ACTIVITY_COLUMN_SELECTOR, clickOnFunctionLink, getTotalTimeFromSummary, navigateToBottomUpTab, navigateToCallTreeTab, navigateToPerformanceTab, retrieveActivity, searchForComponent, startRecording, stopRecording} from '../../helpers/performance-helpers.js';
...
describe('The Performance panel', async function() {
  ...
  it.only('is able to inspect the call stack for a wasm function from the bottom up', async () => {
    ...
    await step(
        'expand the tree for the "mainWasm" activity and check that it displays the correct values', async () => {
          const mainWasmActivity = await retrieveActivity(expectedActivities[0]);
          if (!mainWasmActivity) {
