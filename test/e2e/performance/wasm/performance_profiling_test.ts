...
import {ACTIVITY_COLUMN_SELECTOR, clickOnFunctionLink, getTotalTimeFromSummary, navigateToBottomUpTab, navigateToCallTreeTab, navigateToPerformanceTab, searchForComponent, startRecording, stopRecording} from '../../helpers/performance-helpers.js';
  ...
  it('is able to inspect the call stack for a wasm function from the bottom up', async () => {
    ...
    await step(
        'expand the tree for the "mainWasm" activity and check that it displays the correct values', async () => {
          ...
          const bottomUpTab = await $('.timeline-tree-view') as puppeteer.ElementHandle<HTMLSelectElement>;
          const mainWasmActivity = await waitForElementWithTextContent('mainWasm', bottomUpTab);
          if (!mainWasmActivity) {
