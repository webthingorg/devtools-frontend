...
import {click, getBrowserAndPages, getHostedModeServerPort, waitForFunction} from '../../shared/helper.js';
...
describe('The Application Tab', async () => {
  ...
  it('deletes only first party cookies when clearing site data', async () => {
    ...
    assert.deepEqual(dataGridRowValuesBefore, [
      {
        name: 'third_party',
        value: 'test',
      },
    ...
    assert.deepEqual(dataGridRowValuesAfter, [{
                       name: 'third_party',
                       value: 'test',
                     }]);
  });
