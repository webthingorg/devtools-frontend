...
describe('The Reporting API Page', async () => {
  ...
  it('shows reports', async () => {
    ...
    console.log('ROWS', rows.length);  // eslint-disable-line no-console

    let jsonView;
    await waitForFunction(async () => {
      await click(rows[rows.length - 1][0]);
      jsonView = await waitFor('.json-view');
      return Boolean(jsonView);
    });
    assert.isTrue(Boolean(jsonView));
    assert.isDefined(jsonView);
