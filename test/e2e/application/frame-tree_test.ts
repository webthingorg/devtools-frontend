...
describe('The Application Tab', async () => {
  ...
  it('can handle when JS writes to frame', async () => {
    ...
    await waitForFunction(async () => {
      const fieldValues = await getReportValues();
      const expected = [
        `https://localhost:${getHostedModeServerPort()}/test/e2e/resources/application/iframe.html`,
        '',
    ...
    await waitForFunction(async () => {
      const fieldValues = await getReportValues();
      const expected = [
        `https://localhost:${getHostedModeServerPort()}/test/e2e/resources/application/main-frame.html`,
        '',
        `https://localhost:${getHostedModeServerPort()}`,
        '<iframe>',
        '',
