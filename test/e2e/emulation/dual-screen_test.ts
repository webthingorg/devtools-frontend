...
import {getButtonDisabled, selectSpanButton} from '../helpers/emulation-helpers.js';
...
describe('Dual screen mode', async () => {
  ...
  it('User may not click span button for a non-dual screen device', async () => {
    ...
    // span button should be disabled.
    const spanButton = await selectSpanButton();
    const isDisabled = await getButtonDisabled(spanButton);
    assert(isDisabled);
  });
});
