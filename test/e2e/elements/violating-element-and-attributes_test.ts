 ...
 */
describe('[crbug.com/1399414]: Element has violating properties', async function() {
  ...
  it('navigate to issues panel on hover', async () => {
    ...
    const expectedTitle = 'A form field element should have an id or name attribute';
    assert.strictEqual(popupText, expectedTitle);
    // Open the issue panel and look for the title.
    await click('div.widget.has-padding');
    ...
    assert.strictEqual(issueTitle, expectedTitle);
  });
});
