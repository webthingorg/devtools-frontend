...
describe('The Computed pane', async () => {
  ...
  it('can display inherited CSS properties of the selected element', async () => {
    ...
    assert.isTrue(!!alignContentProperty, 'H1 element should inherit the align-content computed property');
    assert.strictEqual(alignContentProperty && alignContentProperty.value, 'normal');
  });
});
