...
describe('The Styles pane', async () => {
  ...
  it('can display inherited CSS highlight pseudo styles', async () => {
    ...
    const h1Rules = await getDisplayedStyleRules();
    // The 4 rule blocks for the h1 are:
    ...
    // 5. The h1's inherited selection pseudo
    // And there is no 6th block for the ::first-letter style, since only
    // highlight pseudos are inherited.
    assert.strictEqual(h1Rules.length, 5, 'The h1 should have 4 style rule blocks');
    assert.deepEqual(
    ...
    assert.deepEqual(
        h1Rules[4], {selectorText: 'body::selection', propertyNames: ['text-shadow']},
    ...
    const sidebarSeparators = await waitForFunction(async () => {
      ...
      return separators.length === 3 ? separators : null;
    });
    ...
    assert.deepEqual(layerText, [
      ...
      'Inherited from pseudo ::highlight of ',
      'Inherited from pseudo ::selection of ',
    ]);
  });
});
