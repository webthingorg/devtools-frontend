...
import {assertSearchResultMatchesText, summonAndWaitForSearchBox, waitForElementsStyleSection, waitForSelectedNodeToBeExpanded} from '../helpers/elements-helpers.js';
...
describe('The Elements tab', async function() {
  ...
  describe('when searchAsYouType setting is disabled', () => {
    ...
    it.only('search should jump to next match when Enter is pressed when the input is not changed',
      async () => {
        await goToResource('elements/elements-search-test.html');
    ...
    it.only('search should be performed with the new query when the input is changed and Enter is pressed', async () => {
      await goToResource('elements/elements-search-test.html');
      await waitForElementsStyleSection();
      await summonAndWaitForSearchBox();
      const {frontend} = getBrowserAndPages();
