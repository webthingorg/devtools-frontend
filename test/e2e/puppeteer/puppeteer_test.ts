...
describe('Puppeteer', () => {
  ...
  it('should connect to the browser via DevTools own connection using the tab target', async () => {
    ...
    const result = await frontend.evaluate(`(async () => {
      ...
      const mainTarget = SDK.TargetManager.TargetManager.instance().rootTarget();
      if (!mainTarget) {
        throw new Error('Could not find main target');
      }
      const childTargetManager = mainTarget.model(SDK.ChildTargetManager.ChildTargetManager);
