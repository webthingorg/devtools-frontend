...
import {click, enableExperiment, getBrowserAndPages, getTestServerPort, goToResource, timeout, waitFor, waitForFunction} from '../../shared/helper.js';
...
(async () => {
    ...
    }
    {
        const targetPage = page;
        const frame = targetPage.mainFrame();
        const element = await frame.waitForSelector("input#input");
...
`);
  ...
  // TODO(https://github.com/puppeteer/puppeteer/issues/2548): Enable again when puppeteer supports OOP iframes properly
  it.skip('[crbug.com/]should support oo', () => {
    await target.bringToFront();
    const oopIframeTarget = await browser.waitForTarget(t => t.url().endsWith('oop-iframe.html'));
    console.log(oopIframeTarget);
    await oopIframe.click('aria/Button in OOP iframe');
    await waitForScriptToChange();
  });
});
