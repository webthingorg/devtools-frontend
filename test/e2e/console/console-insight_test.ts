...
describe('ConsoleInsight', async function() {
 ...
 Error: Error 2`,
    ...
    const messageGetter = async(consoleElement: Element, includeInlineStackTrace: boolean): Promise<string> => {
      // @ts-ignore
      const Console = await import('./panels/console/console.js');
      const consoleViewMessage = Console.ConsoleViewMessage.getMessageForElement(consoleElement);
    ...
    for (let testIdx = 0; testIdx < messages.length; testIdx++) {
      const messageWithoutStacktrace = await frontend.evaluate(messageGetter, messages[testIdx], false);
      assert.deepStrictEqual(messageWithoutStacktrace, tests[testIdx].expectedWithoutStracktrace);
    }

    for (let testIdx = 0; testIdx < messages.length; testIdx++) {
      const messageWithStacktrace = await frontend.evaluate(messageGetter, messages[testIdx], true);
