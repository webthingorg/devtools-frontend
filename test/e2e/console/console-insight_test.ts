...
describe('ConsoleInsight', function() {
  ...
  async function setupMocks(
    ...
    await frontend.evaluateOnNewDocument(
        `globalThis.hostConfigForTesting = ${JSON.stringify({aidaAvailability, devToolsConsoleInsights})};`);
    await frontend.reload({
      waitUntil: 'networkidle0',
    });
  }

  // Disabled until corresponding backend changes have landed.
  it.skip('[crbug.com/348136212] shows an insight for a console message via the context menu', async () => {
