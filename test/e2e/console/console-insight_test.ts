...
describe('ConsoleInsight', function() {
  ...
  it('shows an insight for a console message via the context menu', async () => {
    ...
    await setupMocks({'DevToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true}});
  ...
  it('does not show context menu if AIDA is not available', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks({'DevToolsConsoleInsights': {'blockedByFeatureFlag': true, 'enabled': false}});
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
  ...
  it('does not show the hover button if age check is not passing', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'DevToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByAge': true}});
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
  ...
  it('does not show the hover button if the feature is not rolled out', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'DevToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByRollout': true}});
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
  ...
  it('does not show the hover button if it is restriced by geography', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'DevToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByGeo': true}});
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
  ...
  it('does not show the hover button if disabled by default', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks({'DevToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'optIn': true}});
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
