...
describe('ConsoleInsight', function() {
  ...
  it('shows an insight for a console message via the context menu', async () => {
    ...
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true}} as Root.Runtime.HostConfig);
  ...
  it('does not show context menu if AIDA is not available', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': true, 'enabled': false}} as Root.Runtime.HostConfig);
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
    });
  ...
  it('does not show the hover button if age check is not passing', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByAge': true}} as
  ...
  it('does not show the hover button if policy does not allow it', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks({
      'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByEnterprisePolicy': true},
  ...
  it('does not show the hover button if the feature is not rolled out', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByRollout': true}} as
        Root.Runtime.HostConfig);
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
  ...
  it('does not show the hover button if it is restriced by geography', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'blockedByGeo': true}} as
        Root.Runtime.HostConfig);
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
  ...
  it('does not show the hover button if disabled by default', async () => {
    const {target} = getBrowserAndPages();
    await setupMocks(
        {'devToolsConsoleInsights': {'blockedByFeatureFlag': false, 'enabled': true, 'optIn': true}} as
        Root.Runtime.HostConfig);
    await click(CONSOLE_TAB_SELECTOR);
    await target.evaluate(() => {
      console.error(new Error('Unexpected error'));
