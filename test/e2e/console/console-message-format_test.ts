...
describe('The Console Tab', async () => {
  it('shows BigInts formatted', async () => {
    const messages = await getConsoleMessages(5, 'big-int');
  ...
  it('shows uncaught promises', async () => {
    const messages = await getConsoleMessages(2, 'uncaught-promise');
  ...
  it('shows structured objects', async () => {
    const messages = await getConsoleMessages(9, 'structured-objects');
  ...
  it('escapes and substitutes correctly', async () => {
    const messages = await getConsoleMessages(9, 'escaping');
  ...
  it('shows built-in objects', async () => {
    const messages = await getConsoleMessages(29, 'built-ins');
  ...
  it('shows primitives', async () => {
    const messages = await getConsoleMessages(15, 'primitives');
  ...
  it('can handle prototype fields', async () => {
    const messages = await getConsoleMessages(13, 'prototypes');
  ...
  it('can show DOM interactions', async () => {
    const messages = await getConsoleMessages(13, 'dom-interactions');
  ...
  it('can handle sourceURLs in exceptions', async () => {
    const messages = await getConsoleMessages(1, 'source-url-exceptions');
  ...
  it('can show stackoverflow exceptions', async () => {
    const messages = await getConsoleMessages(1, 'stack-overflow');
  ...
  it('can show document.write messages', async () => {
    const messages = await getConsoleMessages(2, 'document-write');
  ...
  it('can show verbose promise unhandledrejections', async () => {
    const messages = await getConsoleMessages(4, 'onunhandledrejection', false, showVerboseMessages);
  ...
  describe('shows messages from before', async () => {
    it('iframe removal', async () => {
      const messages = await getConsoleMessages(3, 'navigation/after-removal');
    ...
    it('and after iframe navigation', async () => {
      const messages = await getConsoleMessages(3, 'navigation/after-navigation');
