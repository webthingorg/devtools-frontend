...
describe('The Console Tab', async () => {
  const tests = [
    {
      description:
        'produces console messages when a page logs using console.log',
      evaluate: () => console.log('log'),
      expectedMessageCount: 1,
    },
    {
      description:
        'produces console messages when a page logs using console.debug',
      evaluate: () => console.debug('debug'),
      expectedMessageCount: 1,
    },
    {
      description:
        'produces console messages when a page logs using console.warn',
      evaluate: () => console.warn('warn'),
      expectedMessageCount: 1,
    },
    {
      description:
        'produces console messages when a page logs using console.error',
      evaluate: () => console.error('error'),
      expectedMessageCount: 1,
    },
    {
      description:
        'produces a single console message when messages are repeated',
      evaluate: () => {
        for (let i = 0; i < 5; ++i) {
    ...
    {
      description:
        'counts how many time console.count has been called with the same message',
      evaluate: () => {
        for (let i = 0; i < 2; ++i) {
          console.count('count');
        }
    ...
    {
      description:
        'creates an empty group message using console.group/console.groupEnd',
      evaluate: () => {
        console.group('group');
        console.groupEnd();
      },
    ...
    {
      description:
        'creates a collapsed group using console.groupCollapsed with all messages in between hidden',
      evaluate: () => {
  ...
  for (const test of tests) {
    it(test.description, async () => {
      ...
      const actualMessages = await waitForFunction(async () => {
        ...
        return messages.length === test.expectedMessageCount ? messages : undefined;
      });
      assertMatchesJSONSnapshot(actualMessages);
    });
  }
  ...
  describe('keyboard navigation', () => {
    it('can navigate between individual messages', async () => {
      ...
      await tabBackward();
      assert.strictEqual(
        await activeElementTextContent(),
        'focus-interaction.html:9',
      );

      await frontend.keyboard.press('ArrowUp');
      assert.strictEqual(
        await activeElementTextContent(),
        'focus-interaction.html:9 Third message',
      );

      await frontend.keyboard.press('ArrowUp');
      assert.strictEqual(
        await activeElementTextContent(),
        'focus-interaction.html:8',
      );
      ...
      assert.strictEqual(
        await activeElementAccessibleName(),
        'Console settings',
      );
  ...
  describe('Console log message formatters', () => {
    async function getConsoleMessageTextChunksWithStyle(
      ...
      return await frontend.evaluate(
        (selector, styles) => {
          return [...document.querySelectorAll(selector)].map(message =>
            [...message.childNodes].map(node => {
              // For all nodes, extract text.
              const result = [node.textContent as string];
              // For element nodes, get the requested styles.
              for (const style of styles) {
                result.push(
                  ((node as HTMLElement).style?.[style] as string) ?? '',
                );
              }
              return result;
            }),
    ...
    it('expand %c formatter with color style', async () => {
      ...
      await navigateToConsoleTab();
      await target.evaluate(() =>
        console.log('PRE%cRED%cBLUE', 'color:red', 'color:blue'),
      );

      await waitForConsoleMessages(1);

      // Extract the text and color.
      const textsAndStyles = await getConsoleMessageTextChunksWithStyle(
        frontend,
        ['color'],
      );
      assert.deepEqual(textsAndStyles, [
        [
          ['PRE', ''],
          ['RED', 'red'],
          ['BLUE', 'blue'],
  ...
  describe('for memory objects', () => {
    const MEMORY_ICON_SELECTOR =
      '[aria-label="Reveal in Memory Inspector panel"]';
    ...
    it('shows two memory icons to open memory inspector for a TypedArray (description, buffer)', async () => {
      ...
      const arrayBufferMemoryIcon = await $(
        MEMORY_ICON_SELECTOR,
        arrayBufferProperty,
      );
    ...
    it('shows two memory icons to open memory inspector for a DataView (description, buffer)', async () => {
      const { frontend } = getBrowserAndPages();
      await navigateToConsoleTab();
      await typeIntoConsoleAndWaitForResult(
        frontend,
        'new DataView(new Uint8Array(10).buffer)',
      );
