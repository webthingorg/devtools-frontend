...
import {$, click, getBrowserAndPages, goToResource, typeText} from '../../shared/helper.js';
...
describe('The Console Tab', async () => {
  it('shows messages with stack traces', async () => {
    ...
    await frontend.waitForSelector('.console-message-stack-trace-wrapper');
    await click('.console-message-stack-trace-wrapper');
    const stack = await $('.stack-preview-container');
    const stackTraceRows = await frontend.evaluate((stack: Element) => {
      return Array.from(stack.querySelectorAll('tr')).map(node => ({text: node.textContent, classes: node.classList}));
    }, stack);
    ...
    const expected = [
      ...
      {text: '\nShow 0 more frames', classes: {'0': 'show-all-link'}},
    ];
    assert.strictEqual(JSON.stringify(stackTraceRows), JSON.stringify(expected));
  });

  it('shows messages with stack traces containing ignore-listed frames', async () => {
    const {frontend} = getBrowserAndPages();
    await openSettingsTab('Ignore List');
    await click('button[aria-label="Add filename pattern"]');
    await typeText('ignoreMe.js');
    await click('.editor-buttons .primary-button');
    await click('.close-button[aria-label="Close"]');
