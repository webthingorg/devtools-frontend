...
import {default as ConsoleTabPO} from '../PO/ConsoleTabPO'
...
async function obtainMessagesForTest(testName: string, callback?: (page: puppeteer.Page) => Promise<void>) {
  ...
  await click(ConsoleTabPO.tabConsole);
  // Obtain console messages that were logged
  await frontend.waitForSelector(ConsoleTabPO.loggedConsoleMessages);
  ...
  }, ConsoleTabPO.firstMessageFromConsole);
}
...
error message
  ...
  it('can show verbose promise unhandledrejections', async () => {
    const messages = await obtainMessagesForTest('onunhandledrejection', async () => {
      await click(ConsoleTabPO.defaultLevel);

      await click(ConsoleTabPO.verboseLevel);
    });
