...
describe('Source Tab', async () => {
  it('shows the module, local, and stack scope while pausing', async () => {
    ...
    const scopeElement = await $$('.scope-chain-sidebar-pane-section-title');
    const scopeTitles = await scopeElement.evaluate(nodes => nodes.map((n: HTMLElement) => n.textContent));
