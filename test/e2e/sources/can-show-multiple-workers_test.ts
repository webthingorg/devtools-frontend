...
import {getNestedWorkerFilename} from '../helpers/sources-helpers.js';
import {createSelectorsForWorkerFile} from '../helpers/sources-helpers.js';
...
describe('The Sources Tab', async function() {
  ...
  it('can show multiple dedicated workers with different scripts', async () => {
    ...
    const worker1FileName = await getNestedWorkerFilename(WORKER1_SELECTORS);
    assert.strictEqual(worker1FileName, 'worker1.js');

    const worker2FileName = await getNestedWorkerFilename(WORKER2_SELECTORS);
