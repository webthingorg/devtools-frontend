...
describe('The Sources Tab', async () => {
  ...
  it('hits breakpoint upon refresh for a sourcemapped wasm module', async () => {
    ...
    await step('wait for all the source code to appear', async () => {
      await waitFor(await sourceLineNumberSelector(2));
    });
  ...
  it('[crbug.com/1073071]: does not hit the breakpoint after it is removed for a sourcemapped wasm module',
     async () => {
       const {target, frontend} = getBrowserAndPages();
  ...
  // breakpoint activates upon refresh after it is removed
  it('[crbug.com/1073071]: hits two breakpoints that are set and activated separately', async function() {
    const {target, frontend} = getBrowserAndPages();
