...
describe('The Breakpoints Sidebar', () => {
  ...
  it('shows set breakpoints on source mapped files correctly', async () => {
    const {frontend} = getBrowserAndPages();
    ...
    await waitForFunction(async () => {
      breakpointItems = await $$(BREAKPOINT_ITEM_SELECTOR, breakpointView);
      return breakpointItems.length === expectedLocations.length;
    });

    const locations = await Promise.all(breakpointItems.map(breakpoint => extractLineNumber(breakpoint)));
