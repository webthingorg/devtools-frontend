...
describe('Sources Tab', async () => {
  ...
  it('is able to step with state', async () => {
    ...
    await step('check that the labels in the scope view shows the correct values', async () => {
      ...
      const labels_content = await (await $(SCOPE_LOCAL_VALUES_SELECTOR)).evaluate(element => {
        return element.innerText;
      });

      assert.strictEqual(labels_content, '"": 42', 'lables do not contain the correct contents');
    });
    ...
    await step('check that the labels in the scope view shows the correct values', async () => {
      ...
      assert.strictEqual(labels_content, '"": 50', 'lables do not contain the correct contents');
    });
  ...
  it('is able to step with state in multi-threaded code', async () => {
    ...
    await step('check that the worker thread is selected', async () => {
      ...
      assert.strictEqual(
          selectedThreadName, 'worker-stepping-with-state-and-threads.js', 'the Main thread is not active');
    });
