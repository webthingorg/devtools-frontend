...
import {addBreakpointForLine, clearSourceFilesAdded, getBreakpointDecorators, getLinesWithoutMapping, listenForSourceFilesAdded, openSourceCodeEditorForFile, openSourcesPanel, RESUME_BUTTON, retrieveSourceFilesAdded, retrieveTopCallFrameScriptLocation, waitForAdditionalSourceFiles} from '../helpers/sources-helpers.js';
...
describe('Source Tab', async () => {
  ...
  it('cannot add a breakpoint in unbreakable line raw wasm', async () => {
    const {target, frontend} = getBrowserAndPages();

    await openSourceCodeEditorForFile(target, 'add.wasm', 'wasm/call-to-add-wasm.html');
    assert.deepEqual(await getLinesWithoutMapping(frontend), [
      1,
      2,
      3,
      4,
