...
describe('The Sources Tab', async () => {
  ...
  it('retains __puppeteer_evaluation_script__ sourceURL annotations', async () => {
    const rootSelector = '.navigator-frame-tree-item[aria-label="top, frame"]';
    const domainSelector = `${rootSelector} + ol > [aria-label="(no domain), domain"]`;
    const fileSelector = `${domainSelector} + ol > [aria-label="__puppeteer_evaluation_script__, file"]`;

    await openFileInSourcesPanel('sourceurl.html');
    await expandFileTree({rootSelector, domainSelector, fileSelector});
  });
});
