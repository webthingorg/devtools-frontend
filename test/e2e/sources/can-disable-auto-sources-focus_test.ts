...
import {getBrowserAndPages, getPendingEvents, installEventListener, step, waitFor, waitForFunction, waitForNone} from '../../shared/helper.js';
...
async function breakAndCheckFocusedPanel(expectedPanel: string) {
  ...
  await step('trigger a Debugger.pause event', async () => {
    target.evaluate('f2();');
  });
...
describe('Sources Panel', async () => {
  ...
  it('is not openend on Debugger.pause if autoFocusOnDebuggerPauseEnabled is false', async () => {
    ...
    await waitForNone('.panel[aria-label="sources"]');
  });

  it('is opened on Debugger.pause if autoFocusOnDebuggerPauseEnabled is true (default)', async () => {
