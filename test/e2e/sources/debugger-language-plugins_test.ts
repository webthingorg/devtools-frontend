...
declare function RegisterExtension(
    extensionAPI: object, pluginImpl: TestPluginImpl, name: string,
...
describe('The Debugger Language Plugins', async () => {
  beforeEach(async () => {
    ...
    await frontend.evaluate(resourcePath => {
      globalThis.installExtensionPlugin = function(
          registerPluginCallback: (extensionServerClient: object, extensionAPI: object) => void) {
  ...
  it('can show C filenames after loading the module', async () => {
    ...
    await frontend.evaluate(
        () => globalThis.installExtensionPlugin((extensionServerClient: object, extensionAPI: object) => {
  ...
  it('shows variable value in popover', async () => {
    ...
    const popover = await waitFor('[aria-label="Object popover"]');
