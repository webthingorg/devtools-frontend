...
describe.only('The Debugger Language Plugins', async () => {
  ...
  it.repeat(200, 'shows variable value in popover', async () => {
    ...
    const pausedPosition = await asyncScope.exec(() => waitForFunction(async () => {
                                                   const element = await $('.cm-execution-line-tail', undefined);
                                                   if (element && element.evaluate(e => e.isConnected)) {
                                                     return element;
                                                   }
                                                   return undefined;
                                                 }, asyncScope));
   ...
   const { x, y } = await pausedPosition._clickablePoint();
                                                 // @ts-ignore
   await pausedPosition._page.mouse.move(x, y);
