...
describe('Display error information next to affected lines', async () => {
  it('Error icon should be displayed', async () => {
    ...
    const issueIconComponents = await getIconComponents('text-editor-line-decoration-icon-error');
  ...
  it('Error icons should be correct', async () => {
    await openFileInSourceTab('trusted-type-violations-report-only.rawresponse');
    const bucketIssueIconComponents = await getIconComponents('text-editor-line-decoration-icon-error');
    for (const bucketIssueIconComponent of bucketIssueIconComponents) {
      ...
      for (const issueIconComponent of issueIconComponents) {
        assert.strictEqual(await getIconFile(issueIconComponent), 'error_icon.svg');
      }

      assert.strictEqual(await getIconFile(bucketIssueIconComponent), 'error_icon.svg');
    }
  });
});
