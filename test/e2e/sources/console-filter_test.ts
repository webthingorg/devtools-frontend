...
import {deleteConsoleMessagesFilter, filterConsoleMessages, getConsoleMessages, getCurrentConsoleMessages, showVerboseMessages} from '../helpers/console-helpers.js';

const CONSOLE_MESSAGE_WRAPPER = '.console-group-messages .console-message-wrapper';
type MessageCheck = (element?: string, index?: number) => boolean;
...
function getExpectedMessages(unfilteredMessages: string[], filter: MessageCheck) {
  return unfilteredMessages.filter((msg: string, index: number) => {
    // console.group() outputs are not filtered
    if (/outerGroup$|innerGroup$/.test(msg)) {
      return true;
    }
    return filter(msg, index);
  });
}
...
describe('The Console Tab', async () => {
  ...
  it('can exclude messages from a source url', async () => {
    ...
    for (const urlToExclude of uniqueUrls!) {
      ...
      const expectedMessageFilter: MessageCheck = (msg, _) => {
  ...
  it('can apply empty filter', async () => {
    const filter = '';
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const expectedMessageFilter: MessageCheck = (_, __) => true;
    await testMessageFilter(filter, expectedMessageFilter);
  });
  ...
  it('can apply filter on anchor', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const expectedMessageFilter: MessageCheck = (msg, _) => {
      return /^log-source\.js:\d+/.test(msg!);
    };
    await testMessageFilter(filter, expectedMessageFilter);
  });
