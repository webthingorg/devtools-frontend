...
<<<<<<< PATCH SET (18a8ea Add "Show all overrides" in the Network request's context me)
  it('can show all overrides in the Sources panel', async () => {
    await step('when overrides setting is disabled', async () => {
      ...
      const assertElements = await $$('aria/Select folder for overrides', undefined, 'aria');
      assert.strictEqual(assertElements.length, 0);
    });
    ...
    await step('when overrides setting is disabled', async () => {
      ...
      const assertElements = await $$('aria/Enable Local Overrides', undefined, 'aria');
      assert.strictEqual(assertElements.length, 0);
=======
  it('overrides indicator on the Network panel title', async () => {
    ...
    await step('shows indicator after enabling override in Overrides tab', async () => {
      ...
      assert.strictEqual(icons.length, 1);
      assert.isDefined(iconTitleElement);
>>>>>>> BASE      (4465f5 Clearly indicate overridden resources in the Network panel)
    });
  });
});
