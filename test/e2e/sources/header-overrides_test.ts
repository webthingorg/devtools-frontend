...
describe('The Overrides Panel', async function() {
  ...
  it('can override headers via network panel', async () => {
    ...
    await waitForFunction(async () => {
      ...
      const headersViewText =
          await headersView.evaluate(el => el.shadowRoot?.textContent?.trim().replace(/[ \n]{2,}/g, ''));
      return headersViewText === 'Apply to:hello.htmlfoo:barAdd override rule';
    });
