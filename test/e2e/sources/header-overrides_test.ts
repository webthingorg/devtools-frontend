...
describe('The Overrides Panel', async function() {
  ...
  it('can create header overrides', async () => {
    ...
    await typeText('aaa\n');
    await typeText('bbb');
    await waitFor('[aria-label="NewFile, file"] .tree-element-title');
    const title = await $('[aria-label="NewFile, file"] .tree-element-title');
    // assert.isNotNull(title);
    let foo = await title?.evaluate(el => el.textContent);
    assert.strictEqual(foo, '*.headers');

    await pressKey('s', {control: true});
    await waitForFunction(async () => {
      foo = await title?.evaluate(el => el.textContent);
      return foo === '.headers';
    });
