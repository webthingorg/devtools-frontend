...
*/`);
  ...
  */`);
  ...
  describe('generateInterfaces', () => {
    ...
    it('errors loudly if it could not find the interface', () => {
      const state = parseCode(`class Breadcrumbs extends HTMLElement {
        public update(people: ThisInterfaceDoesNotExist) {}
      }`);

      assert.throws(() => generateInterfaces(state), 'Could not find interface: ThisInterfaceDoesNotExist');
    });
