...
`);
  ...
  html {
    color:green;foo-property:bar-value}
}
...
`);
  ...
  font-family:MyHelvetica;src:local(\'Helvetica Neue Bold\'),local(\'HelveticaNeue-Bold\'),url(MgOpenModernaBold.ttf);font-weight:bold;}
...
`);
  ...
  it('formats import rule with media query correctly', () => {
    ...
    assert.strictEqual(formattedCode, `@import url(\'landscape.css\') screen and (orientation:landscape);article {
  background:yellow}
`);
  });
