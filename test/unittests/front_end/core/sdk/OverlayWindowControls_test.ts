...
describe('OverlayWindowControls', () => {
  it('parseStyleSheet parses styles and replaces variables for style sheet correctly', () => {
    ...
    `;
    let parsedStyleSheet = windowControls.parseStyleSheet(x, y, width, height, originalStyleSheet);
    assert.strictEqual(parsedStyleSheet, expectedStyleSheet);

    originalStyleSheet = '';
    parsedStyleSheet = windowControls.parseStyleSheet(x, y, width, height, originalStyleSheet);
