...
describeWithMockConnection('DebuggerModel', () => {
  ...
  it('resumeModel properly awaits the debugger to be enabled', async () => {
    ...
    // We can only set the mock connection once. Debugger.enable is first called
    // when we create the target (createTarget()), but we do not want to test
    // the actualy behavior until later. Use this flag to send the appropriate
    // response in the mock connection.
    ...
    setMockConnectionResponseHandler('Debugger.enable', async () => {
      ...
      while (!events.includes(Events.WaitingStarted)) {
    ...
    // Wait until the debugger is enabled before pushing the WaitingDone event.
