...
function resetSavedSetting(
    highlighter: SDKModule.OverlayPersistentHighlighter.OverlayPersistentHighlighter,
    forcedState: Array<PersistentHighlightSettingItem> = []): void {
  const setting = highlighter.persistentHighlightSettingForTest();
  setting.set(forcedState);
}

function assertSavedSettingState(
    highlighter: SDKModule.OverlayPersistentHighlighter.OverlayPersistentHighlighter, expected: unknown): void {
  const setting = highlighter.persistentHighlightSettingForTest();
  assert.deepEqual(setting.get(), expected);
}

const NON_RELATED_DOCUMENT_URL_FOR_TEST = 'https://not-this-document.com/' as Platform.DevToolsPath.UrlString;
const DOCUMENT_URL_FOR_TEST = 'https://document-url-for-test.com/' as Platform.DevToolsPath.UrlString;
...
describeWithEnvironment('OverlayPersistentHighlighter', () => {
  ...
  it('is able to highlight flexbox elements', () => {
    ...
    assertSavedSettingState(highlighter, [{
