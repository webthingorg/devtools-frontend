...
describeWithMockConnection('EmulationModel', () => {
  ...
  it('should `emulateTouch` enable touch emulation', async () => {
    ...
    const spy = sinon.spy(emulationAgent, 'invoke_setTouchEmulationEnabled');
    assert.isTrue(spy.notCalled);

    await emulationModel.emulateTouch(true, true);

    assert.isTrue(spy.calledOnce);
    assert.isTrue(spy.calledWith({enabled: true, maxTouchPoints: 1}));
  });

  it('should `emulateTouch` not enable touch emulation when `setCanEmulateTouchEmulation` is called with false',
     async () => {
       const parentTarget = createTarget();
       const target = createTarget({parentTarget});
       const emulationModel = target.model(SDK.EmulationModel.EmulationModel);
       ...
       emulationModel.setCanEnableTouchEmulation(false);
       await emulationModel.emulateTouch(true, true);
