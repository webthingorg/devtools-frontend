...
describe('ServerSentEventsParser', () => {
  ...
  async function enqueueOneByOne(str: string, options?: {prefixBOM?: true}): Promise<void> {
    if (options?.prefixBOM) {
      await parser.addBase64Chunk(window.btoa(String.fromCodePoint(0xef)));
      await parser.addBase64Chunk(window.btoa(String.fromCodePoint(0xbb)));
      await parser.addBase64Chunk(window.btoa(String.fromCodePoint(0xbf)));
    }
