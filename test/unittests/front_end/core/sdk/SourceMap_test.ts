...
describe('TextSourceMap', () => {
  ...
  it('can resolve relative and absolute source URLs', () => {
    const cases = [
    ...
    for (const {sourceRoot, sourceURL, sourceMapURL, expected} of cases) {
      ...
      const sourceMap = new SDK.SourceMap.TextSourceMap('compiled.js', sourceMapURL, mappingPayload, fakeInitiator);

      const sourceURLs = sourceMap.sourceURLs();
      assert.lengthOf(sourceURLs, 1, 'unexpected number of original source URLs');
      assert.equal(
          sourceURLs[0], expected,
          `Unexpected result for sourceRoot ${sourceRoot}, raw sourceURL ${sourceURL}, sourceMapURL ${sourceMapURL}`);
    }
  });
});
