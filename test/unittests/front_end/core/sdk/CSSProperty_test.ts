...
describeWithEnvironment('CSSProperty', () => {
  describe('formatStyle', () => {
    ...
    it('formats shorthand declaration with a variable correctly', () => {
      assert.strictEqual(
          formatStyle('border: 1px solid var(--border-color);;', '', ''), 'border: 1px solid var(--border-color);');
    });
    it('formats shorthand declaration with a function correctly', () => {
      assert.strictEqual(formatStyle('border: 1px solid rgb(0,0,0);;', '', ''), 'border: 1px solid rgb(0,0,0);');
    });
    it('formats declaration with unknown property that contains a function correctly', () => {
      assert.strictEqual(formatStyle('unknownProperty: rgba(0,0,0,0);;', '', ''), 'unknownProperty: rgba(0,0,0,0);');
    });
    // Regression test for crbug/1392813
    it('formats complex CSS variable declaration', () => {
      assert.strictEqual(
