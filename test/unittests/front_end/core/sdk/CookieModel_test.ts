...
describeWithMockConnection('CookieModel', () => {
  ...
  it('can delete cookie', async () => {
    let cookies = [PROTOCOL_COOKIE, PROTOCOL_COOKIE_PARTITIONED];

    // CDP Connection mock: for Network.getCookies, respond with `cookies`.
    setMockConnectionResponseHandler('Network.getCookies', () => {
      return {
        cookies: cookies,
      };
    ...
    const target = createTarget();
    const model = new SDK.CookieModel.CookieModel(target);
    const getcookies = await model.getCookies(['https://www.example.com']);
    assert.isArray(getcookies);
    assert.lengthOf(getcookies, 2);

    await model.deleteCookie(SDK.Cookie.Cookie.fromProtocolCookie(PROTOCOL_COOKIE));

    const getcookies2 = await model.getCookies(['https://www.example.com']);
    assert.isArray(getcookies2);
    assert.lengthOf(getcookies2, 1);
    assert.strictEqual(getcookies2[0].domain(), '.example.com');
    assert.strictEqual(getcookies2[0].name(), 'name');
    assert.strictEqual(getcookies2[0].partitionKey(), 'https://example.net');
  });
});
