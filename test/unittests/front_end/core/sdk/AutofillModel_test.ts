...
describeWithMockConnection('AutofillModel', () => {
  ...
  it('dispatches addressFormFilledEvents on autofill events and on navigation', () => {
    const target = createTarget();
    const autofillModel = target.model(SDK.AutofillModel.AutofillModel);
    assertNotNullOrUndefined(autofillModel);
    ...
    assert.deepStrictEqual(dispatchedEvents[1].event, {filledFields: [], addressUi: {addressFields: []}});
  });
});
