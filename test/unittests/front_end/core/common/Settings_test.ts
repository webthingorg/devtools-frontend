...
describe('Settings instance', () => {
  ...
  it('registers settings with the backing store when creating them', () => {
    ...
    const storage = new SettingsStorage({}, mockBackingStore);
    Common.Settings.registerSettingExtension({
    ...
    assert.isTrue(registeredSettings.has('staticGlobalSetting'));
    assert.isTrue(registeredSettings.has('dynamicLocalSetting'));
    assert.isTrue(registeredSettings.has('dynamicSyncedSetting'));
  });
});
