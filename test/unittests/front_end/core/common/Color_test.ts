...
describe('Color', () => {
  ...
  it('correctly detects and clips out-of-gamut colors', () => {
    assert.isTrue(Color.parse('rgb(-1 1 256)')?.isCanonical());
    assert.deepEqual(Color.parse('rgb(-1 1 256)')?.asString(), 'rgb(0 1 255)');
    assert.isFalse(new Color.Legacy([-1, 0, 1.1, 1], Color.Format.RGBA).isCanonical());
    assert.deepEqual(
        new Color.Legacy([-1, 0, 1.1, 1], Color.Format.RGBA)
            .canonicalize({clipToGamut: true, zeroPowerlessComponents: true})
            .asString(),
        'rgb(0 0 255)');

    const parseAndCanonicalize = (spec: string): Common.Color.Color|undefined => {
      return Color.parse(spec)?.canonicalize({clipToGamut: true, zeroPowerlessComponents: true});
    };
    ...
    assert.isTrue(parseAndCanonicalize('oklch(0.5 -70 -70)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('oklch(0.5 70 -70)')?.asString(), 'oklch(0.5 70 -70)');

    assert.isTrue(parseAndCanonicalize('lab(10 -70 -70)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('lab(10 -70 -70)')?.asString(), 'lab(10 -70 -70)');

    assert.isTrue(parseAndCanonicalize('oklab(0.5 -70 -70)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('oklab(0.5 -70 -70)')?.asString(), 'oklab(0.5 -70 -70)');

    assert.isTrue(parseAndCanonicalize('color(srgb-linear -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(srgb-linear -1 1 1.5)')?.asString(), 'color(srgb-linear 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(srgb -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(srgb -1 1 1.5)')?.asString(), 'color(srgb 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(display-p3 -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(display-p3 -1 1 1.5)')?.asString(), 'color(display-p3 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(a98-rgb -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(a98-rgb -1 1 1.5)')?.asString(), 'color(a98-rgb 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(prophoto-rgb -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(prophoto-rgb -1 1 1.5)')?.asString(), 'color(prophoto-rgb 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(rec2020 -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(rec2020 -1 1 1.5)')?.asString(), 'color(rec2020 0 1 1)');

    assert.isTrue(parseAndCanonicalize('color(xyz-d50 -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(xyz-d50 -1 1 1.5)')?.asString(), 'color(xyz-d50 -1 1 1.5)');

    assert.isTrue(parseAndCanonicalize('color(xyz-d65 -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(xyz-d65 -1 1 1.5)')?.asString(), 'color(xyz-d65 -1 1 1.5)');

    assert.isTrue(parseAndCanonicalize('color(xyz -1 1 1.5)')?.isCanonical());
    assert.deepEqual(parseAndCanonicalize('color(xyz -1 1 1.5)')?.asString(), 'color(xyz -1 1 1.5)');

    // Being just a tiny bit out of gamut doesn't matter:
    assert.isTrue(parseAndCanonicalize('color(srgb-linear 1.0001 1.0001 1.0001)')?.isCanonical());
    assert.isTrue(parseAndCanonicalize('color(srgb 1.0001 1.0001 1.0001)')?.isCanonical());
    assert.isTrue(parseAndCanonicalize('color(display-p3 1.0001 1.0001 1.0001)')?.isCanonical());
    assert.isTrue(parseAndCanonicalize('color(a98-rgb 1.0001 1.0001 1.0001)')?.isCanonical());
    assert.isTrue(parseAndCanonicalize('color(prophoto-rgb 1.0001 1.0001 1.0001)')?.isCanonical());
    assert.isTrue(parseAndCanonicalize('color(rec2020 1.0001 1.0001 1.0001)')?.isCanonical());
  });

  it('correctly represents powerless components', () => {
    const parseAndCanonicalize = (spec: string): Common.Color.Color|undefined => {
      return Color.parse(spec)?.canonicalize({zeroPowerlessComponents: true});
    };
    // With lightness at 0% or 100% the color is black or white and all other components are powerless:
    assert.deepEqual(parseAndCanonicalize('lab(0 15 15)')?.asString(), 'lab(0 0 0)');
    assert.deepEqual(parseAndCanonicalize('lab(100 15 15)')?.asString(), 'lab(100 0 0)');
    assert.deepEqual(parseAndCanonicalize('lch(0 15 15)')?.asString(), 'lch(0 0 0)');
    assert.deepEqual(parseAndCanonicalize('lch(100 15 15)')?.asString(), 'lch(100 0 0)');
    assert.deepEqual(parseAndCanonicalize('oklab(0 15 15)')?.asString(), 'oklab(0 0 0)');
    assert.deepEqual(parseAndCanonicalize('oklab(1 15 15)')?.asString(), 'oklab(1 0 0)');
    assert.deepEqual(parseAndCanonicalize('oklch(0 15 15)')?.asString(), 'oklch(0 0 0)');
    assert.deepEqual(parseAndCanonicalize('oklch(1 15 15)')?.asString(), 'oklch(1 0 0)');
    assert.deepEqual(parseAndCanonicalize('hsl(120deg 10% 0%)')?.asString(), 'hsl(0deg 0% 0%)');
    assert.deepEqual(parseAndCanonicalize('hsl(120deg 10% 100%)')?.asString(), 'hsl(0deg 0% 100%)');

    // With saturation or chroma at 0, the hue becomes powerless:
    assert.deepEqual(parseAndCanonicalize('lch(0.3 0 15)')?.asString(), 'lch(0.3 0 0)');
    assert.deepEqual(parseAndCanonicalize('oklch(0.3 0 15)')?.asString(), 'oklch(0.3 0 0)');
    assert.deepEqual(parseAndCanonicalize('hsl(120deg 0% 50%)')?.asString(), 'hsl(0deg 0% 50%)');

    // If whiteness + blackness is 100% the color is a shade of gray and the hue is powerless:
    assert.deepEqual(parseAndCanonicalize('hwb(120deg 50% 50%)')?.asString(), 'hwb(0deg 50% 50%)');
    // whiteness + blackness exceeding 100% is normalized to 100% retaining the relative magnitude:
    assert.deepEqual(parseAndCanonicalize('hwb(120deg 80% 40%)')?.asString(), 'hwb(0deg 66.67% 33.33%)');
  });
});
