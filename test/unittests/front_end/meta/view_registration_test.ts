...
describeWithEnvironment('View registration', () => {
  ...
  it('Retrieves a registered view', async () => {
    try {
      ...
      assert.strictEqual(
          preRegisteredView.commandPrompt(), commandPrompt, 'Command for view is not returned correctly');
    } catch (error) {
  ...
  it('Adds command for showing a pre registered view', () => {
    ...
    const filteredCommands =
        allCommands.filter(command => command.title() === commandPrompt && command.category() === 'Panel');
