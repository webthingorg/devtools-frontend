...
describeWithMockConnection('NameResolver', () => {
  ...
  function getIdentifiersFromScopeDescriptor(
    ...
    while (current < scopeDescriptor.length) {
      ...
      let end = start + 1;
      while (end < scopeDescriptor.length && scopeDescriptor[end] === 'I') {
        end++;
      }
  ...
  // and uses the variable 'a' at the correct offsets. Such an assertion
  ...
  for (const test of tests) {
    it(test.name, async () => {
      ...
      assert.deepEqual(boundIdentifiers, getIdentifiersFromScopeDescriptor(test.source, test.scopes));
    });
  }
});
