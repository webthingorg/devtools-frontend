...
describeWithMockConnection('PersistenceImpl', () => {
  ...
  const SCRIPT_DESCRIPTION = {
    url: 'http://www.google.com/script.js' as DevToolsPath.UrlString,
  ...
  it('moves breakpoint from file system uiSourceCode to the network uiSourceCode when binding is created', async () => {
    ...
    // Disposing the file system copies the breakpoint back to the file system uiSourceCode (probably to update the storage).
