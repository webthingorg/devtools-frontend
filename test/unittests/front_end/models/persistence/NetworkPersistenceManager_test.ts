...
describeWithMockConnection('NetworkPersistenceManager', () => {
  ...
  it('translates URLs into raw and encoded paths', async () => {
    ...
    let toTest = [
      ...
      {url: 'localhost:8090/lpt1', raw: 'localhost:8090/lpt1', encoded: 'localhost:8090/lpt1'},  // check windows
      {
    ...
    toTest.forEach(testStrings => {
      assert.deepEqual(
          networkPersistenceManager.rawPathFromUrl(
              testStrings.url as Platform.DevToolsPath.UrlString, Host.Platform.isWin()),
          testStrings.raw);
      assert.deepEqual(
          networkPersistenceManager.encodedPathFromUrl(
              testStrings.url as Platform.DevToolsPath.UrlString, Host.Platform.isWin()),
          testStrings.encoded);
    });
  });
