...
describe('ProfileTreeModel', function() {
  ...
  it('calculates depth correctly for the nodes in a profile tree', () => {
    ...
    assert.strictEqual(profileTreeModel.maxDepth, 4);
    assert.strictEqual(root.depth, 0);
    assert.strictEqual(nodeA.depth, 1);
    assert.strictEqual(nodeB.depth, 2);
    assert.strictEqual(nodeC.depth, 2);
    assert.strictEqual(nodeD.depth, 1);
    assert.strictEqual(nodeE.depth, 2);
    assert.strictEqual(nodeF.depth, 3);
  });
});
...
name: bar ts: 17 dur: 1 selfTime: 0
  ...
  it('parses a CPU with GC nodes correctly', () => {
    ...
    const treeAsString = getFrameTreeAsString(cpuProfileDataModel);
    assert.strictEqual(treeAsString, `
name: foo ts: 1.5 dur: 3.125 selfTime: 0.25
  name: bar ts: 1.5 dur: 1.25 selfTime: 1.25
  name: bar ts: 3 dur: 1.625 selfTime: 1
    name: (garbage collector) ts: 4 dur: 0.625 selfTime: 0.625`);
  });
...
name: B ts: 3.25 dur: 1.1875 selfTime: 0.6875
