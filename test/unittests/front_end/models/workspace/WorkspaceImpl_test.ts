...
describe('WorkspaceImpl', () => {
  ...
  it('can remove the current instance', () => {
    ...
    assert.isFalse(sutBefore === sutAfter);
  });
  ...
  it('can retrieve UI source code with project Id and URL', () => {
    ...
    sut.uiSourceCode('exampleProjectID', exampleUrl);
  ...
  it('awaiting UI source code returns it directly if it is available', async () => {
    ...
    sinon.stub(sut, 'uiSourceCodeForURL').returns(uiSourceCodeStub);
  ...
  it('can explicitly await the UI source code if not yet available', async () => {
    ...
    const exampleUrl = 'https://example.com/' as Platform.DevToolsPath.UrlString;
    const uiSourceCodeStub = sinon.createStubInstance(Workspace.UISourceCode.UISourceCode);
    sinon.stub(sut, 'uiSourceCodeForURL').returns(null);
  ...
  it('can add a project', () => {
    ...
    assert.deepStrictEqual(sut.projects(), [projectStub]);
  });
  ...
  it('can check if there are trcking extensions', async () => {
    const sut = Workspace.Workspace.WorkspaceImpl.instance({forceNew: true});
