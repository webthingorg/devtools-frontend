...
describe('TimelineJSProfile', () => {
  ...
  it('generateJSFrameEvents creates JS frame events with v8.run trace event as parent', () => {
    ...
    const v8RunEvent =
        new SDK.TracingModel.ConstructedEvent('v8', 'v8.run', TraceEngine.Types.TraceEvents.Phase.COMPLETE, 10, thread);
  ...
  it('generateJSFrameEvents restarts the call frame stack when a new top level event is encountered', () => {
    function createEvent(
      ...
      // timestamps are assumed to come in microseconds on from raw
      // event payloads, but the rest of the processing is done in
      // milliseconds.
      const ts = tsMs * 1000;
