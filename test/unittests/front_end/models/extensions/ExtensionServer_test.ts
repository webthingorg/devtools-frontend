...
describeWithDummyExtension('Extensions', context => {
  ...
  it('can not show a view for Recorder without using the replay trigger', async () => {
    ...
    // Sending the stringify should flush the message queue and make sure
    // that the view requested command was not actually dispatched.
    const data = await manager.plugins()[0].stringify({});
