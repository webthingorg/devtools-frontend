...
describeWithDevtoolsExtension('Runtime hosts policy', {hostsPolicy}, context => {
  ...
  function createRequest(
    ...
    const dataProvider = () =>
        Promise.resolve(new SDK.ContentData.ContentData('content', false, request.resourceType(), request.mimeType));
...
describeWithDevtoolsExtension('Wasm extension API', {}, context => {
  ...
  it('getWasmGlobal does not block on invalid indices', async () => {
    const result = await context.chrome.devtools?.languageServices.getWasmGlobal(0, stopId) as undefined |
        Chrome.DevTools.WasmValue | Extensions.ExtensionServer.Record;
  ...
  it('getWasmLocal does not block on invalid indices', async () => {
    const result = await context.chrome.devtools?.languageServices.getWasmLocal(0, stopId) as undefined |
        Chrome.DevTools.WasmValue | Extensions.ExtensionServer.Record;
    assertIsStatus(result);
    assert.strictEqual(result.code, 'E_BADARG');
  ...
  it('getWasmOp does not block on invalid indices', async () => {
    const result = await context.chrome.devtools?.languageServices.getWasmOp(0, stopId) as undefined |
        Chrome.DevTools.WasmValue | Extensions.ExtensionServer.Record;
    assertIsStatus(result);
    assert.strictEqual(result.code, 'E_BADARG');
