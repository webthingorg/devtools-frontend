  ...
  describe('treify', () => {
    ...
    it('correctly calculates the total and self times of a hierarchy of events', async () => {
      ...
      const data = [
        ...
        makeCompleteEvent('C', 2, 1),   // 2..3 (finishes when B finishes)
        makeCompleteEvent('E', 10, 3),  // 10..13 (starts when A finishes)
      ] as TraceModel.Types.TraceEvents.RenderererEntry[];
    ...
    describe('building hierarchies trace events and profile calls', () => {
      it('builds a hierarchy from trace events and profile calls', async () => {
        ...
        const traceEvents: TraceModel.Types.TraceEvents.RenderererEntry[] = [evaluateScript, v8Run, parseFunction];
...
-ProfileCall (b) [0.2ms]
  ...
  describe('walking trees', () => {
    it('walkTree walks the entire tree and visits all the roots as well as all children', async () => {
      ...
      function onEntryStart(entry: TraceModel.Types.TraceEvents.RenderererEntry): void {
        callOrder.push({type: 'START', entryName: entry.name});
      }
      function onEntryEnd(entry: TraceModel.Types.TraceEvents.RenderererEntry): void {
