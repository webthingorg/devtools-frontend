...
describe('StackManipulator', function() {
  it('parses a stack and returns all the entries', async function() {
    ...
    const stack = new TraceEngine.StackManipulator.StackManipulator(mainThread, data.Renderer.entryToNode);
  ...
  it('supports the user merging an entry into its parent', async function() {
     ...
     *
     **/
    const entryTwo = mainThread.entries.find(entry => {
    ...
    // Only one entry - entryTwo - should have been hidden.
  ...
  it('supports collapsing an entry', async function() {
     ...
     **/
    const basicTwoCallEntry = mainThread.entries.find(entry => {
      // Processing this trace ends up with two distinct stacks for basicTwo()
      // So we find the first one so we can focus this test on just one stack.
      return TraceEngine.Types.TraceEvents.isProfileCall(entry) && entry.callFrame.functionName === 'basicTwo' &&
          entry.dur === 408;
    });
    if (!basicTwoCallEntry) {
      throw new Error('Could not find expected entry.');
    }

    // Gather the fib() calls under the first basicTwo stack, by finding all the calls whose end time is less than or equal to the end time of the entryTwo event.
