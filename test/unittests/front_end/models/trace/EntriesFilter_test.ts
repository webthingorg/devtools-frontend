...
describe('EntriesFilter', function() {
  ...
  it('adds the collapsed entry into the modifiedVisibleEntries array', async function() {
    ...
    const stack = new TraceEngine.EntriesFilter.EntriesFilter(data.Renderer.entryToNode);
    stack.applyAction({type: TraceEngine.EntriesFilter.FilterAction.COLLAPSE_FUNCTION, entry: entryTwo});
  ...
  it('adds the next visible parent of the merged entry into the modifiedVisibleEntries array if the direct parent is hidden',
     async function() {
       ...
       stack.applyAction({type: TraceEngine.EntriesFilter.FilterAction.MERGE_FUNCTION, entry: foo2Calls[1]});
  ...
  it('supports undo all filter actions by applying context menu undo action', async function() {
    ...
    stack.applyAction({type: TraceEngine.EntriesFilter.FilterAction.MERGE_FUNCTION, entry: basicStackOne});
    ...
    stack.applyAction({type: TraceEngine.EntriesFilter.FilterAction.UNDO_ALL_ACTIONS, entry: basicTwoCallEntry});
  ...
  it('supports resetting all hidden children of a selected entry', async function() {
    ...
    stack.applyAction({type: TraceEngine.EntriesFilter.FilterAction.RESET_CHILDREN, entry: foo2Calls[1]});
