...
describeWithEnvironment('PageLoadMetricsHandler', function() {
  ...
  describe('Marker events', () => {
    ...
    beforeEach(async function() {
      const {PageLoadMetrics} = await TraceLoader.traceEngine(this, 'multiple-navigations-with-iframes.json.gz');
    ...
    it('extracts all marker events from a trace correctly', () => {
      ...
      for (let markerIndex = 0; markerIndex < TraceModel.Handlers.ModelHandlers.PageLoadMetrics.MarkerName.length;
           markerIndex++) {
