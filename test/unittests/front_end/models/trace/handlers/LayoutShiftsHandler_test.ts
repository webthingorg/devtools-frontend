...
async function processTrace(url: string): Promise<void> {
  ...
  TraceModel.Handlers.LayoutShifts.reset();
  TraceModel.Handlers.LayoutShifts.initialize();
  ...
  try {
    ...
    for (const event of events) {
      ...
      TraceModel.Handlers.LayoutShifts.handleEvent(event);
    }
  } catch (error) {
    assert.fail(error);
  }
  ...
  await TraceModel.Handlers.LayoutShifts.finalize();
}
...
describe('LayoutShiftsHandler', function() {
  ...
  it('clusters a single frame correctly', async () => {
    ...
    const layoutShifts = TraceModel.Handlers.LayoutShifts.data();
  ...
  it('creates a cluster after the maximum time gap between shifts', async () => {
    ...
    assert.strictEqual(
        ...
        TraceModel.Handlers.LayoutShifts.MAX_SHIFT_TIME_DELTA);
  ...
  it('creates a cluster after a navigation', async () => {
    ...
    const layoutShifts = TraceModel.Handlers.LayoutShifts.data();
    const {navigationsByFrameId, mainFrameId} = TraceModel.Handlers.Meta.data();
  ...
  it('creates a cluster after exceeding the continuous shift limit', async () => {
    ...
    assert.strictEqual(
        ...
        TraceModel.Handlers.LayoutShifts.MAX_CLUSTER_DURATION);
  });
  it('sets the end of the last session window to the trace end time correctly', async () => {
    ...
    const layoutShifts = TraceModel.Handlers.LayoutShifts.data();
    assert.strictEqual(
        layoutShifts.clusters.at(-1)?.clusterWindow.max, TraceModel.Handlers.Meta.data().traceBounds.max);
  });
  ...
  it('sets the end of the last session window to the max gap between duration correctly', async () => {
    ...
    assert.strictEqual(lastWindow.max, lastShiftInWindow.ts + TraceModel.Handlers.LayoutShifts.MAX_SHIFT_TIME_DELTA);
    assert.isBelow(lastWindow.range, TraceModel.Handlers.LayoutShifts.MAX_CLUSTER_DURATION);
  });
  it('sets the end of the last session window to the max session duration correctly', async () => {
    ...
    assert.strictEqual(lastWindow.range, TraceModel.Handlers.LayoutShifts.MAX_CLUSTER_DURATION);
  });
  ...
  it('demarcates cluster score windows correctly', async () => {
    ...
    for (const cluster of layoutShifts.clusters) {
      ...
      for (const event of cluster.events) {
        ...
        if (scoreBeforeEvent < TraceModel.Handlers.LayoutShifts.LayoutShiftsThreshold.NEEDS_IMPROVEMENT &&
            clusterScore >= TraceModel.Handlers.LayoutShifts.LayoutShiftsThreshold.NEEDS_IMPROVEMENT &&
            clusterScore < TraceModel.Handlers.LayoutShifts.LayoutShiftsThreshold.BAD) {
        ...
        if (scoreBeforeEvent < TraceModel.Handlers.LayoutShifts.LayoutShiftsThreshold.BAD &&
            clusterScore >= TraceModel.Handlers.LayoutShifts.LayoutShiftsThreshold.BAD) {
  ...
  describe('findNextScreenshotEvent', () => {
    it('gets the first screenshot after a trace', async () => {
      await processTrace('cls-cluster-navigation.json.gz');
      const screenshots = TraceModel.Handlers.Screenshots.data();
      const {clusters} = TraceModel.Handlers.LayoutShifts.data();
      const shifts = clusters.flatMap(cluster => cluster.events);
      for (const shift of shifts) {
        const screenshotIndex = TraceModel.Handlers.LayoutShifts.findNextScreenshotEventIndex(screenshots, shift.ts);
