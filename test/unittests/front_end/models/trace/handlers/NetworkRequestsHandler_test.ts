...
type DataArgsProcessedData =
    TraceModel.Types.TraceEvents.TraceEventSyntheticNetworkRequest['args']['data']['processedData'];
...
function assertDataArgsProcessedDataStats<D extends keyof DataArgsProcessedData>(
  ...
  for (const [name, value] of stats.entries()) {
    if (typeof request.args.data.processedData[name] === 'number') {
      const expectedValue = value as DataArgsProcessedData[D];
      const actualValueRounded =
          Number((request.args.data.processedData[name] as number).toPrecision(5)) as DataArgsProcessedData[D];
      assert.strictEqual(actualValueRounded, expectedValue, url);
    } else {
      assert.strictEqual(request.args.data.processedData[name], value, url);
    }
  }
}
