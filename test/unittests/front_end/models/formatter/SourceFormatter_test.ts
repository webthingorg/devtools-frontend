...
describeWithEnvironment('SourceFormatter', () => {
  ...
  before(() => {
    ...
    sourceFormatter = Formatter.SourceFormatter.SourceFormatter.instance();
  });
...
`);
  ...
  it('caches formatted documents', async () => {
    ...
    assert.strictEqual(sourceFormatter.hasFormatted(uiSourceCode), true);
  });
  ...
  it('removes cached formatted documents on request', async () => {
    ...
    assert.strictEqual(sourceFormatter.hasFormatted(uiSourceCode), false);
  });
  ...
  it('can handle duplicate file names for different ui source code instances', async () => {
    const formattedResult = await sourceFormatter.format(uiSourceCode);
    assert.strictEqual(formattedResult.formattedSourceCode.url().startsWith(`${DOCUMENT_URL}:formatted`), true);
    ...
    assert.strictEqual(
        duplicateFormattedResult.formattedSourceCode.url().startsWith(`${DOCUMENT_URL}:formatted`), true);
