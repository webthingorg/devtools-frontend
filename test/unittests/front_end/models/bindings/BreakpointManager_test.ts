...
describeWithRealConnection('BreakpointManager', () => {
  ...
  it('triggers breakpoint synchronization on an instrumentation break', async () => {
    ...
    debuggerModel.setSynchronizeBreakpointsCallback(
        (script: SDK.Script.Script) => breakpointManager.restoreBreakpointsForScript(script));
    ...
    // Fake an instrumentation pause event, check if the restoreBreakpointsForScript was called and
    // whether we have successfully updated the breakpoint as a result.
    const spy = sinon.spy(breakpointManager, 'restoreBreakpointsForScript');
