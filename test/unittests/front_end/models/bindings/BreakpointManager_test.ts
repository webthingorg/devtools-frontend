...
import {
  ...
  registerListenerOnIncomingMethod as registerListenerOnIncomingMessage,
...
describeWithMockConnection('BreakpointManager (mock backend)', () => {
  ...
  it('can restore breakpoints in source mapped scripts', async () => {
    ...
    // Register our interest in the breakpoint request.
    const breakpointResponder = backend.responderToBreakpointByUrlRequest(URL, 0);
    ...
    const reloadedBreakpointResponder = backend.responderToBreakpointByUrlRequest(URL, 0);
    ...
    const resumeSentPromise = registerListenerOnIncomingMessage('Debugger.resume');
    backend.dispatchDebuggerPause(reloadedScript, 'foo', Protocol.Debugger.PausedEventReason.Instrumentation);
