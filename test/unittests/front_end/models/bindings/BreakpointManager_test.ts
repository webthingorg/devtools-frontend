...
describeWithMockConnection('BreakpointManager (mock backend)', () => {
  ...
  beforeEach(async () => {
    ...
    // Wait for the resource tree model to load; otherwise, our uiSourceCodes will be asynchronousely invalidated during the test.
  ...
  it('removes ui source code from breakpoint even after breakpoint live location update', async () => {
    ...
    // Map the breakpoint location to a different file.
    ...
    // Remove the target and verify that the UI source codes were removed from the brekapoint.
