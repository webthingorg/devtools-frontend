...
describeWithMockConnection('IgnoreListManager', () => {
  ...
  beforeEach(async () => {
    ...
    sourceMapFile1UiSourceCode = await debuggerWorkspaceBinding.waitForUISourceCodeAdded(sourceMapFile1Url, target);
    sourceMapFile2UiSourceCode = await debuggerWorkspaceBinding.waitForUISourceCodeAdded(sourceMapFile2Url, target);
  });
