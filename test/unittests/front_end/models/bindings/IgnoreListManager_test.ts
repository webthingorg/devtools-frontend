...
describeWithMockConnection('IgnoreListManager', () => {
  ...
  async function waitForUiSourceCode(
    ...
    const compilerScriptMapping = debuggerWorkspaceBinding.getCompilerScriptMappingForTest(debuggerModel);
    assertNotNullOrUndefined(compilerScriptMapping);

    let uiSourceCode;
    let debugcount = 0;  // DO NOT CHECK IN
    while (true) {
      if (debugcount++ > 10) {
        assert.fail('Too many attempts to wait for UISourceCode');
      }
      uiSourceCode = compilerScriptMapping.uiSourceCodeForURL(sourceURL, false);
      if (uiSourceCode) {
        break;
      }
  ...
  beforeEach(async () => {
    ...
    thirdPartyUiSourceCode = await waitForUiSourceCode(debuggerWorkspaceBinding, sourceMapThirdPartyUrl);
    sourceMapFile1UiSourceCode = await waitForUiSourceCode(debuggerWorkspaceBinding, sourceMapFile1Url);
    sourceMapFile2UiSourceCode = await waitForUiSourceCode(debuggerWorkspaceBinding, sourceMapFile2Url);
  });
