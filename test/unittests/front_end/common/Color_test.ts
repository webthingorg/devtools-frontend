...
describe('Color', () => {
  ...
  it('is able to create a color class from an HSVA value', () => {
    ...
    assert.equal(color.asString(), 'hsl(180 33% 38% / 1)', 'original text was not set correctly');
...
describe('Generator', () => {
  ...
  it('able to return the color for an ID that was not set', () => {
    const generator = new Color.Generator();
    assert.equal(generator.colorForID('r'), 'hsl(133 67% 80% / 1)', 'color was not generated correctly');
  });
});
