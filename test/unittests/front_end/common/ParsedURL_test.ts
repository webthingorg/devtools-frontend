...
describe('Parsed URL', () => {
  ...
  it('converts platform path with variety of special characters to URL and back consistently with Chrome', () => {
    ...
    assert.strictEqual(ParsedURL.platformPathToURL(platformPathTest), urlTest);
    assert.strictEqual(ParsedURL.urlToPlatformPath(urlTest), platformPathTest);
  });
  ...
  it('escapes, unescapes partial path with email address', () => {
    const pathTest = 'username:password@example.com';  // valid filename on unix
    const escapedPath = ParsedURL.escapeFilePath(pathTest);
    assert.strictEqual(pathTest, escapedPath, 'changed during escaping');
    const convertedPath = ParsedURL.unescapeFilePath(escapedPath);
    assert.strictEqual(convertedPath, pathTest, 'path was not converted successfully');
  });

  it('escapes, unescapes partial path', () => {
  ...
  it('escapes, unescapes absolute path', () => {
    const pathTest = '/C:/Program%20Files/Google';
    const escapedPath = ParsedURL.escapeFilePath(pathTest);
    const convertedPath = ParsedURL.unescapeFilePath(escapedPath);
    assert.strictEqual(convertedPath, pathTest, 'path was not converted successfully');
