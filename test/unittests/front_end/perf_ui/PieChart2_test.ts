...
describe('PieChart2', () => {
  describe('with legend', () => {
    ...
    it('does not show a focus ring for click selection', () => {
      ...
      const legendRow = chart.shadowRoot.querySelector('.pie-chart-legend-row[data-keyboard-focus="true"]');
      assert.isNull(legendRow);
    });

    // TODO(petermarshall): This causes the component to access the global UI object which doesn't
    // exist during unit tests. We would like to test this but don't have a solution yet for
    // components that rely on global state like this. Find a way to enable this test.
