...
interface ExpectedMutation {
  max?: number;
  target: keyof HTMLElementTagNameMap|typeof TEXT_NODE, type?: MutationType
}
...
const gatherMatchingObservedExpectationsForExpectedMutation =
...
export const withMutations = async(
  ...
  for (const expectedMutation of expectedMutations) {
    const matchingMutations =
        gatherMatchingObservedExpectationsForExpectedMutation(expectedMutation, observedMutations);
