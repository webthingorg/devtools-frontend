...
export function createContentProviderUISourceCode(options: {
  ...
  project.addUISourceCodeWithProvider(uiSourceCode, contentProvider, options.metadata || null, options.mimeType);
...
export function createFileSystemUISourceCode(options: {
  url: Platform.DevToolsPath.UrlString,
  metadata?: Workspace.UISourceCode.UISourceCodeMetadata, content: string, mimeType: string,
  ...
  const project = new TestFileSystem(
      {fileSystemWorkspaceBinding, platformFileSystem, workspace, metadata, content: options.content});
