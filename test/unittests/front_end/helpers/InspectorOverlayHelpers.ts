...
import {AreaBounds, Bounds} from '../../../../front_end/inspector_overlay/tool_highlight_impl.js';
...
export function getGridLineNumberLabelContainer() {
  const el = document.querySelector(`#${GRID_LABEL_CONTAINER_ID} .${GRID_LINE_NUMBER_LABEL_CONTAINER_CLASS}`);
  assertNotNull(el);
  return el;
}

export function getGridAreaNameLabelContainer() {
  const el = document.querySelector(`#${GRID_LABEL_CONTAINER_ID} .${GRID_LINE_AREA_LABEL_CONTAINER_CLASS}`);
  assertNotNull(el);
  return el;
}
