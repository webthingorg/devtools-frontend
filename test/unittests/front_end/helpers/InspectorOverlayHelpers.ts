...
const GRID_TEACK_SIZES_LABEL_CONTAINER_CLASS = 'track-sizes';
...
export function createGridLabelContainer(layerId?: number) {
  ...
  layerEl.createChild('div', GRID_TEACK_SIZES_LABEL_CONTAINER_CLASS);
...
export function getGridTrackSizesLabelContainer(layerId?: number): HTMLElement {
  ...
  const el =
      document.querySelector(
          `#${GRID_LABEL_CONTAINER_ID} #${CSS.escape(id)} .${GRID_TEACK_SIZES_LABEL_CONTAINER_CLASS}`) as HTMLElement;
  assertNotNull(el);
  return el;
}
