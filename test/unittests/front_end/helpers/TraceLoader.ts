...
// The new engine cache is a map of maps.
// The first map is a Map of string (which is the name of the trace file) to a
// new map, where the key is the JSON-stringified trace engine configuration.
...
export class TraceLoader {
  ...
  static async allModels(
      context: Mocha.Context|Mocha.Suite|null, name: string,
      traceEngineConfig?: TraceEngine.Types.Configuration.Configuration): Promise<AllModelsLoaded> {
    ...
    const traceEngineData =
        await executeTraceEngineOnFileContents(fileContents, /* emulate fresh recording */ false, traceEngineConfig);
