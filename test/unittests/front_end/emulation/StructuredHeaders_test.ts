...
function assertItemParams(
  ...
  assert.lengthOf(result.parameters.items, expectParams.length);
  for (let i = 0; i < expectParams.length; ++i) {
    assert.strictEqual(result.parameters.items[i].name.value, expectParams[i][0]);
    assert(bareItemEquals(result.parameters.items[i].value, expectParams[i][1]), 'Param ' + i + ' value matches');
  }
}
