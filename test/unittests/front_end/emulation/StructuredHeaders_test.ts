...
function makeParams(params: [string, StructuredHeaders.BareItem][]): StructuredHeaders.Parameters {
  ...
  for (const param of params) {
    typedParams.items.push({
      ...
      name: {kind: StructuredHeaders.ResultKind.ParamName, value: param[0]},
      value: param[1],
    });
  }
  return typedParams;
...
function makeInnerList(
  ...
  return {kind: StructuredHeaders.ResultKind.InnerList, items: items, parameters: makeParams(params)};
}
...
describe('StructuredHeaders', () => {
  ...
  describe('Serialization', () => {
    it('Basic integer serialization', () => {
