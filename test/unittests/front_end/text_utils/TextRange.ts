...
function assertIsUnitTextRange(range, line, column) {
  assert.isTrue(range instanceof TextRange, `range is not a TextRange`);
  assert.equal(range.startLine, range.endLine, `the range is not a unit range: start/end lines differ`);
  assert.equal(range.startColumn, range.endColumn, `the range is not a unit range: start/end columns differ`);
  assert.equal(range.startLine, line, `the line was not set correctly`);
  assert.equal(range.startColumn, column, `the column was not set correctly`);
}
...
describe(`TextRange`, () => {
  ...
  it(`can be created from a location`, () => {
    ...
    assertIsUnitTextRange(textRange, line, column);
  });
  ...
  describe(`#immediatelyFollows()`, () => {
    it(`can handle non-range inputs`, () => {
      const textRange = TextRange.fromObject({startLine: 1, startColumn: 2, endLine: 3, endColumn:4});
      assert.isFalse(textRange.immediatelyFollows(), `invalid ranges should not be judged as immediatelly preceeding`);
    });
  ...
  it(`can be collapsed to start`, () => {
    ...
    assertIsUnitTextRange(textRangeACollapsed, rangeA.startLine, rangeA.startColumn);
    const textRangeBCollapsed = textRangeB.collapseToStart();
    assertIsUnitTextRange(textRangeBCollapsed, rangeB.startLine, rangeB.startColumn);
  ...
  it(`can be collapsed to end`, () => {
    ...
    assertIsUnitTextRange(textRangeACollapsed, rangeA.endLine, rangeA.endColumn);
    const textRangeBCollapsed = textRangeB.collapseToEnd();
    assertIsUnitTextRange(textRangeBCollapsed, rangeB.endLine, rangeB.endColumn);
    assertIsTextRangeAndEqualsRange(textRangeA, rangeA, `original TextRange should be unchanged`);
    assertIsTextRangeAndEqualsRange(textRangeB, rangeB, `original TextRange should be unchanged`);
  });
  ...
  it(`can adjusted relative to a position`, () => {
  ...
  it(`can adjusted relative from a position`, () => {
    const textRange = TextRange.fromObject({startLine: 4, startColumn: 3, endLine: 6, endColumn:7});
  ...
  it(`can be check if a position is contained`, () => {
    const textRangeA = TextRange.fromObject({startLine: 1, startColumn: 2, endLine: 3, endColumn:4});
