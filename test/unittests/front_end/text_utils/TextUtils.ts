...
describe('FilterParser', () => {
  ...
  describe('parse', () => {
    ...
    it('returns correct results for a range of inputs', () => {
      ...
      assert.deepEqual(result[0], {text: 'text', negative: false});

      result = filterParser.parse('spaced text');
      assert.deepEqual(result[0], {text: 'spaced', negative: false});
      assert.deepEqual(result[1], {text: 'text', negative: false});

      result = filterParser.parse('-');
      assert.deepEqual(result[0], {text: '-', negative: false});

      result = filterParser.parse('-text');
      assert.deepEqual(result[0], {text: 'text', negative: true});

      result = filterParser.parse('//');
      assert.deepEqual(result[0], {text: '//', negative: false});

      result = filterParser.parse('/regex/');
      assert.deepEqual(result[0], {regex: /regex/i, negative: false});

      result = filterParser.parse('/regex/ /another/');
      assert.deepEqual(result[0], {regex: /regex/i, negative: false});
      assert.deepEqual(result[1], {regex: /another/i, negative: false});

      result = filterParser.parse('/complex\/regex/');
      assert.deepEqual(result[0], {regex: /complex\/regex/i, negative: false});
      ...
      assert.deepEqual(result[0], {key: 'key1', text: 'foo', negative: false});

      result = filterParser.parse('-key1:foo');
      assert.deepEqual(result[0], {key: 'key1', text: 'foo', negative: true});

      result = filterParser.parse('key1:foo key2:bar');
      assert.deepEqual(result[0], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[1], {key: 'key2', text: 'bar', negative: false});
      ...
      result = filterParser.parse('key1:foo -key2:bar');
      assert.deepEqual(result[0], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[1], {key: 'key2', text: 'bar', negative: true});
      ...
      assert.deepEqual(result[0], {key: 'key1', text: '/regex/', negative: false});

      result = filterParser.parse('key1:foo innerText key2:bar');
      assert.deepEqual(result[0], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[1], {text: 'innerText', negative: false});
      assert.deepEqual(result[2], {key: 'key2', text: 'bar', negative: false});

      result = filterParser.parse('bar key1 foo');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {text: 'key1', negative: false});
      assert.deepEqual(result[2], {text: 'foo', negative: false});

      result = filterParser.parse('bar key1:foo');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {key: 'key1', text: 'foo', negative: false});

      result = filterParser.parse('bar key1:foo baz');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[2], {text: 'baz', negative: false});

      result = filterParser.parse('bar key1:foo yek:roo baz');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[2], {text: 'yek:roo', negative: false});
      assert.deepEqual(result[3], {text: 'baz', negative: false});

      result = filterParser.parse('bar key1:foo -yek:roo baz');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[2], {text: 'yek:roo', negative: true});
      assert.deepEqual(result[3], {text: 'baz', negative: false});

      result = filterParser.parse('bar baz key1:foo goo zoo');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {text: 'baz', negative: false});
      assert.deepEqual(result[2], {key: 'key1', text: 'foo', negative: false});
      assert.deepEqual(result[3], {text: 'goo', negative: false});
      assert.deepEqual(result[4], {text: 'zoo', negative: false});

      result = filterParser.parse('bar key1:key1:foo');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {key: 'key1', text: 'key1:foo', negative: false});

      result = filterParser.parse('bar :key1:foo baz');
      assert.deepEqual(result[0], {text: 'bar', negative: false});
      assert.deepEqual(result[1], {text: ':key1:foo', negative: false});
      assert.deepEqual(result[2], {text: 'baz', negative: false});
