...
describe('UISourceCode', () => {
  ...
  describe('renaming', () => {
    it('nothing changes when rename fails', async () => {
      class RenameFailsProject extends Project {
        rename(uiSourceCode, newName, callback) {
          callback(false);
        }
      };
      const project = new RenameFailsProject();
      const url = 'https://www.google.com/search/result?abc=123';
      const contentType = new ResourceType('name', 'title', new ResourceCategory('title', 'shortTitle'), false);
      const uiSourceCode = new UISourceCode(project, url, contentType);

      let renamed = await uiSourceCode.rename('newName');
      assert.equal(renamed, false);
      assert.equal(uiSourceCode.name(), 'result?abc=123', 'name was updated');
      assert.equal(uiSourceCode.url(), 'https://www.google.com/search/result?abc=123', 'url was updated');
      assert.deepEqual(uiSourceCode.contentType(), contentType, 'contentType was updated');
    });

    it('name and url change when rename succeeds', async () => {
      const newContentType = new ResourceType('name2', 'title2',
          new ResourceCategory('title2', 'shortTitle2'), true);
      class RenameSucceedsProject extends Project {
        rename(uiSourceCode, newName, callback) {
          callback(true, newName, 'https://www.google.com/search/newName', newContentType);
        }
        workspace() {
          return {
      ...
      const project = new RenameSucceedsProject();
      const url = 'https://www.google.com/search/result?abc=123';
      const contentType = new ResourceType('name', 'title', new ResourceCategory('title', 'shortTitle'), false);
      const uiSourceCode = new UISourceCode(project, url, contentType);
      ...
      assert.equal(uiSourceCode.name(), 'newName', 'name was not updated');
      assert.equal(uiSourceCode.url(), 'https://www.google.com/search/newName', 'url was not updated');
      assert.deepEqual(uiSourceCode.contentType(), newContentType, 'contentType was not updated');
    });

    it('canRename defers to Project', () => {
      class CanRenameProject extends Project {
        canRename() { return true; }
      };
      const project = new CanRenameProject();
      const url = 'https://www.google.com/search/result?abc=123';
      const contentType = new ResourceType('name', 'title', new ResourceCategory('title', 'shortTitle'), false);
      const uiSourceCode = new UISourceCode(project, url, contentType);

      assert.equal(uiSourceCode.canRename(), true);

      project.canRename = () => { return false };
      assert.equal(uiSourceCode.canRename(), false);
    });
  });
});
