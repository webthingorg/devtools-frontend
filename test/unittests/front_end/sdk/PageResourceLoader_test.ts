...
describe('PageResourceLoader', () => {
  ...
  it('deals with page reloads correctly', async () => {
    ...
    assert.deepEqual(results[1], 'Load aborted due to reload of inspected page');
    assert.deepEqual(results[2], 'Load aborted due to reload of inspected page');
    assert.deepEqual(loader.getNumberOfResources(), {loading: 0, queued: 0, resources: 0});
  });
  ...
  it('handles the load timeout correctly', async () => {
    ...
    const loader =
        PageResourceLoader.instance({forceNew: true, loadOverride: load, maxConcurrentLoads: 1, loadTimeout: 30});
    const loading = [
      loader.loadResource('foo1', '123').catch(e => e.message),
      loader.loadResource('foo2', '123').catch(e => e.message),
      loader.loadResource('foo3', '123').catch(e => e.message),
    ...
    assert.isTrue(
        results.every(x => x === 'Resource load timeout exceeded'),
  ...
  it('respects the max concurrent loads', async () => {
    ...
    const loading =
        [loader.loadResource('foo1', '123'), loader.loadResource('foo2', '123'), loader.loadResource('foo3', '123')];
    assert.deepEqual(loader.getNumberOfResources(), {loading: 2, queued: 1, resources: 3});
