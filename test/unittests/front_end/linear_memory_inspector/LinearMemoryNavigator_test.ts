...
describe('LinearMemoryNavigator', () => {
  ...
  async function assertNavigationEvents(eventType: string) {
    const pageNavigationButtons = component.shadowRoot!.querySelectorAll(`[data-button=${eventType}]`);
  ...
  it('sends event when clicking on refresh', async () => {
    ...
    const refreshButton = component.shadowRoot!.querySelector('[data-button=refresh]');
