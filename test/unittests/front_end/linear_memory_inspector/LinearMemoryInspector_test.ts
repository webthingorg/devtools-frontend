...
describe('LinearMemoryInspector', () => {
  ...
  it('listens on change events and leaves the navigator address, as is, on edit', () => {
    const {component} = setUpComponent();
    const navigator = getNavigator(component);
  ...
  it('listens on change events and updates the navigator address (to hex) on valid submit', () => {
    const {component} = setUpComponent();
    const navigator = getNavigator(component);
  ...
  it('listens on change events and leaves the navigator address on invalid edit and sets an error message', () => {
    const {component} = setUpComponent();
    const navigator = getNavigator(component);
    triggerAddressChangedEvent(component, '-2', LinearMemoryInspector.LinearMemoryNavigator.Mode.Edit);
  ...
  it('listens on change events and leaves the navigator address on invalid submit and sets an error message', () => {
    const {component} = setUpComponent();
    const navigator = getNavigator(component);
    triggerAddressChangedEvent(component, '-2', LinearMemoryInspector.LinearMemoryNavigator.Mode.Submitted);
    assertUpdatesInNavigator(navigator, '-2', 'Address has to be a number between 0x00000000 and 0x000003E8');
