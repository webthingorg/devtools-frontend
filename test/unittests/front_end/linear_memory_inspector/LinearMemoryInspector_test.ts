...
import {getComponentElement, getComponentElements, getEventPromise, renderElementIntoDOM} from '../helpers/DOMHelpers.js';
...
describe('LinearMemoryInspector', () => {
  function getViewer(component: LinearMemoryInspector) {
    return getComponentElement(component, VIEWER_SELECTOR, LinearMemoryViewer);
  }

  function getNavigator(component: LinearMemoryInspector) {
    return getComponentElement(component, NAVIGATOR_SELECTOR, LinearMemoryNavigator);
  }
  ...
  it('can navigate addresses back and forth in history', async () => {
    ...
    const navigator = getNavigator(component);
    const buttons = getComponentElements(navigator, NAVIGATOR_HISTORY_BUTTON_SELECTOR, HTMLButtonElement);
    const [backwardButton, forwardButton] = buttons;

    const viewer = getViewer(component);
    const byteCells = getComponentElements(viewer, VIEWER_BYTE_CELL_SELECTOR, HTMLSpanElement);
    ...
    for (let i = historyLength - 1; i >= 0; --i) {
      const currentByteValue = getComponentElement(viewer, VIEWER_BYTE_CELL_SELECTOR + '.selected', HTMLSpanElement);
  ...
  it('can turn the page back and forth', async () => {
    const {component} = setUpComponent();
    const navigator = getNavigator(component);
    const buttons = getComponentElements(navigator, NAVIGATOR_PAGE_BUTTON_SELECTOR, HTMLButtonElement);
    const [backwardButton, forwardButton] = buttons;

    const address = getComponentElement(navigator, NAVIGATOR_ADDRESS_SELECTOR, HTMLInputElement);
  ...
  it('synchronizes selected addresses in navigator and viewer', async () => {
    ...
    const address = getComponentElement(navigator, NAVIGATOR_ADDRESS_SELECTOR, HTMLInputElement);
    const viewer = getViewer(component);
    const selectedByte = getComponentElement(viewer, VIEWER_BYTE_CELL_SELECTOR + '.selected', HTMLSpanElement);
