...
describeWithMockConnection('OutermostTargetSelector', () => {
  ...
  it('updates selected target when UI context flavor changes', () => {
    ...
    assert.strictEqual(selector.item().element.title, 'Outermost target: Main');
    UI.Context.Context.instance().setFlavor(SDK.Target.Target, prerenderTarget);
    assert.strictEqual(selector.item().element.title, 'Outermost target: prerender1');
  });
  ...
  it('updates when target info changes', () => {
    ...
    assert.strictEqual(selector.item().element.title, 'Outermost target: prerender3');
  });
