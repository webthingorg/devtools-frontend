...
describe.only('LocalizedStringSet', () => {
  ...
  it('should throw an error when a requested message is not part of the UIStrings string structure', () => {
    ...
    const stringSet = i18nInstance.registerFileStrings('test.ts', {}).getLocalizedStringSetFor('en-US');

    assert.throws(() => stringSet.getLocalizedString('en-US', 'string to translate'));
  });
