...
describeWithRealConnection('StylePropertyTreeElement', async () => {
  ...
  describe('updateTitle', () => {
    ...
    describe('color-mix swatch', () => {
      ...
      it('shows a popover with it\'s computed color', () => {
        const cssPropertyWithColorMix = new SDK.CSSProperty.CSSProperty(
            mockCssStyleDeclaration, 0, 'color', 'color-mix(in rgba, red 50%, yellow)', true, false, true, false, '',
            undefined);
        const stylePropertyTreeElement = new Elements.StylePropertyTreeElement.StylePropertyTreeElement({
          stylesPane: stylesSidebarPane,
          matchedStyles: mockMatchedStyles,
        ...
        assert.strictEqual(addPopoverSpy.args[0][1]()?.textContent, 'color(srgb 1 0.5 0)');
      });

      it('propagates updates to outer color-mixes', () => {
        const cssPropertyWithColorMix = new SDK.CSSProperty.CSSProperty(
            mockCssStyleDeclaration, 0, 'color', 'color-mix(in rgba, color-mix(in oklch, red, green), blue)', true,
            false, true, false, '', undefined);
        const stylePropertyTreeElement = new Elements.StylePropertyTreeElement.StylePropertyTreeElement({
          stylesPane: stylesSidebarPane,
          matchedStyles: mockMatchedStyles,
        ...
        assert.strictEqual(outerColorMix.getText(), 'color-mix(in rgba, color-mix(in oklch, red, green), blue)');
        assert.strictEqual(innerColorMix.getText(), 'color-mix(in oklch, red, green)');
        innerColorMix.setFirstColor('blue');
        assert.deepStrictEqual(
            handler.args[0][0].data, {text: 'color-mix(in rgba, color-mix(in oklch, blue, green), blue)'});
        assert.strictEqual(outerColorMix.getText(), 'color-mix(in rgba, color-mix(in oklch, blue, green), blue)');
        ...
        assert.strictEqual(outerColorMix.getText(), 'color-mix(in rgba, color-mix(in oklch, #ff0000, green), blue)');
        assert.deepStrictEqual(
            handler.args[0][0].data, {text: 'color-mix(in rgba, color-mix(in oklch, #ff0000, green), blue)'});
      });
    });
