...
describeWithRealConnection('StylePropertyHighlighter', () => {
  ...
  it('highlighte elements', async () => {
    const {stylesSidebarPane} = await setupStylesPane();
    const highlighter = new Elements.StylePropertyHighlighter.StylePropertyHighlighter(stylesSidebarPane);

    const element = sinon.createStubInstance(HTMLElement);
    highlighter.highlightElement(element);
    assert.isTrue(element.animate.calledOnce);
    assert.isTrue(element.scrollIntoViewIfNeeded.calledOnce);
  });
  ...
  it('highlights layers', async () => {
    ...
    const highlighter = new Elements.StylePropertyHighlighter.StylePropertyHighlighter(stylesSidebarPane);
    const highlightSpy = sinon.spy(highlighter, 'highlightElement');
  ...
  it('highlights sections', async () => {
    ...
    const highlighter = new Elements.StylePropertyHighlighter.StylePropertyHighlighter(stylesSidebarPane);
    const highlightSpy = sinon.stub(highlighter, 'highlightElement');
