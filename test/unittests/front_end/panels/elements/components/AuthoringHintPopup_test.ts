...
describeWithEnvironment('AuthoringHintPopup', async () => {
    ...
    it('renders every section', async () => {
        ...
        const popupComponent = new ElementsComponents.AuthoringHintPopup.AuthoringHintPopup(hintMessage);
        ...
        const popupLearnMoreRendeded = shadowRoot.querySelector('#learn-more') !== null;

        assert.isTrue(popupReasonRendered);
        assert.isTrue(popupPossibleFixRendered);
        assert.isTrue(popupLearnMoreRendeded);
    });
    ...
    it('does not render learn more', async () => {
        ...
        assert.isTrue(popupReasonRendered);
        assert.isTrue(popupPossibleFixRendered);
        assert.isFalse(popupLearnMoreRendeded);
    });
});
