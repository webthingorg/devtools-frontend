  ...
  it('renames nodes from the profile models when the corresponding scripts and source maps have loaded',
     async function() {
       ...
       const sourceMapAttachedPromise = new Promise<void>(
           resolve => performanceModel.addEventListener(
               Timeline.PerformanceModel.Events.NamesResolved,
               (event: Common.EventTarget.EventTargetEvent<Timeline.PerformanceModel.NamesResolvedCause>) => {
                 if (event.data === Timeline.PerformanceModel.NamesResolvedCause.AttachedSourcemap) {
                   resolve();
                 }
               }));
       ...
       // Now that the script and source map have loaded, test the
