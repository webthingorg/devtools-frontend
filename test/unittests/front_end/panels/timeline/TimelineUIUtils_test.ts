...
describe('TimelineUIUtils', () => {
  ...
  function createEvent(eventName: string) {
    const event =
        new SDK.TracingModel.Event('devtools.timeline', eventName, SDK.TracingModel.Phase.Complete, 10, thread);
  ...
  it('creates top frame location text for function calls', async () => {
    const event = createEvent('FunctionCall');
    assert.strictEqual(
  ...
  it('creates top frame location text as a fallback', async () => {
    // 'CPUProfile' is chosen such that we run into the 'default' case.
    const event = createEvent('CPUProfile');
    assert.strictEqual(
        'test.js:1:1', await Timeline.TimelineUIUtils.TimelineUIUtils.buildDetailsTextForTraceEvent(event));
  });
});
