...
import {Types} from '../../../../../front_end/models/trace/trace.js';
import {flattenBreadcrumbs} from '../../../../../front_end/panels/timeline/components/Breadcrumbs.js';
...
describeWithEnvironment('TimelineMiniMap', function() {
  ...
  it('creates the first breadcrumb when breadcrumbsPerformancePanel experiment is enabled', async function() {
    ...
    if (minimap.breadcrumbs) {
      assert.strictEqual(flattenBreadcrumbs(minimap.breadcrumbs.initialBreadcrumb).length, 1);
      assert.notStrictEqual(minimap.breadcrumbs.initialBreadcrumb, {window: initialTraceWindow, child: null});
    }
  });
