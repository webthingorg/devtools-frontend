    ...
    it('returns same events as old engine', async () => {
      ...
      for (let i = 0; i < 136; i++) {
        ...
        assert.strictEqual(
            ...
            Math.round(networkEventsFromNewEngine[i].args.data.processedData.finishTime));

        const {sendStartTime, headersEndTime} = networkEventsFromOldEngine[i].getSendReceiveTiming();
        const sendStartTimeNew = networkEventsFromNewEngine[i].args.data.processedData.sendStartTime;
        const headersEndTimeNew = networkEventsFromNewEngine[i].args.data.processedData.downloadStart;
