...
import {describeWithMockConnection, setMockConnectionResponseHandler, dispatchEvent} from '../../../helpers/MockConnection.js';
...
describeWithMockConnection('Interactions lane', () => {
  const noop = () => ({});
  it('creates data for the input lane from input latency trace events.', async () => {
    const traceEvents = [
      {
        'args': {
          'data': {
    ...
    // The following requests are fired as part of suspending all targets in the perf panel.
    ...
    const flamechartdataprovider = new Timeline.TimelineFlameChartDataProvider.TimelineFlameChartDataProvider();
    const controller = new Timeline.TimelineController.TimelineController(target, client);
    ...
    const myEvent = flamechartdataprovider.entryDataByIndex(1);
    assert.deepEqual(eventFromInputTrack.steps[0], myEvent);
    const inputDelayForEvent = TimelineModel.TimelineModel.TimelineData.forEvent(myEvent).timeWaitingForMainThread;
