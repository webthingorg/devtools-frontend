...
describeWithEnvironment('ThreadAppender', function() {
  ...
  it('does not append a track if there are no visible events on it', async function() {
    ...
    assert.strictEqual(flameChartData.groups.length, 4);
    assert.strictEqual(
        flameChartData.groups[0].name,
        '[RPP] Main — https://chromedevtools.github.io/performance-stories/long-interaction/index.html?x=40');
    assert.strictEqual(flameChartData.groups[1].name, '[RPP] Chrome_ChildIOThread');
    ...
    // should be drawn.
    assert.strictEqual(flameChartData.groups[2].name, '[RPP] ThreadPoolForegroundWorker');
    assert.strictEqual(flameChartData.groups[3].name, '[RPP] ThreadPoolServiceThread');
  });
