...
describeWithEnvironment('TimingTrackAppender', () => {
  ...
  describe('highlightedEntryInfo', () => {
    ...
    it('returns the info for a performance.measure calls correctly', () => {
      ...
      // Due to the formatting going under the i18n pipeline from which the string comes,
      // we can not directly compare for string equality.
      assert.isTrue(highlightedEntryInfo.formattedTime.includes('500.00'));
    });
  });
});
