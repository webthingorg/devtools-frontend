  ...
  it('Adds Hidden Ancestors Arrow as a decoration when TreeModified event is dispatched on a node', async function() {
    ...
    const firstNodeWithChildren = nodeOfGroup?.find(node => {
      const childrenAmount =
          data.Renderer.entryToNode.get(node as TraceEngine.Types.TraceEvents.TraceEntry)?.children.length;
      if (!childrenAmount) {
        return false;
      }
      return childrenAmount > 0 && node.cat === 'devtools.timeline';
    });
    const node = data.Renderer.entryToNode.get(firstNodeWithChildren as TraceEngine.Types.TraceEvents.TraceEntry);
    if (!node) {
