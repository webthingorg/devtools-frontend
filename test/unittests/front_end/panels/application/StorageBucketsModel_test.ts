...
describeWithMockConnection('StorageBucketsModel', () => {
  ...
  const getStorageBucketListStub = ({storageKey}: {storageKey: string}) => {
    return Promise.resolve({storageBuckets: getBucketsForStorageKeys(storageKey), getError: () => undefined});
  };
  ...
  describe('StorageKeyAdded', () => {
    it('only gets storage buckets when model is enabled', async () => {
      const storageKey = STORAGE_KEYS[0];
      const getStorageBucketListSpy = sinon.stub(storageBucketsModel.storageAgent, 'invoke_getStorageBucketList')
                                          .callsFake(getStorageBucketListStub);

      storageKeyManager.updateStorageKeys(new Set([storageKey]));
      assert.isTrue(getStorageBucketListSpy.notCalled);

      storageBucketsModel.enable();
      assert.isTrue(getStorageBucketListSpy.calledOnceWithExactly({storageKey}));
    });
    ...
    it('adds buckets for storage keys that already have been added', async () => {
      ...
      sinon.stub(storageBucketsModel.storageAgent, 'invoke_getStorageBucketList').callsFake(getStorageBucketListStub);
  ...
  describe('CreatedUpdatedDeletedBucket', () => {
    ...
    it('notifies when a bucket is deleted', async () => {
      ...
      const STORAGE_BUCKET_LOCATOR_REMOVED = {
        storageKey: STORAGE_BUCKET_REMOVED.storageKey,
        id: STORAGE_BUCKET_REMOVED.id,
      };
      const storageKeys = [STORAGE_KEYS[0], STORAGE_KEYS[2]];
      sinon.stub(storageBucketsModel.storageAgent, 'invoke_getStorageBucketList').callsFake(getStorageBucketListStub);
      ...
      storageBucketsModel.storageBucketDeleted({bucketLocator: STORAGE_BUCKET_LOCATOR_REMOVED});
  ...
  describe('GetBucketFunctions', () => {
    ...
    it('gets buckets by id', async () => {
      ...
      await listener.waitForBucketEvents(BucketEvents.BucketAdded, 4);
      assert.deepEqual(
          STORAGE_BUCKETS[0], storageBucketsModel.getBucketById(STORAGE_BUCKETS[0].storageKey, STORAGE_BUCKETS[0].id));
      assert.deepEqual(
          STORAGE_BUCKETS[1], storageBucketsModel.getBucketById(STORAGE_BUCKETS[1].storageKey, STORAGE_BUCKETS[1].id));
      assert.deepEqual(
          STORAGE_BUCKETS[2], storageBucketsModel.getBucketById(STORAGE_BUCKETS[2].storageKey, STORAGE_BUCKETS[2].id));
      assert.deepEqual(
          STORAGE_BUCKETS[3], storageBucketsModel.getBucketById(STORAGE_BUCKETS[3].storageKey, STORAGE_BUCKETS[3].id));
    });
