...
describeWithMockConnection('StorageBucketsModel', () => {
  ...
  describe('CreatedUpdatedDeletedBucket', () => {
    ...
    it('notifies when a bucket is deleted', async () => {
      ...
      storageBucketsModel.storageBucketDeleted({bucketId: STORAGE_BUCKET_INFO_REMOVED.bucket.id});
  ...
  describe('GetBucketFunctions', () => {
    ...
    it('gets buckets by id', async () => {
      ...
      await listener.waitForBucketEvents(BucketEvents.BucketAdded, 4);
      assert.deepEqual(STORAGE_BUCKET_INFOS[0], storageBucketsModel.getBucketById(STORAGE_BUCKET_INFOS[0].bucket.id));
      assert.deepEqual(STORAGE_BUCKET_INFOS[1], storageBucketsModel.getBucketById(STORAGE_BUCKET_INFOS[1].bucket.id));
      assert.deepEqual(STORAGE_BUCKET_INFOS[2], storageBucketsModel.getBucketById(STORAGE_BUCKET_INFOS[2].bucket.id));
      assert.deepEqual(STORAGE_BUCKET_INFOS[3], storageBucketsModel.getBucketById(STORAGE_BUCKET_INFOS[3].bucket.id));
    });
    ...
    it('gets default bucket when name isnt given', async () => {
      ...
      await listener.waitForBucketEvents(BucketEvents.BucketAdded, 4);
      assert.deepEqual(
          STORAGE_BUCKET_INFOS[0], storageBucketsModel.getBucketByName(STORAGE_BUCKET_INFOS[0].bucket.storageKey));
      assert.deepEqual(
          STORAGE_BUCKET_INFOS[2], storageBucketsModel.getBucketByName(STORAGE_BUCKET_INFOS[2].bucket.storageKey));
      assert.deepEqual(
          STORAGE_BUCKET_INFOS[3], storageBucketsModel.getBucketByName(STORAGE_BUCKET_INFOS[3].bucket.storageKey));
    });
  });

  it('deletes the bucket', () => {
    const {bucket: storageBucket} = STORAGE_BUCKET_INFOS[0];
    const setStorageBucketTrackingSpy =
