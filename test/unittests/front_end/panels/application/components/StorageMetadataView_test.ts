...
describeWithLocale('SharedStorageMetadataView', () => {
  ...
  it('renders with an top-level site', async () => {
    ...
    const values = getCleanTextContentFromElements(component.shadowRoot, 'devtools-report-value');
    assert.deepEqual(
        values, ['https://example.com', 'https://test.example', 'Yes (because origin is outside of top-level site)']);
  });
  ...
  it('renders with an opaque top-level site', async () => {
    ...
    assert.deepEqual(values, [
      'https://example.com',
      '(opaque)',
      'Yes (because top-level site is opaque)',
      'Yes (because top-level site is opaque)',
    ]);
  });
  ...
  it('renders with an opaque key', async () => {
    ...
    const values = getCleanTextContentFromElements(component.shadowRoot, 'devtools-report-value');
    assert.deepEqual(values, ['https://example.com', 'Yes (because the key is opaque)', 'Yes']);
  });
  ...
  it('renders with a cross-site ancestor chain', async () => {
    ...
    const values = getCleanTextContentFromElements(component.shadowRoot, 'devtools-report-value');
    assert.deepEqual(
        values, ['https://example.com', 'Yes (because there is a third-party origin in the ancestry chain)']);
  });
});
