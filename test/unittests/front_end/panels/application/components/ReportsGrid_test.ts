...
const renderReportsGrid =
    async(data: ApplicationComponents.ReportsGrid.ReportsGridData): Promise<DataGrid.DataGrid.DataGrid> => {
...
describe('ReportsGrid', async () => {
  ...
  it('displays placeholder text if no data', async () => {
    ...
    const footer = component.shadowRoot.querySelector('.reporting-footer div');
    assert.strictEqual(footer?.textContent, 'No reports to display');
  });
