...
describeWithRealConnection('BackForwardCacheView', () => {
  it('can handle delayed navigation history when testing for BFcache availability', async () => {
    ...
    const spy = sinon.spy(resourceTreeModel, 'navigateToHistoryEntry');
    ...
    // A console error about 'storage key not being found' is emitted.
    ...
    await new Promise(resolve => {
      resourceTreeModel.addEventListener(SDK.ResourceTreeModel.Events.FrameNavigated, () => {
        resourceTreeModel.addEventListener(SDK.ResourceTreeModel.Events.FrameNavigated, resolve);
      });
    });
    assert.isTrue(spy.calledOnceWithExactly(entries[0]));
  });
});
