...
import {
  treeNodeKeyText,
  type VisibleTreeNodeFromDOM,
  visibleNodesToTree,
  waitForRenderedTreeNodeCount,
} from '../../../helpers/TreeOutlineHelpers.js';
...
describeWithEnvironment('BackForwardCacheView', () => {
  ...
  it('renders explanation tree', async () => {
    ...
    assertElement(treeOutline, HTMLElement);
    assertShadowRoot(treeOutline.shadowRoot);
    await treeOutline.expandRecursively();
    await waitForRenderedTreeNodeCount(treeOutline.shadowRoot, 5);

    const visible = visibleNodesToTree(treeOutline.shadowRoot, treeNodeKeyText);
    const expected: VisibleTreeNodeFromDOM<NodeData>[] = [
      {
        renderedKey: {
