...
async function unpromisify(node: TreeOutline.TreeOutlineUtils.TreeNode<NodeData>): Promise<Node> {
  ...
  if (node.children) {
    ...
    result.children = await Promise.all(children.map(async child => await unpromisify(child)));
  }
  return result;
}
...
describeWithEnvironment('BackForwardCacheView', () => {
  ...
  it('renders explanation tree', async () => {
    ...
    const treeData = await Promise.all(treeOutline.data.tree.map(
        async node => await unpromisify(node as TreeOutline.TreeOutlineUtils.TreeNode<NodeData>)));
