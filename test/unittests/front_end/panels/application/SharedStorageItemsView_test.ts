...
function selectNodeByKey(dataGrid: DataGrid.DataGrid.DataGridImpl<unknown>, key: string): boolean {
...
describeWithMockConnection('SharedStorageItemsView', function() {
  ...
  it('updates sidebarWidget upon receiving SelectedNode Event', async () => {
    ...
    assert.isTrue(selectNodeByKey(view.getDataGridForTesting(), 'key2'));
    await raf();
