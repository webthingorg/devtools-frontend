...
describeWithMockConnection('SharedStorageItemsView', () => {
  ...
  it('initially has placeholder sidebar', async () => {
    const getMetadataSpy = sinon.spy(sharedStorageModel.storageAgent, 'invoke_getSharedStorageMetadata');
    const getEntriesSpy = sinon.spy(sharedStorageModel.storageAgent, 'invoke_getSharedStorageEntries');
    ...
    assert.isTrue(
        view.getOuterSplitWidgetForTesting().sidebarWidget()?.contentElement.firstChild?.textContent?.includes(
            'Select'));
  });
  ...
  it('updates sidebarWidget upon receiving cellFocusedEvent', async () => {
    ...
    const node = new DataGrid.DataGrid.DataGridNode({data: {key: 'key2', value: 'b'}});

    // Use a spy to assert that the sidebar preview pane gets updated when expected.
    const spy = sinon.spy(view.getOuterSplitWidgetForTesting(), 'setSidebarWidget');
