...
describeWithMockConnection('SharedStorageItemsView', function() {
  ...
  it('has placeholder sidebar when there are no entries', async () => {
    ...
    assert.notDeepEqual(view.outerSplitWidget.sidebarWidget()?.constructor.name, 'SearchableView');
    assert.isTrue(
        view.outerSplitWidget.sidebarWidget()?.contentElement.firstChild?.nextSibling?.textContent?.includes('Select'));
  ...
  it('updates sidebarWidget upon receiving SelectedNode Event', async () => {
    ...
    assert.deepEqual(view.outerSplitWidget.sidebarWidget()?.constructor.name, 'SearchableView');
