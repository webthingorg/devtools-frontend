...
describeWithMockConnection('ServiceWorkerCacheModel', () => {
  ...
  it('removes caches for storage key on clearForStorageKey', async () => {
    setMockConnectionResponseHandler(
        ...
        () => ({
          caches: [
            {cacheId: 'id1', storageKey: testKey, storageBucketId: testStorageBucketInfo.id, cacheName: 'test-cache-1'},
            {cacheId: 'id2', storageKey: testKey, storageBucketId: testStorageBucketInfo.id, cacheName: 'test-cache-2'},
          ],
        }));
    cacheStorageModel.enable();
    manager?.storageBucketCreatedOrUpdated({bucketInfo: testStorageBucketInfo});
