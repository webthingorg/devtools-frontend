...
describeWithMockConnection('SharedStorageEventsView', () => {
  ...
  it('records events only from the target page if the view can retrieve a nonempty main frame id', () => {
    const view = new View.SharedStorageEventsView();
    view.setDefaultIdForTesting(ID_A);
  ...
  it('records all events if the view is unable to retrieve a nonempty main frame id', () => {
    const view = new View.SharedStorageEventsView();
    view.setDefaultIdForTesting(EMPTY_ID);
    for (const event of MULTI_PAGE_EVENTS) {
      view.addEvent(event);
    }
    assert.deepEqual(view.getEventsForTesting(), MULTI_PAGE_EVENTS);
  });
});
