...
function dispatchEventsForNavigationWithSpeculationRules(
    target: SDK.Target.Target, speculationrules?: string, early = false) {
  const seq = ++seqLast;
  const loaderId = `loaderId:${seq}`;

  if (speculationrules !== undefined && early) {
...
`);
  ...
  it('renders out-of-document SpeculationRules', async () => {
    const target = createTarget();
    const view = createView(target);

    dispatchEventsForNavigationWithSpeculationRules(
        target, `
{
  "prerender":[
    {
