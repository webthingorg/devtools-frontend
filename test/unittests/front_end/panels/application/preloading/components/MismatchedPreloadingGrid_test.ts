...
const BG_GREEN = 'background-color: var(--color-background-diff-addition);';
const BG_RED = 'background-color: var(--color-background-diff-deletion);';
...
describeWithEnvironment('MismatchedPreloadingGrid', async () => {
  ...
  it('renderes edit diff', async () => {
    ...
    assertGridContents(
        ...
        ['Preloaded URLs', 'Action', 'Status'],
        [
          ['https://example.com/prefetched.html?q=\n2\n1', 'prefetch', 'Ready'],
        ],
    );

    assertDiff(component, {row: 1, column: 0}, [
      {textContent: 'https://example.com/prefetched.html?q=', partOfStyle: ''},
      {textContent: '2', partOfStyle: BG_RED},
      {textContent: '1', partOfStyle: BG_GREEN},
    ]);
  });
  ...
  it('renderes add diff', async () => {
    ...
    assertDiff(component, {row: 1, column: 0}, [
      ...
      {textContent: '?q=1', partOfStyle: BG_GREEN},
    ]);
  });
  ...
  it('renderes delete diff', async () => {
    ...
    assertDiff(component, {row: 1, column: 0}, [
      {textContent: 'https://example.com/prefetched.html', partOfStyle: ''},
      {textContent: '?q=1', partOfStyle: BG_RED},
    ]);
  });
  ...
  it('renderes complex diff', async () => {
    ...
    assertDiff(component, {row: 1, column: 0}, [
      ...
      {textContent: 'r', partOfStyle: BG_RED},
      {textContent: 'f', partOfStyle: BG_GREEN},
      {textContent: 'e', partOfStyle: ''},
      {textContent: 'nder', partOfStyle: BG_RED},
      {textContent: 'tch', partOfStyle: BG_GREEN},
      {textContent: 'ed.html?', partOfStyle: ''},
      {textContent: 'x', partOfStyle: BG_RED},
      {textContent: 'q', partOfStyle: BG_GREEN},
