...
describeWithEnvironment('PreloadingDetailsReportView', async () => {
  it('renders place holder if not selected', async () => {
    ...
    assertShadowRoot(component.shadowRoot);
    const placeholder = component.shadowRoot.querySelector('div.preloading-noselected div p');
  ...
  it('renders prerendering details', async () => {
    ...
    const data: PrerenderingAttempt = {
      ...
      startedAt: Date.parse('2006-01-02T15:04:05Z'),
      trigger: {
    ...
    const entries = zip2(keys, values);
    entries[1][1] = new Date(entries[1][1]).toISOString();
    assert.deepEqual(entries, [
      ['URL', url],
      ['Started at', '2006-01-02T15:04:05.000Z'],
