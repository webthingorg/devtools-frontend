...
describeWithEnvironment('RuleSetDetailsView', async () => {
  it('renders nothing if not selected', async () => {
    ...
    const component = await renderRuleSetDetailsView(data, prerenderedUrl);
    assertShadowRoot(component.shadowRoot);
...
`,
    ...
    assert.strictEqual(
        noWhiteSpaceAndNoNewLine(textEditor.state.doc.toString()), noWhiteSpaceAndNoNewLine(data.sourceText));
  });
