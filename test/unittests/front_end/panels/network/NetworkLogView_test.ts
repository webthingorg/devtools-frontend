...
describeWithMockConnection('NetworkLogView', () => {
  const tests = (targetFactory: () => SDK.Target.Target) => {
    ...
    function createEnvironment(urls: string[], blockedResponse: boolean) {
    ...
    it('hide Chrome extension requests from checkbox', async () => {
      let rootNode;
      let filterBar;
      ({rootNode, filterBar, networkLogView} = createEnvironment(['chrome-extension://url1', 'url2'], false));
    ...
    it('can filter requests with blocked response cookies from checkbox', async () => {
      let rootNode;
      let filterBar;
      ({rootNode, filterBar, networkLogView} = createEnvironment(['url1', 'url2'], true));
