  ...
  const tests = (targetFactory: () => SDK.Target.Target) => {
    ...
    it('hide Chrome extension requests from checkbox', async () => {
      ...
      ({rootNode, filterBar, networkLogView} = createEnvironment(
           createNetworkRequest, target, networkLogView, createNetworkLogView, ['chrome-extension://url1', 'url2'],
           false));
    ...
    it('can hide Chrome extension requests from dropdown', async () => {
      ...
      await clickItem(hideExtensionURL);
    ...
    it('can filter requests with blocked response cookies from checkbox', async () => {
      ...
      let filterBar;
      ({rootNode, filterBar, networkLogView} = createEnvironment(
           createNetworkRequest, target, networkLogView, createNetworkLogView, ['url1', 'url2'], true));
    ...
    it('can filter requests with blocked response cookies from dropdown', async () => {
      ...
      await clickItem(blockedResponseCookies);
...
function getCheckbox(filterBar: UI.FilterBar.FilterBar, title: string) {
  ...
  return blockedCookiesCheckbox;
}

function createEnvironment(
    createNetworkRequest: (url: string, options: {
      requestHeaders?: SDK.NetworkRequest.NameValue[]|undefined,
      finished?: boolean|undefined,
      target?: SDK.Target.Target|undefined,
    }) => SDK.NetworkRequest.NetworkRequest,
    target: SDK.Target.Target, networkLogView: Network.NetworkLogView.NetworkLogView,
    createNetworkLogView: (filterBar?: UI.FilterBar.FilterBar) => Network.NetworkLogView.NetworkLogView, urls: string[],
    blockedResponse: boolean) {
  for (const url of urls) {
    const request = createNetworkRequest(url, {target});
    if (blockedResponse && url === urls[0]) {
      request.blockedResponseCookies = () => [{
...
async function clickItem(item: HTMLElement) {
  if (item) {
...
function getDropdownItem(softMenu: HTMLElement, label: string) {
  const blockedResponseCookies = softMenu?.querySelector(`[aria-label^="${label}"]`);
  assertElement(blockedResponseCookies, HTMLElement);
  return blockedResponseCookies;
}

function getCheckmark(item: HTMLElement) {
  const blockedResponseCookiesCheckmark = item.querySelector<IconButton.Icon.Icon>('.checkmark');
  assertElement(blockedResponseCookiesCheckmark, HTMLElement);
  return blockedResponseCookiesCheckmark;
}
