...
describeWithMockConnection('RequestHeadersView', () => {
  ...
  it('does not render a link to \'.headers\' if that file does not exist', async () => {
    ...
    assert.isTrue(component.responseHeadersCategory.treeOutline?.contentElement.textContent?.includes(
        'GeneralRequest URL: https://www.example.com/foo.htmlResponse Headers (1)server: DevTools Test Server'));
    component.detach();
  });
  ...
  it('renders a link to \'.headers\'', async () => {
    ...
    const component = renderHeadersView(request);
    assert.isTrue(component.responseHeadersCategory.treeOutline?.contentElement.textContent?.includes(
        'GeneralRequest URL: https://www.example.com/foo.htmlResponse Headers (1).headersserver: DevTools Test Server'));
    component.detach();
  });
});
