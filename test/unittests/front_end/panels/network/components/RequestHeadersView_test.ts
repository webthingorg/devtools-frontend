...
describeWithEnvironment('RequestHeadersView', () => {
  ...
  it('displays decoded "x-client-data"-header', async () => {
    ...
    assert.strictEqual(
        getCleanTextContentFromElements(requestHeadersCategory, '.header-value')[0],
        'CJa2yQEIpLbJAQiTocsBDecoded:message ClientVariations {// Active client experiment variation IDs.repeated int32 variation_id = [3300118, 3300132, 3330195];\n}',
    );
  });
});
