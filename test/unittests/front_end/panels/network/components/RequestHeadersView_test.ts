...
describeWithEnvironment('RequestHeadersView\'s Category', () => {
  ...
  it('dispatches an event when its button is clicked', async () => {
    let eventCounter = 0;
    const component = new NetworkComponents.RequestHeadersView.Category();
    renderElementIntoDOM(component);
    component.data = {
      ...
      headerCount: 3,
      toggleRawButtonText: 'View source' as Common.UIString.LocalizedString,
    };
    assertShadowRoot(component.shadowRoot);
    await coordinator.done();
    ...
    const button = getElementWithinComponent(component, 'devtools-button', HTMLElement);
