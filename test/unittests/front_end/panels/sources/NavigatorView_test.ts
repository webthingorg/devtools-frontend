...
describeWithMockConnection('NavigatorView', () => {
  it('can discard multiple childless frames', async () => {
    ...
    childFrame?.addResource(new SDK.Resource.Resource(
        resourceTreeModel, null, url, url, childFrameId, null, Common.ResourceType.resourceTypes.Document, 'text/html',
        null, null));
    ...
    const contentProvider = TextUtils.StaticContentProvider.StaticContentProvider.fromString(
        url, Common.ResourceType.resourceTypes.Document, '');
    const metadata = new Workspace.UISourceCode.UISourceCodeMetadata(null, null);
    project.addUISourceCodeWithProvider(uiSourceCode, contentProvider, metadata, 'text/html');
