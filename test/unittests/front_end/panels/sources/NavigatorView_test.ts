...
describeWithMockConnection('Navigator tree throttler', () => {
  ...
  it('flushes after condition becomes true', async () => {
    ...
    // Submit two tasks. Those are only queued (because the flushing condition only triggers after two tasks).
    throttler(() => {
      finishedTaskCount++;
    });
    throttler(() => {
    ...
    // Once we submit the third task, the throttler flushes.
    throttler(() => {
      finishedTaskCount++;
    });

    // Check that the throttler flushes and deactivates the timeout.
    assert.strictEqual(finishedTaskCount, 3);
    assert.isFalse(timeoutControl.timeoutActive());
  });
});
