...
describeWithMockConnection('Inline variable view scope helpers', () => {
  ...
  async function toOffset(sourceMap: SDK.SourceMap.SourceMap|null, l: SDK.DebuggerModel.Location|null) {
    if (!l || !sourceMap) {
      return null;
    }
    const entry = sourceMap.findEntry(l.lineNumber, l.columnNumber);
  ...
  it('can resolve single scope mappings', async () => {
    ...
    // This was minified with
    // 'terser index.js -m --toplevel -o example.min.js --source-map "url=example.min.js.map,includeSources"' v5.7.0.
  ...
  it('can resolve nested scope mappings', async () => {
    // Construct call frame.
    const sourceMapUrl = 'file:///tmp/example.js.min.map';
    // This example was minified with
    // 'terser index.js -m --toplevel -o example.min.js --source-map "url=example.min.js.map,includeSources"' v5.7.0.
    const source =
...
describeWithEnvironment('Inline variable view parser', () => {
  it('parses simple identifier', () => {
    ...
    assert.deepEqual(variables, [{line: 1, from: 0, id: 'c'}]);
  });
  ...
  it('parses simple function', () => {
    ...
    assert.deepEqual(variables, [{line: 1, from: 11, id: 'o'}, {line: 2, from: 26, id: 'a'}]);
  });
  ...
  it('parses function with nested block', () => {
    ...
    const variables = Sources.DebuggerPlugin.getVariableNamesByLine(state, 10, code.length);
    assert.deepEqual(
        variables, [{line: 1, from: 11, id: 'o'}, {line: 2, from: 26, id: 'a'}, {line: 4, from: 53, id: 'a'}]);
  });
});
...
describeWithEnvironment('Inline variable view scope value resolution', () => {
  it('resolves single variable in single scope', () => {
    ...
    assert.strictEqual(valuesByLine?.get(2)?.size, 1);
    assert.strictEqual(valuesByLine?.get(2)?.get('a')?.value, 42);
  });
  ...
  it('resolves shadowed variables', () => {
    ...
    const variableNames = [
      {line: 1, from: 5, id: 'a'},    // Falls into the outer scope.
      {line: 11, from: 15, id: 'a'},  // Inner scope.
      {line: 21, from: 25, id: 'a'},  // Outer scope.
      {line: 31, from: 35, id: 'a'},  // Outside of any scope.
    ];
    const valuesByLine = Sources.DebuggerPlugin.getVariableValuesByLine(scopeMappings, variableNames);
  ...
  it('resolves multiple variables on the same line', () => {
    ...
    const variableNames = [
      {line: 11, from: 11, id: 'a'},
      {line: 11, from: 13, id: 'b'},
      {line: 11, from: 15, id: 'a'},
    ];
    const valuesByLine = Sources.DebuggerPlugin.getVariableValuesByLine(scopeMappings, variableNames);
