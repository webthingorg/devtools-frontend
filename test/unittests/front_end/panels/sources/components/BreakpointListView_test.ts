...
async function renderSingleBreakpoint() {
  const component = new SourcesComponents.BreakpointListView.BreakpointListView();
        ...
        breakpointItems: [
          {
            ...
            isHit: true,
            status: SourcesComponents.BreakpointListView.BreakpointStatus.ENABLED,
          },
        ],
      },
    ],
  } as SourcesComponents.BreakpointListView.BreakpointListViewData;
    ...
    groups: [
      {
        ...
        breakpointItems: [
          ...
          },
          {
            ...
            codeSnippet: 'if (x > a) {',
            isHit: true,
            status: SourcesComponents.BreakpointListView.BreakpointStatus.DISABLED,
          },
        ],
      },
      {
...
function extractVisibleBreakpointItems(data: SourcesComponents.BreakpointListView.BreakpointListViewData) {
...
function checkCodeSnippet(
    renderedBreakpointItem: HTMLDivElement, breakpointItem: SourcesComponents.BreakpointListView.BreakpointItem) {
...
function checkCheckboxState(
    checkbox: HTMLInputElement, breakpointItem: SourcesComponents.BreakpointListView.BreakpointItem) {
  const checked = checkbox.checked;
  const indeterminate = checkbox.indeterminate;
  if (breakpointItem.status === SourcesComponents.BreakpointListView.BreakpointStatus.INDETERMINATE) {
    assert.isTrue(indeterminate);
  } else {
    assert.isFalse(indeterminate);
    assert.strictEqual(
        (breakpointItem.status === SourcesComponents.BreakpointListView.BreakpointStatus.ENABLED), checked);
  }
}

describeWithEnvironment('BreakpointListView', () => {
  ...
  it('triggers an event on clicking the checkbox of a breakpoint', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointListView.CheckboxToggledEvent>(
        component, SourcesComponents.BreakpointListView.CheckboxToggledEvent.eventName);
    checkbox.click();
  ...
  it('triggers an event on clicking on the snippet text', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointListView.BreakpointSelectedEvent>(
        component, SourcesComponents.BreakpointListView.BreakpointSelectedEvent.eventName);
    snippet.click();
    const event = await eventPromise;
    assert.deepEqual(event.data.breakpointItem, data.groups[0].breakpointItems[0]);
  });
  ...
  it('triggers an event on expanding/unexpanding', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointListView.ExpandedStateChangedEvent>(
        component, SourcesComponents.BreakpointListView.ExpandedStateChangedEvent.eventName);
