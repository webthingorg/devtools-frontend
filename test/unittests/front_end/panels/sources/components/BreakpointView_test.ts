...
function extractVisibleBreakpointItems(data: SourcesComponents.BreakpointView.BreakpointViewData) {
  const visibleGroups = data.groups.filter(group => group.expanded);
  const breakpointItems = visibleGroups.flatMap(group => group.breakpointEntries);
  assert.isAbove(breakpointItems.length, 0);
  return breakpointItems;
}

function checkCodeSnippet(
    renderedBreakpointItem: HTMLDivElement, breakpointItem: SourcesComponents.BreakpointView.BreakpointItem) {
...
function checkCheckboxState(
    checkbox: HTMLInputElement, breakpointItem: SourcesComponents.BreakpointView.BreakpointItem) {
  const checked = checkbox.checked;
  const indeterminate = checkbox.indeterminate;
  if (breakpointItem.status === SourcesComponents.BreakpointView.BreakpointStatus.INDETERMINATE) {
    assert.isTrue(indeterminate);
  } else {
    assert.isFalse(indeterminate);
    assert.strictEqual((breakpointItem.status === SourcesComponents.BreakpointView.BreakpointStatus.ENABLED), checked);
  }
}
...
describe('BreakpointView', () => {
  it('renders breakpoint entries of groups that are expanded', async () => {
    ...
    assert.lengthOf(renderedExpandedGroups, renderedExpandedGroups.length);
    ...
    for (let i = 0; i < renderedExpandedGroups.length; ++i) {
      ...
      const expectedLength = expandedGroups[i].breakpointEntries.length;
  ...
  it('triggers an event on clicking the checkbox of a breakpoint', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointView.CheckboxToggledEvent>(
        component, SourcesComponents.BreakpointView.CheckboxToggledEvent.eventName);
    checkbox.click();
  ...
  it('triggers an event on clicking on the snippet text', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointView.BreakpointSelectedEvent>(
        component, SourcesComponents.BreakpointView.BreakpointSelectedEvent.eventName);
    snippet.click();
    const event = await eventPromise;
    assert.deepEqual(event.data.item, data.groups[0].breakpointEntries[0]);
  });
  ...
  it('triggers an event on expanding/unexpanding', async () => {
    ...
    const eventPromise = getEventPromise<SourcesComponents.BreakpointView.ExpandedStateChangedEvent>(
        component, SourcesComponents.BreakpointView.ExpandedStateChangedEvent.eventName);
