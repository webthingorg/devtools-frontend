...
async function renderSingleBreakpoint(
  ...
  const data = createSingleBreakpointViewData(type, hoverText);
...
function createSingleBreakpointViewData(type: SourcesComponents.BreakpointsView.BreakpointType, hoverText?: string) {
  return {
    groups: [
      {
        name: 'test1.js',
