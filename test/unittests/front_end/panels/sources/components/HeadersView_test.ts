...
import {assertElement, assertShadowRoot, dispatchKeyDownEvent, getCleanTextContentFromElements, renderElementIntoDOM} from '../../../helpers/DOMHelpers.js';
...
describe('HeadersView', async () => {
  ...
  it('displays data and allows editing', async () => {
    ...
    let rows = getCleanTextContentFromElements(editor.shadowRoot, '.row');
    ...
    rows = getCleanTextContentFromElements(editor.shadowRoot, '.row');
    assert.deepEqual(rows, [
      'Apply to:index.html',
  ...
  it('allows adding "ApplyTo"-blocks', async () => {
    ...
    await pressButton(editor.shadowRoot, 0, '.add-block');
    ...
    const editables = editor.shadowRoot?.querySelectorAll('.editable');
    await changeEditable(editables[5] as HTMLElement, 'articles/*');
    await changeEditable(editables[6] as HTMLElement, 'cache-control');
    await changeEditable(editables[7] as HTMLElement, 'max-age=1000');
