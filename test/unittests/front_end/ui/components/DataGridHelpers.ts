...
export const getCellByIndexes = (shadowRoot: ShadowRoot, indexes: { column: number, row: number}) => {
  const cell = shadowRoot.querySelector<HTMLTableCellElement>(`[data-row-index="${indexes.row}"][data-col-index="${indexes.column}"]`);
...
export const getValuesOfBodyRow = (shadowRoot: ShadowRoot, rowIndex: number, options: {onlyVisible: boolean} = {onlyVisible: false}): string[] => {
...
export const getValuesOfAllBodyRows = (shadowRoot: ShadowRoot, options: {onlyVisible: boolean} = {
  ...
  return rows
      .map((row, index) => {
        ...
        return {rowValues: getValuesOfBodyRow(shadowRoot, index + 1, options), hidden: options.onlyVisible && rowIsHidden};
      })
