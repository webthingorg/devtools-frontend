...
import {getAllRows, getHeaderCellForColumnId, getRowByIndex, getSelectedRow, getValuesForColumn, getValuesOfAllRows} from './DataGridHelpers.js';
...
describe('DataGridController', () => {
  ...
  describe('filtering rows', () => {
    ...
    it('only shows rows with values that match the filter', () => {
      ...
      assertShadowRoot(component.shadowRoot);
      const internalDataGridShadow = getInternalDataGridShadowRoot(component);
      const renderedRowValues = getValuesOfAllRows(internalDataGridShadow, {onlyVisible: true});
    ...
    it('shows all rows if the filter is then cleared', () => {
      ...
      assertShadowRoot(component.shadowRoot);
      const internalDataGridShadow = getInternalDataGridShadowRoot(component);
      let renderedRowValues = getValuesOfAllRows(internalDataGridShadow, {onlyVisible: true});
