...
import {Column, Row, SortDirection} from '../../../../../front_end/ui/components/DataGridUtils.js';
...
import {getCellByIndexes, getFocusableCell, getHeaderCellForColumnId, getHeaderCells, getValuesOfAllBodyRows, getValuesOfBodyRowByAriaIndex} from './DataGridHelpers.js';
...
describe('DataGrid', () => {
  ...
  describe('setting the widths of the columns', () => {
    ...
    it('defaults to 100% if there is only one column', () => {
      ...
      const component = renderColumns(columns);
      assertShadowRoot(component.shadowRoot);

      const headers = getHeaderCells(component.shadowRoot);
      assert.lengthOf(headers, 1);
      assert.strictEqual(headers[0].clientWidth, 100);
    });
    ...
    it('splits the widths evenly if no columns have it given', () => {
      ...
      // 50 and 49 because there are borders and clientWidth gets rounded
      assert.strictEqual(headers[0].clientWidth, 50);
      assert.strictEqual(headers[1].clientWidth, 49);
    });
    ...
    it('distributes remaining width if only some columns have a set width', () => {
      ...
      assert.strictEqual(headers[0].clientWidth, 60);
      assert.strictEqual(headers[1].clientWidth, 19);
      assert.strictEqual(headers[2].clientWidth, 19);
    });
  });
