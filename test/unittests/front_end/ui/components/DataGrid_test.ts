...
describe('DataGrid', () => {
  ...
  describe('navigating with the keyboard', () => {
    ...
    it('does not let the user move left if they are at the first visible column', () => {
      ...
      const firstFocusedCell = getFocusableCell(component.shadowRoot);
      firstFocusedCell.focus();
      assert.strictEqual(firstFocusedCell.getAttribute('data-row-index'), '1');
    ...
    it('does let the user move up into the column row when they are sortable', () => {
      ...
      const firstBodyCell = getCellByIndexes(component.shadowRoot, {column: 0, row: 1});
      firstBodyCell.focus();
      assert.strictEqual(firstBodyCell.getAttribute('data-row-index'), '1');
      assert.strictEqual(firstBodyCell.getAttribute('data-col-index'), '0');
      dispatchKeyDownEvent(table, {key: 'ArrowUp'});
      const newFocusedCell = getFocusableCell(component.shadowRoot);
