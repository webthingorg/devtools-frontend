...
import * as Highlighting from '../../../../../../front_end/ui/components/highlighting/highlighting.js';
...
describe('HighlightManager', () => {
  ...
  function toText(range: Range|null): [string, string] {
    Platform.assertNotNullOrUndefined(range);
    const start = range.startContainer;
    assert.strictEqual(start.nodeType, Node.TEXT_NODE);
    const end = range.endContainer;
    assert.strictEqual(end.nodeType, Node.TEXT_NODE);

    Platform.assertNotNullOrUndefined(start.textContent);
    Platform.assertNotNullOrUndefined(end.textContent);

    return [start.textContent.substr(range.startOffset), end.textContent.substr(0, range.endOffset)];
  }

  it('correctly translates ranges', () => {
    // "text"
    //    ^^
    assert.deepStrictEqual(toText(walk('text').nextRange(2, 2)), ['xt', 'text']);  // special case with start == end

    // "abcdef"
    //    ^ ^
    assert.deepStrictEqual(toText(walk('abc<p>def</p>').nextRange(2, 3)), ['c', 'de']);

    // "abcdefghi"
    //    ^    ^
    assert.deepStrictEqual(toText(walk('abc<p>def</p>ghi').nextRange(2, 6)), ['c', 'gh']);
    ...
    // Except for the first range, start offsets are left-leaning, i.e., when the offset falls between two text nodes,
    // the preceding one is returned.
    assert.deepStrictEqual(toText(walker.nextRange(0, 3)), ['abc', 'abc']);
