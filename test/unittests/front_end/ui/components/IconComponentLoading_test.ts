...
const preloadImageWithName = (name: string): void => {
  const preloadLink = document.createElement('link');
  const iconURL = new URL(`../../../../../front_end/Images/${name}.svg`, import.meta.url).toString();
...
describe('Icon component', () => {
  it('captures loading time with icon component and separate icon .svg files', async () => {
    // preload separate icon files
    for (let i = 0; i < 42; i++) {
      if (i < 40) {
        const icName = 'ic_' + i;
        preloadImageWithName(icName);
      }
      const mediumIcName = 'medium_' + i;
      preloadImageWithName(mediumIcName);
    }
