...
const defaultCounter: UIComponents.IconButton.IconWithTextData = {
  iconName: 'error_icon',
  iconColor: '#1a73e8',
  text: '1',
};
...
describe('IconButton', () => {
  it('renders correctly with one icon', () => {
    const {shadowRoot} = renderIconButton({clickHandler: () => {}, groups: [defaultCounter]});

    const icons = extractCounters(shadowRoot);
