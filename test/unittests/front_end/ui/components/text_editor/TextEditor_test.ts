...
describeWithEnvironment('TextEditor', () => {
  describe('component', () => {
    ...
    it.only('can restore scroll to the same position after reconnecting to the DOM', async () => {
      const waitForScrollPromise = new ManualPromise();
      const dispatchSpy = sinon.spy(CodeMirror.EditorView.prototype, 'dispatch');
      const editor = new TextEditor.TextEditor.TextEditor(makeState('line1\nline2\nline3\nline4\nline5\nalooonglinethatnooneknowshowlongitisnalooonglinethatnooneknowshowlongitisnalooonglinethatnooneknowshowlongitisalooonglinethatnooneknowshowlongitisnalooonglinethatnooneknowshowlongitisnalooonglinethatnooneknowshowlongitis',
          [CodeMirror.EditorView.theme({'&.cm-editor': {height: '50px', width: '50px'}, '.cm-scroller': {overflow: 'auto'}})]));
      sinon.stub(editor, 'scrollEventHandledForTest').callsFake(() => {
        // Resolves `waitForScrollPromise` whenever `scrollEventHandledForTest` is called
