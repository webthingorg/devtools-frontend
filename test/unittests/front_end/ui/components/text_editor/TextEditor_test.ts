...
import {RepeatableManualPromise} from '../../../helpers/RepeatableManualPromise.js';
...
describeWithEnvironment('TextEditor', () => {
  describe('component', () => {
    ...
    it('should restore scroll to the same position after reconnecting to DOM when it is scrollable', async () => {
      const waitForScrollPromise = new RepeatableManualPromise();
      ...
      sinon.stub(editor, 'scrollEventHandledToSaveScrollPositionForTest').callsFake(() => {
        // Resolves `waitForScrollPromise` whenever `scrollEventHandledToSaveScrollPositionForTest` is called
      ...
      await waitForScrollPromise.promise();
