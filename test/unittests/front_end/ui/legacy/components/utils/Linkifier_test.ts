...
describeWithMockConnection('Linkifier', async () => {
  ...
  describe('maybeLinkifyConsoleCallFrame', () => {
    it('uses the BreakLocation as a revealable if the option is provided and a breakpoint is at the given location',
       async () => {
         ...
         // Create a raw callframe with the same source position as the logpoint.
         const callFrame: Protocol.Runtime.CallFrame = {
           functionName: 'foo',
           scriptId: script.scriptId,
           url,
           lineNumber,
         ...
         const anchor =
             linkifier.maybeLinkifyConsoleCallFrame(target, callFrame, {inlineFrameIndex: 0, revealBreakpoint: true});
         assertNotNullOrUndefined(anchor);
