...
describeWithEnvironment('FilmStripView', () => {
  ...
  describe('FilmStripView Dialog', () => {
    async function renderDialogWithSDKModel(
      ...
      const dialogWidget = PerfUI.FilmStripView.Dialog.fromSDKFrame(selectedFrame);
    ...
    describe('using the TraceEngine filmstrip', () => {
      ...
      it('does not let the user go beyond the last image', async () => {
        ...
        let renderedImage = shadowRoot.querySelector<HTMLImageElement>('[data-film-strip-dialog-img]');
        assert.isTrue(
            renderedImage?.currentSrc.includes(traceParsedData.Screenshots[numberOfFrames - 1].args.snapshot));
