...
describeWithEnvironment('FlameChart', () => {
  ...
  describe('entryIndexToCoordinates', () => {
    ...
    it('returns the correct coordinates for a given entry', async () => {
      ...
      assert.deepEqual(chartInstance.entryIndexToCoordinates(0), {x: 36 + canvasOffsetX, y: 51 + canvasOffsetY});

      // Emulate two scrolls to force a change in coordinates.
      chartInstance.setSelectedEntry(3);
      assert.deepEqual(chartInstance.entryIndexToCoordinates(0), {x: 36 + canvasOffsetX, y: -17 + canvasOffsetY});
      chartInstance.setWindowTimes(250, 600);
      assert.deepEqual(chartInstance.entryIndexToCoordinates(0), {x: -514 + canvasOffsetX, y: -17 + canvasOffsetY});
    });
  });
});
