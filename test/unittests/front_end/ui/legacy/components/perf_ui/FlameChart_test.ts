...
describeWithEnvironment('FlameChart', () => {
  ...
  describe('highlightEntry', () => {
    ...
    it('updates the chart to highlight the entry and dispatches an event', async () => {
      ...
      // Ensure that the highlighted div is positioned correctly
      assert.strictEqual(chartInstance.highlightElement.style.height, '16px');
      assert.strictEqual(chartInstance.highlightElement.style.top, '51px');
      assert.strictEqual(chartInstance.highlightElement.style.left, '922px');
      assert.strictEqual(chartInstance.highlightElement.style.width, '115.3px');
