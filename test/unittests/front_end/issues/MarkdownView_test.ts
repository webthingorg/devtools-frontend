...
`;
  ...
  describe('component', () => {
    it('renders basic markdown correctly', () => {
      ...
      assert.isTrue(paragraphs[0].innerText.startsWith('Single paragraph with'));

      const listItems = Array.from(component.shadowRoot.querySelectorAll('li'));
      assert.strictEqual(listItems.length, 2);
      assert.deepStrictEqual(
          listItems.map(item => item.textContent), ['Simple unordered list item 1', 'Simple unordered list item 2']);
    });
  });
});
