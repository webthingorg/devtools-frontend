...
import {IssueSurveyLink} from '../../../../front_end/issues/IssueSurveyLink.js';
import {CanShowSurveyResult, ShowSurveyResult} from '../../../../front_end/host/InspectorFrontendHostAPI.js';
import {assertNotNull, assertShadowRoot, renderElementIntoDOM} from '../helpers/DOMHelpers.js';

function canShowSuccessfulCallback(trigger: string, callback: (result: CanShowSurveyResult) => void) {
  callback({canShowSurvey: true});
}
function showSuccessfulCallback(trigger: string, callback: (result: ShowSurveyResult) => void) {
  callback({surveyShown: true});
}
function canShowFailureCallback(trigger: string, callback: (result: CanShowSurveyResult) => void) {
  callback({canShowSurvey: false});
}
function showFailureCallback(trigger: string, callback: (result: ShowSurveyResult) => void) {
