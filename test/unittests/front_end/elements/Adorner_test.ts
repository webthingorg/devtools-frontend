...
function assertIsAdorner(element: HTMLElement) {
  assert.equal(element.tagName, ADORNER_TAG_NAME, `element tag name is not ${ADORNER_TAG_NAME}`);
  assert.isTrue(element instanceof Adorner, 'element is not an instance of Adorner');
  assert.equal(Object.getPrototypeOf(element), Adorner.prototype, 'element is not on Adorner\'s prototype chain');
}
...
describe('Adorner', () => {
  ...
  it('can be created by Adorner.create', () => {
    ...
    assert.equal(adornerWithOptions.category, AdornerCategories.Layout);
  });
});
