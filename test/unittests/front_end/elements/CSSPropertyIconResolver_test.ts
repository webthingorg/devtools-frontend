...
import {findIcon, getPhysicalFlexDirections, PhysicalFlexDirection, reverseDirection, rotateFlexAlignIcon, rotateFlexDirectionIcon} from '../../../../front_end/elements/CSSPropertyIconResolver.js';
...
describe('CSSPropertyIconResolver', () => {
  ...
  it('can rotate an for align-content', () => {
    const iconName = 'iconName';
    assert.deepEqual(rotateFlexAlignIcon(iconName, PhysicalFlexDirection.LEFT_TO_RIGHT), {
      iconName,
      rotate: -90,
      scaleX: 1,
      scaleY: 1,
    });
    assert.deepEqual(rotateFlexAlignIcon(iconName, PhysicalFlexDirection.RIGHT_TO_LEFT), {
      iconName,
      rotate: 90,
      scaleX: 1,
      scaleY: 1,
    });
    assert.deepEqual(rotateFlexAlignIcon(iconName, PhysicalFlexDirection.TOP_TO_BOTTOM), {
      iconName,
      rotate: 0,
      scaleX: 1,
      scaleY: 1,
    });
    assert.deepEqual(rotateFlexAlignIcon(iconName, PhysicalFlexDirection.BOTTOM_TO_TOP), {
      iconName,
      rotate: 0,
      scaleX: 1,
      scaleY: 1,
  ...
  it('can find an icon for align-content properties', () => {
    ...
    for (const test of tests) {
      assert.deepEqual(
          ...
          rotateFlexAlignIcon(test.iconName, test.expected),
