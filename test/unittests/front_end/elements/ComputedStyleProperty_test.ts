...
describe('ComputedStyleProperty', () => {
  ...
  it('can set onNavigateToSource separately', () => {
    const component = new ComputedStyleProperty();
    renderElementIntoDOM(component);
    let isOnNavigateToSourceCalled = false;
    const data = {...initialData};
    component.data = data;
    component.setOnNavigateToSource(() => {
      isOnNavigateToSourceCalled = true;
    });
    ...
    goto.click();
    assert.isTrue(isOnNavigateToSourceCalled, 'onNavigateToSource should be called correctly');
  });
});
