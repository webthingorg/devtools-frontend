...
describe('ComputedStyleProperty', () => {
  ...
  it('renders expanded vs. collapsed property correctly', () => {
    ...
    assertShadowRoot(component.shadowRoot);
    const isDetailsOpenWhenExpanded = component.shadowRoot.querySelector('details')?.open;
    assert.isTrue(isDetailsOpenWhenExpanded, 'details tag should be open when the property is expanded');

    data.expanded = false;
    component.data = data;
    const isDetailsOpenWhenCollapsed = component.shadowRoot.querySelector('details')?.open;
    assert.isFalse(isDetailsOpenWhenCollapsed, 'details tag should not be open when the property is collapsed');
  });
