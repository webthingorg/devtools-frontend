...
if (existsSync(generatedScreenshotFolder)) {
  rimraf.sync(generatedScreenshotFolder);
}
mkdirSync(generatedScreenshotFolder);

const defaultScreenshotOpts: Partial<puppeteer.ScreenshotOptions> = {
  type: 'png',
  fullPage: true,
  encoding: 'binary'
};
export const assertScreenshotUnchanged = async (page: puppeteer.Page, fileName: string, options: Partial<puppeteer.ScreenshotOptions> = {}) => {
  ...
  if (existsSync(generatedScreenshotPath)) {
  ...
  return new Promise((resolve, reject) => {
    compare(generatedScreenshotPath, goldensScreenshotPath, {}, (err, data) => {
      if (err) {
        reject();
        return;
      }
