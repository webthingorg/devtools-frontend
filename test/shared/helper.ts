...
export const click = async (selector: string|puppeteer.ElementHandle, options?: ClickOptions) => {
  ...
  // Take a picture so we see what we clicked on. Adding additional requirement so it never applies. DO NOT CHECK IN
  if (options?.screenshot && options?.clickOptions?.modifier) {
