...
export const closeAllCloseableTabs = async () => {
  ...
  const all_close_buttons = await $$(selector);

  // Get all panel ids
  const panelTabIds = await all_close_buttons.evaluate((buttons: HTMLElement[]) => {
    return buttons.map(button => {
      return button.parentElement ? button.parentElement.id : '';
    });
  });
