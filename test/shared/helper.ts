 ...
 * @see {@link https://github.com/ionic-team/stencil-site/issues/512#issuecomment-839083970}
 **/
const pierceHandle = 'pierce';
...
export const click = async (selector: string|puppeteer.ElementHandle, options?: ClickOptions) => {
  ...
  // click on it instead.
  if (typeof selector === 'string') {
    // `pierce/` prefix will tell Puppeteer to search across shadow doms.
    const piercedSelector = selector.startsWith(`${pierceHandle}/`) ? selector : `${pierceHandle}/${selector}`;
