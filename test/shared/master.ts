...
const envJobs = Number(process.env['JOBS'] || 1);
const workerCount = Math.min(envJobs, 4);
...
export function init() {
  ...
  const createCount = workerCount - numWorkers;
  finishCount = createCount;
  console.log(`Creating ${createCount} worker${(createCount === 1 ? '' : 's')}.`);
...
function locateWorkerByPid(pid: number): cluster.Worker {
  ...
  if (!workerName) {
    throw new Error(`Received message from pid ${pid}`);
  }
...
function onWorkerMessage({pid, details}: {pid: number, details: string}) {
