...
export class ScreenshotError extends Error {
  ...
  static fromBase64Images(error: unknown, targetScreenshot?: string, frontendScreenshot?: string) {
    ...
    const screenshots = {
      'target': {filePath: this.saveArtifact(targetScreenshot, 'target')},
      'frontend': {filePath: this.saveArtifact(frontendScreenshot, 'frontend')},
    };
   ...
   * publication time.
   */
  private static saveArtifact(base64Image: string, tag: string): string {
    const stashedFileName = tag + '-' +
        'screenshot.png';
    const artifactPath = path.join(os.tmpdir(), stashedFileName);
