...
(async function() {
  try {
    ...
    const resetPages =
      ...
      const { enabledExperiments, selectedPanel } = opts;
      ...
      // If there is a selected panel don't wait on the elements selector. Equally, however, because
      // we don't insist on a selector being passed for selectedPanel, should there not be one we
      // return and assume the test knows what selector it's listening for.
      if (selectedPanel) {
      ...
      // await frontend.waitForNavigation({waitUntil: ['networkidle2', 'domcontentloaded']});
