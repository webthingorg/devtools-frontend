...
def dtf_recipe(
        name,
        cipd_package = dtf_defaults.cipd_package,
        cipd_version = dtf_defaults.cipd_version):
...
def dtf_builder(
        recipe_name,
        swarming_tags = dtf_defaults.swarming_tags,
        **kvargs):
    ...
    if 'executable' not in kvargs:
        kvargs['executable'] = dtf_recipe(recipe_name)
